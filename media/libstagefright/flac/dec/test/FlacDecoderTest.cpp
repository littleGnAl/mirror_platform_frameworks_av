...
class FLACDecoderTest : public ::testing::TestWithParam<tuple<string, string, bool>> {
  public:
    ...
    void processFlacDecoder(vector<FrameInfo> Info, int32_t offset, int32_t range, bool outputFloat,
                            ofstream &ostrm);
...
void FLACDecoderTest::processFlacDecoder(vector<FrameInfo> Info, int32_t offset, int32_t range,
    ...
    if (range + offset > Info.size() || range < 0 || offset > Info.size() - 1 || offset < 0) {
        ...
        ASSERT_TRUE(false) << "Invalid offset or range or both passed for decoding";
    }
    while (1) {
        if (frameID == Info.size() || frameID == (offset + range)) break;
        int64_t flags = (Info)[frameID].flags;
        int32_t size = (Info)[frameID].bytesCount;
        ASSERT_GT(size, 0) << "Invalid size for memory allocation";
        ...
        if (flags == CODEC_CONFIG_FLAG && mInputBufferCount == 0) {
            status_t decoderErr = mFLACDecoder->parseMetadata((uint8_t *)mInputBuffer, size);
            if (decoderErr == WOULD_BLOCK) {
                ...
                ASSERT_TRUE(false) << "FLACDecoder parseMetaData returns error " << decoderErr;
            }
        } else {
            ...
            ASSERT_NE(out_buf, nullptr) << "Output buffer allocation failed";
            status_t decoderErr = mFLACDecoder->decodeOneFrame((uint8_t *)mInputBuffer, size,
                                                               out_buf, &outSize, outputFloat);
            if (decoderErr != OK) {
                if (out_buf) {
                    free(out_buf);
                    out_buf = nullptr;
                }
                ASSERT_TRUE(false) << "decodeOneFrame returns error " << decoderErr;
            }
...
TEST_P(FLACDecoderTest, FlushTest) {
    ...
    ASSERT_NO_FATAL_FAILURE(processFlacDecoder(Info, 0, Info.size() / 3, outputFloat, ostrm));
    mFLACDecoder->flush();
    mHasStreamInfo = false;
    ASSERT_NO_FATAL_FAILURE(processFlacDecoder(
            Info, (Info.size() / 3), Info.size() - (Info.size() / 3), outputFloat, ostrm));
    ostrm.close();
    Info.clear();
}
...
TEST_P(FLACDecoderTest, DecodeTest) {
    ...
    ASSERT_NO_FATAL_FAILURE(processFlacDecoder(Info, 0, Info.size(), outputFloat, ostrm));
    ostrm.close();
    Info.clear();
}
