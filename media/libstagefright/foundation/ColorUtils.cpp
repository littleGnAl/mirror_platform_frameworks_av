...
void ColorUtils::setHDRStaticInfoIntoFormat(
    ...
    if (info.mID == HDRStaticInfo::kType2) {
    ...
    } else if (info.mID == HDRStaticInfo::kType1) {
        ...
        ALOGI("ignoring unsupported hdr-static-info format %d", info.mID);
    }
}
...
bool ColorUtils::getHDRStaticInfoFromFormat(const sp<AMessage> &format, HDRStaticInfo *info) {
    ...
    if (*data == HDRStaticInfo::kType2 && buf->size() == kHdrstaticinfo_type2_size) {
        ...
        return true;
    } else if (*data == HDRStaticInfo::kType1 && buf->size() == kHdrstaticinfo_type1_size) {
        ...
        ALOGW ("unsupported static metadatatype %u and size %zu", *data, buf->size());
    }
    return false;
}
