...
TEST_P(OpusHeaderWriteTest, WriteTest) {
    ...
    int32_t headerSize = WriteOpusHeaders(opusHeader, kOpusSampleRate, headerData,
...
TEST_P(OpusHeaderParseTest, ParseTest) {
    ...
    bool status = GetOpusHeaderBuffers(mInputBuffer, fileSize, &opusHeadBuf, &opusHeadSize,
                                       &codecDelayBuf, &codecDelayBufSize, &seekPreRollBuf,
                                       &seekPreRollBufSize);
    if (!isHeaderValid) ASSERT_EQ(opusHeadBuf, nullptr);
    if (!isCodecDelayValid) ASSERT_EQ(codecDelayBuf, nullptr);
    if (!isSeekPreRollValid) ASSERT_EQ(seekPreRollBuf, nullptr);
    if (!status) {
...
INSTANTIATE_TEST_SUITE_P(
        ...
        ::testing::Values(
                ...
                make_tuple("2ch_invalid_headerlength_16.opus", 2, false, true, true, false),
                make_tuple("2ch_invalid_headerlength1_256.opus", 2, false, false, false, false),
                make_tuple("2ch_invalid_headerlength2_256.opus", 2, false, true, true, false),
                make_tuple("2ch_invalid_size.opus", 2, true, true, false, false),
                ...
                make_tuple("3ch_invalid_nodelay.opus", 3, true, false, false, false),
                make_tuple("3ch_invalid_noperoll.opus", 3, true, true, false, false)));
