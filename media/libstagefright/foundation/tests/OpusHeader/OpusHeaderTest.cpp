...
class OpusHeaderParseTest
    : public OpusHeaderTest,
      public ::testing::TestWithParam<tuple<string /* InputFileName */, int32_t /* ChannelCount */,
                                            bool /* isOpusHeadValid */>> {};
...
TEST_P(OpusHeaderParseTest, ParseTest) {
    ...
    bool isOpusHeadValid = get<2>(params);
    ...
    bool status = GetOpusHeaderBuffers(mInputBuffer, fileSize, &opusHeadBuf, &opusHeadSize,
                                       ...
                                       &seekPreRollBufSize);
    if (!status) {
        ASSERT_FALSE(isOpusHeadValid) << "GetOpusHeaderBuffers failed";
        return;
    }
    status = ParseOpusHeader((uint8_t *)opusHeadBuf, opusHeadSize, &header);

    if (status) {
        ASSERT_TRUE(isOpusHeadValid) << "Parse opus header didn't fail for invalid input";
    } else {
        ASSERT_FALSE(isOpusHeadValid);
        return;
    }
