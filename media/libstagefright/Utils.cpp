...
status_t convertMetaDataToMessage(
    ...
    if (meta->findData(kKeyAVCC, &type, &data, &size)) {
        ...
        if (ptl_present_flag) {
            ...
            const uint8_t  num_bytes_constraint_info = br.getBits(6);       // unsigned int(6) num_bytes_constraint_info;
            ...
            if (num_bytes_constraint_info) {
                br.skipBits(8*num_bytes_constraint_info - 2);  // unsigned int(8*num_bytes_constraint_info - 2) general_constraint_info;
            }

            uint8_t  ptl_sublayer_level_present_flag[8] = {0};
            for (int8_t j = num_sublayers - 2; j >= 0; j--) {
                ptl_sublayer_level_present_flag[j] = br.getBits(1);              // unsigned int(1) ptl_sublayer_level_present_flag[i];
            }
            ...
            const uint16_t ptl_num_sub_profiles = br.getBits(8);              // unsigned int(8) ptl_num_sub_profiles;
        ...
        for (uint8_t i = 0; i < num_of_arrays; i++) {
            ...
            for (int j = 0; j < num_nalus; j++) {
                ...
                status_t err = copyNALUToABuffer(&buffer, ptr + (totalBits - br.numBitsLeft())/8, nal_unit_length);
                if (err != OK) {
                    return err;
                }
