...
status_t convertMetaDataToMessage(
    ...
    if (!strncasecmp("video/", mime, 6) ||
        ...
        if (meta->findData(kKeyHdrStaticInfo, &type, &data, &size) && type == 'hdrS') {
            ...
            if ((info->mID == HDRStaticInfo::kType1 && size == kHdrstaticinfo_type1_size)
                    || (info->mID == HDRStaticInfo::kType2 && size == kHdrstaticinfo_type2_size)) {
        ...
        if (msg->contains("hdr-static-info")) {
            ...
            if (ColorUtils::getHDRStaticInfoFromFormat(msg, &info)) {
                if (info.mID == HDRStaticInfo::kType1) {
                    ...
                    meta->setData(kKeyHdrStaticInfo, 'hdrS', &info, kHdrstaticinfo_type2_size);
                } else {
                    ALOGW ("unsupported info type %u", info.mID);
                }
            }
        }
