...
static void getProfileLevelInfo(ALookup<uint8_t, int32_t> &profiles,
                             ALookup<uint8_t, int32_t> &levels) {
    profiles = {
...
static void parseDolbyVisionProfileLevelFromDvcc(const uint8_t *ptr, size_t size, sp<AMessage> &format) {
    ...
    ALookup<uint8_t, int32_t> profiles{};
    ALookup<uint8_t, int32_t> levels{};
    getProfileLevelInfo(profiles, levels);
    ...
    if (msg->findBuffer("csd-0", &csd0)) {
        ...
        if (mime == MEDIA_MIMETYPE_VIDEO_AVC) {
            ...
            if (!msg->findBuffer("csd-2", &csd2)) {
                // we are going to create dvcc here
