...
static_assert(sizeof(kM3U8MarkerSuffix) == sizeof(kBinMarkerSuffix),
              "Marker suffix should be same size");
...
private:
  ...
  void initDataFile(string fileName, const uint8_t* data, size_t size);
...
void HttpLiveFuzzer::initDataFile(string fileName, const uint8_t* data, size_t size) {
  ofstream file;
...
void HttpLiveFuzzer::createFiles(const uint8_t* data, size_t size) {
  ...
  while (searchForMarker()) {
    ...
    if (isM3U8Marker(location)) {
      ...
      filePtr = const_cast<uint8_t*>(&mData[location]);
      fileSize = bytesRemaining - location;
      string m3u8fileName =
          kM3U8IndexFilePrefix + to_string(m3u8fileIndex) + kM3U8IndexFileSuffix;
      initDataFile(m3u8fileName, filePtr, fileSize);
      ++m3u8fileIndex;
      ...
      initDataFile(binfileName, filePtr, fileSize);
      ++binfileIndex;
    }
    bytesRemaining -= (fileSize + kMarkerSize + kMarkerSuffixSize);
    --mReadIndex;
  }
  if (m3u8fileIndex == 0) {
  ...
  if (binfileIndex == 0) {
    string fileName = kBinFilePrefix + to_string(binfileIndex) + kBinFileSuffix;
    initDataFile(fileName, mData, mSize);
  }
}
...
void HttpLiveFuzzer::invokeLiveSession() {
  ...
  mConditionalVariable.wait_until(waitForDownloadComplete, now + std::chrono::seconds(kWaitTime),
