...
#define encodedFile "/data/local/tmp/Mpeg4H263EncoderTest/Mpeg4H263Output"

constexpr int8_t kIDRFrameRefreshIntervalInSec = 1;
constexpr int32_t kOutputBufferSize = 720 * 1024;
    ...
    void SetUp() override {
        tuple<string /* fileName */, bool /* isMpeg4 */, int32_t /* frameWidth */,
          int32_t /* frameHeight */, float /* frameRate */, int32_t /* bitRate */>
            params = GetParam();
        mFilePath = gEnv->getRes();
        mFileName = get<0>(params);
...
void Mpeg4H263EncoderTest::initEncoderParams() {
    ...
    mEncodeHandle->bitRate[0] = mFrameRate * 1024;
...
void Mpeg4H263EncoderTest::processEncoder() {
...
TEST_P(Mpeg4H263EncoderTest, InitEncodeTest) {
    bool status = PVInitVideoEncoder(mEncodeControl, mEncodeHandle);
    ASSERT_TRUE(status) << "Failed to initialize the encoder!";
}

TEST_P(Mpeg4H263EncoderTest, EncodeTest) {
    ASSERT_NO_FATAL_FAILURE(processEncoder());

    bool status;
    int64_t numFramesEncoded = 0;
    int32_t bytesRead;
    ...
    while(1) {
        ...
        videoIn.timestamp = (numFramesEncoded * 1000) / mFrameRate;  // in ms.
        ...
        int32_t dataLength = kOutputBufferSize, nLayer = 0;
