...
void Mpeg4H263DecoderTest::processMpeg4H263Decoder(vector<FrameInfo> Info, int32_t offset,
    ...
    size_t outputBufferSize = maxWidth * maxHeight * 3 / 2;
    ...
    while (1) {
        ...
        bool volHeader = *mInputBuffer == 0xB0010000;
        if (volHeader) {
        ...
        if (!mInitialized) {
            ...
            bool codecConfig = (flags == CODEC_CONFIG_FLAG) ? true : false;
            ...
            PVGetVideoDimensions(mDecHandle, (int32 *)&dispWidth, (int32 *)&dispHeight);

            uint32_t bufWidth, bufHeight;
            PVGetBufferDimensions(mDecHandle, (int32 *)&bufWidth, (int32 *)&bufHeight);

            EXPECT_LE(dispWidth, bufWidth);
            EXPECT_LE(dispHeight, bufHeight);
        ...
        ASSERT_EQ(status, PV_TRUE) << "failed to decode video frame";
...
TEST_P(Mpeg4H263DecoderTest, FlushTest) {
    ...
    MP4DecodingMode inputMode = isMpeg4 ? MPEG4_MODE : H263_MODE;
    ASSERT_NO_FATAL_FAILURE(
            processMpeg4H263Decoder(Info, 0, Info.size() / 3, mEleStream, ostrm, inputMode));
