...
class AmrwbDecoderTest : public ::testing::TestWithParam<string> {
  public:
    ...
    int32_t DecodeFrames(int16_t *decoderCookie, void *decoderBuf, SNDFILE *handle,
...
int32_t AmrwbDecoderTest::DecodeFrames(int16_t *decoderCookie, void *decoderBuf, SNDFILE *handle,
                                       int32_t frameCount) {
    while (frameCount > 0) {
        uint8_t modeByte;
...
TEST_P(AmrwbDecoderTest, DecodeTest) {
    ...
    if (mFpInput == nullptr) {
        cout << "[   WARN   ] Test Skipped. Could not open " << GetParam() << "\n";
        return;
    }
    ...
    int32_t decoderErr = DecodeFrames(decoderCookie, decoderBuf, handle);
...
TEST_P(AmrwbDecoderTest, ResetDecoderTest) {
    ...
    int32_t decoderErr = DecodeFrames(decoderCookie, decoderBuf, handle, kNumFrameReset);
    ASSERT_EQ(decoderErr, 0) << "DecodeFrames returned error";
