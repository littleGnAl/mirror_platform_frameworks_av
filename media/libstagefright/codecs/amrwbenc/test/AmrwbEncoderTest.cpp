...
class AmrwbEncoderTest : public ::testing::TestWithParam<string> {
  public:
...
TEST_F(AmrwbEncoderTest, CreateAmrwbEncoderTest) {
    for (int32_t mode = VOAMRWB_MD66; mode <= VOAMRWB_MD2385; mode++) {
        ...
        ASSERT_EQ(status, VO_ERR_NONE)
                << "Failed to set AMRWB encoder frame type to " << mFrameType;
...
INSTANTIATE_TEST_SUITE_P(AmrwbEncoderTestAll, AmrwbEncoderTest,
                         ::testing::Values(("bbb_raw_1ch_16khz_s16le.raw")));
