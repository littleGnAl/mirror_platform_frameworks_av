...
sp<IMediaExtractor> MediaExtractorFactory::Create(
    const sp<DataSource> &source, const char *mime, const Vector<uint8_t> *drmUuid, const Vector<uint8_t> *drmSessionId) {
...
MediaExtractor::CreatorFunc MediaExtractorFactory::sniff(
    ...
    MediaExtractor::FreeMetaFunc *freeMeta, sp<ExtractorPlugin> &plugin, const Vector<uint8_t> *drmUuid) {
    *confidence = 0.0f;
    *meta = nullptr;
    ...
    for (auto it = plugins->begin(); it != plugins->end(); ++it) {
    	...
    	if (drmUuid != NULL) {
            if ((*it)->def.def_version >= 2 && (curCreator = (*it)->def.cah_handle_drm_scheme(drmUuid)) != NULL) {
                *confidence = 1.0f;
