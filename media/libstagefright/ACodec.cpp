...
status_t ACodec::configureCodec(
    ...
    if (mIsVideo || mIsImage) {
        ...
        if (!msg->findInt32("channel-count", &numChannels)
            ...
            || !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_DTS_HD_PROFILE_MA)
            || !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_DTS_HD_PROFILE_LBR)
            || !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_DTS_UHD)
            || !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_DTS_UHD_PROFILE_P2)) {
        int32_t numChannels;
        int32_t sampleRate;
        if (!msg->findInt32("channel-count", &numChannels)
                || !msg->findInt32("sample-rate", &sampleRate)) {
