...
sp<ABuffer> ElementaryStreamQueue::dequeueAccessUnitH265() {
    ...
    while ((err = getNextNALUnit(&data, &size, &nalStart, &nalSize)) == OK) {
        ...
        if (sliceType == 35) {
            // access_unit_delemeter_rbsp()
            if (foundSlice && (preVCLIndex > 0)) {
                flush = true;
            }
            foundSlice = true;
        } else if (sliceType <=3 || (sliceType >= 16 && sliceType <=21)) {
            // sice_segment_layer_rbsp()

            // first_slice_sgement_in_pic_flag
