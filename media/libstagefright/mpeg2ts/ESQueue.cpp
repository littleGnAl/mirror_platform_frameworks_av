...
static status_t ExtractVarLenBitFields(
    ABitReader *bits, size_t *bitsUsed, uint32_t *value, unsigned ucTable[], bool extractAndAddFlag)
{
    ...
    const uint8_t *ptr, size_t size, unsigned &frameSize, sp<MetaData> *metaData)
{
    ...
    if (isSyncFrameFlag) {
        ...
        if(!(bits.getBits(1))) { //FullChannelBasedMixFlag, ETSI TS 103 491 V1.2.1, Section 6.4.6.1
    ...
    if (metaData != NULL) {
        (*metaData)->setCString(kKeyMIMEType, MEDIA_MIMETYPE_AUDIO_VND_DTSUHD_PROFILE_P2);
...
//Parse DTS Digital Surround, and DTS LBR(Express) stream header
static status_t ParseDTSHDSyncFrame(
    ...
    //Expecting Digital Surround or DTS Express(LBR) streams only 
    if ((dtshdSyncWord != DTSHD_SYNC_CORE_16BIT_BE) && (dtshdSyncWord != DTSHD_SYNC_EXSS_16BIT_BE)){
        return ERROR_MALFORMED;
    }
    ...
    if (dtshdSyncWord == DTSHD_SYNC_CORE_16BIT_BE) { //DTS Digital Surround Header
        ...
        uint32_t nAuPr, nSS, numAudioPresnt = 1, numAssets = 1;
        ...
        if (staticFieldsPresent) {
            ...
            bits.skipBits(3 + 3); //numAudioPresnt, numAssets
    ...
    if (metaData != NULL) {
        if(isLBR) {
            (*metaData)->setCString(kKeyMIMEType, MEDIA_MIMETYPE_AUDIO_VND_DTSHD_PROFILE_LBR);
        } else {
            (*metaData)->setCString(kKeyMIMEType, MEDIA_MIMETYPE_AUDIO_VND_DTS);
        }
    ...
    if (!isScrambled() && (mBuffer == NULL || mBuffer->size() == 0)) {
        switch (mMode) {
            ...
            case DTS_HD:
            {
                ...
                if (startOffset > 0) {
                    ALOGI("found something resembling an DTS-HD syncword at "
                          "offset %zd",
                          startOffset);
                }
            ...
            case DTS_UHD:
            {
                ...
                if (startOffset >= 0) {
                    ALOGI("found something resembling an DTS UHD syncword"
                          "syncword at offset %zd",
                          startOffset);
                }
...
sp<ABuffer> ElementaryStreamQueue::dequeueAccessUnitDTSHD() {
    unsigned syncStartPos = 0; // in bytes
    unsigned payloadSize = 0;
    sp<MetaData> format = new MetaData;

    ALOGV("dequeueAccessUnitDTSHD[%d]: mBuffer %p(%zu)", mAUIndex,
          mBuffer->data(), mBuffer->size());
    ...
    if (mBuffer->size() < syncStartPos + payloadSize) {
        ALOGV("Not enough buffer size for DTS-HD");
        return NULL;
    }
