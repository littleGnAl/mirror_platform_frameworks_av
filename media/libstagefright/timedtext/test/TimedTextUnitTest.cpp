...
TEST_P(SRTDescriptionTest, extractSRTDescriptionTest) {
    char data[mFileSize];
    mEleStream.read(data, mFileSize);
...
TEST_P(Text3GPPDescriptionTest, Text3GPPGlobalDescriptionTest) {
    ...
    int32_t remaining = -1;
    FontInfo fontInfo;
    vector<FontRecord> fontRecordEntry;

    tmpData += 16;
    remaining = mFileSize - 16;
        ...
        if (remaining >= 12) {
            ...
            if (remaining >= 10) {
                ...
                for (int32_t i = 0; i < fontInfo.entryCount; i++) {
                    ...
                    ALOGI("Font: %s", tmpFont);
                    tmpData += tempFontNameLength;
                    remaining -= tempFontNameLength;
                    fontRecordEntry.push_back({tempFontID, tempFontNameLength, tmpFont});
                }
            }
        }
    }
    ...
    for (int32_t i = 0; i < fontInfo.entryCount; i++) {
        ASSERT_EQ(parcel.readInt32(), fontRecordEntry[i].fontID)
                << "Parcel has invalid value of font Id";
        ASSERT_EQ(parcel.readInt32(), fontRecordEntry[i].fontNameLength)
                << "Parcel has invalid value of font name length";
        uint8_t fontName[fontRecordEntry[i].fontNameLength];
        status = parcel.read((void *)fontName, fontRecordEntry[i].fontNameLength);
        ASSERT_EQ(status, 0) << "Failed to read the font name from parcel";
        ASSERT_EQ(memcmp(fontName, fontRecordEntry[i].font, fontRecordEntry[i].fontNameLength), 0)
