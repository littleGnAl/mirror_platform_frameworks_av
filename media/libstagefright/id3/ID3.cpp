...
struct id3_header {
    ...
    if (header.flags & 0x80) {
        ALOGV("has Global unsynchronization (and we'll defer)");
        hasGlobalUnsync = true;
        // we have to wait on applying global unsynchronization
    ...
    } else if (hasGlobalUnsync) {
        // now it's safe to apply the unsynchronization for the non-2.4 frames
