...
#ifndef VIDEO_RENDER_METRICS_TRACKER_H_

#define VIDEO_RENDER_METRICS_TRACKER_H_
...
// A variety of video rendering metrics.
struct VideoRenderMetrics {
    VideoRenderMetrics();
    ...
    // The frame rate as detected by looking frame durations derived from the content timestamps.
...
// This class anaylzes various timestamps related to video rendering to compute a set of metrics
// that attempt to capture the user experience of a video playback.
//
// The following timestamps (in microseconds) are analyzed to compute these metrics:
//   * The content timestamp found in the content stream, indicating the position of each vidoe
//     frame.
...
//
class VideoRenderMetricsTracker {
public:
    class Configuration {
    public:
        Configuration();
        ...
        // tolerance to determine whether the app is intentionally skipping forward or whether the
    ...
    VideoRenderMetricsTracker();
    VideoRenderMetricsTracker(const Configuration& configuration);
    ...
    const VideoRenderMetrics& getMetrics() const;

    // Called when a change in codec state will result in a content discontinuity - e.g. flush.
    void resetForDiscontinuity();

private:
    // Tracking of frames that have been released to be rendered on the display.
    struct FrameInfo {
    ...
    static void updateFrameRate(float & frameRate, FrameDurationUs & durationUs,
    ...
    // Update the metrics beecause a dropped frame was detected.
    ...
    // Various configuration parameters for the internal algorithms for tracking render metrics.
    const Configuration mConfiguration;

    // Metrics that are updated every time a frame event occurs - when the app releases the frame
    // to the surface or skips the frame, or when a frame is rendered to the display.
    VideoRenderMetrics mMetrics;

    // The most recently processed timestamp referring to the position in the content stream.
    int64_t mLastContentTimeUs;

    // The most recently processed timestamp at which a video frame was rendered.
    ...
    // Frame durations derived from timestamps passed in by the app, indicating when the app would
    // like to have the frame rendered.
    FrameDurationUs mDesiredFrameDurationUs;

    // Frame durations derived from timestamps captured when the frame is actually rendered.
    FrameDurationUs mActualFrameDurationUs;
};

}  // namespace android

#endif  // VIDEO_RENDER_METRICS_TRACKER_H_
