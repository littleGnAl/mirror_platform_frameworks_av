...
status_t MPEG4Writer::Track::getDolbyVisionProfile() {
    ...
    ALookup<uint8_t, int32_t> dolbyVisionProfileMap = {
    ...
    // https://dolby.my.salesforce.com/sfc/p/#700000009YuG/a/
    // 4u000000l6FB/076wHYEmyEfz09m0V1bo85_25hlUJjaiWTbzorNmYY4
    if (!dolbyVisionProfileMap.map(((((char *)data)[2] >> 1) & 0x7f),&mDoviProfile)) {
                ...
                if (mIsDovi) {
                    ...
                    if(err == OK) {
                        ...
                        if (mDoviProfile == DolbyVisionProfileDvavSe) {
                            ...
                            ALOGW("Dolby Vision Profiles > DolbyVisionProfileDvavSe are not supported");
                            err = ERROR_MALFORMED;
                        }
                        if (err == OK && data != NULL && copyCodecSpecificData((uint8_t *)data, size) == OK) {
                            mGotAllCodecSpecificData = true;
                        }
                    }
                }
...
void MPEG4Writer::Track::writeVideoFourCCBox() {
    ...
    if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_MPEG4, mime)) {
        ...
        if (mDoviProfile <= DolbyVisionProfileDvheSt) {
          ...
          ALOGE("Unsupported Dolby Vision profile");
          TRESPASS();
        }
        writeDoviConfigBox();
    }
