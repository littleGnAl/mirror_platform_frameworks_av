    ...
    if (!mGetCodecInfo) {
        mGetCodecInfo = [&log = mErrorLog](const AString &name,
            ...
            *info = nullptr;
            ...
            if (!mcl) {
                log.log("Fatal error: failed to initialize MediaCodecList");
            ...
            log.log(base::StringPrintf("Codec with name '%s' is not found on the device.",
                                  name.c_str()));
...
void MediaCodec::updateMediametrics() {
    if (mMetricsHandle == 0) {
        mErrorLog.log("Fatal error: no metrics handle found");
        return;
    }
...
void MediaCodec::updatePlaybackDuration(const sp<AMessage> &msg) {
    ...
    size_t index = 0;
    while (msg->findInt64(base::StringPrintf("%zu-system-nano", index++).c_str(), &renderTimeNs)) {
...
status_t MediaCodec::init(const AString &name) {
    ...
    if (err != OK) {
        mErrorLog.log(base::StringPrintf(
    ...
    if (mCodec == NULL) {
        ...
        mErrorLog.log(base::StringPrintf(
                "Getting codec base with name '%s' (owner='%s') failed", name.c_str(), owner));
        return NAME_NOT_FOUND;
    }
    ...
    if (mDomain == DOMAIN_VIDEO) {
        ...
        if (mCodecLooper == NULL) {
            ...
            if (OK != err) {
                ...
                mErrorLog.log("Fatal error: codec looper failed to start");
                return err;
            }
        }
...
status_t MediaCodec::getBufferAndFormat(
    ...
    if (mReleasedByResourceManager) {
        ...
        mErrorLog.log("resource already released");
        return DEAD_OBJECT;
    }

    if (buffer == NULL) {
        ALOGE("getBufferAndFormat - null MediaCodecBuffer");
        mErrorLog.log("null buffer");
        return INVALID_OPERATION;
    }

    if (format == NULL) {
        ALOGE("getBufferAndFormat - null AMessage");
        mErrorLog.log("null format");
        return INVALID_OPERATION;
    }
    ...
    if (index >= buffers.size()) {
        ...
        mErrorLog.log(base::StringPrintf("Bad index (index=%zu)", index));
        return INVALID_OPERATION;
    }
    ...
    if (!info.mOwnedByClient) {
        ...
        mErrorLog.log(base::StringPrintf("index %zu is not owned by client", index));
        return INVALID_OPERATION;
    }
...
void MediaCodec::cancelPendingDequeueOperations() {
    if (mFlags & kFlagDequeueInputPending) {
        mErrorLog.log("Pending dequeue input buffer request cancelled");
    ...
    if (mFlags & kFlagDequeueOutputPending) {
        mErrorLog.log("Pending dequeue output buffer request cancelled");
...
bool MediaCodec::handleDequeueInputBuffer(const sp<AReplyToken> &replyID, bool newRequest) {
    if (!isExecuting()) {
        ...
        mErrorLog.log("Invalid to call in async mode");
        PostReplyWithError(replyID, INVALID_OPERATION);
    } else if (newRequest && (mFlags & kFlagDequeueInputPending)) {
        mErrorLog.log("Invalid to call while another dequeue input request is pending");
        PostReplyWithError(replyID, INVALID_OPERATION);
        return true;
...
bool MediaCodec::handleDequeueOutputBuffer(const sp<AReplyToken> &replyID, bool newRequest) {
    if (!isExecuting()) {
        ...
        mErrorLog.log("Invalid to call while another dequeue output request is pending");
        PostReplyWithError(replyID, INVALID_OPERATION);
    } else if (mFlags & kFlagStickyError) {
        PostReplyWithError(replyID, getStickyError());
    ...
    switch (msg->what()) {
        ...
        case kWhatSetCallback:
        {
            ...
            if (mState == UNINITIALIZED
                ...
                mErrorLog.log(base::StringPrintf(
                        "Callback can only be set at Initialized state; currently %s",
                        apiStateString().c_str()));
                PostReplyWithError(replyID, INVALID_OPERATION);
                break;
            }
        ...
        case kWhatConfigure:
        {
            ...
            if (flags & CONFIGURE_FLAG_USE_BLOCK_MODEL) {
                if (!(mFlags & kFlagIsAsync)) {
                    mErrorLog.log("Block model is only valid with callback set (async mode)");
                    PostReplyWithError(replyID, INVALID_OPERATION);
                    break;
                }
            ...
            switch (mState) {
                ...
                case FLUSHED:
                {
                    ...
                    if (mSurface == NULL) {
                        ...
                        mErrorLog.log("Cannot set surface if the codec is not configured with "
                                      "a surface already");
                        err = INVALID_OPERATION;
                    } else if (obj == NULL) {
                        // do not support unsetting surface
                        mErrorLog.log("Unsetting surface is not supported");
                        err = BAD_VALUE;
                    } else {
        ...
        case kWhatRelease:
        {
            ...
            if (reclaimed) {
                if (!mReleasedByResourceManager) {
                    ...
                    mErrorLog.log("Released by resource manager");
        ...
        case kWhatDequeueInputBuffer:
        {
            ...
            if (mFlags & kFlagIsAsync) {
                ...
                mErrorLog.log("dequeueInputBuffer can't be used in async mode");
                PostReplyWithError(replyID, INVALID_OPERATION);
                break;
            }
            ...
            if (mHaveInputSurface) {
                ...
                mErrorLog.log("dequeueInputBuffer can't be used with input surface");
                PostReplyWithError(replyID, INVALID_OPERATION);
                break;
            }
        ...
        case kWhatDequeueOutputBuffer:
        {
            ...
            if (mFlags & kFlagIsAsync) {
                ...
                mErrorLog.log("dequeueOutputBuffer can't be used in async mode");
                PostReplyWithError(replyID, INVALID_OPERATION);
                break;
            }
        ...
        case kWhatSignalEndOfInputStream:
        {
            if (!isExecuting()) {
                ...
                mErrorLog.log("signalEndOfInputStream() called without an input surface set");
                PostReplyWithError(msg, INVALID_OPERATION);
                break;
            } else if (mFlags & kFlagStickyError) {
        ...
        case kWhatGetBuffers:
        {
            ...
            if (!isExecuting()) {
                ...
                mErrorLog.log("getInput/OutputBuffers() is not supported with callbacks");
                PostReplyWithError(replyID, INVALID_OPERATION);
                break;
            } else if (mFlags & kFlagStickyError) {
                PostReplyWithError(replyID, getStickyError());
        ...
        case kWhatGetOutputFormat:
        {
            ...
            if (mState != CONFIGURED && mState != STARTING &&
                ...
                mErrorLog.log("Fatal error: format is not initialized");
                PostReplyWithError(replyID, INVALID_OPERATION);
                break;
            } else if (mFlags & kFlagStickyError) {
                PostReplyWithError(replyID, getStickyError());
        ...
        case kWhatGetName:
        {
            ...
            if (mComponentName.empty()) {
                mErrorLog.log("Fatal error: name is not set");
                PostReplyWithError(replyID, INVALID_OPERATION);
                break;
            }
        ...
        if (hasCryptoOrDescrambler()) {
            ...
            if (mem == nullptr) {
                ...
                sDealer = new MemoryDealer(
                        newDealerCapacity,
                        base::StringPrintf("CSD(%luMB)", newDealerCapacity / 1048576).c_str());
            ...
            if (view.error() != C2_OK) {
                mErrorLog.log("Fatal error: failed to allocate and map a block");
                return -EINVAL;
            }
...
status_t MediaCodec::onQueueInputBuffer(const sp<AMessage> &msg) {
    ...
    if (msg->findSize("size", &size)) {
        ...
        if (!hasCryptoOrDescrambler()) {
            ALOGE("[%s] queuing secure buffer without mCrypto or mDescrambler!",
                    mComponentName.c_str());
            mErrorLog.log("queuing secure buffer without mCrypto or mDescrambler!");
            return -EINVAL;
        }
    ...
    if (index >= mPortBuffers[kPortIndexInput].size()) {
        mErrorLog.log(base::StringPrintf("index out of range (index=%zu)",
    ...
    if (c2Buffer || memory) {
        ...
        if (c2Buffer) {
            ...
            mErrorLog.log("Fatal error: invalid queue request without a buffer");
            err = UNKNOWN_ERROR;
        }
...
status_t MediaCodec::onReleaseOutputBuffer(const sp<AMessage> &msg) {
    ...
    if (render && buffer->size() != 0) {
        ...
        if (err == NO_INIT) {
            ALOGE("rendering to non-initialized(obsolete) surface");
            mErrorLog.log("rendering to non-initialized(obsolete) surface");
            return err;
        }
        if (err != OK) {
