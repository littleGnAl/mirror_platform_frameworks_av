...
MediaCodec::MediaCodec(
    ...
    if (!mGetCodecInfo) {
        mGetCodecInfo = [&log = mErrorLog](const AString &name, sp<MediaCodecInfo> *info) -> status_t {
            *info = nullptr;
            ...
            log.log(AStringPrintf("Codec with name '%s' is not found on the device.",
                                  name.c_str()));
...
status_t MediaCodec::getBufferAndFormat(
    ...
    if (index >= buffers.size()) {
        ...
        mErrorLog.log(AStringPrintf("Bad index (index=%zu)", index));
        return INVALID_OPERATION;
    }
    ...
    if (!info.mOwnedByClient) {
        ...
        mErrorLog.log(AStringPrintf("index %zu is not owned by client", index));
        return INVALID_OPERATION;
    }
...
status_t MediaCodec::onQueueInputBuffer(const sp<AMessage> &msg) {
    ...
    if (index >= mPortBuffers[kPortIndexInput].size()) {
        mErrorLog.log(AStringPrintf("index out of range (index=%zu)",
