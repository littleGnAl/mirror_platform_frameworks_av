...
status_t MediaCodec::queueInputBuffers(
    ...
    int infoIdx = 0;
    sp<WrapperObject<std::vector<AccessUnitInfo>>> auInfo{
            (decltype(auInfo.get()))infos.get()};
    if (auInfo->value.size() <= 0) {
        return BAD_VALUE;
    }
...
status_t MediaCodec::queueBuffer(
    ...
    sp<WrapperObject<std::vector<AccessUnitInfo>>> bufferInfos{
