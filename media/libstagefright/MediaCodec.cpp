...
MediaCodec::MediaCodec(
    ...
    if (!mGetCodecInfo) {
        mGetCodecInfo = [](const AString &name, status_t *err) -> sp<MediaCodecInfo> {
            *err = OK;
            const sp<IMediaCodecList> mcl = MediaCodecList::getInstance();
            if (!mcl) {
                *err = NO_INIT;  // if called from Java should raise IOException
                return nullptr;
            }
            AString tmp = name;
            ...
            for (const AString &codecName : { name, tmp }) {
                ...
                return mcl->getCodecInfo(codecIdx);
            }
            *err = NAME_NOT_FOUND;
            return nullptr;
        };
...
status_t MediaCodec::init(const AString &name) {
    ...
    if (!name.startsWith("android.filter.")) {
        status_t err = OK;
        mCodecInfo = mGetCodecInfo(name, &err);
        if (err != OK) {
