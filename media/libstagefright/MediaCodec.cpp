...
static const char *kCodecContentFrameRate = "android.media.mediacodec.framerate.content";
static const char *kCodecDesiredFrameRate = "android.media.mediacodec.framerate.desired";
static const char *kCodecActualFrameRate = "android.media.mediacodec.framerate.actual";
...
void MediaCodec::updateMediametrics() {
    ...
    const VideoRenderMetrics& renderMetrics = mVideoRenderMetricsTracker.getMetrics();
    if (renderMetrics.frameRenderedCount > 0) {
        const VideoRenderMetrics & m = renderMetrics;
        mediametrics_setInt64(mMetricsHandle, kCodecFramesReleased, m.frameReleasedCount);
        mediametrics_setInt64(mMetricsHandle, kCodecFramesRendered, m.frameReleasedCount);
        ...
        mediametrics_setDouble(mMetricsHandle, kCodecContentFrameRate, m.contentFrameRate);
        mediametrics_setDouble(mMetricsHandle, kCodecDesiredFrameRate, m.desiredFrameRate);
        mediametrics_setDouble(mMetricsHandle, kCodecActualFrameRate, m.actualFrameRate);
    }
...
void MediaCodec::processRenderedFrames(const sp<AMessage> &msg) {
    ...
    if (msg->what() != kWhatCodecNotify && what != kWhatOutputFramesRendered) {
        ...
        if (!logged) {
            logged = true;
            ALOGE("processRenderedFrames: expected kWhatOuputFramesRendered (%d)", msg->what());
        }
        return;
    }
    // Rendered frames only matter if the buffers are going to the display
    if (mIsSurfaceToDisplay) {
        ...
        for (size_t index = 0;
            ...
            // Capture which frames are rendered for metrics
            ...
            mVideoRenderMetricsTracker.onFrameRendered(mediaTimeUs, renderTimeNs);
        }
    }
}
        ...
        case kWhatCodecNotify:
        {
            ...
            switch (what) {
                ...
                case kWhatStopCompleted:
                {
                    ...
                    if (mIsSurfaceToDisplay) {
                        mVideoRenderMetricsTracker.resetForDiscontinuity();
                    }
...
status_t MediaCodec::onReleaseOutputBuffer(const sp<AMessage> &msg) {
    ...
    if (render && buffer->size() != 0) {
        ...
        if (mIsSurfaceToDisplay) {
            noRenderTime ? mVideoRenderMetricsTracker.onFrameReleased(mediaTimeUs)
                         : mVideoRenderMetricsTracker.onFrameReleased(mediaTimeUs, renderTimeNs);
        ...
        if (mIsSurfaceToDisplay) {
            int64_t mediaTimeUs = -1;
            buffer->meta()->findInt64("timeUs", &mediaTimeUs);
            mVideoRenderMetricsTracker.onFrameSkipped(mediaTimeUs);
        }
        mBufferChannel->discardBuffer(buffer);
    }
