...
void MediaCodec::updateMediametrics() {
    ...
    AString profile;
    AString mediaType;
    if (mOutputFormat->findString(KEY_PROFILE, &profile)
...
hdr_format MediaCodec::getHDRFormat(const AString &profile, const AString &transfer,
    ...
    if (mIsVideo) {
        ...
        if (mMetricsHandle != 0) {
            ...
            if (format->findString(KEY_COLOR_STANDARD, &colorStandard)) {
                mediametrics_setCString(mMetricsHandle, kCodecConfigColorStandard,
                    colorStandard.c_str());
            }
            AString colorRange;
            if (format->findString(KEY_COLOR_RANGE, &colorRange)) {
                mediametrics_setCString(mMetricsHandle, kCodecConfigColorRange,
                    colorRange.c_str());
            }
            AString colorTransfer;
            if (format->findString(KEY_COLOR_TRANSFER, &colorTransfer)) {
                mConfigColorTransfer = colorTransfer;
                mediametrics_setCString(mMetricsHandle, kCodecConfigColorTransfer,
                    colorTransfer.c_str());
            }
            HDRStaticInfo info;
...
void MediaCodec::handleOutputFormatChangeIfNeeded(const sp<MediaCodecBuffer> &buffer) {
    ...
    if (mMetricsHandle != 0) {
        AString colorStandard;
        if (format->findString(KEY_COLOR_STANDARD, &colorStandard)) {
            mediametrics_setCString(mMetricsHandle, kCodecParsedColorStandard,
                colorStandard.c_str());
        }
        AString colorRange;
        if (format->findString(KEY_COLOR_RANGE, &colorRange)) {
            mediametrics_setCString(mMetricsHandle, kCodecParsedColorRange,
                colorRange.c_str());
        }
        AString colorTransfer;
        if (format->findString(KEY_COLOR_TRANSFER, &colorTransfer)) {
            mediametrics_setCString(mMetricsHandle, kCodecParsedColorTransfer,
                colorTransfer.c_str());
        }
    }
}
