...
struct MediaCodec::ResourceManagerServiceProxy :
    ...
    inline void setImportance(int priority) {
        mImportance = priority;
    }
...
private:
    ...
    inline void getClientInfo(ClientInfoParcel& clientInfo) {
        clientInfo.pid = static_cast<int32_t>(mPid);
        clientInfo.uid = static_cast<int32_t>(mUid);
        clientInfo.id = getId(mClient);
        clientInfo.name = mCodecName;
        clientInfo.importance = mImportance;
    }
...
void MediaCodec::ResourceManagerServiceProxy::notifyClientStarted(
    ...
    getClientInfo(clientConfig.clientInfo);
...
void MediaCodec::updateCodecImportance(const sp<AMessage>& msg) {
    ...
    if (msg->findInt32(KEY_CODEC_IMPORTANCE, &importance)) {
...
status_t MediaCodec::configure(
    ...
    // Update the codec importance.
    int32_t importance = 0;
    if (format->findInt32(KEY_CODEC_IMPORTANCE, &importance)) {
        // Ignoring the negative importance.
        if (importance >= 0) {
            mResourceManagerProxy->setImportance(importance);
        }
