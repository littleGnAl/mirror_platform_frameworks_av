...
void MediaCodec::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        ...
        case kWhatRelease:
        {
            ...
            if (mReplyID) {
                postPendingReply(UNKNOWN_ERROR);
            }
            mReplyID = replyID;
...
void MediaCodec::postPendingReply(status_t err /* = OK */) {
    sp<AMessage> response{new AMessage};
    if (err != OK) {
        response->setInt32("err", err);
    }
...
void MediaCodec::postPendingReply(const sp<AMessage> &response) {
    CHECK(mReplyID);
