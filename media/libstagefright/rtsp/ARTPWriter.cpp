...
void ARTPWriter::addSR(const sp<ABuffer> &buffer) {
    ...
    // A current rtpTime can be calcuated from ALooper::GetNowUs().
    ...
    // Note: ALooper::GetNowUs() is same with System.currentTimeMillis() of java.
...
uint32_t ARTPWriter::getRtpTime(int64_t timeUs) {
    int32_t clockPerMs = mClockRate / 1000LL;
    int64_t rtpTime = mRTPTimeBase + (timeUs * clockPerMs / 1000L);
