            ...
            if (chunk_type != FOURCC('e', 'n', 'c', 'a')) {
                ...
                if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_RAW, FourCC2MIME(chunk_type))) {
                    ...
                    if (chunk_type == FOURCC('t', 'w', 'o', 's')) {
                        mLastTrack->meta->setCString(kKeyPcmByteOrder, "big");
                    } else {
                        mLastTrack->meta->setCString(kKeyPcmByteOrder, "little");
                    }
                }
            }
...
MPEG4Source::MPEG4Source(
    ...
    if (mIsPcm) {
        ...
        if (err != OK || maxSampleSize != static_cast<size_t>(pcmSampleSize)) {
            ALOGW("PCM track with variable block size");
            mIsPcm = false;
        } else {
    ...
    if ((!mIsAVC && !mIsHEVC) || mWantsNALFragments) {
        if (newBuffer) {
            if (mIsPcm) {
                ...
                CHECK(mFormat->findCString(kKeyPcmByteOrder, &byteOrder));

                if (!strcasecmp(byteOrder, "big")) {
                    //Big-endian -> little-endian
                    if (size == 2 || size == 4) {
                        uint16_t *dstData = (uint16_t *)buf;
                        uint16_t *srcData = (uint16_t *)buf;
                        size_t multiplier = size / sizeof(uint16_t);

                        for (size_t j = 0; j < samplesToRead * multiplier; j++) {
