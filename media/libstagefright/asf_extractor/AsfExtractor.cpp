    ...
    while(timeArray.size() < 20) {
        ...
        while(payload) {
            ...
            if (lastTimeStamp == 0) {
                ...
                uint64_t timestamp = payload->presentationTime *1000;
                if (lastTimeStamp != timestamp) {
                    timeArray.push_back(timestamp);
                    lastTimeStamp = timestamp;
                    ALOGV("Add timestamp = %" PRIu64, lastTimeStamp);
                }
            }
            payload = payload->next;
        }
...
status_t AsfExtractor::initialize() {
    ...
    if (fileMediaInfo && fileMediaInfo->seekable) {
        ...
        for (;;) {
            ...
            objectSize = *(ssize_t*)(objectHeader + 16);
...
status_t AsfExtractor::setupTracks() {
    ...
    while (audioInfo || videoInfo) {
        ...
        track->bufferActive = NULL;
        track->bufferGroup = new MediaBufferGroup(8, 64*1024*1024);
...
status_t AsfExtractor::seek_l(Track* track, int64_t seekTimeUs, MediaSource::ReadOptions::SeekMode mode) {
    ...
    ALOGV("data packet offset = %" PRId64, mDataPacketCurrentOffset);
...
status_t AsfExtractor::readPacket() {
    ...
    while (payload) {
        ...
        if (payload->mediaObjectLength == payload->payloadSize ||
            ...
            status = track->bufferGroup->acquire_buffer(
                &buffer, false, payload->mediaObjectLength);
            if (status != OK) {
            ...
            if (payload->mediaObjectLength == payload->payloadSize) {
                ...
                if (track->encrypted) {
                    ...
                    track->bufferGroup->acquire_buffer(&copy, false, payload->payloadSize);
