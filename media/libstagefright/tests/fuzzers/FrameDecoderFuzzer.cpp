...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    sp<IMediaSource> source =
            new IMediaSourceFuzzImpl(&fdp, MAX_MEDIA_BUFFER_SIZE);
        ...
        switch (fdp.ConsumeIntegralInRange<uint8_t>(0, 4)) {
            case 0:
                decoder->init(/*frameTimeUs*/ fdp.ConsumeIntegral<int64_t>(),
                              /*numFrames*/ fdp.ConsumeIntegral<size_t>(),
            ...
            case 3: {
                std::vector<sp<IMemory>> memory_vector;
                decoder->extractFrames(&memory_vector);
                break;
            }
            case 4: {
                sp<MetaData> trackMeta = generateMetaData(&fdp);
    ...
    if(isVideoDecoder) {
        delete reinterpret_cast<VideoFrameDecoder*>(decoder);
    } else {
        delete reinterpret_cast<ImageDecoder*>(decoder);
    }
