...
typedef std::vector<std::function<void(FuzzedDataProvider *, sp<MediaWriter>,
                                       sp<MetaData>, int tmpFileFd)>>
    OperationVec;
typedef std::vector<std::function<void(FuzzedDataProvider *, MPEG4Writer *)>>
    MPEG4OperationVec;
static const OperationVec operations = {
    [](FuzzedDataProvider *, sp<MediaWriter> mediaWriter, sp<MetaData>, int) { 
        mediaWriter->pause();
    },
    [](FuzzedDataProvider *dataProvider, sp<MediaWriter> mediaWriter,
       sp<MetaData>, int tmpFd) {
    ...
    [](FuzzedDataProvider *dataProvider, sp<MediaWriter> mediaWriter,
      ...
      // Likely won't do much, but might as well as do a quick sanity check
      // while we're here.
...
static const MPEG4OperationVec mpeg4Operations = {
    [](FuzzedDataProvider *, MPEG4Writer *mediaWriter) {
      mediaWriter->notifyApproachingLimit();
    },
    ...
    [](FuzzedDataProvider *dataProvider, MPEG4Writer *mediaWriter) {
      std::string strVal =
          dataProvider->ConsumeRandomLengthString(kMaxMPEGStrLen);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  std::string packageName =
      dataProvider.ConsumeRandomLengthString(kMaxPackageNameLen);
  ...
  for (size_t ops_run = 0;
       dataProvider.remaining_bytes() > 0 && ops_run < kMaxOperations - 1;
       ops_run++) {
    ...
    if (op < baseOpLen) {
      ...
      mpeg4Operations[op - baseOpLen](&dataProvider,
                                      (MPEG4Writer *)writer.get());
    } else {
      // Here for sanity, will error out.
      operations[op](&dataProvider, writer, fileMeta, tf.fd);
    }
  }
  writer->stop();
