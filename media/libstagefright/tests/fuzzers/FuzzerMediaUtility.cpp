...
std::string genMimeType(FuzzedDataProvider *dataProvider) {
  uint8_t idx =
      dataProvider->ConsumeIntegralInRange<uint8_t>(0, kMimeTypes.size() - 1);
  return std::string(kMimeTypes[idx]);
}

sp<IMediaExtractor> genMediaExtractor(FuzzedDataProvider *dataProvider,
                                      std::string mimeType,
                                      uint16_t maxDataAmount) {
  uint32_t dataBlobSize =
      dataProvider->ConsumeIntegralInRange<uint16_t>(0, maxDataAmount);
  ...
  sp<DataSource> source = DataSourceFactory::getInstance()->CreateFromURI(
      NULL /* httpService */, uri.c_str());
...
sp<MediaSource> genMediaSource(FuzzedDataProvider *dataProvider,
  ...
  sp<IMediaExtractor> extractor =
      genMediaExtractor(dataProvider, mime, maxMediaBlobSize);
...
sp<MediaWriter> createWriter(int fd, StandardWriters writerType,
                             sp<MetaData> fileMeta) {
