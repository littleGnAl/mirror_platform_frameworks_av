...
// #define LOG_NDEBUG 0
#define LOG_TAG "VideoRenderMetricsTracker_test"
#include <utils/Log.h>
...
using Metrics = VideoRenderMetrics;
using Configuration = VideoRenderMetricsTracker::Configuration;

class Helper {
public:
    Helper(double contentFrameDurationMs, Configuration configuration) :
            mVideoRenderMetricsTracker(configuration) {
    ...
    void render(std::initializer_list<T> renderDurationMsList) {
        for (auto renderDurationMs : renderDurationMsList) {
            mVideoRenderMetricsTracker.onFrameReleased(mMediaTimeUs);
            mVideoRenderMetricsTracker.onFrameRendered(mMediaTimeUs, mClockTimeNs);
    ...
    void skip(int numFrames) {
        for (int i = 0; i < numFrames; ++i) {
            mVideoRenderMetricsTracker.onFrameSkipped(mMediaTimeUs);
            mMediaTimeUs += mContentFrameDurationUs;
    ...
    const Metrics getMetrics() const {
        return mVideoRenderMetricsTracker.getMetrics();
    }
...
class VideoRenderMetricsTrackerTest : public ::testing::Test {
public:
    VideoRenderMetricsTrackerTest() {}
};

TEST_F(VideoRenderMetricsTrackerTest, countsReleasedFrames) {
    Configuration c;
    Helper h(16.66, c);
    h.drop(10);
    h.render({16.66, 16.66, 16.66});
    h.skip(10); // skipped frames aren't released so they shouldn't be counted
    h.render({16.66, 16.66, 16.66, 16.66});
    h.drop(10);
    EXPECT_EQ(27, h.getMetrics().frameReleasedCount);
}

TEST_F(VideoRenderMetricsTrackerTest, countsSkippedFrames) {
    Configuration c;
    Helper h(16.66, c);
    h.drop(10);
    h.skip(10); // frames skipped before rendering a frame aren't counted as skipped
    h.render({16.66, 16.66, 16.66});
    h.skip(10);
    h.render({16.66, 16.66, 16.66, 16.66});
    h.skip(10); // frames skipped at the end of playback aren't counted as skipped
    h.drop(10);
    EXPECT_EQ(10, h.getMetrics().frameSkippedCount);
}

TEST_F(VideoRenderMetricsTrackerTest, whenSkippedFramesAreDropped_countsDroppedFrames) {
    Configuration c;
...
TEST_F(VideoRenderMetricsTrackerTest, whenNotSkippedFramesAreDropped_countsDroppedFrames) {
    Configuration c;
...
TEST_F(VideoRenderMetricsTrackerTest, countsRenderedFrames) {
    Configuration c;
    Helper h(16.66, c);
    h.drop(10);
    h.render({16.66, 16.66, 16.66});
...
TEST_F(VideoRenderMetricsTrackerTest, detectsFrameRate) {
    Configuration c;
...
TEST_F(VideoRenderMetricsTrackerTest, whenLowTolerance_doesntDetectFrameRate) {
    Configuration c;
...
TEST_F(VideoRenderMetricsTrackerTest, whenFrameRateDestabilizes_detectsFrameRate) {
    Configuration c;
    c.frameRateDetectionToleranceUs = 2 * 1000; // 2 ms
    Helper h(16.66, c);
    h.render({16.6, 16.7, 16.6, 16.7});
...
TEST_F(VideoRenderMetricsTrackerTest, detects32Pulldown) {
    Configuration c;
    c.frameRateDetectionToleranceUs = 2 * 1000; // 2 ms
    Helper h(41.66, c);
...
TEST_F(VideoRenderMetricsTrackerTest, whenBad32Pulldown_doesntDetect32Pulldown) {
    Configuration c;
    c.frameRateDetectionToleranceUs = 2 * 1000; // 2 ms
    Helper h(41.66, c);
...
TEST_F(VideoRenderMetricsTrackerTest, whenFramRateChanges_detectsMostRecentFrameRate) {
    Configuration c;
    c.frameRateDetectionToleranceUs = 2 * 1000; // 2 ms
    Helper h(16.66, c);
    h.render({16.6, 16.7, 16.6, 16.7});
...
TEST_F(VideoRenderMetricsTrackerTest, whenFrameRateIsUnstable_doesntDetectFrameRate) {
    Configuration c;
    c.frameRateDetectionToleranceUs = 2 * 1000; // 2 ms
    Helper h(16.66, c);
