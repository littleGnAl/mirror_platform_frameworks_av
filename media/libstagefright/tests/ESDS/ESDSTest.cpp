...
class ESDSUnitTest : public ::testing::TestWithParam<tuple<
  ...
  public:
    ESDSUnitTest() : mESDSData(nullptr) {
        ...
        mESDSParams.streamType = get<6>(GetParam());
    };
    ...
    virtual void SetUp() override {
        string inputFileName = gEnv->getRes() + mESDSParams.inputFile;
        mEleStream.open(inputFileName, ifstream::binary);
        ASSERT_EQ(mEleStream.is_open(), true) << "Failed to open " << mESDSParams.inputFile;

        struct stat buf;
        status_t status = stat(inputFileName.c_str(), &buf);
        ASSERT_EQ(status, 0) << "Failed to get properties of input file: " << mESDSParams.inputFile;
        mFileSize = buf.st_size;

        ASSERT_NO_FATAL_FAILURE(readESDSData());
    }
  ...
  private:
    void readESDSData() {
        ...
        ASSERT_NE(inputFp, nullptr) << "File open failed for file: " << inputFile;
        int32_t fd = fileno(inputFp);
        ASSERT_GE(fd, 0) << "File descriptor invalid for file: " << inputFile;

        sp<DataSource> dataSource =
                DataSourceFactory::getInstance()->CreateFromFd(fd, 0, mFileSize);
        ASSERT_NE(dataSource, nullptr) << "Unable to create data source for file: " << inputFile;

        sp<IMediaExtractor> extractor = MediaExtractorFactory::Create(dataSource);
        ASSERT_NE(extractor, nullptr) << "Unable to create extractor for file: " << inputFile;
    ...
    bool esdsDataPresent(size_t numTracks, sp<IMediaExtractor> extractor) {
        ...
        for (size_t i = 0; i < numTracks; ++i) {
            ...
            if (trackMeta->findData(kKeyESDS, &type, &mESDSData, &mESDSSize)) {
                foundESDS = true;
                break;
            }
        }
...
TEST_P(ESDSUnitTest, InvalidDataTest) {
    void *invalidData = malloc(mESDSSize);
...
TEST(ESDSSanityUnitTest, ConstructorSanityTest) {
    void *invalidData = malloc(0);
    ASSERT_NE(invalidData, nullptr) << "Unable to allocate memory";
    ESDS esds_zero(invalidData, 0);
    free(invalidData);
    ASSERT_NE(esds_zero.InitCheck(), OK) << "invalid ESDS data accepted";

    invalidData = malloc(UINT32_MAX);
    if (invalidData) {
        ESDS esds_large(invalidData, UINT32_MAX);
        free(invalidData);
        ASSERT_NE(esds_large.InitCheck(), OK) << "invalid ESDS data accepted";
    }
}
