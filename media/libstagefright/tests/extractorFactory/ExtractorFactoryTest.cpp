...
int32_t ExtractorFactoryTest::createExtractor(bool createFromService, string inputMime) {
    ...
    if (createFromService) {
        mExtractor = MediaExtractorFactory::MediaExtractorFactory::CreateFromService(
                mDataSource, inputMime.c_str());
    } else {
...
TEST_F(ExtractorFactoryTest, ListExtractorsTest) {
    ...
    unordered_set<std::string> supportedTypes = MediaExtractorFactory::getSupportedTypes();
    ...
    ASSERT_NE(outputLog, nullptr) << "Unable to open output file for writing";
