...
// max size of hvcc box is 1 KB
constexpr uint32_t kHvccBoxMaxSize = 1024;
...
TEST_P(HEVCUtilsUnitTest, NALUnitTest) {
    ...
    while (getline(mInfoFileStream, line)) {
        ...
        offset += 3;
        ASSERT_LT(offset, chunkLength) << "NAL start location must be lesser than the chunk size";
        ...
        if (!((type.compare("VPS") && type.compare("SPS") && type.compare("PPS") &&
            ...
            ASSERT_EQ(err, (status_t)OK) << "Failed to add NAL Unit type: " << type;
    ...
    ASSERT_TRUE(err) << "Unable to create hvcc box";

    ASSERT_GT(hvccBoxSize, 0) << "Failed to create Hvcc box";

    int16_t frameRate = (hvcc[20] << 8) | hvcc[19];
