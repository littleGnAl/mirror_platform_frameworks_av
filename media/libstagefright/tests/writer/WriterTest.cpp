    ...
    for (int32_t i = 0; i < dstBufInfo.size(); i++) {
        ...
        if (mWriterName == MPEG4) {
            ASSERT_LE(abs(mBufferInfo[index][i].timeUs - dstBufInfo[i].timeUs), kMuxToleranceTimeUs)
                    ...
                    << "is greater than tolerance value = " << kMuxToleranceTimeUs
                    << " micro seconds";
        } else {
            ASSERT_EQ(mBufferInfo[index][i].timeUs, dstBufInfo[i].timeUs)
                    << "Input timestamp " << mBufferInfo[index][i].timeUs
                    << " does not match with extracted timestamp " << dstBufInfo[i].timeUs;
        }
    }
    return;
}
