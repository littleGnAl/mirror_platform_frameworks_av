...
static bool compTimeStamp(BufferInfo &lhs, int64_t rhsTimeUs) {
    return lhs.timeUs < rhsTimeUs;
}

static int64_t addSampleSize(int64_t initSize, BufferInfo &infoSize) {
    return initSize + infoSize.size;
}
...
TEST_P(ListenerTest, SetMaxFileLimitsTest) {
    ...
    int64_t totalSampleSizeForMaxDuration = 0;

    size_t inputFileSize = buf.st_size;
    int64_t fileDurationUs = mBufferInfo[mBufferInfo.size() - 1].timeUs;
    if (fileSizeLimit > 0) {
        maxFileSize = (int64_t)(fileSizeLimit * inputFileSize);
        mWriter->setMaxFileSize(maxFileSize);
    }
    if (fileDurationLimit > 0) {
        maxFileDuration = (int64_t)(fileDurationLimit * fileDurationUs);
        mWriter->setMaxFileDuration(maxFileDuration);
        auto lower =
                lower_bound(mBufferInfo.begin(), mBufferInfo.end(), maxFileDuration, compTimeStamp);
        totalSampleSizeForMaxDuration = accumulate(mBufferInfo.begin(), lower, 0, addSampleSize);
    }
    ...
    // Handle cases where % of max file duration and size are very close.
    if ((fileDurationLimit < fileSizeLimit) || (totalSampleSizeForMaxDuration < maxFileSize)) {
        ASSERT_EQ(listener->mSignalledLimit, MEDIA_RECORDER_INFO_MAX_DURATION_REACHED);
    } else {
        ASSERT_EQ(listener->mSignalledLimit, MEDIA_RECORDER_INFO_MAX_FILESIZE_REACHED);
    }
