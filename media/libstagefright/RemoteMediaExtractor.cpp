...
static void* closing_thread_func(void *arg) {
    while (true) {
        ...
        ALOGD("wake up in the worker with %zu entries", pending.size());
        if (!pending.empty()) {
            ds = pending.front();
        ...
        if (ds != nullptr) {
            ALOGD("closing ds.get() = %p", ds.get());
            ds->close();
            ALOGD("finished closing ds.get() = %p", ds.get());
        }
    }
...
static void start_close_thread(sp<DataSource> ds) {
    ...
    std::call_once(sCheckOnce, [&](){
        ...
        ALOGD("start_close_thread started worker thread");
    });

    ALOGD("enqueue our datasource for delayed close ds.get=%p", ds.get());
    {
        std::lock_guard _lm(pending_mutex);     // scoped, no explicit unlock
        ALOGD("lock acquired, inserting ds.get() %p", ds.get());
...
RemoteMediaExtractor::~RemoteMediaExtractor() {
    delete mExtractor;
    // XXX hook for changing behavior this dynamically, drop after testing
    int8_t new_scheme = property_get_bool("debug.rbe.delayedclose", 1);
    if (new_scheme != 0) {
        ALOGD("deferred approach");
        start_close_thread(mSource);
        mSource.clear();
    } else {
        ALOGD("right away, baseline approach");
        mSource->close();
        mSource.clear();
    }
