...
void blend_stripe_10bit_av8(uint16_t *decSampleHbdOffsetY, int16_t *grainStripe,
                            uint32_t widthComp, uint32_t grainStripeWidth,
  ...
  if ((widthComp & 0x7) == 0) {
    for (l = 0; l < blockHeight; l++) {
      for (k = 0; k < widthComp; k += BLK_8) {
        ...
        uint16x8_t v_u16x8_output =
            vsqaddq_u16(v_u16x8_decodeSample, v_16x8_grainStripe);
...
void blend_stripe_8bit_av8(uint8_t *decSampleOffsetY, int8_t *grainStripe,
                           uint32_t widthComp, uint32_t grainStripeWidth,
                           uint32_t blockHeight, uint8_t bitDepth) {
  uint32_t k, l;
  ...
  if ((widthComp & 0xF) == 0) {
    for (l = 0; l < blockHeight; l++) {
      for (k = 0; k < widthComp; k += BLK_16) {
        ...
        uint8x16_t v_u8x16_output =
            vsqaddq_u8(v_u8x16_decodeSample, v_8x16_grainStripe);
...
uint32_t block_avg_8x8_10bit_av8(uint16_t *decSampleBlk8, uint32_t widthComp,
                                 uint16_t *pNumSamples, uint8_t bitDepth) {
...
uint32_t block_avg_16x16_10bit_av8(uint16_t *decSampleBlk8, uint32_t widthComp,
                                   uint16_t *pNumSamples, uint8_t bitDepth) {
  uint32_t blockAvg = 0;
  uint8_t k;
...
uint32_t block_avg_8x8_8bit_av8(uint8_t *decSampleBlk8, uint32_t widthComp,
                                uint16_t *pNumSamples, uint8_t shift_Size) {
  uint32_t blockAvg = 0;
  uint8_t k;
...
uint32_t block_avg_16x16_8bit_av8(uint8_t *decSampleBlk8, uint32_t widthComp,
                                  uint16_t *pNumSamples, uint8_t shift_Size) {
  uint32_t blockAvg = 0;
  uint8_t k;
...
void simulate_grain_blk16x16_8bit_av8(int8_t *grainStripe,
                                      uint32_t grainStripeOffsetBlk8,
                                      GrainSynthesisStruct *grain_synt,
                                      uint32_t width, uint8_t log2ScaleFactor,
                                      int16_t scaleFactor, uint32_t kOffset,
  ...
  for (l = 0; l < BLK_16; l++) {
    ...
    int8x16_t v_8x16_grainStripe =
        vcombine_s8(v_8x8_grainStripe2, v_8x8_grainStripe1);
...
void simulate_grain_blk8x8_8bit_av8(int8_t *grainStripe,
                                    uint32_t grainStripeOffsetBlk8,
                                    GrainSynthesisStruct *grain_synt,
                                    uint32_t width, uint8_t log2ScaleFactor,
                                    int16_t scaleFactor, uint32_t kOffset,
...
void simulate_grain_blk8x8_10bit_av8(int16_t *grainStripe,
                                     uint32_t grainStripeOffsetBlk8,
                                     GrainSynthesisStruct *grain_synt,
                                     uint32_t width, uint8_t log2ScaleFactor,
                                     int16_t scaleFactor, uint32_t kOffset,
  ...
  for (l = 0; l < BLK_8; l++) {
    ...
    int16x8_t v_16x8_grainStripe =
        vcombine_s16(v_16x4_grainStripe2, v_16x4_grainStripe1);
...
void simulate_grain_blk16x16_10bit_av8(int16_t *grainStripe,
                                       uint32_t grainStripeOffsetBlk8,
                                       GrainSynthesisStruct *grain_synt,
                                       uint32_t width, uint8_t log2ScaleFactor,
                                       int16_t scaleFactor, uint32_t kOffset,
...
void blend_stripe_10bit_avx(uint16_t *decSampleHbdOffsetY, int16_t *grainStripe,
                            uint32_t widthComp, uint32_t grainStripeWidth,
                            uint32_t blockHeight, uint8_t bitDepth) {
  uint32_t k, l;
  uint16_t maxRange;
  ...
  if ((widthComp & 0x7) == 0) {
    for (l = 0; l < blockHeight; l++) {
      for (k = 0; k < widthComp; k += BLK_8) {
        __m128i __m128i_decData =
            _mm_loadu_si128((__m128i *)decSampleHbdOffsetY);
        __m128i __m128i_grainSt = _mm_loadu_si128((__m128i *)grainStripe);
        __m128i_grainSt =
            _mm_slli_epi16(__m128i_grainSt, (bitDepth - BIT_DEPTH_8));
...
void blend_stripe_8bit_avx(uint8_t *decSampleOffsetY, int8_t *grainStripe,
                           uint32_t widthComp, uint32_t grainStripeWidth,
                           uint32_t blockHeight, uint8_t bitDepth) {
  uint32_t k, l;
  ...
  if ((widthComp & 0xF) == 0) {
    for (l = 0; l < blockHeight; l++) {
      for (k = 0; k < widthComp; k += BLK_16) {
        ...
        __m128i __m128i_decData2 = _mm_cvtepu8_epi16(
            _mm_loadu_si128((__m128i *)(decSampleOffsetY + BLK_8)));

        __m128i __m128i_grainSt1 = _mm_cvtepi8_epi16(__m128i_grainSt);
        __m128i __m128i_grainSt2 = _mm_cvtepi8_epi16(
            _mm_loadu_si128((__m128i *)(grainStripe + BLK_8)));

        __m128i __m128i_sum1 =
            _mm_add_epi16(__m128i_decData1, __m128i_grainSt1);
        __m128i __m128i_sum2 =
            _mm_add_epi16(__m128i_decData2, __m128i_grainSt2);
...
uint32_t block_avg_8x8_10bit_avx(uint16_t *decSampleBlk8, uint32_t widthComp,
                                 uint16_t *pNumSamples, uint8_t bitDepth) {
  uint32_t blockAvg = 0;
  uint8_t k;
...
uint32_t block_avg_16x16_10bit_avx(uint16_t *decSampleBlk8, uint32_t widthComp,
                                   uint16_t *pNumSamples, uint8_t bitDepth) {
  uint32_t blockAvg = 0;
  uint8_t k;

  __m128i __m128i_sumRegister1 = _mm_loadu_si128((__m128i *)decSampleBlk8);
  __m128i __m128i_sumRegister2 =
      _mm_loadu_si128((__m128i *)(decSampleBlk8 + BLK_8));
  ...
  for (k = 1; k < BLK_16; k++) {
    ...
    __m128i __m128i_decData2 =
        _mm_loadu_si128((__m128i *)(decSampleBlk8 + BLK_8));
    decSampleBlk8 += widthComp;
    __m128i_sumRegister1 =
        _mm_add_epi16(__m128i_sumRegister1, __m128i_decData1);
    __m128i_sumRegister2 =
        _mm_add_epi16(__m128i_sumRegister2, __m128i_decData2);
  }
  __m128i __m128i_zeros = _mm_set1_epi16(0);
  ...
  __m128i __m128i_sumRegister =
      _mm_hadd_epi32(__m128i_sumRegister1, __m128i_sumRegister2);
...
uint32_t block_avg_8x8_8bit_avx(uint8_t *decSampleBlk8, uint32_t widthComp,
                                uint16_t *pNumSamples, uint8_t shift_Size) {
  uint32_t blockAvg = 0;
  uint8_t k;
  __m128i __m128i_decData = _mm_loadu_si128((__m128i *)decSampleBlk8);
...
uint32_t block_avg_16x16_8bit_avx(uint8_t *decSampleBlk8, uint32_t widthComp,
                                  uint16_t *pNumSamples, uint8_t shift_Size) {
  uint32_t blockAvg = 0;
  uint8_t k;

  __m128i __m128i_decData1 =
      _mm_cvtepu8_epi16(_mm_loadu_si128((__m128i *)decSampleBlk8));
  __m128i __m128i_decData2 =
      _mm_cvtepu8_epi16(_mm_loadu_si128((__m128i *)(decSampleBlk8 + BLK_8)));
  __m128i __m128i_sumRegister =
      _mm_add_epi16(__m128i_decData1, __m128i_decData2);
  decSampleBlk8 += widthComp;
...
void simulate_grain_blk16x16_8bit_avx(int8_t *grainStripe,
                                      uint32_t grainStripeOffsetBlk8,
                                      GrainSynthesisStruct *grain_synt,
                                      uint32_t width, uint8_t log2ScaleFactor,
                                      int16_t scaleFactor, uint32_t kOffset,
...
void simulate_grain_blk8x8_8bit_avx(int8_t *grainStripe,
                                    uint32_t grainStripeOffsetBlk8,
                                    GrainSynthesisStruct *grain_synt,
                                    uint32_t width, uint8_t log2ScaleFactor,
                                    int16_t scaleFactor, uint32_t kOffset,
  ...
  for (l = 0; l < BLK_8; l += 2) {
    ...
    __m128i_grainStripe2 = _mm_srai_epi16(__m128i_grainStripe2, shiftFactor);
    __m128i __m128i_grainStripe =
        _mm_packs_epi16(__m128i_grainStripe1, __m128i_zerovals);

    _mm_storel_epi64((__m128i *)grainStripe, __m128i_grainStripe);
    grainStripe += width;
    __m128i_grainStripe =
        _mm_packs_epi16(__m128i_grainStripe2, __m128i_zerovals);
    _mm_storel_epi64((__m128i *)grainStripe, __m128i_grainStripe);
    grainStripe += width;
  }
...
void simulate_grain_blk8x8_10bit_avx(int16_t *grainStripe,
                                     uint32_t grainStripeOffsetBlk8,
                                     GrainSynthesisStruct *grain_synt,
                                     uint32_t width, uint8_t log2ScaleFactor,
                                     int16_t scaleFactor, uint32_t kOffset,
  ...
  for (l = 0; l < BLK_8; l++) {
    ...
    __m128i __m128i_database_h_v2 =
        _mm_loadu_si128((__m128i *)(database_h_v + 4));
    database_h_v += DATA_BASE_SIZE;
    ...
    __m128i __m128i_grainStripe1 =
        _mm_mullo_epi32(__m128i_database_h_v1, __m128i_scaleFactor);
    __m128i __m128i_grainStripe2 =
        _mm_mullo_epi32(__m128i_database_h_v2, __m128i_scaleFactor);
    ...
    __m128i __m128i_grainStripe =
        _mm_packs_epi32(__m128i_grainStripe1, __m128i_grainStripe2);
    _mm_storeu_si128((__m128i *)grainStripe, __m128i_grainStripe);
    grainStripe += width;
  }
...
void simulate_grain_blk16x16_10bit_avx(int16_t *grainStripe,
                                       uint32_t grainStripeOffsetBlk8,
                                       GrainSynthesisStruct *grain_synt,
                                       uint32_t width, uint8_t log2ScaleFactor,
                                       int16_t scaleFactor, uint32_t kOffset,
  ...
  for (l = 0; l < BLK_16; l++) {
    ...
    __m128i __m128i_database_h_v2 =
        _mm_loadu_si128((__m128i *)(database_h_v + 4));
    __m128i __m128i_database_h_v3 =
        _mm_loadu_si128((__m128i *)(database_h_v + 8));
    __m128i __m128i_database_h_v4 =
        _mm_loadu_si128((__m128i *)(database_h_v + 12));
    database_h_v += DATA_BASE_SIZE;
    __m128i_database_h_v1 = _mm_cvtepi8_epi32(__m128i_database_h_v1);
    __m128i_database_h_v2 = _mm_cvtepi8_epi32(__m128i_database_h_v2);
    ...
    __m128i __m128i_grainStripe3 =
        _mm_mullo_epi32(__m128i_database_h_v3, __m128i_scaleFactor);
    __m128i __m128i_grainStripe4 =
        _mm_mullo_epi32(__m128i_database_h_v4, __m128i_scaleFactor);
    __m128i_grainStripe1 = _mm_srai_epi32(__m128i_grainStripe1, shiftFactor);
    __m128i_grainStripe2 = _mm_srai_epi32(__m128i_grainStripe2, shiftFactor);
    ...
    __m128i __m128i_grainStripe_st2 =
        _mm_packs_epi32(__m128i_grainStripe3, __m128i_grainStripe4);
