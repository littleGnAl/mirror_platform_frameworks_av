    ...
    for (h=0; h < NUM_CUT_OFF_FREQ;h++)
    {
        for (v=0;v < NUM_CUT_OFF_FREQ;v++)
        {
            ...
            for( l = 0; l < DATA_BASE_SIZE; l+= fgsBlkSize )
            {
                for( k = 0; k < DATA_BASE_SIZE; k++ )
                {
                    psFgsHandle->dataBase[blk_id][h][v][l][k] = ((psFgsHandle->dataBase[blk_id][h][v][l][k]) * deblockFactor [v]) >> 7;
                    psFgsHandle->dataBase[blk_id][h][v][l+fgsBlkSize-1][k] = ((psFgsHandle->dataBase[blk_id][h][v][l+fgsBlkSize-1][k]) * deblockFactor[v]) >> 7;
                }
            }
...
uint32_t fgs_simulation_blending_10bit_8x8(fgsProcessArgs *inArgs)
{
    ...
    grainStripeWidth = ((widthComp[0] - 1) | 0xF) + 1; //Make next muliptle of 16
    grainStripe = (int16_t *)malloc(grainStripeWidth * BLK_16 * sizeof(int16_t));
                ...
                for(y=0;y < height_forloop; y+=BLK_16)
                {
                    ...
                    for(x=0;x < width_forloop; x+=BLK_16)
                    {
                        ...
                        for(blkId=0;blkId < NUM_8x8_BLKS_16x16;blkId++)
                        {
                            ...
                            blockAvg = block_avg_8x8_10bit(decSampleHbdBlk8, strideComp[compCtr], &numSamples,
                                    BLK_8, BLK_8, bitDepth);
                            ...
                            if(INTENSITY_INTERVAL_MATCH_FAIL != intensityInt)
                            {
                                ...
                                simulate_grain_blk8x8_10bit(grainStripe,grainStripeOffsetBlk8,
                    ...
                    blend_stripe_10bit(decSampleHbdOffsetY, grainStripe,
                ...
                for(y=0;y < height_forloop; y+=BLK_16)
                {
                    ...
                    for(x=0;x < width_forloop; x+=BLK_16)
                    {
...

                        {
                            ...
                            blockAvg = block_avg_16x16_10bit(decSampleHbdBlk16, strideComp[compCtr], &numSamples,
                                    BLK_16, BLK_16, bitDepth);
                            ...
                            if(INTENSITY_INTERVAL_MATCH_FAIL != intensityInt)
                            {
                                ...
                                /* 8x8 block grain simulation */
                                simulate_grain_blk16x16_10bit(grainStripe,grainStripeOffset,
                                        grainSynt, grainStripeWidth,
...
uint32_t fgs_simulation_blending_8bit_8x8(fgsProcessArgs *inArgs){
    ...
    grainStripeWidth = ((widthComp[0] - 1) | 0xF) + 1;
    grainStripe = (int8_t *)malloc(grainStripeWidth * BLK_16 * sizeof(int8_t));
                ...
                for(y=0;y < height_forloop; y+=BLK_16)
                {
                    ...
                    for(x=0;x < width_forloop; x+=BLK_16)
                    {
                        ...
                        for(blkId=0;blkId < NUM_8x8_BLKS_16x16;blkId++)
                        {
                            ...
                            blockAvg = block_avg_8x8_8bit(decSampleBlk8, strideComp[compCtr], &numSamples,
                                    BLK_8, BLK_8, BLK_8_shift);
                            ...
                            if(INTENSITY_INTERVAL_MATCH_FAIL != intensityInt)
                            {
                                ...
                                /* 8x8 block grain simulation */
                                simulate_grain_blk8x8_8bit(grainStripe,grainStripeOffsetBlk8,
                                        grainSynt, grainStripeWidth,
                                        log2ScaleFactor,scaleFactor,kOffset,lOffset,h,v,BLK_8);
                    ...
                    blend_stripe_8bit(decSampleOffsetY, grainStripe,
                            strideComp[compCtr], grainStripeWidth, BLK_16, bitDepth);
                ...
                for(y=0;y < height_forloop; y+=BLK_16)
                {
                    ...
                    for(x=0;x < width_forloop; x+=BLK_16)
                    {
...

                        {
                            ...
                            blockAvg = block_avg_16x16_8bit(decSampleBlk16, strideComp[compCtr], &numSamples,
                                    BLK_16, BLK_16, BLK_16_shift);
                            ...
                            if(INTENSITY_INTERVAL_MATCH_FAIL != intensityInt)
                            {
                                ...
                                /* 8x8 block grain simulation */
                                simulate_grain_blk16x16_8bit(grainStripe,grainStripeOffset,
                                        grainSynt, grainStripeWidth, log2ScaleFactor,scaleFactor,
...
uint32_t fgs_process(GrainCharacteristicApi fgsParamStruct,void *handle)
{
    ...
    uint8_t bitDepth; /*grain bit depth and decoded bit depth are assumed to be same */
    uint8_t color_offset[FGS_MAX_NUM_COMP];
    ...
    uint32_t errorCode;
    uint8_t block_size = fgsParamStruct.fgcParameters.blockSize;
    ...
    errorCode =
        validate_film_grain_parameters(fgsParamStruct.width, fgsParamStruct.height,
    ...
    uint32_t *offsetsArr[FGS_MAX_NUM_COMP];
    ...
    for(compCtr=0; compCtr < numComp; compCtr++)
    {
        ...
        offsetsArr[compCtr] = (uint32_t *)malloc(sizeof(uint32_t) * numBlks_x[compCtr] * numBlks_y[compCtr]);
    }
    ...
    fgsProcessArgs inArgs;
    inArgs.numComp = numComp;

    for (compCtr = 0; compCtr < numComp; compCtr++)
    {
        inArgs.fgsOffsets[compCtr] = offsetsArr[compCtr];
        inArgs.decComp[compCtr] = decComp[compCtr];
        inArgs.widthComp[compCtr] = widthComp[compCtr];
        inArgs.heightComp[compCtr] = heightComp[compCtr];
        inArgs.strideComp[compCtr] = strideComp[compCtr];
    }
    inArgs.fgcParams = fgsParamStruct.fgcParameters;
    inArgs.grainSynt = grainSynt;
    inArgs.bitDepth = bitDepth;
...
void * fgs_create()
{
    ...
    int8_t blkSz_arr[BLOCK_IDX] = {8, 16};
    psFgsHandle = (fgsHandle *)malloc(sizeof(fgsHandle));
    int8_t blk_id;

    for(blk_id = 0; blk_id < BLOCK_IDX; blk_id++)
    {
        data_base_gen((GrainSynthesisStruct *)&psFgsHandle->grainSynt, blkSz_arr[blk_id], blk_id);
    }
    return (void *)psFgsHandle;
}


/* film grain synthesis deinit */
