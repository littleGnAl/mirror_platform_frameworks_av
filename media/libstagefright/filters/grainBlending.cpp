...
void data_base_gen(GrainSynthesisStruct *psFgsHandle, int8_t fgsBlkSize, int8_t blk_id) {
    ...
    int8_t r64IDCTTranspose[DATA_BASE_SIZE][DATA_BASE_SIZE];

    /*Transpose of Matrix*/
    for (l = 0; l < DATA_BASE_SIZE; l++) {
        for (k = 0; k < DATA_BASE_SIZE; k++) {
            r64IDCTTranspose[l][k] = R64_IDCT[k][l];
        }
    }
        ...
        for (v = 0; v < NUM_CUT_OFF_FREQ; v++) {
            ...
            for (i = 0; i < DATA_BASE_SIZE; i++) {
                for (j = 0; j < DATA_BASE_SIZE; j++) {
                    for (k = 0; k < DATA_BASE_SIZE; k++) {
                        bIDCT[i][j] += r64IDCTTranspose[i][k] * B[k][j];
                    }
                    bIDCT[i][j] += 128;
            ...
            for (i = 0; i < DATA_BASE_SIZE; i++) {
                for (j = 0; j < DATA_BASE_SIZE; j++) {
                    ...
                    psFgsHandle->dataBase[blk_id][h][v][j][i] = CLIP3(-127, 127, bGrain[i][j]);
                }
            }

            /* De-blocking at horizontal 8x8 and 16x16 block edges */
            for (l = 0; l < DATA_BASE_SIZE; l += fgsBlkSize) {
                for (k = 0; k < DATA_BASE_SIZE; k++) {
                    psFgsHandle->dataBase[blk_id][h][v][l][k] =
                        ((psFgsHandle->dataBase[blk_id][h][v][l][k]) * deblockFactor[v]) >> 7;
...
uint8_t validate_film_grain_parameters(uint32_t width, uint32_t height, uint8_t chroma_format_idc,
                                       uint8_t bitDepth, FilmGrainCharacteristicsStruct *psFgcParam,
                                       GrainSynthesisStruct *grainSynt) {
    /* number of color components */
    uint8_t numComp = FGS_MAX_NUM_COMP;
    ...
    for (compCtr = 0; compCtr < numComp; compCtr++) {
        if ((psFgcParam->compModelPresentFlag[compCtr] != 0) &&
            ...
            return FGS_INVALID_COMP_MODEL_PRESENT_FLAG;
        }
        if (psFgcParam->compModelPresentFlag[compCtr] &&
            (psFgcParam->numModelValuesMinus1[compCtr] > MAX_ALLOWED_MODEL_VALUES_MINUS1)) {
            return FGS_INVALID_NUM_MODEL_VALUES;
        }
    }

    /* validation of intensity intervals */
    for (compCtr = 0; compCtr < numComp; compCtr++) {
        if (1 == psFgcParam->compModelPresentFlag[compCtr]) {
            for (intensityCtr = 0; intensityCtr <= psFgcParam->numIntensityIntervalsMinus1[compCtr];
                 intensityCtr++) {
                if (psFgcParam->intensityIntervalLowerBound[compCtr][intensityCtr] >
                    ...
                    return FGS_INVALID_INTENSITY_BOUNDARY_VALUES;
                }
                ...
                for (multiGrainCtr = psFgcParam->intensityIntervalLowerBound[compCtr][intensityCtr];
                    ...
                    /* Non over lap intervals only */
                    if (multiGrainCheck[compCtr][multiGrainCtr]) {
                        return FGS_INVALID_INTENSITY_BOUNDARY_VALUES;
                    } else {
                ...
                if (psFgcParam->compModelValue[compCtr][intensityCtr][0] >= (1 << BIT_DEPTH_8)) {
                    ...
                    return FGS_INVALID_CUT_OFF_FREQUENCIES;
                ...
                if (0 == compPairMatch) {
                    ...
                    if (num_comp_model_pairs > MAX_ALLOWED_COMP_MODEL_PAIRS) {
                        return FGS_INVALID_NUM_CUT_OFF_FREQ_PAIRS;
                    }
    ...
    if (0 != psFgcParam->filmGrainCharacteristicsRepetitionPeriod) {
        return FGS_INVALID_FGC_REPETETION_PERIOD;
    }
    return FGS_SUCCESS;
}

template <typename T>
void blend_stripe_generic(T *decSampleOffsetY, int8_t *grainStripe, uint32_t widthComp,
    ...
    for (l = 0; l < blockHeight; l++) {
        for (k = 0; k < widthComp; k++) {
            ...
            *decSampleOffsetY = (uint16_t)grainSample;
            decSampleOffsetY++;
            grainStripe++;
        }
...
template <typename T, int blkSzShift>
uint32_t block_avg_generic(T *decSampleBlk, uint32_t widthComp, uint8_t bitDepth) {
    uint32_t blockAvg = 0;
    uint8_t k, l;
    constexpr int blkSz = 1 << (blkSzShift >> 1);
    ...
    for (k = 0; k < blkSz; k++) {
        ...
        decSampleBlk += widthComp - blkSz;
    }

    blockAvg = blockAvg >> (blkSzShift + (bitDepth - BIT_DEPTH_8));

    return blockAvg;
}

template <int blkSz>
void simulate_grain_generic(int8_t *grainStripe, uint32_t grainStripeOffsetBlk8,
                            GrainSynthesisStruct *grainSynt, uint32_t width,
                            uint8_t log2ScaleFactor, int16_t scaleFactor, uint32_t kOffset,
                            uint32_t lOffset, uint8_t h, uint8_t v) {
    uint32_t k, l;
    constexpr int16_t databaseIdx = blkSz >> 4;
    int8_t *database_h_v = &grainSynt->dataBase[databaseIdx][h][v][lOffset][kOffset];
    ...
    for (l = 0; l < blkSz; l++) {
        ...
        grainStripe += width - blkSz;
        database_h_v += DATA_BASE_SIZE - blkSz;
    }
...
template <typename T>
uint32_t block_average_with_padding(T *decSampleBlk, uint32_t widthComp, uint16_t *pNumSamples,
                                    uint8_t ySize, uint8_t xSize, uint8_t blkSz, uint8_t shift_Size,
                                    uint8_t bitDepth) {
    uint32_t blockAvg = 0;
    uint8_t k, l;
...
void deblock_grain_stripe(int8_t *grainStripe, uint32_t widthComp, uint32_t strideComp, int8_t blkW,
                          int8_t blkH) {
...
template <typename T>
uint32_t fgs_simulation_blending(fgsProcessArgs *inArgs, fgsFxns<T> *fgsFunc) {
    uint8_t numComp, compCtr;
    ...
    T *decSampleBlk, *decSampleOffsetY;
    T *decComp[FGS_MAX_NUM_COMP];
    uint16_t numSamples;
    GrainSynthesisStruct *grainSynt = (GrainSynthesisStruct *)(inArgs->grainSynt);
    int16_t scaleFactor, blockSize, blkSzbinAnd, blockAvgShiftSz;
    ...
    blockSize = inArgs->fgcParams.blockSize;

    if (blockSize == 16) {
        blkSzbinAnd = 0xF;
        blockAvgShiftSz = BLK_16_SHIFT;
    } else if (blockSize == 8) {
        blkSzbinAnd = 0x7;
        blockAvgShiftSz = BLK_8_SHIFT;
    }

    for (compCtr = 0; compCtr < numComp; compCtr++) {
        decComp[compCtr] = (T *)inArgs->decComp[compCtr];
    ...
    if (0 == fgcParams->filmGrainCharacteristicsCancelFlag) {
        for (compCtr = 0; compCtr < numComp; compCtr++) {
            if (1 == fgcParams->compModelPresentFlag[compCtr]) {
                ...
                uint32_t width_forloop = widthComp[compCtr] - (widthComp[compCtr] & blkSzbinAnd);
                uint32_t height_forloop = heightComp[compCtr] - (heightComp[compCtr] & blkSzbinAnd);
                uint32_t *offsetArr = inArgs->fgsOffsets[compCtr];

                /* Make next multiple of block size */
                grainStripeWidth = ((widthComp[compCtr] - 1) | blkSzbinAnd) + 1;
                /* Loop of blockSize*blockSize blocks */
                for (y = 0; y < height_forloop; y += blockSize) {
                    /* Initialization of grain stripe of blockSizexwidth size */
                    memset(grainStripe, 0, (grainStripeWidth * blockSize * sizeof(int8_t)));
                    for (x = 0; x < width_forloop; x += blockSize) {
                        /* start position offset of decoded sample in x direction */
                        grainStripeOffset = x;

                        decSampleBlk = decSampleOffsetY + x;
                        ...
                        blockAvg = fgsFunc->block_avg(decSampleBlk, strideComp[compCtr], bitDepth);
                        ...
                        if (INTENSITY_INTERVAL_MATCH_FAIL != intensityInt) {
                            kOffset = kOffset_const + (x & 0xF);
                            lOffset = lOffset_const + (y & 0xF);

                            scaleFactor = scaleFactor_const *
                            ...
                            /* block grain simulation */
                            fgsFunc->simulate_grain(grainStripe, grainStripeOffset, grainSynt,
                                                    grainStripeWidth, log2ScaleFactor, scaleFactor,
                                                    kOffset, lOffset, h, v);

                        } /* only if average falls in any interval */

                        /* uppdate the PRNG once per block of samples */
                        offsetArr++;
                    }
                    if (widthComp[compCtr] & blkSzbinAnd) {
                        grainStripeOffset = x;
                        ...
                        blockAvg = block_average_with_padding<T>(
                            decSampleBlk, strideComp[compCtr], &numSamples, blockSize,
                            MIN(blockSize, (widthComp[compCtr] - x)), blockSize, blockAvgShiftSz,
                            bitDepth);
                        ...
                        if (numSamples > 0) {
                            ...
                            if (INTENSITY_INTERVAL_MATCH_FAIL != intensityInt) {
                                ...
                                fgsFunc->simulate_grain(grainStripe, grainStripeOffset, grainSynt,
                                                        grainStripeWidth, log2ScaleFactor,
                                                        scaleFactor, kOffset, lOffset, h, v);
                        ...
                        /* uppdate the PRNG once per block of samples */
                        offsetArr++;
                    } /* End of blockSizexwidth grain simulation */

                    /* deblocking at the vertical edges of blocks at blockSizexwidth */
                    deblock_grain_stripe(grainStripe, widthComp[compCtr], grainStripeWidth,
                                         blockSize, blockSize);

                    /* Blending of size blockSizexwidth */
                    fgsFunc->blend_stripe(decSampleOffsetY, grainStripe, strideComp[compCtr],
                                          grainStripeWidth, blockSize, bitDepth);

                    decSampleOffsetY += blockSize * strideComp[compCtr];
                }

                if (heightComp[compCtr] & blkSzbinAnd) {
                    /* Initialization of grain stripe of blockSizexwidth size */
                    memset(grainStripe, 0, (grainStripeWidth * blockSize * sizeof(int8_t)));
                    for (x = 0; x < width_forloop; x += blockSize) {
                        grainStripeOffset = x;
                        ...
                        blockAvg = block_average_with_padding<T>(
                            decSampleBlk, strideComp[compCtr], &numSamples,
                            MIN(blockSize, (heightComp[compCtr] - y)), blockSize, blockSize,
                            blockAvgShiftSz, bitDepth);
                    ...
                    if (widthComp[compCtr] & blkSzbinAnd) {
...

                        {
                            blockAvg = block_average_with_padding<T>(
                                decSampleBlk, strideComp[compCtr], &numSamples,
                                MIN(blockSize, (heightComp[compCtr] - y)),
                                MIN(blockSize, (widthComp[compCtr] - x)), blockSize,
                                blockAvgShiftSz, bitDepth);
                    ...
                    fgsFunc->blend_stripe(decSampleOffsetY, grainStripe, strideComp[compCtr],
                                          grainStripeWidth,
                                          MIN(blockSize, (heightComp[compCtr] - y)), bitDepth);
                    decSampleOffsetY +=
                        MIN(blockSize, heightComp[compCtr] - y) * strideComp[compCtr];
...
template <typename T>
void function_selector(uint32_t processorArch, fgsFxns<T> *fptr, int16_t blkSz) {
    switch (processorArch) {
#if defined(__aarch64__)
        case ARCH_ARMV8_INTRINSICS:
            fptr->blend_stripe = blend_stripe_av8;
            if (blkSz == BLK_8) {
                fptr->block_avg = block_avg_8x8_av8;
                fptr->simulate_grain = simulate_grain_blk8x8_av8;
            } else if (blkSz == BLK_16) {
                fptr->block_avg = block_avg_16x16_av8;
                fptr->simulate_grain = simulate_grain_blk16x16_av8;
            }
            break;
#endif

#if defined(__x86_64__)
        case ARCH_X86_SSE42_INTRINSICS:
            fptr->blend_stripe = blend_stripe_sse42;
            if (blkSz == BLK_8) {
                fptr->block_avg = block_avg_8x8_sse42;
                fptr->simulate_grain = simulate_grain_blk8x8_sse42;
            } else if (blkSz == BLK_16) {
                fptr->block_avg = block_avg_16x16_sse42;
                fptr->simulate_grain = simulate_grain_blk16x16_sse42;
            }
            break;
#endif
        ...
        default:
            fptr->blend_stripe = blend_stripe_generic;
            if (blkSz == BLK_8) {
                fptr->block_avg = block_avg_generic<T, BLK_8_SHIFT>;
                fptr->simulate_grain = simulate_grain_generic<BLK_8>;
            } else if (blkSz == BLK_16) {
                fptr->block_avg = block_avg_generic<T, BLK_16_SHIFT>;
                fptr->simulate_grain = simulate_grain_generic<BLK_16>;
            }
            break;
    }
}
...
int32_t fgsWorkerProcess(fgsProcessArgs *inArgs) {
    uint32_t errorCode;
    uint8_t bitDepth = inArgs->bitDepth;
    int16_t blockSize = inArgs->fgcParams.blockSize;

    if (bitDepth == 8) {
        fgsFxns<uint8_t> fptr;
        function_selector(inArgs->processorArch, &fptr, blockSize);
        errorCode = fgs_simulation_blending<uint8_t>(inArgs, &fptr);
    } else if (bitDepth == 10) {
        fgsFxns<uint16_t> fptr;
        function_selector(inArgs->processorArch, &fptr, blockSize);
        errorCode = fgs_simulation_blending<uint16_t>(inArgs, &fptr);
    } else {
        return FGS_FAIL;
    }
    return errorCode;
...
int32_t workerThread(threadCntxt *args) {
    while (1) {
        sem_wait((sem_t *)args->semVarProcStart);
        /* Terminate the thread */
        pthread_mutex_lock((pthread_mutex_t *)args->mutexVar);

        if (args->terminateCmd) {
            args->processComplete = 1;
            sem_post((sem_t *)args->semVarProcEnd);
            args->processErrorCode = FGS_SUCCESS;
            pthread_mutex_unlock((pthread_mutex_t *)args->mutexVar);
            break;
        }
...
uint32_t fgs_process(GrainCharacteristicApi fgsParamStruct, void *handle) {
    ...
    uint8_t blockSize;
    /* grain bit depth and decoded bit depth are assumed to be same */
    uint8_t bitDepth;
    ...
    blockSize = fgsParamStruct.fgcParameters.blockSize;
    errorCode = validate_film_grain_parameters(fgsParamStruct.width, fgsParamStruct.height,
                                               fgsParamStruct.chromaFormat, fgsParamStruct.bitDepth,
                                               &(fgsParamStruct.fgcParameters), grainSynt);
    ...
    if (fgsParamStruct.width > psFgsHandle->initParams.maxWidth) return FGS_INVALID_WIDTH;
    if (fgsParamStruct.height > psFgsHandle->initParams.maxHeight) return FGS_INVALID_HEIGHT;
    if (fgsParamStruct.bitDepth > psFgsHandle->initParams.maxbitDepth) return FGS_INVALID_BIT_DEPTH;
    ...
    for (compCtr = 0; compCtr < numComp; compCtr++) {
        numBlks_x[compCtr] =
            ((widthComp[compCtr] >> WHSz) + ((widthComp[compCtr] & blkSzbinAnd) ? 1 : 0));
        numBlks_y[compCtr] =
            ((heightComp[compCtr] >> WHSz) + ((heightComp[compCtr] & blkSzbinAnd) ? 1 : 0));
        numSripesPerThr[compCtr] = numBlks_y[compCtr] / numThreads;
        numSripesPerThr[compCtr] -= (numSripesPerThr[compCtr] & 0x1);
        heightforLastThr[compCtr] =
            heightComp[compCtr] - numSripesPerThr[compCtr] * (numThreads - 1) * blockSize;
    }
    if (blockSize == 16) {
        for (compCtr = 0; compCtr < numComp; compCtr++) {
        ...
        for (compCtr = 0; compCtr < numComp; compCtr++) {
            if (1 == fgsParamStruct.fgcParameters.compModelPresentFlag[compCtr]) {
                ...
                /* Seed initialization for current picture */
                pseudoRandValEc = seedLUT[((picOffset + colorOffset[compCtr]) & 0xFF)];
                int16_t BlkYSz = numBlks_y[compCtr] - (numBlks_y[compCtr] & 0x1);
                int16_t BlkXSz = numBlks_x[compCtr] - (numBlks_x[compCtr] & 0x1);
                for (i = 0; i < BlkYSz; i += 2) {
                    for (j = 0; j < BlkXSz; j += 2) {
                        *tmp = pseudoRandValEc;
                        *(tmp + 1) = *tmp;
                        *(tmp + numBlks_x[compCtr]) = *tmp;
                        *(tmp + numBlks_x[compCtr] + 1) = *tmp;
                        pseudoRandValEc = prng(pseudoRandValEc);
                        tmp += 2;
                    }
                    if (numBlks_x[compCtr] & 0x1) {
                        *tmp = pseudoRandValEc;
                        *(tmp + numBlks_x[compCtr]) = *tmp;
                        pseudoRandValEc = prng(pseudoRandValEc);
                        tmp++;
                    }
                    tmp += numBlks_x[compCtr];
                }
                if (numBlks_y[compCtr] & 0x1) {
                    for (j = 0; j < BlkXSz; j += 2) {
                        *tmp = pseudoRandValEc;
                        *(tmp + 1) = *tmp;
                        pseudoRandValEc = prng(pseudoRandValEc);
    ...
    for (int z = 0; z < numThreads; z++) {
        ...
        args->fgsArgs.processorArch = psFgsHandle->initParams.processorArch;
        for (compCtr = 0; compCtr < numComp; compCtr++) {
            ...
            if (1 == fgsParamStruct.fgcParameters.compModelPresentFlag[compCtr]) {
                ...
                args->fgsArgs.fgsOffsets[compCtr] =
                    offsetArr[compCtr] + z * numSripesPerThr[compCtr] * numBlks_x[compCtr];

                args->fgsArgs.decComp[compCtr] =
                    (void *)(decComp[compCtr] + offset_dec_bufs * z * numSripesPerThr[compCtr] *
                                                    blockSize * strideComp[compCtr]);

                args->fgsArgs.heightComp[compCtr] = numSripesPerThr[compCtr] * blockSize;
            }
        }
        ...
        args->fgsArgs.grainSynt = grainSynt;
        args->terminateCmd = 0;
        args->processComplete = 0;
        sem_post((sem_t *)args->semVarProcStart);
        pthread_mutex_unlock((pthread_mutex_t *)args->mutexVar);
    }
    ...
    for (int i = 0; i < (numThreads - 1); i++) {
        ...
        while (!psFgsHandle->threadCntxArr[i].processComplete) {
            sem_wait((sem_t *)psFgsHandle->threadCntxArr[i].semVarProcEnd);
        }

        if (psFgsHandle->threadCntxArr[i].processErrorCode != FGS_SUCCESS) {
            errorCode = psFgsHandle->threadCntxArr[i].processErrorCode;
        }
    }
...
void *fgs_create(FGSInitParams *initParams) {
    ...
    if ((initParams->maxWidth < MIN_WIDTH) || (initParams->maxWidth > MAX_WIDTH)) {
        initParams->errorCode = FGS_INVALID_WIDTH;
        return NULL;
    }
    if ((initParams->maxHeight < MIN_HEIGHT) || (initParams->maxHeight > MAX_HEIGHT)) {
    ...
    if ((initParams->numThreads <= 0) || (initParams->numThreads > FGS_MAX_NUM_THREADS)) {
        initParams->errorCode = FGS_INVALID_NUM_THREADS;
        return NULL;
    }

    int8_t blkSzArr[NUM_BLOCK_IDX] = {8, 16};
    psFgsHandle = (fgsHandle *)malloc(sizeof(fgsHandle));
    int8_t blkId;
    for (blkId = 0; blkId < NUM_BLOCK_IDX; blkId++) {
    ...
    uint32_t maxMacroblocks = (wdMult16 >> 3) * (htMult16 >> 3);
    uint32_t compCtr;
    ...
    for (compCtr = 0; compCtr < FGS_MAX_NUM_COMP; compCtr++) {
        psFgsHandle->offsetArr[compCtr] = (uint32_t *)malloc(maxMacroblocks * sizeof(uint32_t));
    }

    for (int i = 0; i < initParams->numThreads; i++) {
        psFgsHandle->threadCntxArr[i].thrdHandle = malloc(sizeof(pthread_t));
        psFgsHandle->threadCntxArr[i].semVarProcStart = malloc(sizeof(sem_t));
        psFgsHandle->threadCntxArr[i].semVarProcEnd = malloc(sizeof(sem_t));
        psFgsHandle->threadCntxArr[i].mutexVar = malloc(sizeof(pthread_mutex_t));
        pthread_mutex_init((pthread_mutex_t *)psFgsHandle->threadCntxArr[i].mutexVar, NULL);
        sem_init((sem_t *)psFgsHandle->threadCntxArr[i].semVarProcStart, 0, 0);
        sem_init((sem_t *)psFgsHandle->threadCntxArr[i].semVarProcEnd, 0, 0);

        psFgsHandle->threadCntxArr[i].terminateCmd = 0;
        /* No process running for now */
        psFgsHandle->threadCntxArr[i].processComplete = 1;
        psFgsHandle->threadCntxArr[i].fgsArgs.scratchMem =
            (void *)((uint8_t *)psFgsHandle->scratchMem + scratchMemPerThr * i);
    }
    /*Last thread is the master thread, not called at create time*/
    for (int i = 0; i < initParams->numThreads - 1; i++) {
        pthread_create((pthread_t *)psFgsHandle->threadCntxArr[i].thrdHandle, NULL,
                       (void *(*)(void *))(&workerThread), (void *)&psFgsHandle->threadCntxArr[i]);
    }
...
void fgs_delete(void *handle) {
    ...
    for (int i = 0; i < psFgsHandle->initParams.numThreads; i++) {
        pthread_mutex_lock((pthread_mutex_t *)psFgsHandle->threadCntxArr[i].mutexVar);
        psFgsHandle->threadCntxArr[i].terminateCmd = 1;
        psFgsHandle->threadCntxArr[i].processComplete = 0;
        sem_post((sem_t *)psFgsHandle->threadCntxArr[i].semVarProcStart);
        pthread_mutex_unlock((pthread_mutex_t *)psFgsHandle->threadCntxArr[i].mutexVar);

        /* Last thread is the master thread, so there is no thread_join */
        if ((psFgsHandle->initParams.numThreads - 1) != i)
            pthread_join(*(pthread_t *)psFgsHandle->threadCntxArr[i].thrdHandle, NULL);

        pthread_mutex_destroy((pthread_mutex_t *)psFgsHandle->threadCntxArr[i].mutexVar);
        sem_destroy((sem_t *)(psFgsHandle->threadCntxArr[i].semVarProcStart));
        sem_destroy((sem_t *)(psFgsHandle->threadCntxArr[i].semVarProcEnd));

        free(psFgsHandle->threadCntxArr[i].mutexVar);
        free(psFgsHandle->threadCntxArr[i].semVarProcStart);
        free(psFgsHandle->threadCntxArr[i].semVarProcEnd);
        free(psFgsHandle->threadCntxArr[i].thrdHandle);
    }
