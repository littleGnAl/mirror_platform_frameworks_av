      ...
      for (l = 0; l < DATA_BASE_SIZE; l += fgsBlkSize) {
        for (k = 0; k < DATA_BASE_SIZE; k++) {
          ...
          psFgsHandle->dataBase[blk_id][h][v][l + fgsBlkSize - 1][k] =
              ((psFgsHandle->dataBase[blk_id][h][v][l + fgsBlkSize - 1][k]) * deblockFactor[v]) >>
              7;
        }
      }
    }
        ...
        if ((3 != chroma_format_idc) && (compCtr > 0)) {
          ...
          } else if (2 == chroma_format_idc) /* 4:2:2 */
          {
            psFgcParam->compModelValue[compCtr][intensityCtr][0] =
                (psFgcParam->compModelValue[compCtr][intensityCtr][0] * SCALE_DOWN_422) >>
                Q_FORMAT_SCALING;
...
void blend_stripe_generic(uint16_t *decSampleOffsetY, int8_t *grainStripe, uint32_t widthComp,
  ...
  for (l = 0; l < blockHeight; l++) {
    for (k = 0; k < widthComp; k++) {
      decodeSampleHbd = *decSampleOffsetY;
...
void blend_stripe_generic(uint8_t *decSampleOffsetY, int8_t *grainStripe, uint32_t widthComp,
                          uint32_t grainStripeWidth, uint32_t blockHeight, uint8_t bitDepth) {
  uint32_t k, l;
  int32_t grainSample;
...
uint32_t block_avg_generic(uint16_t *decSampleBlk, uint32_t widthComp, uint8_t bitDepth,
...
uint32_t block_avg_generic(uint8_t *decSampleBlk, uint32_t widthComp, uint8_t bitDepth,
                           int16_t blkSz, int16_t blkShift) {
  uint32_t blockAvg = 0;
  uint8_t k, l;
...
void simulate_grain_generic(int8_t *grainStripe, uint32_t grainStripeOffsetBlk8,
  ...
  uint32_t k, l;
  int8_t *database_h_v = &grainSynt->dataBase[(blkSz >> 4)][h][v][lOffset][kOffset];
...
template <typename T>
uint32_t block_average(T *decSampleBlk, uint32_t widthComp, uint16_t *pNumSamples, uint8_t ySize,
                       uint8_t xSize, uint8_t blkSz, uint8_t shift_Size, uint8_t bitDepth) {
  uint32_t blockAvg = 0;
  uint8_t k, l;
...
template <typename T>
uint32_t fgs_simulation_blending_8x8(fgsProcessArgs *inArgs, fgsFxns<T> *fgsFunc) {
  ...
  T *decSampleBlk16, *decSampleBlk8, *decSampleOffsetY;
  ...
  if (0 == fgcParams->filmGrainCharacteristicsCancelFlag) {
    for (compCtr = 0; compCtr < numComp; compCtr++) {
      if (1 == fgcParams->compModelPresentFlag[compCtr]) {
        ...
        /* Next multiple of 16 */
        grainStripeWidth = ((widthComp[compCtr] - 1) | 0xF) + 1;
        ...
        for (y = 0; y < height_forloop; y += BLK_16) {
          ...
          for (x = 0; x < width_forloop; x += BLK_16) {
            ...
            for (blkId = 0; blkId < NUM_8x8_BLKS_16x16; blkId++) {
              ...
              grainStripeOffsetBlk8 =
                  grainStripeOffset + (xOffset8x8 + (yOffset8x8 * grainStripeWidth));

              decSampleBlk8 = decSampleBlk16 + offsetBlk8x8;

              blockAvg = fgsFunc->block_avg_8x8(decSampleBlk8, strideComp[compCtr], bitDepth, BLK_8,
                                                BLK_8_shift);
              ...
              if (INTENSITY_INTERVAL_MATCH_FAIL != intensityInt) {
                ...
                scaleFactor =
                    scaleFactor_const * fgcParams->compModelValue[compCtr][intensityInt][0];
                ...
                fgsFunc->simulate_grain_blk8x8(grainStripe, grainStripeOffsetBlk8, grainSynt,
                                               grainStripeWidth, log2ScaleFactor, scaleFactor,
                                               kOffset, lOffset, h, v, BLK_8);
          ...
          if ((widthComp[compCtr] & 0xF)) {
            ...
            for (blkId = 0; blkId < NUM_8x8_BLKS_16x16; blkId++) {
              ...
              decSampleBlk8 = decSampleBlk16 + offsetBlk8x8;
              blockAvg = block_average<T>(decSampleBlk8, strideComp[compCtr], &numSamples, BLK_8,
          ...
          fgsFunc->blend_stripe(decSampleOffsetY, grainStripe, strideComp[compCtr],
                                grainStripeWidth, BLK_16, bitDepth);
        ...
        if ((heightComp[compCtr] & 0xF)) {
          ...
          for (x = 0; x < width_forloop; x += BLK_16) {
            ...
            for (blkId = 0; blkId < NUM_8x8_BLKS_16x16; blkId++) {
              ...
              decSampleBlk8 = decSampleBlk16 + offsetBlk8x8;
              blockAvg = block_average<T>(decSampleBlk8, strideComp[compCtr], &numSamples,
          ...
          fgsFunc->blend_stripe(decSampleOffsetY, grainStripe, strideComp[compCtr],
                                grainStripeWidth, MIN(BLK_16, (heightComp[compCtr] - y)), bitDepth);
...
template <typename T>
uint32_t fgs_simulation_blending_16x16(fgsProcessArgs *inArgs, fgsFxns<T> *fgsFunc) {
  uint8_t numComp, compCtr;
  uint8_t log2ScaleFactor, h, v;
  uint8_t bitDepth; /* grain bit depth and decoded bit depth are assumed to be same */
  uint32_t widthComp[FGS_MAX_NUM_COMP], heightComp[FGS_MAX_NUM_COMP], strideComp[FGS_MAX_NUM_COMP];
  T *decSampleBlk16, *decSampleOffsetY;
  T *decComp[FGS_MAX_NUM_COMP];
  uint16_t numSamples;
  GrainSynthesisStruct *grainSynt = (GrainSynthesisStruct *)(inArgs->grainSynt);
  int16_t scaleFactor;
      ...
      if (1 == fgcParams->compModelPresentFlag[compCtr]) {
        ...
        for (y = 0; y < height_forloop; y += BLK_16) {
          ...
          for (x = 0; x < width_forloop; x += BLK_16) {
            ...
            {
              blockAvg = fgsFunc->block_avg_16x16(decSampleBlk16, strideComp[compCtr], bitDepth,
                                                  BLK_16, BLK_16_shift);
              ...
              if (INTENSITY_INTERVAL_MATCH_FAIL != intensityInt) {
                ...
                /* 8x8 block grain simulation */
                fgsFunc->simulate_grain_blk16x16(grainStripe, grainStripeOffset, grainSynt,
                                                 grainStripeWidth, log2ScaleFactor, scaleFactor,
                                                 kOffset, lOffset, h, v, BLK_16);
          ...
          if (widthComp[compCtr] & 0xF) {
            ...
            {
              blockAvg = block_average<T>(decSampleBlk16, strideComp[compCtr], &numSamples, BLK_16,
                                          MIN(BLK_16, (widthComp[compCtr] - x)), BLK_16,
        ...
        if (heightComp[compCtr] & 0xF) {
          ...
          for (x = 0; x < width_forloop; x += BLK_16) {
            ...
            {
              blockAvg = block_average<T>(decSampleBlk16, strideComp[compCtr], &numSamples,
...
template <typename T>
void function_selector(uint32_t processorArch, fgsFxns<T> *fptr) {
  switch (processorArch) {
#if defined(__aarch64__)
    case ARCH_ARMV8_INTRINSICS:
      fptr->blend_stripe = blend_stripe_av8;
      fptr->block_avg_8x8 = block_avg_8x8_av8;
      fptr->block_avg_16x16 = block_avg_16x16_av8;
      fptr->simulate_grain_blk8x8 = simulate_grain_blk8x8_av8;
      fptr->simulate_grain_blk16x16 = simulate_grain_blk16x16_av8;
      break;
#endif
    ...
    case ARCH_X86_AVX_INTRINSICS:
      ...
      fptr->block_avg_8x8 = block_avg_8x8_avx;
      fptr->block_avg_16x16 = block_avg_16x16_avx;
      fptr->simulate_grain_blk8x8 = simulate_grain_blk8x8_avx;
      fptr->simulate_grain_blk16x16 = simulate_grain_blk16x16_avx;
      break;
#endif
    ...
    default:
      ...
      fptr->block_avg_8x8 = block_avg_generic;
      fptr->block_avg_16x16 = block_avg_generic;
      fptr->simulate_grain_blk8x8 = simulate_grain_generic;
      fptr->simulate_grain_blk16x16 = simulate_grain_generic;
      break;
  }
}
...
int32_t fgsWorkerProcess(fgsProcessArgs *inArgs) {
  ...
  if (bitDepth == 8) {
    ...
    function_selector(inArgs->processorArch, &fptr);
    if (block_size == 8)
      errorCode = fgs_simulation_blending_8x8<uint8_t>(inArgs, &fptr);
    else if (block_size == 16)
      errorCode = fgs_simulation_blending_16x16<uint8_t>(inArgs, &fptr);
    else
      errorCode = FGS_FAIL;
  } else if (bitDepth == 10) {
    ...
    function_selector(inArgs->processorArch, &fptr);
    if (block_size == 8)
      errorCode = fgs_simulation_blending_8x8<uint16_t>(inArgs, &fptr);
    else if (block_size == 16)
      errorCode = fgs_simulation_blending_16x16<uint16_t>(inArgs, &fptr);
    else
      errorCode = FGS_FAIL;
  } else {
    return FGS_FAIL;
...
uint32_t fgs_process(GrainCharacteristicApi fgsParamStruct, void *handle) {
  ...
  for (compCtr = 0; compCtr < numComp; compCtr++) {
    ...
    heightforLastThr[compCtr] =
        heightComp[compCtr] - numSripesPerThr[compCtr] * (numThreads - 1) * BLK_16;
  }
...
void *fgs_create(FGSInitParams *initParams) {
  ...
  for (int z = 0; z < initParams->numThreads; z++) {
    ...
    psFgsHandle->threadCntxArr[z].semVar = malloc(sizeof(sem_t));
    ...
    sem_init((sem_t *)psFgsHandle->threadCntxArr[z].semVar, 0, 0);
...
void fgs_delete(void *handle) {
  ...
  for (int z = 0; z < psFgsHandle->initParams.numThreads; z++) {
    ...
    sem_post((sem_t *)psFgsHandle->threadCntxArr[z].semVar);
    ...
    sem_destroy((sem_t *)(psFgsHandle->threadCntxArr[z].semVar));
