            ...
            for( l = 0; l < DATA_BASE_SIZE; l+= fgsBlkSize )
            {
                for( k = 0; k < DATA_BASE_SIZE; k++ )
                {
                    ...
                    psFgsHandle->dataBase[blk_id][h][v][l+fgsBlkSize-1][k] =
                        ((psFgsHandle->dataBase[blk_id][h][v][l+fgsBlkSize-1][k]) * deblockFactor[v]) >> 7;
                }
            }
...
uint32_t fgs_process(GrainCharacteristicApi fgsParamStruct,void *handle)
{
    ...
    uint32_t width,height,widthComp[FGS_MAX_NUM_COMP],heightComp[FGS_MAX_NUM_COMP], strideComp[FGS_MAX_NUM_COMP];
    uint8_t *decComp[FGS_MAX_NUM_COMP];
    ...
    /*Verify paramters are not violating init time params*/
