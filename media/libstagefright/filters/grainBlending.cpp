...
void data_base_gen(GrainSynthesisStruct *psFgsHandle, int8_t fgsBlkSize, int8_t blk_id) {
    ...
    int8_t R64_IDCT_TR[DATA_BASE_SIZE][DATA_BASE_SIZE];

    /*Transpose of Matrix*/
    for (l = 0; l < DATA_BASE_SIZE; l++) {
        for (k = 0; k < DATA_BASE_SIZE; k++) {
            R64_IDCT_TR[l][k] = R64_IDCT[k][l];
        }
    }
...
template <typename T>
uint32_t block_avg_generic_8x8(T *decSampleBlk, uint32_t widthComp, uint8_t bitDepth) {
    uint32_t blockAvg = 0;
    uint8_t k, l;
...
template <typename T>
uint32_t block_avg_generic_16x16(T *decSampleBlk, uint32_t widthComp, uint8_t bitDepth) {
    uint32_t blockAvg = 0;
    uint8_t k, l;
...
void simulate_grain_generic_8x8(int8_t *grainStripe, uint32_t grainStripeOffsetBlk8,
...
void simulate_grain_generic_16x16(int8_t *grainStripe, uint32_t grainStripeOffsetBlk8,
                                  GrainSynthesisStruct *grainSynt, uint32_t width,
                                  uint8_t log2ScaleFactor, int16_t scaleFactor, uint32_t kOffset,
                                  uint32_t lOffset, uint8_t h, uint8_t v) {
    uint32_t k, l;
            ...
            if (1 == fgcParams->compModelPresentFlag[compCtr]) {
                ...
                for (y = 0; y < height_forloop; y += blockSize) {
                    ...
                    for (x = 0; x < width_forloop; x += blockSize) {
                        ...
                        {
                            blockAvg =
                                fgsFunc->block_avg(decSampleBlk, strideComp[compCtr], bitDepth);
...
void function_selector(uint32_t processorArch, fgsFxns<T> *fptr, int16_t blkSz) {
    switch (processorArch) {
        ...
        case ARCH_X86_AVX_INTRINSICS:
            fptr->blend_stripe = blend_stripe_avx;
            if (blkSz == BLK_8) {
                fptr->block_avg = block_avg_8x8_avx;
                fptr->simulate_grain = simulate_grain_blk8x8_avx;
            } else if (blkSz == BLK_16) {
                fptr->block_avg = block_avg_16x16_avx;
                fptr->simulate_grain = simulate_grain_blk16x16_avx;
            }
            break;
#endif
        ...
        default:
            ...
            if (blkSz == BLK_8) {
                fptr->block_avg = block_avg_generic_8x8;
                fptr->simulate_grain = simulate_grain_generic_8x8;
            } else if (blkSz == BLK_16) {
                fptr->block_avg = block_avg_generic_16x16;
                fptr->simulate_grain = simulate_grain_generic_16x16;
            }
            break;
    }
}
...
uint32_t fgs_process(GrainCharacteristicApi fgsParamStruct, void *handle) {
    ...
    for (int z = 0; z < (numThreads - 1); z++) {
        ...
        while (!psFgsHandle->threadCntxArr[z].processComplete) {
            sem_wait((sem_t *)psFgsHandle->threadCntxArr[z].semVarProcEnd);
        }
...
void *fgs_create(FGSInitParams *initParams) {
    ...
    for (int z = 0; z < initParams->numThreads; z++) {
        psFgsHandle->threadCntxArr[z].thrdHandle = malloc(sizeof(pthread_t));
        psFgsHandle->threadCntxArr[z].semVarProcStart = malloc(sizeof(sem_t));
        psFgsHandle->threadCntxArr[z].semVarProcEnd = malloc(sizeof(sem_t));
        psFgsHandle->threadCntxArr[z].mutexVar = malloc(sizeof(pthread_mutex_t));
        pthread_mutex_init((pthread_mutex_t *)psFgsHandle->threadCntxArr[z].mutexVar, NULL);
        sem_init((sem_t *)psFgsHandle->threadCntxArr[z].semVarProcStart, 0, 0);
        sem_init((sem_t *)psFgsHandle->threadCntxArr[z].semVarProcEnd, 0, 0);

        psFgsHandle->threadCntxArr[z].terminateCmd = 0;
        /* No process running for now */
        psFgsHandle->threadCntxArr[z].processComplete = 1;
    ...
    for (int z = 0; z < initParams->numThreads - 1; z++) {
        pthread_create((pthread_t *)psFgsHandle->threadCntxArr[z].thrdHandle, NULL,
                       (void *(*)(void *))(&workerThread), (void *)&psFgsHandle->threadCntxArr[z]);
    }
...
void fgs_delete(void *handle) {
    ...
    for (int z = 0; z < psFgsHandle->initParams.numThreads; z++) {
        pthread_mutex_lock((pthread_mutex_t *)psFgsHandle->threadCntxArr[z].mutexVar);
        psFgsHandle->threadCntxArr[z].terminateCmd = 1;
        psFgsHandle->threadCntxArr[z].processComplete = 0;
        sem_post((sem_t *)psFgsHandle->threadCntxArr[z].semVarProcStart);
        pthread_mutex_unlock((pthread_mutex_t *)psFgsHandle->threadCntxArr[z].mutexVar);

        /* Last thread is the master thread, so there is no thread_join */
        if ((psFgsHandle->initParams.numThreads - 1) != z)
            pthread_join(*(pthread_t *)psFgsHandle->threadCntxArr[z].thrdHandle, NULL);

        pthread_mutex_destroy((pthread_mutex_t *)psFgsHandle->threadCntxArr[z].mutexVar);
        sem_destroy((sem_t *)(psFgsHandle->threadCntxArr[z].semVarProcStart));
        sem_destroy((sem_t *)(psFgsHandle->threadCntxArr[z].semVarProcEnd));

        free(psFgsHandle->threadCntxArr[z].mutexVar);
        free(psFgsHandle->threadCntxArr[z].semVarProcStart);
        free(psFgsHandle->threadCntxArr[z].semVarProcEnd);
