...
uint32_t fgs_thread_get_handle_size(void)
{
...
int32_t fgs_thread_create(void *thread_handle, void *attribute, void *strt, void *argument)
{
...
uint32_t fgs_thread_get_sem_struct_size(void)
{
    return (sizeof(HANDLE));
}
...
void fgs_thread_exit(void *val_ptr)
{
...
void data_base_gen(GrainSynthesisStruct* psFgsHandle,
        int8_t fgsBlkSize, int8_t blk_id)
{
...
/* Function validates film grain parameters  and returns 0 for valid parameters of SMPTE-RDD5
 * else 1.
 * Also down converts the chroma model values for 4:2:0 and 4:2:2 chromqa_formats */
uint8_t validate_film_grain_parameters(uint32_t width,
        uint32_t height,
        ...
        GrainSynthesisStruct *grainSynt)
{
    ...
    if((chroma_format_idc < MIN_CHROMA_FORMAT_IDC) || (chroma_format_idc > MAX_CHROMA_FORMAT_IDC))
    {
        ...
        if(1 == psFgcParam->compModelPresentFlag[compCtr])
        {
                    ...
                    intensityCtr++)
            {
                ...
                for(multiGrainCtr = psFgcParam->intensityIntervalLowerBound[compCtr][intensityCtr];
                    multiGrainCtr <= psFgcParam->intensityIntervalUpperBound[compCtr][intensityCtr];
                    multiGrainCtr++)
                {
                ...
                if((3 != chroma_format_idc) && (compCtr > 0))
                {
                    ...
                    else if(2 == chroma_format_idc)/* 4:2:2 */
                    {
                        psFgcParam->compModelValue[compCtr][intensityCtr][0] =
                            (psFgcParam->compModelValue[compCtr][intensityCtr][0] *
                                                            SCALE_DOWN_422)>>Q_FORMAT_SCALING;
...
void blend_stripe_10bit_generic(uint16_t *decSampleHbdOffsetY, int16_t *grainStripe,
...
uint32_t block_avg_8x8_8bit_generic(uint8_t *decSampleBlk8,
        uint32_t widthComp,
        uint16_t *pNumSamples,
        uint8_t shift_Size)
{
...
void simulate_grain_blk8x8_8bit_generic(int8_t *grainStripe,uint32_t grainStripeOffsetBlk8,
...
void simulate_grain_blk16x16_8bit_generic(int8_t *grainStripe,uint32_t grainStripeOffsetBlk8,
        GrainSynthesisStruct *grainSynt,uint32_t width,uint8_t log2ScaleFactor,
        int16_t scaleFactor,uint32_t kOffset, uint32_t lOffset,
        uint8_t h,uint8_t v)
{
...
void simulate_grain_blk8x8_10bit_generic(int16_t *grainStripe,uint32_t grainStripeOffsetBlk8,
        GrainSynthesisStruct *grainSynt,uint32_t width,uint8_t log2ScaleFactor,
        int16_t scaleFactor,uint32_t kOffset, uint32_t lOffset,
        uint8_t h,uint8_t v)
{
...
void simulate_grain_blk16x16_10bit_generic(int16_t *grainStripe,uint32_t grainStripeOffsetBlk8,
        GrainSynthesisStruct *grainSynt,uint32_t width,uint8_t log2ScaleFactor,
        int16_t scaleFactor,uint32_t kOffset, uint32_t lOffset,
        uint8_t h,uint8_t v)
{
    ...
    for(l = 0; l < BLK_16; l++) /* y direction */
    {
        ...
        {
            *grainStripe =
                (int16_t)(((int32_t)scaleFactor * (*database_h_v)
                          )>>(log2ScaleFactor+GRAIN_SCALE));
            grainStripe++;
            database_h_v++;
        }
...
uint32_t block_average(uint8_t *decSampleBlk,
        uint32_t widthComp,
        uint16_t *pNumSamples,
        uint8_t ySize,
        uint8_t xSize,
...
/* Function to calculate block average for bit depths > 8 */
uint32_t block_average_hbd(uint16_t *decSampleBlk,
        uint32_t widthComp,
        uint16_t *pNumSamples,
        uint8_t ySize,
        uint8_t xSize,
...
void deblock_grain_stripe(int8_t *grainStripe, uint32_t widthComp, uint32_t strideComp,
                            int8_t blkW, int8_t blkH)
{
...
void deblock_grain_stripe_hbd(int16_t *grainStripe, uint32_t widthComp, uint32_t strideComp,
                                int8_t blkW, int8_t blkH)
{
    int32_t left1,left0,right0,right1;
    uint32_t pos8;
...
uint32_t fgs_simulation_blending_10bit_8x8(fgsProcessArgs *inArgs)
{
    ...
    uint32_t x, y;
    /* ec : seed to be used for the psudo random generator for a given color component */
    ...
    if(0 == fgcParams->filmGrainCharacteristicsCancelFlag)
    {
        for(compCtr=0; compCtr < numComp; compCtr++)
        {
            if(1 == fgcParams->compModelPresentFlag[compCtr])
            {
                ...
                uint32_t width_forloop = widthComp[compCtr] - (widthComp[compCtr] & 0xF);
                uint32_t height_forloop = heightComp[compCtr] - (heightComp[compCtr] & 0xF);
                ...
                for(y=0;y < height_forloop; y+=BLK_16)
                {
                    ...
                    for(x=0;x < width_forloop; x+=BLK_16)
                    {
                        ...
                        for(blkId=0;blkId < NUM_8x8_BLKS_16x16;blkId++)
                        {
                            ...
                            grainStripeOffsetBlk8 = grainStripeOffset +
                                                    (xOffset8x8 + (yOffset8x8 * grainStripeWidth));

                            decSampleHbdBlk8 = decSampleHbdBlk16 + offsetBlk8x8;
                            blockAvg = fgsFunc->block_avg_8x8_10bit(decSampleHbdBlk8,
                                                                strideComp[compCtr], &numSamples,
                                                                bitDepth);
                            ...
                            if(INTENSITY_INTERVAL_MATCH_FAIL != intensityInt)
                            {
                                ...
                                fgsFunc->simulate_grain_blk8x8_10bit(grainStripe,
                                                                    grainStripeOffsetBlk8,
                                                                    grainSynt, grainStripeWidth,
                    ...
                    if((widthComp[compCtr] & 0xF))
                    {
                        ...
                        for(blkId=0;blkId < NUM_8x8_BLKS_16x16;blkId++)
                        {
                            ...
                            grainStripeOffsetBlk8 = grainStripeOffset + (xOffset8x8 +
                                                                (yOffset8x8 * grainStripeWidth));
                            if (((widthComp[compCtr] - x) <= xOffset8x8))
                            {
                                continue;
                            }
                            decSampleHbdBlk8 = decSampleHbdBlk16 + offsetBlk8x8;
                            blockAvg = block_average_hbd(decSampleHbdBlk8, strideComp[compCtr],
                                    &numSamples,
                                    BLK_8,
                    ...
                    deblock_grain_stripe_hbd(grainStripe, widthComp[compCtr], grainStripeWidth,
                                                BLK_8, BLK_16);

                    /* Blending of size 16xwidth*/

                    fgsFunc->blend_stripe_10bit(decSampleHbdOffsetY, grainStripe,
                            strideComp[compCtr], grainStripeWidth, BLK_16, bitDepth);
                ...
                if((heightComp[compCtr] & 0xF))
                {
                    ...
                    for(x=0;x < width_forloop ; x+=BLK_16)
                    {
                        ...
                        for(blkId=0;blkId < NUM_8x8_BLKS_16x16;blkId++)
                        {
                            ...
                            if(INTENSITY_INTERVAL_MATCH_FAIL != intensityInt)
                            {
                                /* 8x8 grain block offset using co-ordinates of decoded 8x8 block
                                 * in the frame */
                                kOffset = kOffset_const + ((x + xOffset8x8) & 0x0008);
            ...
            if(1 == fgcParams->compModelPresentFlag[compCtr])
            {
                ...
                for(y=0;y < height_forloop; y+=BLK_16)
                {
                    ...
                    for(x=0;x < width_forloop; x+=BLK_16)
                    {
...

                        {
                            ...
                            if(INTENSITY_INTERVAL_MATCH_FAIL != intensityInt)
                            {
                                ...
                                /* 8x8 block grain simulation */
                                fgsFunc->simulate_grain_blk16x16_10bit(grainStripe,
                                        grainStripeOffset,
                                        grainSynt, grainStripeWidth,
                                        log2ScaleFactor,scaleFactor,kOffset,lOffset,h,v);
                    ...
                    if(widthComp[compCtr] & 0xF)
                    {
...

                        {
                            ...
                            blockAvg = block_average_hbd(decSampleHbdBlk16, strideComp[compCtr],
                                    &numSamples,
                                    BLK_16,
                                    MIN(BLK_16, (widthComp[compCtr]-x)),
                                    BLK_16, BLK_16_shift,
                                    bitDepth);
                ...
                if(heightComp[compCtr] & 0xF)
                {
                    ...
                    fgsFunc->blend_stripe_10bit(decSampleHbdOffsetY, grainStripe,
                            strideComp[compCtr], grainStripeWidth,
                            MIN(BLK_16, (heightComp[compCtr]-y)), bitDepth);
                    decSampleHbdOffsetY += MIN(BLK_16, heightComp[compCtr]-y) *
                                                                            strideComp[compCtr];
                } /* end of component loop */
            }
        }
        ...
        for(compCtr=0; compCtr < numComp; compCtr++)
        {
            if(1 == fgcParameters->compModelPresentFlag[compCtr])
            {
                ...
                for(y=0;y < height_forloop; y+=BLK_16)
                {
                    ...
                    for(x=0;x < width_forloop; x+=BLK_16)
                    {
                        ...
                        for(blkId=0;blkId < NUM_8x8_BLKS_16x16;blkId++)
                        {
                            ...
                            blockAvg = fgsFunc->block_avg_8x8_8bit(decSampleBlk8,
                                                                strideComp[compCtr], &numSamples,
                                                                BLK_8_shift);
                            ...
                            if(INTENSITY_INTERVAL_MATCH_FAIL != intensityInt)
                            {
                                ...
                                /* 8x8 block grain simulation */
                                fgsFunc->simulate_grain_blk8x8_8bit(grainStripe,
                                        grainStripeOffsetBlk8,
                                        grainSynt, grainStripeWidth,
                                        log2ScaleFactor,scaleFactor,kOffset,lOffset,h,v);
                    ...
                    if(widthComp[compCtr] & 0xF)
                    {
                        ...
                        for(blkId=0;blkId < NUM_8x8_BLKS_16x16;blkId++)
                        {
                            ...
                            decSampleBlk8 = decSampleBlk16 + offsetBlk8x8;
                            blockAvg = block_average(decSampleBlk8, strideComp[compCtr],
                                    &numSamples,
                                    MIN(BLK_8, (heightComp[compCtr]-y-yOffset8x8)),
                                    MIN(BLK_8, (widthComp[compCtr]-x-xOffset8x8)),
                                    BLK_8, BLK_8_shift);
                ...
                if(heightComp[compCtr] & 0xF)
                {
                        ...
                        for(blkId=0;blkId < NUM_8x8_BLKS_16x16;blkId++)
                        {
                            ...
                            if(numSamples > 0)
                            {
                                ...
                                if(INTENSITY_INTERVAL_MATCH_FAIL != intensityInt)
                                {
                                    ...
                                    fgsFunc->simulate_grain_blk8x8_8bit(grainStripe,
                                            grainStripeOffsetBlk8,
                                            grainSynt, grainStripeWidth,log2ScaleFactor,scaleFactor,
                                            kOffset,lOffset,h,v);
                    ...
                    fgsFunc->blend_stripe_8bit(decSampleOffsetY, grainStripe,
                            strideComp[compCtr], grainStripeWidth, MIN(BLK_16,
                                                                        (heightComp[compCtr]-y)),
                                                                    bitDepth);
            ...
            if(1 == fgcParameters->compModelPresentFlag[compCtr])
            {
                ...
                for(y=0;y < height_forloop; y+=BLK_16)
                {
                    ...
                    for(x=0;x < width_forloop; x+=BLK_16)
                    {
                        ...
                        {
                            blockAvg = fgsFunc->block_avg_16x16_8bit(decSampleBlk16,
                                    strideComp[compCtr], &numSamples,
                                    BLK_16_shift);
                            ...
                            if(INTENSITY_INTERVAL_MATCH_FAIL != intensityInt)
                            {
                                ...
                                /* 8x8 block grain simulation */
                                fgsFunc->simulate_grain_blk16x16_8bit(grainStripe,grainStripeOffset,
                                        grainSynt, grainStripeWidth, log2ScaleFactor,scaleFactor,
                                        kOffset,lOffset,h,v);
...

                        {
                            ...
                            if(numSamples > 0)
                            {
                                ...
                                if(INTENSITY_INTERVAL_MATCH_FAIL != intensityInt)
                                {
                                    ...
                                    /* 8x8 block grain simulation */
                                    fgsFunc->simulate_grain_blk16x16_8bit(grainStripe,
                                            grainStripeOffset,
                                            grainSynt, grainStripeWidth,
                                            log2ScaleFactor,scaleFactor,
                                            kOffset,lOffset,h,v);
...
uint32_t fgs_process(GrainCharacteristicApi fgsParamStruct,void *handle)
{
    ...
    uint32_t width,height,widthComp[FGS_MAX_NUM_COMP],heightComp[FGS_MAX_NUM_COMP],
    ...
    for(compCtr=0; compCtr < numComp; compCtr++)
    {
        ...
        heightforLastThr[compCtr] = heightComp[compCtr] -
                                        numSripesPerThr[compCtr]*(numThreads - 1)*BLK_16;
    }
    ...
    for(int z=0; z < numThreads; z++)
    {
        ...
        for(compCtr=0; compCtr < numComp; compCtr++)
        {
            ...
            if(1 == fgsParamStruct.fgcParameters.compModelPresentFlag[compCtr])
            {
                ...
                args->fgsArgs.fgsOffsets[compCtr] = offsetArr[compCtr] +
                    z * numSripesPerThr[compCtr] * numBlks_x[compCtr];

                args->fgsArgs.decComp[compCtr] = (void *)(decComp[compCtr] +
                        offset_dec_bufs * z * numSripesPerThr[compCtr] *
                                            BLK_16 * strideComp[compCtr]);
...
void * fgs_create(FGSInitParams *initParams)
{
    ...
    {
        psFgsHandle->scratchMem = malloc(wdMult16 * BLK_16 * initParams->numThreads *
                                    sizeof(int8_t));
    ...
    for(int z=0; z < initParams->numThreads; z++){
        ...
        psFgsHandle->threadCntxArr[z].thrdHandle = malloc(fgs_thread_get_handle_size());
        psFgsHandle->threadCntxArr[z].semVar = malloc(fgs_thread_get_sem_struct_size());
        psFgsHandle->threadCntxArr[z].mutexVar = malloc(fgs_thread_get_mutex_lock_size());
    ...
    for(int z=0; z < initParams->numThreads -1; z++){
        fgs_thread_create(psFgsHandle->threadCntxArr[z].thrdHandle, NULL, (void*)(&workerThread),
...
uint32_t fgs_validate_input_params(GrainCharacteristicApi fgsParamStruct)
{
    GrainSynthesisStruct grainSynt;
    uint32_t errorCode;
    errorCode =  validate_film_grain_parameters(fgsParamStruct.width,
            fgsParamStruct.height, fgsParamStruct.chromaFormat,
            fgsParamStruct.bitDepth, &(fgsParamStruct.fgcParameters),
            &grainSynt);
    return errorCode;
}
