...
{
  int32_t i = 0;
  /* Check for occurence of any int8_tacters other than ' ' and '\t' */
  for (i = 0; ; i++)
  {
    if (' ' != line[i] && '\t' != line[i])
      break;
  ...
  for (;;)
  {
    if (0 != fgets(line, UNIT_APP_MAX_LINE_LEN, cur_fp))
    {
      ...
      else if (0 == strcmp(g_param[0], "intensity_interval_lower_bound"))
      {
        ...
        for (i = 0; i < FGS_MAX_NUM_COMP; i++)
        {
          max_num_of_values += (psFgcParameters->compModelPresentFlag[i] * (psFgcParameters->numIntensityIntervalsMinus1[i] + 1));
        }
        printf("intensity_interval_lower_bound : ");
        i = 0;
        for (j = 0; j < FGS_MAX_NUM_COMP; j++)
        {
          num_intensity_itvls = (1 + psFgcParameters->numIntensityIntervalsMinus1[j])*psFgcParameters->compModelPresentFlag[j];
      ...
      else if (0 == strcmp(g_param[0], "intensity_interval_upper_bound"))
      {
        ...
        for (i = 0; i < FGS_MAX_NUM_COMP; i++)
        {
          max_num_of_values += psFgcParameters->compModelPresentFlag[i] * (1 + psFgcParameters->numIntensityIntervalsMinus1[i]);
        }
    ...
    else
    {
      ...
      if (psFgsAppParams->startFrameNum >= frameNum)
      {
        ...
        while ((frameNum < psFgsAppParams->endFrameNum) && (frameNum < psFgsAppParams->numFrames))
        {
          ...
          if(fgcCTx.bitDepth > 8)
          {
            fgcCTx.decBufU = (void *)((uint16_t *)pdecPelFrm + psFgsAppParams->frameWidth * psFgsAppParams->frameHeight);
            fgcCTx.decBufV = (void *)((uint16_t *)fgcCTx.decBufU + (psFgsAppParams->frameWidth >> width_shift) * (psFgsAppParams->frameHeight>>height_shift));
          }
          else
          {
            fgcCTx.decBufU = (void *)((uint8_t *)pdecPelFrm + psFgsAppParams->frameWidth * psFgsAppParams->frameHeight);
            fgcCTx.decBufV = (void *)((uint8_t *)fgcCTx.decBufU + (psFgsAppParams->frameWidth >> width_shift) * (psFgsAppParams->frameHeight >> height_shift));
          }
