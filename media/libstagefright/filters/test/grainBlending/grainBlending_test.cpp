...
int32_t get_next_parameter(char *line, uint32_t line_length,
                           uint32_t *position_in_line, char *data,
  ...
  while (' ' == line[*position_in_line] || '\t' == line[*position_in_line] ||
         ...
         '\0' == line[*position_in_line] || '=' == line[*position_in_line] ||
...
int32_t get_next_parameter_allow_spaces(char *line, uint32_t line_length,
                                        uint32_t *position_in_line, char *data,
                                        uint32_t data_length) {
  uint32_t i = 0;
  if (*position_in_line > line_length) {
...
void get_all_params_of_line(
  ...
  for (param_number = 0;; param_number++) {
    if (0 == allow_spaces_for_params) {
      status = get_next_parameter(line, line_length, &position_in_line,
                                  param[param_number], UNIT_APP_MAX_PARAM_LEN);

      if ((0 == status) || (0 == param[param_number]))
        break;
    } else {
      status = get_next_parameter_allow_spaces(
          line, line_length, &position_in_line, param[param_number],
...
int32_t set_input_params(FILE *cur_fp, fgs_app_params *psAppParams) {
  ...
  for (;;) {
    if (0 != fgets(line, UNIT_APP_MAX_LINE_LEN, cur_fp)) {
      ...
      get_all_params_of_line(line, UNIT_APP_MAX_LINE_LEN, g_param, &num_params,
                             1);
    ...
    if (0 != fgets(line, UNIT_APP_MAX_LINE_LEN, cur_fp)) {
      ...
      if (0 == strcmp(g_param[0], "start_frame_num")) {
        ...
        printf("endFrameNum : %d\n", psAppParams->endFrameNum);
                 ...
                 strcmp(g_param[0], "film_grain_characteristics_cancel_flag")) {
        ...
        while (token != NULL) {
          if (i < FGS_MAX_NUM_COMP) {
            ...
            printf("Invalid number of values [%d] being passed to parameter: "
                   "[%s]\n",
                   i, g_param[0]);
            return FAILURE_RET;
          }
        ...
        for (i = 0; i < FGS_MAX_NUM_COMP; i++) {
          max_num_of_values +=
              (psFgcParameters->compModelPresentFlag[i] *
        ...
        for (j = 0; j < FGS_MAX_NUM_COMP; j++) {
          num_intensity_itvls =
              (1 + psFgcParameters->numIntensityIntervalsMinus1[j]) *
          ...
          for (j = 0; j < FGS_MAX_NUM_COMP; j++) {
            ...
            for (k = 0; k < (num_intensity_itvls); k++) {
              num_comp_model_values =
                  (1 + psFgcParameters->numModelValuesMinus1[j]) *
                  psFgcParameters->compModelPresentFlag[j];
              for (v = 0; v < (num_comp_model_values); v++) {
                if (token == NULL) {
                  printf("Invalid number of values [%d] being passed to "
                         "parameter: [%s]\n",
                         i, g_param[0]);
                  return FAILURE_RET;
                } else {
...
int32_t
fgs_parse_config_params(fgs_app_params *psAppParams, FILE *cur_fp,
                        FilmGrainCharacteristicsStruct *psFgcParameters) {
...
int main(int argc, char *argv[]) {
  ...
  void *psFgsHandle;
  fgs_app_params *psFgsAppParams =
      (fgs_app_params *)malloc(sizeof(fgs_app_params));
  ...
  status =
      fgs_parse_config_params(psFgsAppParams, cur_fp, &(fgcCTx.fgcParameters));
  ...
  {
    sizeInBytes =
        (psFgsAppParams->frameWidth * psFgsAppParams->frameHeight * 3) >> 1;
    height_shift = 1;
    width_shift = 1;
  ...
  {
    sizeInBytes =
        (psFgsAppParams->frameWidth * psFgsAppParams->frameHeight * 3);
    height_shift = 0;
    width_shift = 0;
  ...
  for (;;) {
    if (getc(cur_fp) == EOF) {
      ...
      status = fgs_parse_config_params(psFgsAppParams, cur_fp,
                                       &(fgcCTx.fgcParameters));
      if (SUCCESS_RET != status) {
        printf("Failed to parse configuration file: %s\n",
               psFgsAppParams->initParamsCfgFile);
        fclose(cur_fp);
      ...
      if (psFgsAppParams->startFrameNum >= frameNum) {
        if (psFgsAppParams->startFrameNum > frameNum) {
          while ((frameNum < psFgsAppParams->startFrameNum) &&
            ...
            bytesRead =
                fread(pdecPelFrm, sizeInBytes, sizeof(uint8_t), inputFile);
            if (!bytesRead) {
              printf("Reached end of input file\n");
              break;
            }
        ...
        while ((frameNum < psFgsAppParams->endFrameNum) &&
          ...
          if (fgcCTx.bitDepth > 8) {
            ...
            fgcCTx.decBufU = (void *)((uint8_t *)pdecPelFrm +
                                      psFgsAppParams->frameWidth *
                                          psFgsAppParams->frameHeight);
            fgcCTx.decBufV =
        ...
        printf("Invalid value specified for startFrameNum in configuration "
               "file: %s\n",
               psFgsAppParams->initParamsCfgFile);
        fclose(cur_fp);
        fclose(inputFile);
