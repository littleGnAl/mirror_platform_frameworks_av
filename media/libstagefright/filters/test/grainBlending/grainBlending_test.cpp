...
int32_t is_blank(char *line) {
    ...
    if ('\r' != line[i] && '\n' != line[i] && (int8_t)EOF != line[i] && '\0' != line[i])
        return 0;
    else
...
int32_t get_next_parameter(char *line, uint32_t line_length, uint32_t *position_in_line, char *data,
    ...
    while (' ' == line[*position_in_line] || '\t' == line[*position_in_line] ||
           '\n' == line[*position_in_line] || (char)EOF == line[*position_in_line] ||
    ...
    /* found                                                                */
    while (flag || ('=' != line[*position_in_line] && '(' != line[*position_in_line] &&
                    ')' != line[*position_in_line] && (char)EOF != line[*position_in_line] &&
                    '\n' != line[*position_in_line] && '\0' != line[*position_in_line] &&
                    ' ' != line[*position_in_line] && ':' != line[*position_in_line])) {
        ...
        if (*position_in_line > line_length || i > data_length) return 0;
    }
...
int32_t get_next_parameter_allow_spaces(char *line, uint32_t line_length,
    ...
    while (' ' == line[*position_in_line] || '\t' == line[*position_in_line] ||
        ...
        (*position_in_line)++;
        if (*position_in_line > line_length) return 0;
    }
...
void get_all_params_of_line(char *line, uint32_t line_length,
    ...
    for (param_number = 0;; param_number++) {
        if (0 == allow_spaces_for_params) {
            status = get_next_parameter(line, line_length, &position_in_line, param[param_number],
                                        UNIT_APP_MAX_PARAM_LEN);

            if ((0 == status) || (0 == param[param_number])) break;
        } else {
            status = get_next_parameter_allow_spaces(line, line_length, &position_in_line,
                    ...
                    for (j = 0; j < FGS_MAX_NUM_COMP; j++) {
                        ...
                        for (k = 0; k < (num_intensity_itvls); k++) {
                            ...
                            for (v = 0; v < (num_comp_model_values); v++) {
                                if (token == NULL) {
                                    printf(
                                        "Invalid number of values [%d] being passed to parameter: "
                                        "[%s]\n",
                                        i, g_param[0]);
                                    return FAILURE_RET;
                                } else {
...
int32_t fgs_parse_config_params(fgs_app_params *psAppParams, FILE *cur_fp,
    ...
    for (;;) {
        if (0 != fgets(line, UNIT_APP_MAX_LINE_LEN, cur_fp)) {
            ...
            /* Ignore blank lines and comments */
            if (1 == is_comment(line) || 1 == is_blank(line)) continue;
