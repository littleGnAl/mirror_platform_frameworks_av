...
static struct option long_options[] = {
    {"input_file", required_argument, NULL, 'i'},     {"output_file", required_argument, NULL, 'o'},
    {"num_frames", optional_argument, NULL, 'f'},     {"width", required_argument, NULL, 'w'},
    {"height", required_argument, NULL, 'h'},         {"bit_depth", required_argument, NULL, 'b'},
    {"chroma_format", required_argument, NULL, 'c'},  {"num_threads", optional_argument, NULL, 't'},
    {"processor_arch", optional_argument, NULL, 'p'}, {NULL, 0, NULL, 0}};
...
void getCfgFileTokens(ifstream &file, string startMarker, string endMarker, char *tokenWords[],
                      int *numTokens) {
    int startFlag = 0;
    string str;
    ...
    *numTokens = 1;
    ...
    while (std::getline(file, str)) {
        ...
        if (str[0] == '#' || regex_match(str, blankLine)) {
            continue;
        }
        if (!startFlag) {
            if (!regex_match(str, input_start))
                continue;
            else
                startFlag = 1;
            continue;
        ...
        if (regex_match(str, input_end)) break;

        strcpy(tokenWords[0], str.c_str());
        tokenWords[*numTokens] = strtok(tokenWords[0], " \t");
...
void parseInputParams(ifstream &file, fgs_app_params *fgsAppParams) {
    ...
    fargv[0] = NULL;
    while (file.getline(elementArr[curLine], UNIT_APP_MAX_LINE_LEN)) {
        /* Blank and empty lines */
        if (elementArr[curLine][0] == '#' || regex_match(elementArr[curLine], blankLine)) {
            continue;
        }
        if (!startFlag) {
            if (!regex_match(elementArr[curLine], input_start))
                continue;
            else
                startFlag = 1;
            continue;
        ...
        if (regex_match(elementArr[curLine], input_end)) break;

        fargv[fargc] = strtok(elementArr[curLine], " \t");
        curLine++;

        while (fargv[fargc] != NULL) {
            fargc++;
...
int32_t parseFGCParams(ifstream &file, fgs_app_params *fgsAppParams,
    ...
    regex fgs_params_start{"^\\s*FGC_PARAMS_PROFILE_START\\s*"};
    regex fgs_params_end{"^\\s*FGC_PARAMS_PROFILE_END\\s*"};
    regex blankLine{"^\\s*$"};
    ...
    while (file.getline(elementArr[curLine], UNIT_APP_MAX_LINE_LEN)) {
        ...
        if (!startFlag) {
            if (!regex_match(elementArr[curLine], fgs_params_start))
                continue;
            else
                startFlag = 1;
            continue;
        ...
        if (regex_match(elementArr[curLine], fgs_params_end)) break;
    ...
    while ((opt = getopt_long(fargc, fargv, "t:e:f:s:b:p:n:l:u:m:v:", long_options_fgc, NULL)) !=
           -1) {
        switch (opt) {
            case 't':
...
int main(int argc, char *argv[]) {
    ...
    for (;;) {
        if (cfgFile.peek() == EOF) {
            ...
            printf("\n------------------ FGS Start ------------------\n\n");
            printf("start_frame_num : %d\n", psFgsAppParams->startFrameNum);
            printf("end_frame_num : %d\n", psFgsAppParams->endFrameNum);
            ...
            printf("block_size : %d\n", fgcCTx.fgcParameters.blockSize);
            printf("comp_model_present_flag : ");
            for (c = 0; c < 3; c++) {
                printf("%d ", fgcCTx.fgcParameters.compModelPresentFlag[c]);
            }
            printf("\n");
            printf("num_intensity_intervals_minus1 : ");
            for (c = 0; c < 3; c++) {
                if (fgcCTx.fgcParameters.compModelPresentFlag[c]) {
                    printf("%d ", fgcCTx.fgcParameters.numIntensityIntervalsMinus1[c]);
                }
            }
            printf("\n");
            ...
            for (c = 0; c < 3; c++) {
                if (fgcCTx.fgcParameters.compModelPresentFlag[c]) {
                    printf("%d ", fgcCTx.fgcParameters.numModelValuesMinus1[c]);
                }
            }
            printf("\n");
            ...
            for (c = 0; c < 3; c++) {
                if (fgcCTx.fgcParameters.compModelPresentFlag[c]) {
                    for (i = 0; i <= fgcCTx.fgcParameters.numIntensityIntervalsMinus1[c]; i++) {
                        printf("%d ", fgcCTx.fgcParameters.intensityIntervalUpperBound[c][i]);
                    }
                }
            }
            printf("\n");
            ...
            for (c = 0; c < 3; c++) {
                if (fgcCTx.fgcParameters.compModelPresentFlag[c]) {
                    for (i = 0; i <= fgcCTx.fgcParameters.numIntensityIntervalsMinus1[c]; i++) {
                        printf("%d ", fgcCTx.fgcParameters.intensityIntervalLowerBound[c][i]);
                    }
                }
            }
            printf("\n");
            ...
            for (c = 0; c < 3; c++) {
                if (fgcCTx.fgcParameters.compModelPresentFlag[c]) {
                    for (i = 0; i <= fgcCTx.fgcParameters.numIntensityIntervalsMinus1[c]; i++) {
                        for (j = 0; j <= fgcCTx.fgcParameters.numModelValuesMinus1[i]; j++) {
                            printf("%d ", fgcCTx.fgcParameters.compModelValue[c][i][j]);
                        }
                    }
                }
            }
