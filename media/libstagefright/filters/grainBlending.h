...
#ifndef GRAIN_BLENDING_H_
#define GRAIN_BLENDING_H_
...
/* in Q-format of 8 : 1/sqrt(2) */
#define SCALE_DOWN_422 181
...
/* 0 = blksize 8, 1 = blksize 16 */
#define NUM_BLOCK_IDX 2
...
typedef struct GrainSynthesisStruct_t {
    int8_t dataBase[NUM_BLOCK_IDX][NUM_CUT_OFF_FREQ][NUM_CUT_OFF_FREQ][DATA_BASE_SIZE]
                   [DATA_BASE_SIZE];
...
template <typename T>
struct fgsFxns {
    void (*blend_stripe)(T *decSampleOffsetY, int8_t *grainStripe, uint32_t widthComp,
                         uint32_t grainStripeWidth, uint32_t blockHeight, uint8_t bitDepth);

    uint32_t (*block_avg)(T *decSampleBlk, uint32_t widthComp, uint8_t bitDepth);

    void (*simulate_grain)(int8_t *grainStripe, uint32_t grainStripeOffsetBlk,
...
typedef enum {
    ...
    FGS_SUCCESS = 0,
    /* Invalid input width */
    FGS_INVALID_WIDTH = 0x01,
    /* Invalid input height */
    FGS_INVALID_HEIGHT = 0x02,
    /* Invalid Chroma format idc */
    FGS_INVALID_CHROMA_FORMAT = 0x03,
    /* Invalid bit depth */
    FGS_INVALID_BIT_DEPTH = 0x04,
...
#ifdef __x86_64__
void blend_stripe_sse42(uint16_t *decSampleHbdOffsetY, int8_t *grainStripe, uint32_t widthComp,
                        uint32_t grainStripeWidth, uint32_t blockHeight, uint8_t bitDepth);

void blend_stripe_sse42(uint8_t *decSampleOffsetY, int8_t *grainStripe, uint32_t widthComp,
                        uint32_t grainStripeWidth, uint32_t blockHeight, uint8_t bitDepth);

uint32_t block_avg_8x8_sse42(uint16_t *decSampleBlk8, uint32_t widthComp, uint8_t bitDepth);

uint32_t block_avg_16x16_sse42(uint16_t *decSampleBlk8, uint32_t widthComp, uint8_t bitDepth);

uint32_t block_avg_8x8_sse42(uint8_t *decSampleBlk8, uint32_t widthComp, uint8_t bitDepth);

uint32_t block_avg_16x16_sse42(uint8_t *decSampleBlk8, uint32_t widthComp, uint8_t bitDepth);

void simulate_grain_blk16x16_sse42(int8_t *grainStripe, uint32_t grainStripeOffsetBlk8,
                                   GrainSynthesisStruct *grain_synt, uint32_t width,
                                   uint8_t log2ScaleFactor, int16_t scaleFactor, uint32_t kOffset,
                                   uint32_t lOffset, uint8_t h, uint8_t v);

void simulate_grain_blk8x8_sse42(int8_t *grainStripe, uint32_t grainStripeOffsetBlk8,
                                 GrainSynthesisStruct *grain_synt, uint32_t width,
                                 uint8_t log2ScaleFactor, int16_t scaleFactor, uint32_t kOffset,
                                 uint32_t lOffset, uint8_t h, uint8_t v);
#endif
