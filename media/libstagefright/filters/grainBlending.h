...
/*!
 **************************************************************************
 * \file grainBlending.h
 *
 * \brief
 *    Declarartions of functions, structures and macros for grain blending
 *
 * \date
 *    14/09/2021
 *
...
#ifndef GRAIN_BLEND_H_
#define GRAIN_BLEND_H_
...
typedef enum {
  ARCH_ARM_GENERIC = 0x100,
  ARCH_ARMV8_INTRINSICS,
  ARCH_X86_GENERIC = 0x200,
  ARCH_X86_AVX_INTRINSICS
...
typedef struct FilmGrainCharacteristicsStruct_t {
  ...
  uint8_t intensityIntervalLowerBound[FGS_MAX_NUM_COMP]
                                     [FGS_MAX_NUM_INTENSITIES];
  /* Upper bound of intensity interval */
  uint8_t intensityIntervalUpperBound[FGS_MAX_NUM_COMP]
                                     [FGS_MAX_NUM_INTENSITIES];
  /* Component model values for each intensity interval */
  uint32_t compModelValue[FGS_MAX_NUM_COMP][FGS_MAX_NUM_INTENSITIES]
                         [FGS_MAX_NUM_MODEL_VALUES];
...
typedef struct GrainSynthesisStruct_t {
  int8_t dataBase[BLOCK_IDX][NUM_CUT_OFF_FREQ][NUM_CUT_OFF_FREQ][DATA_BASE_SIZE]
                 [DATA_BASE_SIZE];
...
typedef struct fgsFxns {
  void (*blend_stripe_10bit)(uint16_t *decSampleHbdOffsetY,
                             int16_t *grainStripe, uint32_t widthComp,
                             uint32_t grainStripeWidth, uint32_t blockHeight,
                             uint8_t bitDepth);
  ...
  uint32_t (*block_avg_8x8_10bit)(uint16_t *decSampleBlk8, uint32_t widthComp,
                                  uint16_t *pNumSamples, uint8_t bitDepth);
  uint32_t (*block_avg_16x16_10bit)(uint16_t *decSampleBlk8, uint32_t widthComp,
                                    uint16_t *pNumSamples, uint8_t bitDepth);
  uint32_t (*block_avg_8x8_8bit)(uint8_t *decSampleBlk8, uint32_t widthComp,
                                 uint16_t *pNumSamples, uint8_t shift_Size);
  uint32_t (*block_avg_16x16_8bit)(uint8_t *decSampleBlk8, uint32_t widthComp,
                                   uint16_t *pNumSamples, uint8_t shift_Size);
  void (*simulate_grain_blk8x8_8bit)(int8_t *grainStripe,
  ...
  void (*simulate_grain_blk16x16_8bit)(int8_t *grainStripe,
                                       uint32_t grainStripeOffsetBlk8,
                                       GrainSynthesisStruct *grain_synt,
                                       uint32_t width, uint8_t log2ScaleFactor,
                                       int16_t scaleFactor, uint32_t kOffset,
                                       uint32_t lOffset, uint8_t h, uint8_t v);
  void (*simulate_grain_blk8x8_10bit)(int16_t *grainStripe,
                                      uint32_t grainStripeOffsetBlk8,
                                      GrainSynthesisStruct *grain_synt,
                                      uint32_t width, uint8_t log2ScaleFactor,
                                      int16_t scaleFactor, uint32_t kOffset,
                                      uint32_t lOffset, uint8_t h, uint8_t v);
  void (*simulate_grain_blk16x16_10bit)(int16_t *grainStripe,
                                        uint32_t grainStripeOffsetBlk8,
                                        GrainSynthesisStruct *grain_synt,
                                        uint32_t width, uint8_t log2ScaleFactor,
                                        int16_t scaleFactor, uint32_t kOffset,
...
typedef enum {
  ...
  /* Invalid num threads value */
  FGS_INVALID_NUM_THREADS = FGS_PARAM_ERROR + 0x0E,
  /* Failure error code */
  FGS_FAIL = 0xFF
} FGS_ERROR_T;
