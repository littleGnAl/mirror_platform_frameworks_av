...
class StagefrightRecorderTest
  ...
  public:
    ...
    void SetUp() override {
        ...
        ASSERT_NE(mOutputAudioFp, nullptr) << "Failed to open output file %s"
                                           << OUTPUT_FILE_NAME_AUDIO << " for stagefright recorder";

        int32_t fd = fileno(mOutputAudioFp);
        ASSERT_GE(fd, 0) << "Failed to get the file descriptor of the output file for %s"
                         << OUTPUT_FILE_NAME_AUDIO;

        status_t status = mStfRecorder->setOutputFile(fd);
        ASSERT_EQ(status, OK) << "Failed to set the output file %s" << OUTPUT_FILE_NAME_AUDIO
    ...
    void TearDown() override {
        ...
        if (!kDebug) {
            ...
            ASSERT_EQ(status, 0) << "Unable to delete the output file %s" << OUTPUT_FILE_NAME_AUDIO;
        }
    }
...
void StagefrightRecorderTest::setAudioRecorderFormat(output_format outputFormat,
    ...
    ASSERT_EQ(status, OK) << "Failed to set the audio source: %d" << audioSource;

    status = mStfRecorder->setOutputFormat(outputFormat);
    ASSERT_EQ(status, OK) << "Failed to set the output format: %d" << outputFormat;

    status = mStfRecorder->setAudioEncoder(encoder);
    ASSERT_EQ(status, OK) << "Failed to set the audio encoder: %d" << encoder;
}
...
void StagefrightRecorderTest::recordMedia(bool isPause, int32_t numStart, int32_t numPause) {
    ...
    if (isPause) {
        ...
        std::this_thread::sleep_for(std::chrono::seconds(kClipDurationInSec));
        auto tEnd = std::chrono::high_resolution_clock::now();
        mExpectedDurationInMs = mExpectedDurationInMs +
...
void StagefrightRecorderTest::setupExtractor(AMediaExtractor *extractor, int32_t &trackCount) {
    ...
    ASSERT_GE(fd, 0) << "Failed to open recorder's output file %s" << OUTPUT_FILE_NAME_AUDIO
                     << " to validate";

    struct stat buf;
    int32_t status = fstat(fd, &buf);
    ASSERT_EQ(status, 0) << "Failed to get properties of input file %s" << OUTPUT_FILE_NAME_AUDIO
                         << " for extractor";

    size_t fileSize = buf.st_size;
    ASSERT_GT(fileSize, 0) << "Size of input file %s" << OUTPUT_FILE_NAME_AUDIO
...
void StagefrightRecorderTest::validateOutput(bool isPaused) {
    ...
    ASSERT_EQ(trackCount, 1) << "No tracks reported by extractor";
    ...
    for (int32_t idx = 0; idx < trackCount; idx++) {
        ...
        int32_t totalClipDurationInUs = mExpectedDurationInMs * 1000;
        if (isPaused) {
            totalClipDurationInUs = (mExpectedDurationInMs - mExpectedPauseInMs) * 1000;
        }
