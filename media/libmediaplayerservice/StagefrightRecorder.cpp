...
status_t StagefrightRecorder::setupVideoEncoder(
    ...
    auto createEncoder = [](const sp<ALooper> &looper, const sp<AMessage> &format,
            ...
            const AttributionSourceState& attributionSource) -> status_t {
        uid_t uid = VALUE_OR_RETURN_STATUS(aidl2legacy_int32_t_uid_t(attributionSource.uid));
        pid_t pid = VALUE_OR_RETURN_STATUS(aidl2legacy_int32_t_pid_t(attributionSource.pid));
    ...
    if (createAsync && futureVal) {
        *futureVal = std::async(std::launch::async, createEncoder,
            ...
            flags, mGraphicBufferProducer, mAttributionSource);
        return OK;
    }

    auto err = createEncoder(mLooper, format, cameraSource, source,
        mPersistentSurface, flags, mGraphicBufferProducer, mAttributionSource);
