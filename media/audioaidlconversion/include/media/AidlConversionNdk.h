...
template <typename T, typename aidl::android::hardware::audio::effect::Parameter::Specific::Tag tag>
ConversionResult<T> getParameterSpecific(
        const aidl::android::hardware::audio::effect::Parameter& u) {
    const auto& spec = VALUE_OR_RETURN(UNION_GET(u, specific));
    return unionGetField<aidl::android::hardware::audio::effect::Parameter::Specific, tag>(spec);
}

#define GET_PARAMETER_SPECIFIC(u, specific, tag) \
    getParameterSpecific<specific,               \
                         aidl::android::hardware::audio::effect::Parameter::Specific::Tag::tag>(u)

template <typename T, typename aidl::android::hardware::audio::effect::Parameter::Specific::Tag tag,
          typename T::Tag field, typename F>
ConversionResult<F> getParameterSpecificField(
        const aidl::android::hardware::audio::effect::Parameter& u) {
    const auto& spec = VALUE_OR_RETURN((getParameterSpecific<T, tag>(u)));
    return VALUE_OR_RETURN((unionGetField<T, field>(spec)));
}

#define GET_PARAMETER_SPECIFIC_FIELD(u, specific, tag, field, fieldType)                        \
    getParameterSpecificField<specific,                                                         \
