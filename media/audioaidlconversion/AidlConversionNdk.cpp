...
aidl2legacy_Flags_uint32(Flags aidl) {
    uint32_t legacy = 0;
    switch (aidl.type) {
...
ConversionResult<Flags>
legacy2aidl_uint32_Flags(const uint32_t legacy) {
    Flags aidl;
...
legacy2aidl_AudioConfigBase_buffer_config_t(const buffer_config_t& legacy, bool isInput) {
    ...
    if (legacy.mask & EFFECT_CONFIG_SMP_RATE) {
        aidl.sampleRate = VALUE_OR_RETURN(convertIntegral<int32_t>(aidl.sampleRate));
    }
