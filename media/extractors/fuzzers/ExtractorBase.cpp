...
bool ExtractorBase::setDataSource(const uint8_t* data, size_t size) {
  if ((!data) || (size == 0)) {
    return false;
  }
...
bool ExtractorBase::getExtractorDef() {
  float confidence;
  void* meta = nullptr;
...
bool ExtractorBase::extractTracks() {
...
void ExtractorBase::extractTrack(MediaTrackHelper* track, MediaBufferGroup* bufferGroup) {
...
bool ExtractorBase::getTracksMetadata() {
...
bool ExtractorBase::getMetadata() {
  AMediaFormat* format = AMediaFormat_new();
...
void ExtractorBase::setDataSourceFlags(uint32_t flags) {
