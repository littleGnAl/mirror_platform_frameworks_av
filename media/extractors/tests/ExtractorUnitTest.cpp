...
constexpr int32_t kRandomSeekToleranceUs = 40000;  // 40 ms;
...
void randomSeekTest(MediaTrackHelper *track, int64_t clipDuration) {
    ...
    do {
        ...
        if (buffer) {
            ...
            ASSERT_TRUE(hasTimestamp)
                    << "Extractor didn't set timestamp for the given sample";

            buffer->release();
            EXPECT_LE(abs(timeStamp - seekToTimeStamp), kRandomSeekToleranceUs)
                    << "Seek unsuccessful. Expected timestamp range ["
                    << seekToTimeStamp - kRandomSeekToleranceUs << ", "
                    << seekToTimeStamp + kRandomSeekToleranceUs << "] "
                    << "received " << timeStamp;
        }
        delete options;
...
TEST_P(ExtractorUnitTest, SeekTest) {
    ...
    for (int32_t idx = 0; idx < numTracks; idx++) {
        ...
        // Flac, Wav and Midi extractor can give samples from any pts and mark the given sample as
