...
TEST_P(ExtractorUnitTest, CreateExtractorTest) {
    ...
    EXPECT_EQ(setDataSource(inputFileName), 0)
            << "SetDataSource failed for" << GetParam().first << "extractor";

    EXPECT_EQ(createExtractor(), 0)
...
TEST_P(ExtractorUnitTest, MetaDataComparisonTest) {
    ...
    for (int32_t idx = 0; idx < numTracks; idx++) {
        ...
        if (!strncmp(extractorMime, "audio/", 6)) {
            ...
            ASSERT_TRUE(AMediaFormat_getInt32(extractorFormat, AMEDIAFORMAT_KEY_CHANNEL_COUNT, &exChannelCount));
            ASSERT_TRUE(AMediaFormat_getInt32(extractorFormat, AMEDIAFORMAT_KEY_SAMPLE_RATE, &exSampleRate));
            ASSERT_TRUE(AMediaFormat_getInt32(trackFormat, AMEDIAFORMAT_KEY_CHANNEL_COUNT, &trackChannelCount));
            ASSERT_TRUE(AMediaFormat_getInt32(trackFormat, AMEDIAFORMAT_KEY_SAMPLE_RATE, &trackSampleRate));
...
TEST_P(ExtractorUnitTest, SeekTest) {
    ...
    if (!(seekFlag &
          (MediaExtractorPluginHelper::CAN_SEEK_BACKWARD |
           MediaExtractorPluginHelper::CAN_SEEK_FORWARD | MediaExtractorPluginHelper::CAN_SEEK))) {
    ...
    for (int32_t idx = 0; idx < numTracks; idx++) {
        ...
        for (int32_t mode = CMediaTrackReadOptions::SEEK_PREVIOUS_SYNC;
            ...
            for (int32_t seekCount = 0; seekCount < kMaxCount; seekCount++) {
                ...
                EXPECT_NE(options, nullptr) << "Cannot create read option";
                ...
                if (buffer) {
                    ...
                    switch (mode & 0x7) {
                        ...
                        case CMediaTrackReadOptions::SEEK_CLOSEST:
                            // For Extractors like MPEG2TS, seek_closest is not supported.
                            // The timestamp will be previous seek pts. Hence we check for
                            // EXPECT_LE instead of a EXPECT_EQ
                            EXPECT_LE(timeStamp, seekablePoints[seekIdx]);
                            break;
                        default:
                            break;
                    }
