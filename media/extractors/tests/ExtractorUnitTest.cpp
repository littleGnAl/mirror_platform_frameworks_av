  ...
  public:
    ...
    void setupExtractor(string writerFormat) {
        ...
        static const std::map<std::string, standardExtractors> mapExtractor = {
                {"aac", AAC},         {"amr", AMR},         {"flac", FLAC},   {"mid", MIDI},
                {"midi", MIDI},       {"mkv", MKV},         {"mp3", MP3},     {"mp4", MPEG4},
                {"mpeg2ps", MPEG2PS}, {"mpeg2ts", MPEG2TS}, {"mpeg4", MPEG4}, {"mpg", MPEG2PS},
                {"ogg", OGG},         {"opus", OGG},        {"ts", MPEG2TS},  {"wav", WAV},
                {"webm", MKV}};
...
size_t allocateOutputBuffers(string inputFileName, AMediaFormat *extractorFormat) {
    ...
    if (!strcmp(mime, MEDIA_MIMETYPE_AUDIO_RAW) && !strncmp(mime, "audio/", 6)) {
        ...
        if (!AMediaFormat_getInt64(extractorFormat, AMEDIAFORMAT_KEY_DURATION, &clipDuration) ||
            clipDuration <= 0) {
        ...
        bufferSize = bitsPerSampple * channelCount * sampleRate * (clipDuration / 1000000 + 1);
    } else {
        struct stat buf;
