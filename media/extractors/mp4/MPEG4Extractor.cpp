...
status_t MPEG4Extractor::parseChunk(off64_t *offset, int depth) {
    ...
    switch(chunk_type) {
        ...
        {
            // ISO/IEC 23008-3
            const uint32_t mhac_header_size = 4 /* mhaC size */ + 4 /* mhaC string */
            ...
            uint32_t mhac_size = U16_AT(&mhac_info[11]);
            if (chunk_size != mhac_header_size + mhac_size) {
                return ERROR_MALFORMED;
            }

            data_offset += sizeof(mhac_info);
            uint8_t cookie[chunk_size - sizeof(mhac_info)];
            if (mDataSource->readAt(
