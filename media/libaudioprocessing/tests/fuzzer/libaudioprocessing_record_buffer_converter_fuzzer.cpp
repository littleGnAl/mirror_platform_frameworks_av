...
// http://cs/android/system/media/audio/include/system/audio-hal-enums.h?l=571&rcl=d1beeda6c9de201a4fc95bf1e1e5f51fb7529635
const uint32_t FUZZ_AUDIO_FORMATS[] = {
...
const size_t NUM_AUDIO_FORMATS = sizeof(FUZZ_AUDIO_FORMATS) / sizeof(FUZZ_AUDIO_FORMATS[0]);

extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  FuzzedDataProvider fdp(data, size);
  fdp.ConsumeIntegral<int>();

  audio_channel_mask_t srcChannelMask = fdp.ConsumeIntegral<int>();
  audio_format_t srcFormat =
      (audio_format_t)FUZZ_AUDIO_FORMATS[fdp.ConsumeIntegralInRange<int>(0, NUM_AUDIO_FORMATS - 1)];
  uint32_t srcSampleRate = fdp.ConsumeIntegralInRange<int>(1, 0x7fffffff);
  audio_channel_mask_t dstChannelMask = fdp.ConsumeIntegral<int>();
  audio_format_t dstFormat =
      (audio_format_t)FUZZ_AUDIO_FORMATS[fdp.ConsumeIntegralInRange<int>(0, NUM_AUDIO_FORMATS - 1)];
  uint32_t dstSampleRate = fdp.ConsumeIntegralInRange<int>(1, 0x7fffffff);

  // Certain formats will result in fatal errors that aren't interesting crashes
  // fuzzing.  Don't use those ones.
  uint32_t dstChannelCount = audio_channel_count_from_in_mask(dstChannelMask);
  android::AudioResampler::src_quality quality =
  ...
  uint32_t srcChannelCount = audio_channel_count_from_in_mask(srcChannelMask);
  ...
  uint32_t srcFrameSize = srcChannelCount * audio_bytes_per_sample(srcFormat);
  int srcNumFrames = fdp.ConsumeIntegralInRange<int>(0, MAX_FRAMES);
  const size_t metadataSize = 2 + 3 * sizeof(int) + 2 * sizeof(float);
  ...
  uint32_t dstFrameSize = dstChannelCount * audio_bytes_per_sample(dstFormat);
  size_t frames = fdp.ConsumeIntegralInRange<size_t>(0, MAX_FRAMES + 1);
