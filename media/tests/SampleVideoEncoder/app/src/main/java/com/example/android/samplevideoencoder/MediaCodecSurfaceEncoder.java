...
package com.example.android.samplevideoencoder;
...
public class MediaCodecSurfaceEncoder {
    ...
    private final int MAX_B_FRAMES = 1;
    ...
    private final boolean mIsEncoderTypeSoftware;
    ...
    public MediaCodecSurfaceEncoder(Context context, int resId, String mime, boolean isSoftware,
        ...
        mResID = resId;
        mMime = mime;
        mIsEncoderTypeSoftware = isSoftware;
        mOutputPath = outputPath;
    }

    void startEncodingSurface() throws IOException, InterruptedException {
        ...
        if (decoder == null) {
            mExtractor.release();
            Log.e(TAG, "No suitable decoder found for format: " + decoderFormat.toString());
            return;
        }
        ...
        ArrayList<String> listOfEncoders =
                MediaCodecBase.selectCodecs(mMime, null, null, true, mIsEncoderTypeSoftware);
        if (listOfEncoders.isEmpty()) {
            Log.e(TAG, "No suitable codecs found for mime: " + mMime);
            return;
        }
    ...
    private MediaFormat setUpEncoderFormat(MediaFormat decoderFormat) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            encoderFormat.setInteger(MediaFormat.KEY_MAX_B_FRAMES, MAX_B_FRAMES);
    ...
    private void dequeueDecoderOutput(int bufferIndex, MediaCodec.BufferInfo info) {
        ...
        Log.d(TAG, "output: id: " + bufferIndex + " flags: " + info.flags + " size: " + info.size +
    ...
    private void enqueueDecoderInput(int bufferIndex) {
        ...
        if (size < 0) {
            ...
            Log.d(TAG,
                    "input: id: " + bufferIndex + " size: " + size + " pts: " + pts + " flags: " +
                            codecFlags);
    ...
    private void doWork(int frameLimit) throws InterruptedException {
        ...
        while (!hasSeenError() && !mSawDecInputEOS && frameCount < frameLimit) {
            ...
            // encoder output
            if (mDecOutputCount - mEncOutputCount > MAX_B_FRAMES) {
                tryEncoderOutput();
            }
        }
    }
