...
public class NalUnitUtil {
    ...
    public static int findAVCFrameType(ByteBuffer buf) {
        int limit = buf.limit();
        ...
        for (int pos = 0; pos + 3 < limit; ) {
            ...
            if (startOffset != 0) {
                ...
                if (nalUnitType == 1 || nalUnitType == 5) {
                    sliceType = parseAVCNALUnitData(dataArray, (pos + startOffset),
    ...
    public static int findHEVCFrameType(ByteBuffer buf) {
        int limit = buf.limit();
        byte[] dataArray = new byte[buf.remaining()];
        buf.get(dataArray);
        int sliceType = -1;
        for (int pos = 0; pos + 3 < limit; ) {
            ...
            if (startOffset != 0) {
                ...
                if (nalUnitType >= 0 && nalUnitType <= 21) {
                    sliceType = parseHEVCNALUnitData(dataArray, (pos + startOffset),
