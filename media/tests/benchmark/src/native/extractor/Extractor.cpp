...
int32_t Extractor::setupTrackFormat(int32_t trackId) {
    ...
    bool durationFound = AMediaFormat_getInt64(mFormat, AMEDIAFORMAT_KEY_DURATION, &mDuration);
    if (!durationFound) return -1;
...
void Extractor::dumpStatistics(std::string inputFile) {
    mTimer->dumpStatistics(inputFile);
    double timeTakenPerSec = ((1.0 * mTimer->getTotalTime()) / mDuration) * 1000000;
    std::cout << "Time taken per second : " << timeTakenPerSec << endl;
}
