...
public class Stats {
    ...
    public void setInitTime(long initTime) {
        mInitTimeNs = initTime;
    }

    public void setDeInitTime(long deInitTime) {
        mDeInitTimeNs = deInitTime;
    }
    ...
    public void addFrameSize(int size) {
        mFrameSizes.add(size);
    }

    public void addInputTime() {
        mInputTimer.add(System.nanoTime());
    }
    ...
    public void reset() {
        if (mFrameSizes.size() != 0) mFrameSizes.clear();
        if (mInputTimer.size() != 0) mInputTimer.clear();
        if (mOutputTimer.size() != 0) mOutputTimer.clear();
    }
    ...
    public long getDeInitTime() {
        return mDeInitTimeNs;
    }

    public long getTimeDiff(long sTime, long eTime) {
        return (eTime - sTime);
    }

    private long getTotalTime() {
        if (mOutputTimer.size() == 0) return -1;
     ...
     */
    public void dumpStatistics(String operation, String inputReference, long duarationUs) {
        if (mOutputTimer.size() == 0) {
            Log.e(TAG, "No output produced");
            return;
        }
        long totalTimeTakenNs = getTotalTime();
        long timeTakenPerSec = (totalTimeTakenNs * 1000000) / duarationUs;
