...
status_t DeviceHalAidl::createAudioPatch(unsigned int num_sources,
    ...
    // In tests it's possible that it's value is AUDIO_PATCH_HANDLE_NONE.
    ...
    auto fwkHandleIt = *patch != AUDIO_PATCH_HANDLE_NONE ?
            mFwkHandleToPatchId.find(*patch) : mFwkHandleToPatchId.end();
    AudioPatch aidlPatch;
    if (fwkHandleIt != mFwkHandleToPatchId.end()) {
        existingPatchIt = mPatches.find(fwkHandleIt->second);
    ...
    if (existingPatchIt != mPatches.end()) {
        ...
        if (fwkHandleIt != mFwkHandleToPatchId.end()) {
            fwkHandleIt->second = aidlPatch.id;
            ...
            if (*patch == AUDIO_PATCH_HANDLE_NONE) {
                ...
                *patch = aidlPatch.id;
                LOG_ALWAYS_FATAL_IF(mFwkHandleToPatchId.count(*patch) > 0,
                        "%s: patch id %d clashes with another framework patch handle",
                        __func__, *patch);
            }
            mFwkHandleToPatchId.emplace(*patch, aidlPatch.id);
        }
    }
    cleanups.disarmAll();
    return OK;
...
status_t DeviceHalAidl::releaseAudioPatch(audio_patch_handle_t patch) {
    TIME_CHECK();
    if (!mModule) return NO_INIT;
    auto idMapIt = mFwkHandleToPatchId.find(patch);
    if (idMapIt == mFwkHandleToPatchId.end()) {
        return BAD_VALUE;
    }
    RETURN_STATUS_IF_ERROR(statusTFromBinderStatus(mModule->resetAudioPatch(idMapIt->second)));
    mFwkHandleToPatchId.erase(idMapIt);
    return OK;
}
