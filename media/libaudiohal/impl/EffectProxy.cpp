...
ndk::ScopedAStatus EffectProxy::setOffloadParam(const effect_offload_param_t* offload) {
    ...
    ALOGI("%s: active %soffload sub-effect: %s, flags %s", __func__, offload->isOffload ? "" : "non-",
          mActiveSub.uuid.toString().c_str(), std::get<SubEffectTupleIndex::DESCRIPTOR>(itor->second).common.flags.toString().c_str());
    return ndk::ScopedAStatus::ok();
}
