...
ndk::ScopedAStatus EffectProxy::setActiveSubEffect(const ActiveCheckerCallback& activeChecker) {
    ALOGV("%s: %s", __func__, mIdentity.type.toString().c_str());
...
ndk::ScopedAStatus EffectProxy::open(const Parameter::Common& common,
    ...
    for (auto& sub : mSubEffects) {
        ...
        if (effect) {
            ...
            ALOGI("%s: %s FMQ [%p %p %p] vs [%p %p %p] ret %p", __func__,
                  sub.first.uuid.toString().c_str(), &openRet.statusMQ, &openRet.inputDataMQ,
                  &openRet.outputDataMQ,
                  &std::get<SubEffectTupleIndex::RETURN>(sub.second).statusMQ,
                  &std::get<SubEffectTupleIndex::RETURN>(sub.second).inputDataMQ,
                  &std::get<SubEffectTupleIndex::RETURN>(sub.second).outputDataMQ, &openRet);
        }
    }
