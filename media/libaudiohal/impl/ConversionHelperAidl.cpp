...
status_t parseAndGetVendorParameters(
    ...
    RETURN_STATUS_IF_ERROR(statusTFromBinderStatus(vendorExt->parseVendorParameterIds(
                            IHalAdapterVendorExtension::ParameterScope(recipient.index()),
    ...
    if (recipient.index() == static_cast<int>(ParameterScope::MODULE)) {
        ...
        LOG_ALWAYS_FATAL("%s: unexpected recipient variant index: %zu", __func__, recipient.index());
    }
    if (!parameters.empty()) {
        ...
        if (!vendorParameters.empty()) {
            ...
            values->append(vendorParameters.c_str());
        }
    }
    return OK;
}
