...
DevicesFactoryHalAidl::DevicesFactoryHalAidl(std::shared_ptr<IModule> iModule) {
    ALOG_ASSERT(iModule != nullptr, "Provided default IModule service is NULL");
    mIModule = iModule;
}
...
AudioHalVersionInfo DevicesFactoryHalAidl::getHalVersion() const {
    ...
    if (mIModule) {
        if (!mIModule->getInterfaceVersion(&versionNumber).isOk()) {
...
extern "C" __attribute__((visibility("default"))) void* createCoreFactoryImpl() {
    auto service = IModule::fromBinder(
            ndk::SpAIBinder(AServiceManager_waitForService(IModule::descriptor)));
