          ...
          for (const auto& desc : mHalDescList) {
              ...
              if (desc.common.id.proxy.has_value()) {
                  ...
                  if (0 == proxyMap.count(uuid)) {
                      proxyMap.insert(
                              {uuid, ndk::SharedRefBase::make<EffectProxy>(uuid, mFactory)});
                  }
      ...
      mProxyDescList([this]() {
          ...
          for (const auto& proxy : mUuidProxyMap) {
              if (Descriptor desc; proxy.second && proxy.second->getDescriptor(&desc).isOk()) {
                  list.emplace_back(desc);
              }
          }
          return list;
      }()),
      mEffectNums(mHalDescList.size() + mProxyDescList.size()) {
    ALOG_ASSERT(effectsFactory != nullptr, "Provided IEffectsFactory service is NULL");
    ALOGI("%s with %zu halEffects %zu proxyEffects", __func__, mHalDescList.size(),
          mProxyDescList.size());
}
...
status_t EffectsFactoryHalAidl::getDescriptor(uint32_t index, effect_descriptor_t* pDescriptor) {
    ...
    if (index >= mEffectNums) {
        ALOGE("%s index %d exceed max number %zu", __func__, index, mEffectNums);
...
status_t EffectsFactoryHalAidl::createEffect(const effect_uuid_t* uuid, int32_t sessionId,
    ...
    if (isProxyEffect(aidlUuid)) {
        ...
        RETURN_STATUS_IF_ERROR(mUuidProxyMap.at(aidlUuid)->create());
    } else {
