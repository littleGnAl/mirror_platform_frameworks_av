...
status_t EffectConversionHelperAidl::setAecParameter(EffectParamReader& param) {
    ...
    if (!param.validateParamValueSize(sizeof(uint32_t), sizeof(uint32_t)) ||
        !param.readFromParameter(&type) ||
        !param.readFromValue(&value, param.getPaddedParameterSize())) {
        return BAD_VALUE;
    }
...
status_t EffectConversionHelperAidl::getAecParameter(EffectParamWriter& param) {
    ...
    if (!param.validateParamValueSize(sizeof(uint32_t), sizeof(uint32_t)) ||
        !param.readFromParameter(&type) || !param.readFromValue(&value)) {
...
status_t EffectConversionHelperAidl::getAgcParameter(EffectParamWriter& param) {
    uint32_t type = 0, value = 0;
    if (!param.validateParamValueSize(sizeof(uint32_t), sizeof(uint32_t)) || !param.readFromParameter(&type) ||
        !param.readFromValue(&value)) {
        return BAD_VALUE;
    }
    Parameter aidlParam;
    switch (type) {
...
status_t EffectConversionHelperAidl::setBassBoostParameter(EffectParamReader& param) {
    uint32_t type = 0;
    uint16_t value = 0;
    if (!param.validateParamValueSize(sizeof(uint32_t), sizeof(uint16_t)) || !param.readFromParameter(&type) ||
        !param.readFromValue(&value, param.getPaddedParameterSize())) {
        return BAD_VALUE;
    }
    Parameter aidlParam;
...
status_t EffectConversionHelperAidl::setDownmixParameter(EffectParamReader& param) {
    uint32_t type = 0;
    int16_t value = 0;
    if (!param.validateParamValueSize(sizeof(uint32_t), sizeof(int16_t)) || !param.readFromParameter(&type) ||
        !param.readFromValue(&value)) {
        return BAD_VALUE;
    }
    Parameter aidlParam;
