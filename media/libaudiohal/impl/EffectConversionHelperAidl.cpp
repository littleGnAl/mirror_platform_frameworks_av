...
status_t EffectConversionHelperAidl::handleGetParameter(uint32_t cmdSize, const void* pCmdData,
    ...
    memcpy(pReplyData, pCmdData, sizeof(effect_param_t) + reader.getPSize());
...
status_t EffectConversionHelperAidl::setAecParameter(EffectParamReader& param) {
    ...
    if (!param.validatePVSize(sizeof(uint32_t), sizeof(uint32_t)) || !param.readNParameter(&type) ||
        !param.readNValue(&value, param.getPaddedPSize())) {
        return BAD_VALUE;
    }
...
status_t EffectConversionHelperAidl::setBassBoostParameter(EffectParamReader& param) {
    uint32_t type = 0;
    uint16_t value = 0;
    if (!param.validatePVSize(sizeof(uint32_t), sizeof(uint16_t)) || !param.readNParameter(&type) ||
        !param.readNValue(&value, param.getPaddedPSize())) {
        return BAD_VALUE;
    }
    Parameter aidlParam;
...
status_t EffectConversionHelperAidl::setDownmixParameter(EffectParamReader& param) {
    uint32_t type = 0;
    int16_t value = 0;
    if (!param.validatePVSize(sizeof(uint32_t), sizeof(int16_t)) || !param.readNParameter(&type) ||
        !param.readNValue(&value)) {
        return BAD_VALUE;
    }
    Parameter aidlParam;
