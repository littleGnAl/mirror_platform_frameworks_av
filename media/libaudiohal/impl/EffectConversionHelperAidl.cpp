...
status_t EffectConversionHelperAidl::handleSetParameter(uint32_t cmdSize, const void* pCmdData,
    ...
    if (!validateCommandSize(*param, cmdSize)) {
        ALOGE("%s illegal param %s size %u", __func__, aidl::android::aidl_utils::toString(*param).c_str(),
              cmdSize);
        return BAD_VALUE;
    }
...
status_t EffectConversionHelperAidl::handleGetParameter(uint32_t cmdSize, const void* pCmdData,
    ...
    if (!validateCommandSize(*param, *replySize)) {
        ALOGE("%s illegal param %s, replysize %u", __func__,
              aidl::android::aidl_utils::toString(*param).c_str(), *replySize);
        return BAD_VALUE;
    }
...
status_t EffectConversionHelperAidl::handleSetConfig(uint32_t cmdSize, const void* pCmdData,
    ...
    RETURN_STATUS_IF_ERROR(statusTFromBinderStatus(mEffect->setParameter(Parameter::make<Parameter::common>(common))));
...
status_t EffectConversionHelperAidl::handleSetVolume(uint32_t cmdSize, const void* pCmdData,
    ...
    RETURN_STATUS_IF_ERROR(statusTFromBinderStatus(mEffect->setParameter(Parameter::make<Parameter::volumeStereo>(volume))));
    return *static_cast<int32_t*>(pReplyData) = OK;
}
