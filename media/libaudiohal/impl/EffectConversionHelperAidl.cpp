...
status_t EffectConversionHelperAidl::handleSetVolume(uint32_t cmdSize, const void* pCmdData,
    ...
    Parameter::VolumeStereo volumeToSet = {.left = (float)(*(uint32_t*)pCmdData) / unityGain,
                                      .right = (float)(*(uint32_t*)pCmdData + 1) / unityGain};

    RETURN_STATUS_IF_ERROR(statusTFromBinderStatus(
            mEffect->setParameter(Parameter::make<Parameter::volumeStereo>(volumeToSet))));

    // get volume from effect and set if changed.
    Parameter::Id id = Parameter::Id::make<Parameter::Id::commonTag>(Parameter::volumeStereo);
    Parameter actualVolumeSet;
    RETURN_STATUS_IF_ERROR(statusTFromBinderStatus(mEffect->getParameter(id, &actualVolumeSet)));
    Parameter::VolumeStereo appliedVolume = actualVolumeSet.get<Parameter::volumeStereo>();

    if (volumeToSet != appliedVolume) {
        if (replySize && *replySize == 2 * sizeof(uint32_t) && pReplyData) {
            ALOGI("%s change volume from %s to %s ", __func__, volumeToSet.toString().c_str(),
                  appliedVolume.toString().c_str());
