...
status_t Hal2AidlMapper::prepareToOpenStream(
    ...
    if (isInput && setConfigFromPortConfig(config, *mixPortConfig)->base != initialConfig.base) {
        // If the resulting config is different, we must stop here and provide it to the framework
        // so that it can retry.
        mixPortConfig->id = 0;
    } else if (!isInput && initialConfig.base.format.type == AudioFormatType::PCM
            && mixPortConfig->id == 0) {
        // The framework does not retry opening PCM outputs, so apply the suggested config.
    ...
    if (mixPortConfig->id == 0) {
        ALOGD("%p %s: suggested config: %s", this, __func__, config->toString().c_str());
        return OK;
    }
    if (isInput) {
