...
status_t EffectHalAidl::process() {
    ...
    if (!statusQ->isValid() || !inputQ->isValid() || !outputQ->isValid()) {
        ALOGE("%s got invalid FMQ", __func__);
        return BAD_VALUE;
    }

    size_t availableSize = inputQ->availableToWrite();
    size_t rSize = std::min(availableSize, mInBuffer->getSize());
    if (rSize == 0) {
        ALOGW("%s not able to write, buffer size %zu, avail %zu", __func__, mInBuffer->getSize(),
              availableSize);
        return INVALID_OPERATION;
    }
    if (!inputQ->writeBlocking((float*)mInBuffer->ptr(), rSize / sizeof(float))) {
        ALOGW("%s failed to write %zu into inputQ", __func__, rSize);
    ...
    availableSize = outputQ->availableToWrite();
    size_t wSize = std::min(availableSize, mOutBuffer->getSize());
    if (wSize == 0) {
        ALOGW("%s not able to read, buffer size %zu, available %zu", __func__,
              mOutBuffer->getSize(), availableSize);
        return INVALID_OPERATION;
    }
    if (!outputQ->readBlocking((float*)mOutBuffer->ptr(), wSize / sizeof(float))) {
        ALOGW("%s failed to read %zu from outputQ", __func__, wSize);
        return FAILED_TRANSACTION;
    }

    ALOGD("%s %s consumed %zu produced %zu", __func__, mDesc.common.name.c_str(), rSize, wSize);
    return OK;
}
