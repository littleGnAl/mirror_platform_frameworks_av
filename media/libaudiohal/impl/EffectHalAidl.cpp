...
EffectHalAidl::EffectHalAidl(const std::shared_ptr<IEffect>& effect, uint64_t effectId,
    ...
    : EffectConversionHelperAidl("EffectHalAidl"),
      mEffectId(effectId),
...
status_t EffectHalAidl::handleGetConfig(uint32_t cmdCode, uint32_t cmdSize, void* pCmdData,
    ...
    if (state != State::INIT) {
        ...
        RETURN_IF_BINDER_FAIL(mEffect->getParameter(id, &param));
        //convert param to effect_config_t
    } else {
        ALOGW("%s get in state %s, return cached config", __func__,
...
// TODO: remove this logic and either update EffectHalInterface or add logic in
// AudioFlinger/Effects.cpp to adapt to AIDL.
