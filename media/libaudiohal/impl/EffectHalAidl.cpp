...
status_t EffectHalAidl::process() {
    ...
    if (!statusQ || statusQ->isValid() || !inputQ || inputQ->isValid() || !outputQ ||
        !outputQ->isValid()) {
        ALOGW("%s invalid FMQ [Status %p I %p O %p]", __func__, statusQ.get(), inputQ.get(),
              outputQ.get());
    ...
    if (mInBuffer->audioBuffer() ||
        ...
        ALOGW("%s failed to write %zu floats from audiobuffer %p to inputQ", __func__,
              floatsToWrite, mInBuffer->audioBuffer());
        return INVALID_OPERATION;
    }
