...
class EffectsFactoryHalInterface : public RefBase
{
  public:
    ...
    virtual std::vector<effectsConfig::InputStream> getPreProcessings() const = 0;
    virtual std::vector<effectsConfig::OutputStream> getPostProcessings() const = 0;
    virtual std::vector<effectsConfig::DeviceEffects> getDeviceProcessings() const = 0;
    // skipped elements only meaningful for HIDL, AIDL return status of IFactory::queryProcessing
    virtual std::pair<status_t /* parser status */, int /* skipped elements */>
    getEffectParsingResult() const = 0;
