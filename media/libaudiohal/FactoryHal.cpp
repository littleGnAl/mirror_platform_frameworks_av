...
//#include "include/media/audiohal/DevicesFactoryHalInterface.h"
//#include "include/media/audiohal/EffectsFactoryHalInterface.h"
...
static const std::array<AudioHalVersionInfo, 5> sAudioHALVersions = {
    // TODO: remove the comment for AIDL 1.0
...
bool hasAidlHalService(const InterfaceName& interface, const AudioHalVersionInfo& version) {
    ...
    ALOGE("%s Service %s exist xxx", __func__, name.c_str());
    return true;
}
...
bool hasHidlHalService(const InterfaceName& interface, const AudioHalVersionInfo& version) {
    ...
    const std::string fqName = interface.first + "@" + version.toVersionString() + "::" + interface.second;
