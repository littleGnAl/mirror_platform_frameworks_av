...
class MockFactory : public IFactory {
  public:
    MockFactory(int version) : IFactory(), mVersion(version) {}
...
class MockEffect : public IEffect {
  public:
    MockEffect(int version) : IEffect(), mVersion(version) {}
...
class EffectHalVersionCompatibilityTest : public ::testing::TestWithParam<TestParam> {
  public:
    ...
    void TearDown() override { EXPECT_CALL(*mMockFactory, destroyEffect(_)); }
  ...
  protected:
    ...
    std::shared_ptr<MockFactory> mMockFactory = ndk::SharedRefBase::make<MockFactory>(mVersion);
    std::shared_ptr<MockEffect> mMockEffect = ndk::SharedRefBase::make<MockEffect>(mVersion);
    sp<EffectHalAidl> mEffectHalAidl;
};
