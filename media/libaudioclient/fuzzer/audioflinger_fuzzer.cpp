...
static const std::vector<audio_input_flags_t> kInputFlags = [] {
    std::vector<audio_input_flags_t> result;
    for (const auto xsdEnumVal : xsdc_enum_range<xsd::AudioInOutFlag>{}) {
        audio_input_flags_t enumVal;
        std::string enumString = toString(xsdEnumVal);
        if (enumString.find("_INPUT_") != std::string::npos &&
            audio_input_flag_from_string(enumString.c_str(), &enumVal)) {
            result.push_back(enumVal);
        }
    }
    return result;
...
static const std::vector<audio_output_flags_t> kOutputFlags = [] {
    ...
    for (const auto xsdEnumVal : xsdc_enum_range<xsd::AudioInOutFlag>{}) {
        audio_output_flags_t enumVal;
        std::string enumString = toString(xsdEnumVal);
        if (enumString.find("_OUTPUT_") != std::string::npos &&
            audio_output_flag_from_string(enumString.c_str(), &enumVal)) {
            result.push_back(enumVal);
        }
    }
    return result;
...
void AudioFlingerFuzzer::invokeAudioTrack() {
    ...
    audio_content_type_t contentType = getValueFromArray(&mFdp, kContentType);
...
void AudioFlingerFuzzer::invokeAudioInputDevice() {
    ...
    device = getValue(&mFdp, kDevices);
    source = getValue(&mFdp, kInputSources);
...
void AudioFlingerFuzzer::invokeAudioOutputDevice() {
    ...
    device = new DeviceDescriptorBase(getValue(&mFdp, kDevices));
