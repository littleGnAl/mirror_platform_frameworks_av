...
static const std::vector<audio_stream_type_t> kStreamtypes = [] {
    std::vector<audio_stream_type_t> result;
    for (const auto xsdEnumVal : xsdc_enum_range<xsd::AudioStreamType>{}) {
        audio_stream_type_t enumVal;
        std::string enumString = toString(xsdEnumVal);
        if (audio_stream_type_from_string(enumString.c_str(), &enumVal)) {
...
static const std::vector<audio_format_t> kFormats = [] {
    ...
    for (const auto xsdEnumVal : xsdc_enum_range<xsd::AudioFormat>{}) {
        audio_format_t enumVal;
        std::string enumString = toString(xsdEnumVal);
        if (audio_format_from_string(enumString.c_str(), &enumVal)) {
            result.push_back(enumVal);
        }
    }
    return result;
...
static const std::vector<audio_channel_mask_t> kChannelMasks = [] {
    ...
    for (const auto xsdEnumVal : xsdc_enum_range<xsd::AudioChannelMask>{}) {
        audio_channel_mask_t enumVal;
        std::string enumString = toString(xsdEnumVal);
        if (audio_channel_mask_from_string(enumString.c_str(), &enumVal)) {
            result.push_back(enumVal);
        }
    }
    return result;
...
static const std::vector<audio_usage_t> kUsages = [] {
    ...
    for (const auto xsdEnumVal : xsdc_enum_range<xsd::AudioUsage>{}) {
        audio_usage_t enumVal;
        std::string enumString = toString(xsdEnumVal);
        if (audio_usage_from_string(enumString.c_str(), &enumVal)) {
            result.push_back(enumVal);
        }
    }
    return result;
...
static const std::vector<audio_content_type_t> kContentType = [] {
    ...
    for (const auto xsdEnumVal : xsdc_enum_range<xsd::AudioContentType>{}) {
        audio_content_type_t enumVal;
        std::string enumString = toString(xsdEnumVal);
        if (audio_content_type_from_string(enumString.c_str(), &enumVal)) {
            result.push_back(enumVal);
        }
    }
    return result;
...
static const std::vector<audio_source_t> kInputSources = [] {
    ...
    for (const auto xsdEnumVal : xsdc_enum_range<xsd::AudioSource>{}) {
        audio_source_t enumVal;
        std::string enumString = toString(xsdEnumVal);
        if (audio_source_from_string(enumString.c_str(), &enumVal)) {
            result.push_back(enumVal);
        }
    }
    return result;
...
static const std::vector<audio_gain_mode_t> kGainModes = [] {
    std::vector<audio_gain_mode_t> result;
    for (const auto xsdEnumVal : xsdc_enum_range<xsd::AudioGainMode>{}) {
        audio_gain_mode_t enumVal;
        std::string enumString = toString(xsdEnumVal);
        if (audio_gain_mode_from_string(enumString.c_str(), &enumVal)) {
            result.push_back(enumVal);
        }
    }
    return result;
...
std::vector<T> getFlags(std::vector<T> arr, std::string findString, FUNC func) {
    for (const auto xsdEnumVal : xsdc_enum_range<xsd::AudioInOutFlag>{}) {
        T enumVal;
        std::string enumString = toString(xsdEnumVal);
        if (enumString.find(findString) != std::string::npos &&
            func(enumString.c_str(), &enumVal)) {
            arr.push_back(enumVal);
        }
    }
    return arr;
}
...
static const std::vector<audio_input_flags_t> kInputFlags = [] {
    std::vector<audio_input_flags_t> tempVector;
    return getFlags(tempVector, "_INPUT_", audio_input_flag_from_string);
}();

static const std::vector<audio_output_flags_t> kOutputFlags = [] {
    std::vector<audio_output_flags_t> tempVector;
    return getFlags(tempVector, "_OUTPUT_", audio_output_flag_from_string);
}();

static const std::vector<audio_devices_t> kDevices = [] {
    std::vector<audio_devices_t> result;
    for (const auto xsdEnumVal : xsdc_enum_range<xsd::AudioDevice>{}) {
        audio_devices_t enumVal;
        std::string enumString = toString(xsdEnumVal);
        if (audio_device_from_string(enumString.c_str(), &enumVal)) {
            result.push_back(enumVal);
        }
    }
    return result;
...
void AudioFlingerFuzzer::invokeAudioTrack() {
    ...
    bool offload = false;
    bool fast = (flags & AUDIO_OUTPUT_FLAG_FAST) ? true : false;
...
void AudioFlingerFuzzer::invokeAudioRecord() {
    ...
    audio_attributes_t attributes = {};
    bool fast = (flags & AUDIO_INPUT_FLAG_FAST) ? true : false;
...
void AudioFlingerFuzzer::invokeAudioEffect() {
    ...
    sp<IEffect> effect =
        af->createEffect(&descriptor, effectClient, priority, io, sessionId, device, opPackageName,
                         getpid(), false, &status, &id, &enabled);
    if (!effect) {
        return;
    }
...
void AudioFlingerFuzzer::invokeAudioInputDevice() {
    ...
    status_t status = af->openInput(module, &input, &config, &device, address, source, flags);
    if (status != NO_ERROR) {
        return;
    }
...
void AudioFlingerFuzzer::invokeAudioOutputDevice() {
    ...
    status_t status = af->openOutput(module, &output, &config, device, &latencyMs, flags);
    if (status != NO_ERROR) {
        return;
    }
