...
std::vector<T> getFlags(const xsdc_enum_range<X> &range, const FUNC &func) {
    ...
    for (const auto &xsdEnumVal : range) {
        ...
        if (func(enumString.c_str(), &enumVal)) {
...
template <typename T, typename X, typename FUNC>
std::vector<T> getFlags(const xsdc_enum_range<X> &range, const std::string &findString,
                        const FUNC &func) {
    std::vector<T> vec;
    for (const auto &xsdEnumVal : range) {
        T enumVal;
...
#define INSTANTIATE_VECTOR(NAME, TYPE, XSDTYPE, FROMSTRINGFUNC)                                   \
    static const std::vector<TYPE> NAME = getFlags<TYPE, xsd::XSDTYPE, decltype(FROMSTRINGFUNC)>( \
        xsdc_enum_range<xsd::XSDTYPE>{}, FROMSTRINGFUNC)

#define INSTANTIATE_VECTOR_FROM_STRING(NAME, TYPE, XSDTYPE, SEARCHSTRING, FROMSTRINGFUNC)         \
    static const std::vector<TYPE> NAME = getFlags<TYPE, xsd::XSDTYPE, decltype(FROMSTRINGFUNC)>( \
        xsdc_enum_range<xsd::XSDTYPE>{}, SEARCHSTRING, FROMSTRINGFUNC)

INSTANTIATE_VECTOR(kStreamtypes, audio_stream_type_t, AudioStreamType,
                   audio_stream_type_from_string);
INSTANTIATE_VECTOR(kFormats, audio_format_t, AudioFormat, audio_format_from_string);
INSTANTIATE_VECTOR(kChannelMasks, audio_channel_mask_t, AudioChannelMask,
                   audio_channel_mask_from_string);
INSTANTIATE_VECTOR(kUsages, audio_usage_t, AudioUsage, audio_usage_from_string);
INSTANTIATE_VECTOR(kContentType, audio_content_type_t, AudioContentType,
                   audio_content_type_from_string);
INSTANTIATE_VECTOR(kInputSources, audio_source_t, AudioSource, audio_source_from_string);
INSTANTIATE_VECTOR(kGainModes, audio_gain_mode_t, AudioGainMode, audio_gain_mode_from_string);
INSTANTIATE_VECTOR(kDevices, audio_devices_t, AudioDevice, audio_device_from_string);
INSTANTIATE_VECTOR_FROM_STRING(kInputFlags, audio_input_flags_t, AudioInOutFlag, "_INPUT_",
                               audio_input_flag_from_string);
INSTANTIATE_VECTOR_FROM_STRING(kOutputFlags, audio_output_flags_t, AudioInOutFlag, "_OUTPUT_",
                               audio_output_flag_from_string);
