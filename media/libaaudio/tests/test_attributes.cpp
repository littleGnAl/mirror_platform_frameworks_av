...
static void checkAttributes(aaudio_performance_mode_t perfMode, aaudio_usage_t usage,
                            aaudio_content_type_t contentType, const char* tags = nullptr,
    ...
    if (tags != nullptr) {
        AAudioStreamBuilder_setTags(aaudioBuilder, tags);
    }
    if (preset != DONT_SET) {
    ...
    const char* readTags = AAudioStream_getTags(aaudioStream);
    EXPECT_TRUE(strlen(readTags) < AUDIO_ATTRIBUTES_TAGS_MAX_SIZE)
            << "expected tags len " << strlen(readTags) << " lesser than "
            << AUDIO_ATTRIBUTES_TAGS_MAX_SIZE;

    const char* expectedTags =
            tags == nullptr ? "" : (strlen(tags) > AUDIO_ATTRIBUTES_TAGS_MAX_SIZE - 1 ? "" : tags);
...
static const char* const sTags[] = {nullptr, "", "oem=routing_extension",
                                    "VX_OEM_ROUTING_EXTENSION", maxSizeTags.c_str(),
                                    // intentionnaly use oversized tags
                                    oversizedTags.c_str(), oversizedTags2.c_str()};
...
static void checkAttributesInputPreset(aaudio_performance_mode_t perfMode) {
    for (aaudio_input_preset_t inputPreset : sInputPresets) {
        checkAttributes(perfMode, DONT_SET, DONT_SET, nullptr, inputPreset, DONT_SET, DONT_SET,
...
static void checkAttributesAllowedCapturePolicy(aaudio_performance_mode_t perfMode) {
    for (aaudio_allowed_capture_policy_t policy : sAllowCapturePolicies) {
        checkAttributes(perfMode, DONT_SET, DONT_SET, nullptr, DONT_SET, policy,
                        AAUDIO_DIRECTION_INPUT);
    }
}
...
static void checkAttributesPrivacySensitive(aaudio_performance_mode_t perfMode) {
    for (int privacyMode : sPrivacyModes) {
        checkAttributes(perfMode, DONT_SET, DONT_SET, nullptr, DONT_SET, DONT_SET, privacyMode,
                        AAUDIO_DIRECTION_INPUT);
    }
}
