...
TEST_P(Codec2VideoDecDecodeTest, DecodeTest) {
    ...
    ASSERT_GE(numCsds, 0) << "Error in parsing input mInfoFile file: " << mInfoFile;
    ...
    if (mMd5Enable) {
        ...
        ALOGV("mChksumFile Size %zu ", refChksmSize);
    ...
    if (mMd5Enable && refChksmSize != mMd5Offset) {
        ALOGE("refChksum size and generated mChksumFile size mismatch refChksum size %zu generated "
              "mChksumFile size %" PRId64 "",
...
TEST_P(Codec2VideoDecCsdInputTests, CSDFlushTest) {
    ...
    int32_t numCsds = populateInfoVector(mInfoFile, &Info, mTimestampDevTest, &mTimestampUslist);
    ASSERT_GE(numCsds, 0) << "Error in parsing input mInfoFile file";
