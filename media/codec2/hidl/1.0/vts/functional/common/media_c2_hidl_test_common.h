...
template <typename... T>
static inline std::string PrintToStringParamName(
        const testing::TestParamInfo<std::tuple<T...>>& info) {
        std::stringstream ss;
        std::apply([&ss](auto&&... args) {((ss << args << '_'), ...);}, info.param);
        ss << std::to_string(info.index);
        std::string s = ss.str();
        auto isNotAlphaNum = [](char c) { return !std::isalnum(c); };
        std::replace_if(s.begin(), s.end(), isNotAlphaNum, '_');
        return s;
}
