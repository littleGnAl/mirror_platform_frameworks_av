...
void Codec2Client::Component::stopUseOutputSurface(
        C2BlockPool::local_id_t blockPoolId) {
    sp<HGraphicBufferProducer2> nullHgbp;
    ...
    Return<Status> transStatus = mBase1_0->setOutputSurface(
            ...
            nullHgbp);
    if (!transStatus.isOk()) {
        LOG(ERROR) << "setOutputSurface(stopUseOutputSurface) -- transaction failed.";
    } else {
        c2_status_t status =
                static_cast<c2_status_t>(static_cast<Status>(transStatus));
        if (status != C2_OK) {
            LOG(DEBUG) << "setOutputSurface(stopUseOutputSurface) -- call failed: "
                       << status << ".";
        }
    }
}
