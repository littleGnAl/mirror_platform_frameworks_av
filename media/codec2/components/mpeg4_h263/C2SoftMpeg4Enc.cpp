   ...
   public:
    ...
    static C2R ProfileLevelSetter(
        ...
        constexpr LevelLimits kLimits[] = {
            ...
            { LEVEL_MP4V_1, 380160, 176, 144, 15, 128000, 163840 },
            { LEVEL_MP4V_2, 1520640, 352, 288, 30, 128000, 655360 },
#else
            { LEVEL_H263_10, 380160, 176, 144, 15, 64000 },
            { LEVEL_H263_45, 380160, 176, 144, 15, 128000 },
#endif
        };
        ...
        bool needsUpdate = (me.v.level == LEVEL_MP4V_0B);
#else
...
#ifdef MPEG4
                    vbvSize <= limit.vbvSize &&
#endif
                    size.v.height <= limit.height &&
                    bitrate.v.value <= limit.bitrate &&
                    frameRate.v.value <= limit.frameRate) {
    ...
    ProfileLevelType getProfileLevel_l() const {
        ProfileLevelType type = CORE_PROFILE_LEVEL2;
#ifdef MPEG4
        if (mProfileLevel->level == LEVEL_MP4V_0) type = SIMPLE_PROFILE_LEVEL0;
        else if (mProfileLevel->level == LEVEL_MP4V_1) type = SIMPLE_PROFILE_LEVEL1;
        else if (mProfileLevel->level == LEVEL_MP4V_2) type = SIMPLE_PROFILE_LEVEL2;
#endif
        return type;
    }
