...
c2_status_t C2SoftAvcDec::onFlush_sm() {
    ...
    uint32_t infoBufferSize = ALIGN8(mWidth) * ALIGN8(mHeight) >> BLK_8X8_INFO_SHIFT;
    if (mFrameMetaDataEnable) {
    ...
    while (inPos < inSize && inSize - inPos >= kMinInputBytes) {
        ...
        ivd_video_decode_op_t *ps_decode_op = &s_h264d_decode_op.s_ivd_video_decode_op_t;
        {
            ...
            if (mFrameMetaDataEnable) {
                ...
                if (qpView->error()) {
                    ALOGE("write view map failed %d", qpView->error());
                    work->result = qpView->error();
                    return;
                }
                mbTypeView.reset(new C2WriteView(mMbTypeBlock->map().get()));
                if (mbTypeView->error()) {
                    ALOGE("write view map failed %d", mbTypeView->error());
