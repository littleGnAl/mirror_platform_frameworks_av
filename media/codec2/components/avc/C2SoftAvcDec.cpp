...
public:
    explicit IntfImpl(const std::shared_ptr<C2ReflectorHelper> &helper)
        ...
        mC2HdrStaticInfo = std::make_shared<C2StreamHdrStaticInfo::output>();
        mC2HdrStaticInfo->hdrType = 1.f;
        mC2HdrStaticInfo->validFields = 0.f;
        mC2HdrStaticInfo->mastering = {
            .red   = { .x = 0.708,  .y = 0.292 },
        ...
        mC2HdrStaticInfo->maxCll = 1000;
        mC2HdrStaticInfo->maxFall = 120;
        mC2HdrStaticInfo->ave = {
            .ambientIlluminance = 1.0,
            .ambientLight = {.x = 0.0, .y = 0.0},
        };
        mC2HdrStaticInfo->ccv = {
            .cancelFlag = 0.0,
            .persistenceFlag = 0.0,
        ...
        addParameter(
                ...
                .withDefault(mC2HdrStaticInfo)
                .withFields({
                    C2F(mC2HdrStaticInfo, hdrType).inRange(HDRStaticInfo::kType1, HDRStaticInfo::kType2),
                    C2F(mC2HdrStaticInfo, validFields).inRange(0, 15),
                    C2F(mC2HdrStaticInfo, mastering.red.x).inRange(0, 1),
                    C2F(mC2HdrStaticInfo, mastering.red.y).inRange(0, 1),
                    C2F(mC2HdrStaticInfo, mastering.green.x).inRange(0, 1),
                    C2F(mC2HdrStaticInfo, mastering.green.y).inRange(0, 1),
                    C2F(mC2HdrStaticInfo, mastering.blue.x).inRange(0, 1),
                    C2F(mC2HdrStaticInfo, mastering.blue.y).inRange(0, 1),
                    C2F(mC2HdrStaticInfo, mastering.white.x).inRange(0, 1),
                    C2F(mC2HdrStaticInfo, mastering.white.x).inRange(0, 1),
                    C2F(mC2HdrStaticInfo, mastering.maxLuminance).inRange(0, 65535),
                    C2F(mC2HdrStaticInfo, mastering.minLuminance).inRange(0, 6.5535),
                    C2F(mC2HdrStaticInfo, maxCll).inRange(0, 65535),
                    C2F(mC2HdrStaticInfo, maxFall).inRange(0, 65535),
                    C2F(mC2HdrStaticInfo, ave.ambientIlluminance).inRange(0, 65535),
                    C2F(mC2HdrStaticInfo, ave.ambientLight.x).inRange(0, 1),
                    C2F(mC2HdrStaticInfo, ave.ambientLight.y).inRange(0, 1),
                    ...
                    C2F(mC2HdrStaticInfo, ccv.red.x).inRange(-100, 100),
                    C2F(mC2HdrStaticInfo, ccv.red.y).inRange(-100, 100),
                    C2F(mC2HdrStaticInfo, ccv.green.x).inRange(-100, 100),
                    C2F(mC2HdrStaticInfo, ccv.green.y).inRange(-100, 100),
                    C2F(mC2HdrStaticInfo, ccv.blue.x).inRange(-100, 100),
                    C2F(mC2HdrStaticInfo, ccv.blue.y).inRange(-100, 100),
                    C2F(mC2HdrStaticInfo, ccv.maxLuminance).inRange(0, 65535),
                    C2F(mC2HdrStaticInfo, ccv.minLuminance).inRange(0, 65535),
                    C2F(mC2HdrStaticInfo, ccv.avgLuminance).inRange(0, 65535),
                    // TODO
                })
...
static bool getMDCV(iv_obj_t* mDecHandle, HDRStaticInfo* mHdrStaticInfoLocal) {
    WORD32 ret = 0;
    ...
    if (IV_SUCCESS != ret) {
        ALOGE("Failed to get MDCV params: 0x%x", s_mdcv_op.u4_error_code);
        return false;
    }

    if (mHdrStaticInfoLocal->mID == HDRStaticInfo::kType2)
    {
        mHdrStaticInfoLocal->sType2.mValidFields |= HDRStaticInfo::Type2::kDisplayColorVolume;
        mHdrStaticInfoLocal->sType2.mG.x = s_mdcv_op.au2_display_primaries_x[0];
        mHdrStaticInfoLocal->sType2.mB.x = s_mdcv_op.au2_display_primaries_x[1];
        mHdrStaticInfoLocal->sType2.mR.x = s_mdcv_op.au2_display_primaries_x[2];
        mHdrStaticInfoLocal->sType2.mG.y = s_mdcv_op.au2_display_primaries_y[0];
        mHdrStaticInfoLocal->sType2.mB.y = s_mdcv_op.au2_display_primaries_y[1];
        mHdrStaticInfoLocal->sType2.mR.y = s_mdcv_op.au2_display_primaries_y[2];
        mHdrStaticInfoLocal->sType2.mW.x = s_mdcv_op.u2_white_point_x;
        mHdrStaticInfoLocal->sType2.mW.y = s_mdcv_op.u2_white_point_y;
        mHdrStaticInfoLocal->sType2.mMaxDisplayLuminance =
            (UWORD16)(s_mdcv_op.u4_max_display_mastering_luminance / 10000);
        mHdrStaticInfoLocal->sType2.mMinDisplayLuminance =
...
static bool getCLL(iv_obj_t* mDecHandle, HDRStaticInfo*  mHdrStaticInfoLocal) {
    WORD32 ret = 0;
    ...
    if (IV_SUCCESS != ret) {
        ALOGE("Failed to get CLL params: 0x%x", s_cll_op.u4_error_code);
        return false;
    }

    if (mHdrStaticInfoLocal->mID == HDRStaticInfo::kType2)
    {
        mHdrStaticInfoLocal->sType2.mValidFields |= HDRStaticInfo::Type2::kContentLightLevel;

        mHdrStaticInfoLocal->sType2.mMaxContentLightLevel =
            s_cll_op.u2_max_content_light_level;
        mHdrStaticInfoLocal->sType2.mMaxFrameAverageLightLevel =
            s_cll_op.u2_max_pic_average_light_level;
    }
    return true;
}

static bool getAVE(iv_obj_t* mDecHandle, HDRStaticInfo*  mHdrStaticInfoLocal) {
    WORD32 ret = 0;
    ...
    if (IV_SUCCESS != ret) {
        ALOGE("Failed to get AVE params: 0x%x", s_ave_op.u4_error_code);
        return false;
    }

    if (mHdrStaticInfoLocal->mID == HDRStaticInfo::kType2)
    {
        mHdrStaticInfoLocal->sType2.mValidFields |= HDRStaticInfo::Type2::kAmbientViewingEnv;

        mHdrStaticInfoLocal->sType2.mAmbientLight.x = s_ave_op.u2_ambient_light_x;
        mHdrStaticInfoLocal->sType2.mAmbientLight.y = s_ave_op.u2_ambient_light_y;
        mHdrStaticInfoLocal->sType2.mAmbientIlluminance = s_ave_op.u4_ambient_illuminance;
    }
    return true;
}

static bool getCCV(iv_obj_t* mDecHandle, HDRStaticInfo*  mHdrStaticInfoLocal) {
    WORD32 ret = 0;
    ...
    if (IV_SUCCESS != ret) {
        ALOGE("Failed to get CCV params: 0x%x", s_ccv_op.u4_error_code);
        return false;
    }

    if (mHdrStaticInfoLocal->mID == HDRStaticInfo::kType2)
    {
        mHdrStaticInfoLocal->sType2.mValidFields |= HDRStaticInfo::Type2::kContentColorVolume;

        mHdrStaticInfoLocal->sType2.mCCVPrimariesPresentFlag =
            s_ccv_op.u1_ccv_primaries_present_flag;

        mHdrStaticInfoLocal->sType2.mCCVG.x = s_ccv_op.ai4_ccv_primaries_x[0];
        mHdrStaticInfoLocal->sType2.mCCVB.x = s_ccv_op.ai4_ccv_primaries_x[1];
        mHdrStaticInfoLocal->sType2.mCCVR.x = s_ccv_op.ai4_ccv_primaries_x[2];
        mHdrStaticInfoLocal->sType2.mCCVG.y = s_ccv_op.ai4_ccv_primaries_y[0];
        mHdrStaticInfoLocal->sType2.mCCVB.y = s_ccv_op.ai4_ccv_primaries_y[1];
        mHdrStaticInfoLocal->sType2.mCCVR.y = s_ccv_op.ai4_ccv_primaries_y[2];

        mHdrStaticInfoLocal->sType2.mCCVMinContentLuminancePresentFlag =
            s_ccv_op.u1_ccv_min_luminance_value_present_flag;

        mHdrStaticInfoLocal->sType2.mCCVMaxContentLuminancePresentFlag =
            s_ccv_op.u1_ccv_max_luminance_value_present_flag;

        mHdrStaticInfoLocal->sType2.mCCVAvgContentLuminancePresentFlag =
            s_ccv_op.u1_ccv_avg_luminance_value_present_flag;

        mHdrStaticInfoLocal->sType2.mMinContentLuminance = s_ccv_op.u4_ccv_min_luminance_value;
        mHdrStaticInfoLocal->sType2.mMaxContentLuminance = s_ccv_op.u4_ccv_max_luminance_value;
        mHdrStaticInfoLocal->sType2.mAvgContentLuminance = s_ccv_op.u4_ccv_avg_luminance_value;

        mHdrStaticInfoLocal->sType2.mCCVCancelFlag = s_ccv_op.u1_ccv_cancel_flag;
        mHdrStaticInfoLocal->sType2.mCCVPersistenceFlag = s_ccv_op.u1_ccv_persistence_flag;
    }
...
bool C2SoftAvcDec::getHDRStaticParams(ivd_video_decode_op_t *ps_decode_op,
    ...
    HDRStaticInfo mHdrStaticInfoLocal;
    memset(&mHdrStaticInfoLocal, 0, sizeof(HDRStaticInfo));
    mHdrStaticInfoLocal.mID = HDRStaticInfo::kType2;
    ...
    if (1 == ps_decode_op->s_sei_decode_op.u1_sei_ave_params_present_flag) {
        getAVE(mDecHandle,&mHdrStaticInfoLocal);
    }

    // Get content color volume parameters if they have changed
    if (1 == ps_decode_op->s_sei_decode_op.u1_sei_ccv_params_present_flag) {
        getCCV(mDecHandle,&mHdrStaticInfoLocal);
    }

    // Check if any fields have changed
    if (0 != memcmp(&mHdrStaticInfo.sType2, &mHdrStaticInfoLocal.sType2, SIZE_TYPE2)) {
        mHdrStaticInfo.mID = HDRStaticInfo::kType2;
        mHdrStaticInfo.sType2 = mHdrStaticInfoLocal.sType2;
        ...
        //Convert HDRStaticInfo to C2StreamHdrStaticInfo
        {
            ...
            c2HdrStaticInfo.mastering.red.x = mHdrStaticInfo.sType2.mR.x * 0.00002;
            c2HdrStaticInfo.mastering.red.y = mHdrStaticInfo.sType2.mR.y * 0.00002;
            c2HdrStaticInfo.mastering.green.x = mHdrStaticInfo.sType2.mG.x * 0.00002;
            c2HdrStaticInfo.mastering.green.y = mHdrStaticInfo.sType2.mG.y * 0.00002;
            c2HdrStaticInfo.mastering.blue.x = mHdrStaticInfo.sType2.mB.x * 0.00002;
            c2HdrStaticInfo.mastering.blue.y = mHdrStaticInfo.sType2.mB.y * 0.00002;
            c2HdrStaticInfo.mastering.white.x = mHdrStaticInfo.sType2.mW.x * 0.00002;
            c2HdrStaticInfo.mastering.white.y = mHdrStaticInfo.sType2.mW.y * 0.00002;
            c2HdrStaticInfo.mastering.maxLuminance = mHdrStaticInfo.sType2.mMaxDisplayLuminance;
            c2HdrStaticInfo.mastering.minLuminance =
                    mHdrStaticInfo.sType2.mMinDisplayLuminance * 0.0001;
            ...
            c2HdrStaticInfo.ave.ambientLight.x = mHdrStaticInfo.sType2.mAmbientLight.x * 0.00002;
            c2HdrStaticInfo.ave.ambientLight.y = mHdrStaticInfo.sType2.mAmbientLight.y * 0.00002;
            ...
            c2HdrStaticInfo.ccv.red.x = mHdrStaticInfo.sType2.mCCVR.x * 0.00002;
            c2HdrStaticInfo.ccv.red.y = mHdrStaticInfo.sType2.mCCVR.y * 0.00002;
            c2HdrStaticInfo.ccv.green.x = mHdrStaticInfo.sType2.mCCVG.x * 0.00002;
            c2HdrStaticInfo.ccv.green.y = mHdrStaticInfo.sType2.mCCVG.y * 0.00002;
            c2HdrStaticInfo.ccv.blue.x = mHdrStaticInfo.sType2.mCCVB.x * 0.00002;
            c2HdrStaticInfo.ccv.blue.y = mHdrStaticInfo.sType2.mCCVB.y * 0.00002;
            c2HdrStaticInfo.ccv.maxLuminance =
                    mHdrStaticInfo.sType2.mMaxContentLuminance * 0.0000001;
            c2HdrStaticInfo.ccv.minLuminance =
                    mHdrStaticInfo.sType2.mMinContentLuminance * 0.0000001;
            c2HdrStaticInfo.ccv.avgLuminance =
                    mHdrStaticInfo.sType2.mAvgContentLuminance * 0.0000001;
        ...
        ALOGD("Updated HDR static params!");
    }
...
void C2SoftAvcDec::process(
    ...
    while (inPos < inSize) {
        ...
        /* call getHDRStaticParams only for kType2 */
        if(mHdrStaticInfo.mID == HDRStaticInfo::kType2)
        {
