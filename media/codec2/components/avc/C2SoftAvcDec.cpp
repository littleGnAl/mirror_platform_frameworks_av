...
public:
    explicit IntfImpl(const std::shared_ptr<C2ReflectorHelper> &helper)
        ...
        addParameter(
                ...
                .withFields({
                    C2F(mC2HdrStaticInfo, mastering.red.x).inRange(0,
                                                                   1),
                    C2F(mC2HdrStaticInfo, mastering.red.y).inRange(0,
                                                                   1),
                    C2F(mC2HdrStaticInfo, mastering.green.x).inRange(0,
                                                                     1),
                    C2F(mC2HdrStaticInfo, mastering.green.y).inRange(0,
                                                                     1),
                    C2F(mC2HdrStaticInfo, mastering.blue.x).inRange(0,
                                                                    1),
                    C2F(mC2HdrStaticInfo, mastering.blue.y).inRange(0,
                                                                    1),
                    C2F(mC2HdrStaticInfo, mastering.white.x).inRange(0,
                                                                     1),
                    C2F(mC2HdrStaticInfo, mastering.white.x).inRange(0,
                                                                      1),
                    C2F(mC2HdrStaticInfo, mastering.maxLuminance).inRange(0,
                                                                          65535),
                    C2F(mC2HdrStaticInfo, mastering.minLuminance).inRange(0,
                                                                          6.5535),
                    C2F(mC2HdrStaticInfo, maxCll).inRange(0,
                                                          0XFFFF),
                    C2F(mC2HdrStaticInfo, maxFall).inRange(0,
                                                           0XFFFF)
                })
...
static bool getMDCV(iv_obj_t* mDecHandle, HDRStaticInfo* hdrStaticInfo) {
...

    {
        ...
        hdrStaticInfo->sType1.mMinDisplayLuminance =
...
static bool getCLL(iv_obj_t* mDecHandle, HDRStaticInfo*  hdrStaticInfo) {
    ...
    {
        hdrStaticInfo->sType1.mMaxContentLightLevel =
                              s_cll_op.u2_max_content_light_level;
        hdrStaticInfo->sType1.mMaxFrameAverageLightLevel =
...
void C2SoftAvcDec::process(
    ...
    while (inPos < inSize && inSize - inPos >= kMinInputBytes) {
        ...
        if (!getHDRStaticParams(ps_decode_op, work)) {
            mSignalledError = true;
            work->workletsProcessed = 1u;
            work->result = C2_CORRUPTED;
            return;
