...
size_t getChromaStrideAlignment(const std::shared_ptr<C2BlockPool> &pool)
{
    ...
    // U plane's row increment is alignment requirement of chroma planes as buffer
    // requested was for 2x2 dimensions
    size_t alignment = layout.planes[C2PlanarLayout::PLANE_U].rowInc;
    outBlock.reset();
...
c2_status_t C2SoftAvcDec::ensureDecoderState(const std::shared_ptr<C2BlockPool> &pool) {
    ...
    if (mStrideAlignment <= 0) {
        ...
        if (mStrideAlignment <= 0) {
            ALOGD("Unable to determine stride alignment of allocator. Use 128 alignment");
            mStrideAlignment = 128;
        }
    }
