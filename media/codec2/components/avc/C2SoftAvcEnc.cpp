...
public:
    explicit IntfImpl(const std::shared_ptr<C2ReflectorHelper> &helper)
...

        {
            ...
            mHdrStaticInfo = std::make_shared<C2StreamHdrStaticInfo::input>();
            mHdrStaticInfo->hdrType = 1.f;
            mHdrStaticInfo->validFields = 0.f;
            ...
            mHdrStaticInfo->ave = {
                .ambientIlluminance = 1.0,
                .ambientLight = {.x = 0.0, .y = 0.0},
            };
            mHdrStaticInfo->ccv = {
                .cancelFlag = 0.0,
                .persistenceFlag = 0.0,
                .primariesPresentFlag = 0.0,
                .maxLuminancePresentFlag = 0.0,
                .minLuminancePresentFlag = 0.0,
                .avgLuminancePresentFlag = 0.0,
                .red   = { .x = 0.708,  .y = 0.292 },
                .green = { .x = 0.170,  .y = 0.797 },
                .blue  = { .x = 0.131,  .y = 0.046 },
                .maxLuminance = 1000,
            ...
            addParameter(
                    ...
                    .withFields({
                        C2F(mHdrStaticInfo, hdrType).inRange(0, 1),
                        C2F(mHdrStaticInfo, validFields).inRange(0, 15),
                        ...
                        C2F(mHdrStaticInfo, maxCll).inRange(0, 65535),
                        C2F(mHdrStaticInfo, maxFall).inRange(0, 65535),
                        C2F(mHdrStaticInfo, ave.ambientIlluminance).inRange(0, 65535),
                        C2F(mHdrStaticInfo, ave.ambientLight.x).inRange(0, 1),
                        C2F(mHdrStaticInfo, ave.ambientLight.y).inRange(0, 1),
                        C2F(mHdrStaticInfo, ccv.cancelFlag).inRange(0, 1),
                        C2F(mHdrStaticInfo, ccv.persistenceFlag).inRange(0, 1),
                        C2F(mHdrStaticInfo, ccv.primariesPresentFlag).inRange(0, 1),
                        C2F(mHdrStaticInfo, ccv.maxLuminancePresentFlag).inRange(0, 1),
                        C2F(mHdrStaticInfo, ccv.minLuminancePresentFlag).inRange(0, 1),
                        C2F(mHdrStaticInfo, ccv.avgLuminancePresentFlag).inRange(0, 1),
                        C2F(mHdrStaticInfo, ccv.red.x).inRange(-100, 100),
                        C2F(mHdrStaticInfo, ccv.red.y).inRange(-100, 100),
                        C2F(mHdrStaticInfo, ccv.green.x).inRange(-100, 100),
                        C2F(mHdrStaticInfo, ccv.green.y).inRange(-100, 100),
                        C2F(mHdrStaticInfo, ccv.blue.x).inRange(-100, 100),
                        C2F(mHdrStaticInfo, ccv.blue.y).inRange(-100, 100),
                        C2F(mHdrStaticInfo, ccv.maxLuminance).inRange(0, 65535),
                        C2F(mHdrStaticInfo, ccv.minLuminance).inRange(0, 65535),
                        C2F(mHdrStaticInfo, ccv.avgLuminance).inRange(0, 65535),
                        // TODO
                    })
    ...
    static C2R HdrStaticInfoSetter(bool mayBlock, const C2P<C2StreamHdrStaticInfo::input> &oldMe,
                                   C2P<C2StreamHdrStaticInfo::input> &me) {
        (void)mayBlock;
        C2R res = C2R::Ok();

        if (me.v.hdrType != 1) {
            ALOGW("Hdr info is invalid. Using old Hdr params");
            me.set().hdrType = 1;
            me.set().validFields = oldMe.v.validFields;
            me.set().mastering = oldMe.v.mastering;
            me.set().maxCll = oldMe.v.maxCll;
            me.set().maxFall = oldMe.v.maxCll;
            me.set().ave = oldMe.v.ave;
            me.set().ccv = oldMe.v.ccv;
        }
    ...
    std::shared_ptr<C2StreamHdrStaticInfo::input> getHdrStaticInfo_l() const { return mHdrStaticInfo; }
...
c2_status_t C2SoftAvcEnc::setHdrStaticParams() {
    ...
    ih264e_ctl_set_sei_ave_params_ip_t s_ave_ip;
    ih264e_ctl_set_sei_ave_params_op_t s_ave_op;
    ih264e_ctl_set_sei_ccv_params_ip_t s_ccv_ip;
    ih264e_ctl_set_sei_ccv_params_op_t s_ccv_op;

    // All SEI parameters are valid. Proceed to set SEI messages to codec

    // Set mastering display color volume parameters
    setMDCV(&s_mdcv_ip, &s_mdcv_op);

    // Set content light level parameters
    setCLL(&s_cll_ip, &s_cll_op);

    // Set ambient viewing environment parameters
    setAVE(&s_ave_ip, &s_ave_op);

    // Set content color volume parameters
    setCCV(&s_ccv_ip, &s_ccv_op);
...
c2_status_t C2SoftAvcEnc::setMDCV(
    ...
    memset(ps_mdcv_ip, 0, sizeof(ih264e_ctl_set_sei_mdcv_params_ip_t));
    memset(ps_mdcv_op, 0, sizeof(ih264e_ctl_set_sei_mdcv_params_op_t));

    ps_mdcv_ip->e_cmd = IVE_CMD_VIDEO_CTL;
    ps_mdcv_ip->e_sub_cmd = IVE_CMD_CTL_SET_SEI_MDCV_PARAMS;
    ps_mdcv_ip->u4_size = sizeof(ih264e_ctl_set_sei_mdcv_params_ip_t);
    ps_mdcv_op->u4_size = sizeof(ih264e_ctl_set_sei_mdcv_params_op_t);

    if (0 != ((int32_t)mHdrStaticInfo->validFields & HDRStaticInfo::Type2::kDisplayColorVolume)) {
        ALOGV("New MDCV params detected: mR = (%f, %f), mG = (%f, %f), "
        ...
        mHdrStaticInfo->mastering.maxLuminance,
        mHdrStaticInfo->mastering.minLuminance);

        ps_mdcv_ip->u1_sei_mdcv_params_present_flag = (UWORD8)1;
        ps_mdcv_ip->au2_display_primaries_x[0] =
                mHdrStaticInfo->mastering.green.x / 0.00002 + 0.5;
        ps_mdcv_ip->au2_display_primaries_x[1] =
                mHdrStaticInfo->mastering.blue.x / 0.00002 + 0.5;
        ps_mdcv_ip->au2_display_primaries_x[2] =
                mHdrStaticInfo->mastering.red.x / 0.00002 + 0.5;
        ps_mdcv_ip->au2_display_primaries_y[0] =
                mHdrStaticInfo->mastering.green.y / 0.00002 + 0.5;
        ps_mdcv_ip->au2_display_primaries_y[1] =
                mHdrStaticInfo->mastering.blue.y / 0.00002 + 0.5;
        ps_mdcv_ip->au2_display_primaries_y[2] =
                mHdrStaticInfo->mastering.red.y / 0.00002 + 0.5;
        ps_mdcv_ip->u2_white_point_x =
                mHdrStaticInfo->mastering.white.x / 0.00002 + 0.5;
        ps_mdcv_ip->u2_white_point_y =
                mHdrStaticInfo->mastering.white.y / 0.00002 + 0.5;
        ps_mdcv_ip->u4_max_display_mastering_luminance =
                mHdrStaticInfo->mastering.maxLuminance / 0.0001 + 0.5;
        ps_mdcv_ip->u4_min_display_mastering_luminance =
                mHdrStaticInfo->mastering.minLuminance / 0.0001 + 0.5;
    } else {
        ps_mdcv_ip->u1_sei_mdcv_params_present_flag = (UWORD8)0;
    }

    ps_mdcv_ip->u4_timestamp_high = -1;
    ps_mdcv_ip->u4_timestamp_low = -1;

    status = ive_api_function(mCodecCtx, ps_mdcv_ip, ps_mdcv_op);
    if (status != IV_SUCCESS) {
        ALOGE("Unable to set MDCV params = 0x%x\n", ps_mdcv_op->u4_error_code);
        return C2_BAD_VALUE;
    }
...
c2_status_t C2SoftAvcEnc::setCLL(
    ...
    memset(ps_cll_ip, 0, sizeof(ih264e_ctl_set_sei_cll_params_ip_t));
    memset(ps_cll_op, 0, sizeof(ih264e_ctl_set_sei_cll_params_op_t));

    ps_cll_ip->e_cmd = IVE_CMD_VIDEO_CTL;
    ps_cll_ip->e_sub_cmd = IVE_CMD_CTL_SET_SEI_CLL_PARAMS;
    ps_cll_ip->u4_size = sizeof(ih264e_ctl_set_sei_cll_params_ip_t);
    ps_cll_op->u4_size = sizeof(ih264e_ctl_set_sei_cll_params_op_t);

    if (0 != ((int32_t)mHdrStaticInfo->validFields & HDRStaticInfo::Type2::kContentLightLevel)) {
        ALOGV("New CLL params detected: maxCLL = %f, maxFALL = %f!",
            mHdrStaticInfo->maxCll,
            mHdrStaticInfo->maxFall);

        ps_cll_ip->u1_sei_cll_params_present_flag = (UWORD8)1;
        ps_cll_ip->u2_max_content_light_level = mHdrStaticInfo->maxCll + 0.5;
        ps_cll_ip->u2_max_pic_average_light_level = mHdrStaticInfo->maxFall + 0.5;
    } else {
        ps_cll_ip->u1_sei_cll_params_present_flag = (UWORD8)0;
    }

    ps_cll_ip->u4_timestamp_high = -1;
    ps_cll_ip->u4_timestamp_low = -1;

    status = ive_api_function(mCodecCtx, ps_cll_ip, ps_cll_op);
    if (status != IV_SUCCESS) {
        ALOGE("Unable to set CLL params = 0x%x\n", ps_cll_op->u4_error_code);
        return C2_BAD_VALUE;
    }
...
c2_status_t C2SoftAvcEnc::setAVE(
    ih264e_ctl_set_sei_ave_params_ip_t* ps_ave_ip,
    ih264e_ctl_set_sei_ave_params_op_t* ps_ave_op) {
    IV_STATUS_T status;

    memset(ps_ave_ip, 0, sizeof(ih264e_ctl_set_sei_ave_params_ip_t));
    memset(ps_ave_op, 0, sizeof(ih264e_ctl_set_sei_ave_params_op_t));

    ps_ave_ip->e_cmd = IVE_CMD_VIDEO_CTL;
    ps_ave_ip->e_sub_cmd = IVE_CMD_CTL_SET_SEI_AVE_PARAMS;
    ps_ave_ip->u4_size = sizeof(ih264e_ctl_set_sei_ave_params_ip_t);
    ps_ave_op->u4_size = sizeof(ih264e_ctl_set_sei_ave_params_op_t);

    if (0 != ((int32_t)mHdrStaticInfo->validFields & HDRStaticInfo::Type2::kAmbientViewingEnv)) {
        ALOGV("New AVE params detected: AmbientIlluminance = %f, AmbientLight = (%f, %f)!",
            mHdrStaticInfo->ave.ambientIlluminance,
            mHdrStaticInfo->ave.ambientLight.x,
            mHdrStaticInfo->ave.ambientLight.y);

        ps_ave_ip->u1_sei_ave_params_present_flag = 1;
        ps_ave_ip->u2_ambient_light_x = mHdrStaticInfo->ave.ambientLight.x / 0.00002 + 0.5;
        ps_ave_ip->u2_ambient_light_y = mHdrStaticInfo->ave.ambientLight.y / 0.00002 + 0.5;
        ps_ave_ip->u4_ambient_illuminance = mHdrStaticInfo->ave.ambientIlluminance + 0.5;
    } else {
        ps_ave_ip->u1_sei_ave_params_present_flag = 0;
    }

    ps_ave_ip->u4_timestamp_high = -1;
    ps_ave_ip->u4_timestamp_low = -1;

    status = ive_api_function(mCodecCtx, ps_ave_ip, ps_ave_op);
    if (status != IV_SUCCESS) {
        ALOGE("Unable to set AVE params = 0x%x\n", ps_ave_op->u4_error_code);
        return C2_BAD_VALUE;
    }
...
c2_status_t C2SoftAvcEnc::setCCV(
    ih264e_ctl_set_sei_ccv_params_ip_t* ps_ccv_ip,
    ih264e_ctl_set_sei_ccv_params_op_t* ps_ccv_op) {
    IV_STATUS_T status;

    memset(ps_ccv_ip, 0, sizeof(ih264e_ctl_set_sei_ccv_params_ip_t));
    memset(ps_ccv_op, 0, sizeof(ih264e_ctl_set_sei_ccv_params_op_t));

    ps_ccv_ip->e_cmd = IVE_CMD_VIDEO_CTL;
    ps_ccv_ip->e_sub_cmd = IVE_CMD_CTL_SET_SEI_CCV_PARAMS;
    ps_ccv_ip->u4_size = sizeof(ih264e_ctl_set_sei_ccv_params_ip_t);
    ps_ccv_op->u4_size = sizeof(ih264e_ctl_set_sei_ccv_params_op_t);

    if (0 != ((int32_t)mHdrStaticInfo->validFields & HDRStaticInfo::Type2::kContentColorVolume)) {
        ALOGV("New CCV params detected: mR = (%f, %f), "
              "mG = (%f, %f), mB = (%f, %f), maxDL = %f, minDL = %f, avgDL = %f!",
              mHdrStaticInfo->ccv.red.x, mHdrStaticInfo->ccv.red.y,
              mHdrStaticInfo->ccv.green.x, mHdrStaticInfo->ccv.green.y,
              mHdrStaticInfo->ccv.blue.x, mHdrStaticInfo->ccv.blue.y,
              mHdrStaticInfo->ccv.maxLuminance,
              mHdrStaticInfo->ccv.minLuminance,
              mHdrStaticInfo->ccv.avgLuminance);

        ps_ccv_ip->u1_sei_ccv_params_present_flag = 1;

        if (1 == mHdrStaticInfo->ccv.primariesPresentFlag) {
            ps_ccv_ip->u1_ccv_primaries_present_flag = 1;
            ps_ccv_ip->ai4_ccv_primaries_x[0] = mHdrStaticInfo->ccv.green.x / 0.00002 + 0.5;
            ps_ccv_ip->ai4_ccv_primaries_x[1] = mHdrStaticInfo->ccv.blue.x / 0.00002 + 0.5;
            ps_ccv_ip->ai4_ccv_primaries_x[2] = mHdrStaticInfo->ccv.red.x / 0.00002 + 0.5;
            ps_ccv_ip->ai4_ccv_primaries_y[0] = mHdrStaticInfo->ccv.green.y / 0.00002 + 0.5;
            ps_ccv_ip->ai4_ccv_primaries_y[1] = mHdrStaticInfo->ccv.blue.y / 0.00002 + 0.5;
            ps_ccv_ip->ai4_ccv_primaries_y[2] = mHdrStaticInfo->ccv.red.y / 0.00002 + 0.5;
        } else {
            ps_ccv_ip->u1_ccv_primaries_present_flag = 0;
        }

        if (1 == mHdrStaticInfo->ccv.minLuminancePresentFlag) {
            ps_ccv_ip->u1_ccv_min_luminance_value_present_flag = 1;
            ps_ccv_ip->u4_ccv_min_luminance_value =
                    mHdrStaticInfo->ccv.minLuminance / 0.0000001 + 0.5;
        } else {
            ps_ccv_ip->u1_ccv_min_luminance_value_present_flag = 0;
        }

        if (1 == mHdrStaticInfo->ccv.maxLuminancePresentFlag) {
            ps_ccv_ip->u1_ccv_max_luminance_value_present_flag = 1;
            ps_ccv_ip->u4_ccv_max_luminance_value =
                    mHdrStaticInfo->ccv.maxLuminance / 0.0000001 + 0.5;
        } else {
            ps_ccv_ip->u1_ccv_max_luminance_value_present_flag = 0;
        }

        if (1 == mHdrStaticInfo->ccv.avgLuminancePresentFlag) {
            ps_ccv_ip->u1_ccv_avg_luminance_value_present_flag = 1;
            ps_ccv_ip->u4_ccv_avg_luminance_value =
                    mHdrStaticInfo->ccv.avgLuminance / 0.0000001 + 0.5;
        } else {
            ps_ccv_ip->u1_ccv_avg_luminance_value_present_flag = 0;
        }
    } else {
        ps_ccv_ip->u1_sei_ccv_params_present_flag = 0;
    }

    if (1 == mHdrStaticInfo->ccv.cancelFlag) {
        ps_ccv_ip->u1_ccv_cancel_flag = 1;
    } else {
        ps_ccv_ip->u1_ccv_cancel_flag = 0;
    }

    if (1 == mHdrStaticInfo->ccv.persistenceFlag) {
        ps_ccv_ip->u1_ccv_persistence_flag = 1;
    } else {
        ps_ccv_ip->u1_ccv_persistence_flag = 0;
    }

    ps_ccv_ip->u1_ccv_reserved_zero_2bits = 0;
    ps_ccv_ip->u4_timestamp_high = -1;
    ps_ccv_ip->u4_timestamp_low = -1;

    status = ive_api_function(mCodecCtx, ps_ccv_ip, ps_ccv_op);
    if (status != IV_SUCCESS) {
        ALOGE("Unable to set CCV params = 0x%x\n", ps_ccv_op->u4_error_code);
        return C2_BAD_VALUE;
    }
...
void C2SoftAvcEnc::process(
    ...
    // handle dynamic config parameters
    {
        ...
        if (HdrStaticInfo != mHdrStaticInfo) {
            ALOGV("Hdr params changed. Resetting Hdr params");
