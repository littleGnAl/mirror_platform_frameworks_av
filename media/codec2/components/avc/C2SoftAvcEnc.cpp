...
void C2SoftAvcEnc::fillInfoBuffer(const std::unique_ptr<C2Work> &work,
    ...
    int32_t infoSize = (mSize->width * mSize->height) >> BLK_8X8_INFO_SHIFT;
...
void C2SoftAvcEnc::process(
    ...
    do {
        ...
        if (mFrameMetaDataEnable) {
            ...
            setFrameInfoArgs(&s_video_encode_ip, &s_recon_buf, qpView, mbTypeView,
...
c2_status_t C2SoftAvcEnc::drainInternal(
    ...
    while (true) {
        ...
        if (s_encode_op.output_present) {
            ...
            if (!mFrameMetaDataEnable || s_encode_op.u4_is_last) {
                break;
            }
        }
    }
