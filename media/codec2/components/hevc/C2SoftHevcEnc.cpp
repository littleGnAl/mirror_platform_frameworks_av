  ...
  public:
    explicit IntfImpl(const std::shared_ptr<C2ReflectorHelper> &helper)
        ...
        addParameter(
                ...
                .withSetter(PictureQuantizationSetter, mGop)
                .build());
    }
    ...
    static C2R PictureQuantizationSetter(bool mayBlock,
        ...
        for (size_t i = 0; i < me.v.flexCount(); ++i) {
            ...
            if (layer.min <= layer.max) {
                ...
                int32_t layerMin = std::clamp(layer.min, HEVC_QP_MIN, HEVC_QP_MAX);
                ...
                ALOGV("Ignoring configuration, minQP cannot be greater than maxQP");
            }
        }
        ...
        if (maxBframes == 0) {
            maxTemporalLayer = 0;
        } else if (maxBframes <= 2) {
            maxTemporalLayer = 1;
        } else if (maxBframes <= 6) {
            maxTemporalLayer = 2;
        } else {
            maxTemporalLayer = 3;
        }

        if (maxFrameQP - minFrameQP < maxTemporalLayer + 1) {
            if (minFrameQP + maxTemporalLayer + 1 < HEVC_QP_MAX) {
                maxFrameQP = minFrameQP + maxTemporalLayer + 1;
            } else {
                minFrameQP = maxFrameQP - (maxTemporalLayer + 1);
            }
        }
...
c2_status_t C2SoftHevcEnc::initEncParams() {
    ...
    int32_t maxFrameQP = HEVC_QP_MAX;
    int32_t minFrameQP = HEVC_QP_MIN;
