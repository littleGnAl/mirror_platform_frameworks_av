...
int SimpleC2Component::getHalPixelFormatForBitDepth10(C2StreamColorAspectsTuning::output *colorAspects) {
    // Save supported hal pixel formats for bit depth of 10, the first time this is called
    if (!mBitDepth10HalPixelFormats.size()) {
        std::vector<int> halPixelFormats{ HAL_PIXEL_FORMAT_YCBCR_P010, HAL_PIXEL_FORMAT_RGBA_1010102 };
        ...
        for (int halPixelFormat: halPixelFormats) {
            ...
            c2_status_t status = mOutputBlockPool->fetchGraphicBlock(320, 240, halPixelFormat, usage, &block);
            if (status == C2_OK) {
