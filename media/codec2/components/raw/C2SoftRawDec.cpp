        ...
        for (int inputIndex = 0; inputIndex < work->input.buffers.size(); inputIndex++) {
            ...
            while (metaIndex < inBufferInfo->flexCount()) {
                ...
                if (outputSize + inputMeta.size > maxOutSize) {
                    ...
                    outOrdinal.timestamp = currentBufferInfos.front().timestamp;
                    cloneAndSend(work->input.ordinal.frameIndex.peeku(), work,
                            FillWork(C2FrameData::FLAG_INCOMPLETE, outOrdinal, buffer));
                    ALOGV("Large audio frame sending size: %d, ts: %ld", inputMeta.size,
                            inputMeta.timestamp);
                    block.reset();
                    currentBufferInfos.clear();
                    outputSize = 0;
                    ...
                    memcpy(outPtr + outputSize, rView.data() + outputSize,
                           inputMeta.size);
                    ALOGV("Making %d for ts: %ld", inputMeta.size, inputMeta.timestamp);
