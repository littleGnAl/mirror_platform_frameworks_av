    ...
    while (metaIndex < inBufferInfo->flexCount()) {
        ...
        while (auOffset < inputMeta.size) {
            ...
            uint32_t toCopy = c2_min(
                    inputMeta.size - auOffset, thresholdSize - outputSize);
            toCopy &= ~(0x1);
            int64_t auTimestamp = inputMeta.timestamp + auOffset * sampleTimeUs;
            memcpy(outPtr + outputSize, rView.data() + inputOffset, toCopy);
            auMerge.add(inputMeta.flags, toCopy, auTimestamp);
            if (currentBufferInfos.empty() ||
                    currentBufferInfos.back().flags != inputMeta.flags) {
                currentBufferInfos.emplace_back(inputMeta.flags, toCopy, auTimestamp);
            } else {
            ...
            auOffset += toCopy;
            inputOffset += toCopy;
            ALOGV("Making size %d for ts: %ld, outputSize: %d, metaIndex : %d/%zu",
                toCopy, auTimestamp, outputSize, metaIndex, inBufferInfo->flexCount());
        }
        metaIndex++;
    }
    work->worklets.front()->output.flags = work->input.flags;
