...
#define DRC_DEFAULT_MOBILE_OUTPUT_LOUDNESS 0.25 /*decoder output loudness: -1 = the value is unknown,else dB step value (e.g. 64 for -16 dB) */
...
void C2SoftXaacDec::process(const std::unique_ptr<C2Work>& work,
    ...
    WORD32 loudness_measure = -1;

    float targetRefLevel = mIntf->getDrcTargetRefLevel();
    int32_t tmp_reflevel = (int32_t)targetRefLevel;
    float loudness_runtime;
    ...
    if(targetRefLevel != prevtargetref || prevtargetref == DEFAULT_PREV_STATE) {
    C2StreamDrcTargetReferenceLevelTuning::input currentTargetRefLevel(
        0u, (float)(-0.25*targetRefLevel));
    work->worklets.front()->output.configUpdate.push_back(
        C2Param::Copy(currentTargetRefLevel));
    prevtargetref = targetRefLevel ;
    }

    if(effectType != preveffecttype || preveffecttype == DEFAULT_PREV_STATE ) {
    C2StreamDrcEffectTypeTuning::input currentEffectype(
    ...
    if(mMpegDDRCPresent ) {
        ia_drc_dec_api(mMpegDDrcHandle, IA_API_CMD_GET_CONFIG_PARAM,
           ...
           if(targetRefLevel < 0 ) {
              loudness_runtime = loudness_measure;
           } else {
              loudness_runtime = targetRefLevel;
           }
    } else {
           ixheaacd_dec_api(mXheaacCodecHandle,
                              IA_API_CMD_GET_LOUDNESS_VAL,0,&loudness_measure);
           loudness_runtime = loudness_measure ;
    }

   if(loudness_runtime != prevloudness || prevloudness == DEFAULT_PREV_STATE) {
   C2StreamDrcOutputLoudnessTuning::output
                    drcOutLoudness(0u, (float) (-0.25*loudness_runtime));
    work->worklets.front()->output.configUpdate.push_back(
