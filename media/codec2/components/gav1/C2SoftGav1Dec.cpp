...
void C2SoftGav1Dec::getHDRStaticParams(const libgav1::DecoderBuffer *buffer,
                                       const std::unique_ptr<C2Work> &work) {
  static HDRStaticInfo hdrStaticInfo{};
  if (buffer->has_hdr_cll) {
    hdrStaticInfo.maxCll = buffer->hdr_cll.max_cll;
    hdrStaticInfo.maxFall = buffer->hdr_cll.max_fall;
  }

  if (buffer->has_hdr_mdcv) {
    hdrStaticInfo.redx = (buffer->hdr_mdcv.primary_chromaticity_x[0] / 65536.0);
    hdrStaticInfo.redy = (buffer->hdr_mdcv.primary_chromaticity_y[0] / 65536.0);

    hdrStaticInfo.greenx = (buffer->hdr_mdcv.primary_chromaticity_x[1] / 65536.0);
    hdrStaticInfo.greeny = (buffer->hdr_mdcv.primary_chromaticity_y[1] / 65536.0);

    hdrStaticInfo.bluex = (buffer->hdr_mdcv.primary_chromaticity_x[2] / 65536.0);
    hdrStaticInfo.bluey = (buffer->hdr_mdcv.primary_chromaticity_y[2] / 65536.0);

    hdrStaticInfo.whitex = (buffer->hdr_mdcv.white_point_chromaticity_x / 65536.0);
    hdrStaticInfo.whitey = (buffer->hdr_mdcv.white_point_chromaticity_y / 65536.0);

    // luminance-max is in 24.8 fixed-point format.
    hdrStaticInfo.maxLuminance = buffer->hdr_mdcv.luminance_max / 256.0;
    // luminance-min is in 18.14 format.
    hdrStaticInfo.minLuminance = buffer->hdr_mdcv.luminance_min / 16384.0;
  }

  // config if static info has changed
  if (!(hdrStaticInfo == mHdrStaticInfo)) {
    mHdrStaticInfo = hdrStaticInfo;
    C2StreamHdrStaticInfo::output HDRStaticMetadataInfo = {0u};
    ...
    work->worklets.front()->output.configUpdate.push_back(C2Param::Copy(HDRStaticMetadataInfo));
  }
}
...
void C2SoftGav1Dec::getHDR10PlusInfoData(const libgav1::DecoderBuffer *buffer,
  ...
  if (buffer->has_itut_t35) {
    HDRDynamicInfo hdrDynamicInfo;
    hdrDynamicInfo.setItuT35(&buffer->itut_t35);

    // config if hdr10Plus info has changed
    if (!(hdrDynamicInfo == mHdrDynamicInfo)) {
      mHdrDynamicInfo = hdrDynamicInfo;
      std::unique_ptr<C2StreamHdr10PlusInfo::output> hdr10PlusInfo =
              C2StreamHdr10PlusInfo::output::AllocUnique(hdrDynamicInfo.payload.size());
      if (!hdr10PlusInfo) {
      ...
      memcpy(hdr10PlusInfo->m.value, hdrDynamicInfo.payload.data(), hdrDynamicInfo.payload.size());
