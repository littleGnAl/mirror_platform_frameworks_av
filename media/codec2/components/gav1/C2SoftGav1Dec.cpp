  ...
  if (buffer->bitdepth == 10) {
    ...
    if (format == HAL_PIXEL_FORMAT_RGBA_1010102) {
      ...
      convertPlanar16ToY410OrRGBA1010102(
              dstY, srcY, srcU, srcV, srcYStride,
      ...
      convertPlanar16ToP010((uint16_t *)dstY, (uint16_t *)dstU, srcY, srcU, srcV, srcYStride,
                            srcUStride, srcVStride, dstYStride, dstUStride, dstVStride, mWidth,
                            mHeight, isMonochrome, convFormat, mTmpFrameBuffer.get(), tmpSize);
    } else {
      size_t tmpSize = 0;
      if (buffer->image_format == libgav1::kImageFormatYuv444) {
        tmpSize = dstYStride * mHeight + dstUStride * align(mHeight, 2);
      ...
      convertPlanar16ToYV12(dstY, dstU, dstV, srcY, srcU, srcV, srcYStride, srcUStride, srcVStride,
                            dstYStride, dstUStride, dstVStride, mWidth, mHeight, isMonochrome,
                            convFormat, mTmpFrameBuffer.get(), tmpSize);
    }
  } else {
