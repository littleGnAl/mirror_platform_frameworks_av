...
TEST_F(GraphicsTrackerTest, DropAndReleaseTest) {
    ...
    ret = mTracker->allocate(0, 0, 0, GRALLOC_USAGE_SW_WRITE_OFTEN, &buf1, &fence1);
    ASSERT_EQ(C2_OK, ret);
    mBqStat->mDequeued++;
    ASSERT_EQ(maxDequeueCount - 1, mTracker->getCurDequeueable());

    ret = mTracker->allocate(0, 0, 0, GRALLOC_USAGE_SW_WRITE_OFTEN, &buf2, &fence2);
    ASSERT_EQ(C2_OK, ret);
    mBqStat->mDequeued++;
...
TEST_F(GraphicsTrackerTest, StopAndWaitTest) {
    ...
    ASSERT_EQ(C2_OK, mTracker->allocate(
            0, 0, 0, GRALLOC_USAGE_SW_WRITE_OFTEN, &buf1, &fence));
    mBqStat->mDequeued++;
    AHardwareBuffer_release(buf1);

    ASSERT_EQ(C2_OK, mTracker->allocate(
            0, 0, 0, GRALLOC_USAGE_SW_WRITE_OFTEN, &buf2, &fence));
    mBqStat->mDequeued++;
...
TEST_F(GraphicsTrackerTest, SurfaceChangeTest) {
    ...
    for (int i = 0; i < firstPassRender; ++i) {
        ASSERT_EQ(C2_OK, mTracker->allocate(
                0, 0, 0, GRALLOC_USAGE_SW_WRITE_OFTEN, &bufs[i], &fences[i]));
        mBqStat->mDequeued++;
        numAlloc++;
    ...
    while (numAlloc < firstPassAlloc) {
        c2_status_t ret = mTracker->allocate(
                0, 0, 0, GRALLOC_USAGE_SW_WRITE_OFTEN, &bufs[numAlloc], &fences[numAlloc]);
        if (ret == C2_BLOCKING) {
            mBqStat->mBlocked++;
            c2_status_t waitRes = waitFence.wait(3000000000);
            if (waitRes == C2_TIMED_OUT || waitRes == C2_OK) {
    ...
    // mReleased should not be checked. IProducerListner::onBufferReleased()
