...
c2_status_t C2PlatformAllocatorStoreImpl::fetchAllocator(
    ...
    case C2PlatformAllocatorStore::DMABUFHEAP:
...
void UseComponentStoreForDmaBufAllocator(
    ...
    if (res == C2_OK) {
        ...
        mapper = [store](C2MemoryUsage usage, size_t capacity,
                         std::string *heapName, unsigned *flags) -> c2_status_t {
            if (capacity > UINT32_MAX) {
                return C2_BAD_VALUE;
            }
