...
std::weak_ptr<C2DmaBufAllocator> gIonAllocator;

void UseComponentStoreForIonAllocator(
        const std::shared_ptr<C2DmaBufAllocator> allocator,
...
std::shared_ptr<C2Allocator> C2PlatformAllocatorStoreImpl::fetchIonAllocator() {
    std::lock_guard<std::mutex> lock(gIonAllocatorMutex);
    std::shared_ptr<C2DmaBufAllocator> allocator = gIonAllocator.lock();
    if (allocator == nullptr) {
        ...
        allocator = std::make_shared<C2DmaBufAllocator>(C2PlatformAllocatorStore::ION);
