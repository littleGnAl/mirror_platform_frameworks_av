...
public:
  ...
  // internal methods
  C2DmaBufAllocation(BufferAllocator &alloc, size_t size, size_t align,
...
c2_status_t C2DmaBufAllocation::map(size_t offset, size_t size,
...
c2_status_t C2DmaBufAllocation::status() const { return mInit; }

C2Allocator::id_t C2DmaBufAllocation::getAllocatorId() const { return mId; }

bool C2DmaBufAllocation::equals(
...
const C2Handle *C2DmaBufAllocation::handle() const { return &mHandle; }
...
C2DmaBufAllocation::C2DmaBufAllocation(BufferAllocator &alloc, size_t size,
  ...
  if (!(usage.expected & (C2MemoryUsage::CPU_READ | C2MemoryUsage::CPU_WRITE)))
    bufferFd = alloc.AllocLegacyAlign("system-uncached", alignedSize, align, 0);

  if (bufferFd !=
      -EEXIST) /* If system-uncached doesn't exist, fall back to system */
    bufferFd = alloc.AllocLegacyAlign("system", alignedSize, align, 0);
...
C2DmaBufAllocator::C2DmaBufAllocator(id_t id) : mInit(C2_OK) {
   ...
   */
  mBufferAllocator.MapNameToIonHeap("system", "", 0, heapmask,
                                    ION_FLAG_CACHED | flags);
  mBufferAllocator.MapNameToIonHeap("system-uncached", "", heapmask, flags);
}
...
void C2DmaBufAllocator::setUsageMapper(const UsageMapperFn &mapper __unused,
...
bool C2DmaBufAllocator::isValid(const C2Handle *const o) {
