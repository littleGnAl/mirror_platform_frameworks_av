...
public:
  ...
  // internal methods
  C2DmaBufAllocation(BufferAllocator &alloc, size_t size, std::string heap_name,
...
c2_status_t C2DmaBufAllocator::mapUsage(
        C2MemoryUsage usage, size_t capacity, std::string *heap_name, unsigned *flags) {
    std::lock_guard<std::mutex> lock(mUsageMapperLock);
