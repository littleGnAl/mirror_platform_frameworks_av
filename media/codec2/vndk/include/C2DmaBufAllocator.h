...
public:
  virtual c2_status_t
  newLinearAllocation(uint32_t capacity, C2MemoryUsage usage,
   ...
   * supported minimum and maximum usage masks and default block-size to use for
   * the mapper.
   *
   * \param mapper this method is called to map Codec 2.0 buffer usage to ion
   * flags required by the ion device \param minUsage minimum buffer usage
   * required for supported allocations (defaults to 0) \param maxUsage maximum
   * buffer usage supported by the ion allocator (defaults to SW_READ |
   * SW_WRITE) \param blockSize alignment used prior to calling |mapper| for the
   * buffer capacity. This also helps reduce the size of cache required for
   * caching mapper results. (defaults to the page size)
   */
  void setUsageMapper(const UsageMapperFn &mapper, uint64_t minUsage,
...
private:
  ...
  c2_status_t mapUsage(C2MemoryUsage usage, size_t size,
                     /* => */ std::string *heap_name, unsigned *flags);
  ...
  typedef std::tuple<std::string, unsigned, c2_status_t> MapperValue;
