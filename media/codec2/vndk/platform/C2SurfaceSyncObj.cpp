...
void C2SyncVariables::clearLockIfNecessary() {
    // Waiting for 30ms without acquiring the lock,
    // we will consider the lock was dangling.
    constexpr int kTestLockDurationMs = 30;

    bool locked = tryLock(kTestLockDurationMs);
    unlock();
...
bool C2SyncVariables::tryLock(size_t ms) {
    uint32_t old = FUTEX_UNLOCKED;
