...
c2_status_t static CreateGraphicBlockByAhwb(const AHardwareBuffer *ahwb,
                                            const std::shared_ptr<C2Allocator> &allocator,
                                            std::shared_ptr<C2GraphicBlock> *block) {
    uint64_t origId = 0;
    // TODO: unmark after sdk 31 or above being applied.
    // int ret = AHardwareBuffer_getId(ahwb, &origId);
    int ret = -1;
    if (ret != ::android::OK) {
        ALOGE("Improper AHarewareBuffer");
        return C2_CORRUPTED;
    }
...
std::shared_ptr<C2GraphicBlock> _C2BlockFactory::CreateGraphicBlock(const AHardwareBuffer *ahwb) {
    ...
    std::shared_ptr<C2GraphicBlock> block;
    c2_status_t res = CreateGraphicBlockByAhwb(
            ahwb, std::static_pointer_cast<C2Allocator>(sAllocator), &block);
    if (res != C2_OK) {
        return nullptr;
    }
    return block;
...
c2_status_t C2IgbaBlockPool::_fetchGraphicBlock(
    ...
    if (!mWaitableObj->waitEvent(timeoutNs, &hangUp, &allocatable)) {
        return C2_CANCELED;
    }
    if (hangUp) {
        mValid = false;
    ...
    *origId = 0;
    int ret = -1;
    // TODO after sdk 31 build enabled
    // int ret = AHardwareBuffer_getId(ahwb, origId);
    // actually CHECK here.
    if (ret != ::android::OK) {
        ALOGE("Ahwb got from AIDL call is not valid");
        return C2_CORRUPTED;
    }
    c2_status_t res = CreateGraphicBlockByAhwb(ahwb, mAllocator, block);
