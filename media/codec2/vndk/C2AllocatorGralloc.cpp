...
class C2Gralloc4Mapper {
 public:
    ...
    static std::shared_ptr<C2Gralloc4Mapper> getInstance() {
        static std::shared_ptr<C2Gralloc4Mapper> sInstance(new C2Gralloc4Mapper);
        static std::once_flag once;
        std::call_once(
                once,
                [](std::shared_ptr<C2Gralloc4Mapper> self) {
                    self->mMapper = std::make_unique<Gralloc4Mapper>();
                    if (self->mMapper) {
                        self->mMapper->preload();
                    }
                },
                sInstance);
        return sInstance;
 ...
 private:
    ...
    C2Gralloc4Mapper() : mMapper(nullptr) {}
};
