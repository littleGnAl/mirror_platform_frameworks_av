...
static c2_status_t ParsePixelFormat(
...
static void HandleInterleavedFormats(
        C2PlanarLayout *layout /* nonnull */, uint8_t **addr /* nonnull */) {
...
c2_status_t C2AllocationGralloc::map(
    ...
    c2_status_t ret = ParsePixelFormat(mBuffer, rect, mFormat, grallocUsage, mStride, layout, addr);
    if (ret != C2_OK) {
        return ret;
    }
    mLocked = true;

    HandleInterleavedFormats(layout, addr);
...
c2_status_t C2AllocatorAhwb::Impl::priorGraphicAllocation(
    ...
    hidl_handle hidlHandle;
    hidlHandle.setTo(C2HandleAhwb::UnwrapNativeHandle(ahwbHandle), true);

    allocation->reset(new C2AllocationAhwb(
            width, height, format, layerCount,
            grallocUsage, stride, hidlHandle, ahwbHandle, mTraits->id));
    return C2_OK;
}
