...
C2AllocationBuf::C2AllocationBuf(BufferAllocator &alloc, size_t size, size_t align,
    ...
    if (bufferFd != -EEXIST) /* If system-uncached doesn't exist, fall back to system */
...
C2AllocatorBuf::C2AllocatorBuf(id_t id)
	...
	mBufferAllocator.MapNameToIonHeap("system", "ion_system_heap", ION_FLAG_CACHED, ~0, 0); //not quite right
	mBufferAllocator.MapNameToIonHeap("system-uncached", "ion_system_heap", 0, ~0, 0); //not quite right
...
void C2AllocatorBuf::setUsageMapper(
        const UsageMapperFn &mapper __unused, uint64_t minUsage, uint64_t maxUsage, uint64_t blockSize __unused) {
    std::lock_guard<std::mutex> lock(mUsageMapperLock);
    Traits traits = {
...
c2_status_t C2AllocatorBuf::newLinearAllocation(
    ...
    std::shared_ptr<C2AllocationBuf> alloc
        = std::make_shared<C2AllocationBuf>(mBufferAllocator, capacity, align, usage, getId());
