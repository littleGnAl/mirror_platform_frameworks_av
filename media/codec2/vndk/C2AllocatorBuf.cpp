...
c2_status_t C2AllocationBuf::map(
...
c2_status_t C2AllocationBuf::status() const {
    return mInit;
}

C2Allocator::id_t C2AllocationBuf::getAllocatorId() const {
    return mId;
}

bool C2AllocationBuf::equals(const std::shared_ptr<C2LinearAllocation> &other) const {
    if (!other || other->getAllocatorId() != getAllocatorId()) {
        return false;
    }
    // get user handle to compare objects
    std::shared_ptr<C2AllocationBuf> otherAsBuf = std::static_pointer_cast<C2AllocationBuf>(other);
    return mHandle.bufferFd() == otherAsBuf->mHandle.bufferFd();
}

const C2Handle *C2AllocationBuf::handle() const {
    return &mHandle;
}
...
C2AllocationBuf::C2AllocationBuf(BufferAllocator &alloc, size_t size, size_t align,
                                 unsigned heapMask __unused, unsigned flags __unused,
                                 C2Allocator::id_t id)
    ...
    bufferFd = alloc.Alloc("system", alignedSize, 0); //not quite right
    if (bufferFd < 0)
	ret = bufferFd;
...
/* ======================================= DMABUF ALLOCATOR ====================================== */
C2AllocatorBuf::C2AllocatorBuf(id_t id)
    : mInit(C2_OK) {
	...
	/* So one issue here is we don't have a way to multiplex the ION_FLAG_CACHED onto dmabuf heaps */
	mBufferAllocator.MapNameToIonHeap("system", "ion_system_heap", 0, ~0, 0); //not quite right
...
void C2AllocatorBuf::setUsageMapper(
...
std::size_t C2AllocatorBuf::MapperKeyHash::operator()(const MapperKey &k) const {
    return std::hash<uint64_t>{}(k.first) ^ std::hash<size_t>{}(k.second);
}

c2_status_t C2AllocatorBuf::mapUsage(
...
/* HACK - We don't really have a equivalent to !ION_FLAG_CACHED w/ dmabuf heaps
...
c2_status_t C2AllocatorBuf::newLinearAllocation(
    ...
    std::shared_ptr<C2AllocationBuf> alloc
        = std::make_shared<C2AllocationBuf>(mBufferAllocator, capacity, align, heapMask, flags, getId());
    ret = alloc->status();
    if (ret == C2_OK) {
        *allocation = alloc;
    }
