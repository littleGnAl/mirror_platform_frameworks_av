...
public:
    ...
    // internal methods
    C2AllocationBuf(size_t size, size_t align, unsigned heapMask, unsigned flags, C2Allocator::id_t id);
...
C2AllocationBuf::C2AllocationBuf(size_t size, size_t align,
    ...
    mBufferAllocator.MapNameToIonHeap("system", "system_heap", 0, ~0, 0);
    bufferFd = mBufferAllocator.Alloc("system", alignedSize, 0);
    if (bufferFd < 0)
	ret = bufferFd;
...
c2_status_t C2AllocatorBuf::newLinearAllocation(
    ...
    std::shared_ptr<C2AllocationBuf> alloc
        = std::make_shared<C2AllocationBuf>(capacity, align, heapMask, flags, getId());
    ret = alloc->status();
    if (ret == C2_OK) {
        *allocation = alloc;
    }
