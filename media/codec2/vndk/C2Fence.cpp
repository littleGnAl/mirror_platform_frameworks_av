...
public:
    ...
    SyncFenceImpl(const std::vector<int>& fenceFds) {
        mListFences.clear();
...
std::vector<int>  _C2FenceFactory::ExtractFdsFromC2Fence(const C2Fence& C2fencep) {
    std::vector<int> retFds;
    if (C2fencep.mImpl)
        retFds = C2fencep.mImpl->fds();
    ALOGE("list fence %d", retFds.size());
    return retFds;
}
