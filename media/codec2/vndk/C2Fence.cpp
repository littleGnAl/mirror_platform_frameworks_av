...
public:
    ...
    virtual std::vector<int> fds() const {
        ...
        for (int index = 0; index < mlistFds.size(); index++) {
            sp<Fence> retFence = sp<Fence>::make(mlistFds[index]);
            RetFDs.push_back(retFence->dup());
        }
        return RetFDs;
    }
    ...
    SyncFenceImpl(std::vector<int> fenceFds)
    {
        for (int fenceFd : fenceFds) {
            mlistFds.push_back(fenceFd);
            sp<Fence> withFence = sp<Fence>::make(fenceFd);
            mFence = Fence::merge("syncFence", mFence, withFence);
        }
    }
