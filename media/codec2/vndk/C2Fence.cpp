...
public:
    ...
    PipeFenceImpl(const std::shared_ptr<C2IgbaWaitableObj> &obj) : mObj(obj) {}

private:
    const std::shared_ptr<C2IgbaWaitableObj> mObj;
};

C2Fence _C2FenceFactory::CreatePipeFence(const std::shared_ptr<C2IgbaWaitableObj> &obj) {
    std::shared_ptr<C2Fence::Impl> p;
    if (obj) {
        p = std::make_shared<_C2FenceFactory::PipeFenceImpl>(obj);
        if (!p) {
