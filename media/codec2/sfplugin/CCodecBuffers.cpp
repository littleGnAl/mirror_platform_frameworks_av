...
// C2SkipCutBuffer for buffer and bufferInfos

class C2SkipCutBuffer : public SkipCutBuffer {
public:
    explicit C2SkipCutBuffer(
            int32_t skip, int32_t cut, size_t num16BitChannels):
        SkipCutBuffer(skip, cut, num16BitChannels),
        mFrontPaddingDelay(0), mSize(0) {
    }

    virtual ~C2SkipCutBuffer() {
    ...
    void submitMultiAccessUnits(
        ...
        typedef WrapperObject<std::vector<AccessUnitInfo>> BufferInfoWrapper;
        sp<BufferInfoWrapper> bufferInfos{new BufferInfoWrapper(decltype(bufferInfos->value)())};
...
void OutputBuffers::setSkipCutBuffer(int32_t skip, int32_t cut) {
    ...
    mSkipCutBuffer = new C2SkipCutBuffer(skip, cut, mChannelCount);
}
