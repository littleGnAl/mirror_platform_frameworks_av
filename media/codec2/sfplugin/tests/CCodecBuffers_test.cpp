...
private:
    ...
    static bool VerifyPlane(
            const MediaImage2 *mediaImage, const uint8_t *base, uint32_t index, uint8_t value) {
...
public:
    ...
    static bool VerifyClientBuffer(
        ...
        *errorMsg = "";
        ...
        if (!VerifyPlane(mediaImage, buffer->base(), MediaImage2::Y, 'Y')) {
            *errorMsg = "Y plane does not match";
            return false;
        }
        if (!VerifyPlane(mediaImage, buffer->base(), MediaImage2::U, 'U')) {
            *errorMsg = "U plane does not match";
            return false;
        }
        if (!VerifyPlane(mediaImage, buffer->base(), MediaImage2::V, 'V')) {
            *errorMsg = "V plane does not match";
            return false;
        }
        return true;
    }
