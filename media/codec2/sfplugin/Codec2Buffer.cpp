        ...
        switch (layout.type) {
            case C2PlanarLayout::TYPE_YUV: {
                ...
                switch (copyFormat) {
                    ...
                    case COLOR_FormatYUVP010Flexible:
                        mediaImage->mPlane[mediaImage->Y].mOffset = 0;
...
sp<ConstGraphicBlockBuffer> ConstGraphicBlockBuffer::AllocateEmpty(
    ...
    if (format->findInt32("android._color-format", &colorFormat)) {
        if (colorFormat == COLOR_FormatYUVP010 || colorFormat == COLOR_FormatYUVP010Flexible) {
            bpp = 24;  // 16(Y) + 4(U) + 4(V)
        }
    }
