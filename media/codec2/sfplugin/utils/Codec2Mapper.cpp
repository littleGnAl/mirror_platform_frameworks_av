...
const ALookup<C2Config::profile_t, int32_t> sAc4Profiles = {
...
const ALookup<C2Config::level_t, int32_t> sAc4Levels = {
...
struct Ac4ProfileLevelMapper : ProfileLevelMapperHelper {
    bool simpleMap(C2Config::level_t from, int32_t *to) override {
        return sAc4Levels.map(from, to);
    }
    bool simpleMap(int32_t from, C2Config::level_t *to) override {
        return sAc4Levels.map(from, to);
    }
    bool simpleMap(C2Config::profile_t from, int32_t *to) override {
        return sAc4Profiles.map(from, to);
    }
    bool simpleMap(int32_t from, C2Config::profile_t *to) override {
        return sAc4Profiles.map(from, to);
    }
};
