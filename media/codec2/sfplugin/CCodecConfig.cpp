...
ReflectedParamUpdater::Dict CCodecConfig::getReflectedFormat(
    ...
    {   // reflect set of qpoffset-rect-map into a binary blob
        ...
        if (params->findString(KEY_QP_OFFSET_RECTS_INFO, &qpOffsetRectsInfo)) {
            ...
            char *contour = strtok(strdup(qpOffsetRectsInfo.c_str()), ";");
            while (contour != nullptr) {
                int left, right, top, bot;
                int qpOffset;
                if (sscanf(contour, "(%d,%d,%d,%d)-%d", &left, &right, &top, &bot, &qpOffset) == 5) {
                    C2Rect rect(right - left, bot - top);
                    rect.at(left, top);
                    c2QpOffsetRectsInfo.push_back(C2QpOffsetRectStruct(rect, qpOffset));
                } else {
                    ALOGD("Ignoring contour %s", contour);
                }
                contour = strtok(nullptr, ";");
            }
