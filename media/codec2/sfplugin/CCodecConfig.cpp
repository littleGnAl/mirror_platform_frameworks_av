    ...
    { // convert color info
        ...
        if ((int32_t)hdr.hdrType == HDRStaticInfo::kType1) {
            if (msg->findFloat("smpte2086.red.x", &hdr.mastering.red.x)
                ...
                if (0 == status) {
                    ...
                    msg->setBuffer(KEY_HDR_STATIC_INFO,
                        ABuffer::CreateAsCopy(&meta, SIZE_TYPE1));
                }
                else {
            ...
            if (msg->findFloat("valid-hdr-fields", &hdr.validFields)
                ...
                if (!IN_RANGE_INCLUSIVE(hdr.ccv.maxLuminance, 0, 65535)) {
                    status = -1;
                }
                if (!IN_RANGE_INCLUSIVE(hdr.ccv.minLuminance, 0, 65535)) {
                    status = -1;
                }
                if (!IN_RANGE_INCLUSIVE(hdr.ccv.avgLuminance, 0, 65535)) {
                    status = -1;
                }
                ...
                if (0 == status) {
                    ...
                    msg->setBuffer(KEY_HDR_STATIC_INFO,
                        ABuffer::CreateAsCopy(&meta, SIZE_TYPE2));
                } else {
                    ALOGD("found invalid HDR static metadata %s", msg->debugString(8).c_str());
                }
            }
