...
void CCodecConfig::initializeStandardParams() {
    ...
    add(ConfigMapper(KEY_AVERAGE_QP, C2_PARAMKEY_ENC_STAT_AVERAGE_QP, "value")
        .limitTo(D::ENCODER & D::VIDEO & (D::CONFIG | D::PARAM))
        .withMapper([](C2Value v) -> C2Value {
            int32_t value = 0;
            (void)v.get(&value);
            return value == 0 ? C2_FALSE : C2_TRUE;
    ...
    add(ConfigMapper(KEY_PICTURE_TYPE, C2_PARAMKEY_ENC_STAT_PICTURE_TYPE, "value")
        .limitTo(D::ENCODER & D::VIDEO & (D::CONFIG | D::PARAM))
        .withMapper([](C2Value v) -> C2Value {
            int32_t value = 0;
            (void)v.get(&value);
