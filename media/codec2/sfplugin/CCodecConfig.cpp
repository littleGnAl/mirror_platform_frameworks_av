    ...
    { // convert color info
        ...
        if ((int32_t)hdr.hdrType == 0) {
                ...
                if (hdr.mastering.red.x >= 0                && hdr.mastering.red.x <= 1
                    ...
                    msg->removeEntryAt(msg->findEntryByName("cta861.max-fall"));
                            msg->setBuffer(KEY_HDR_STATIC_INFO,
                                ABuffer::CreateAsCopy(&meta, 1 + sizeof(HDRStaticInfo::Type1)));
                } else {
                    ALOGD("found invalid HDR static metadata %s", msg->debugString(8).c_str());
                }
            }
        } else if ((int32_t)hdr.hdrType == 1) {
            if (msg->findFloat("valid-hdr-fields", &hdr.validFields)
                ...
                if (hdr.validFields >= 0                    && hdr.validFields <= 15
                        && hdr.mastering.red.x >= 0         && hdr.mastering.red.x <= 1
                        && hdr.mastering.red.y >= 0         && hdr.mastering.red.y <= 1
                        && hdr.mastering.green.x >= 0       && hdr.mastering.green.x <= 1
                        && hdr.mastering.green.y >= 0       && hdr.mastering.green.y <= 1
                        && hdr.mastering.blue.x >= 0        && hdr.mastering.blue.x <= 1
                        ...
                        && hdr.maxCll >= 0                  && hdr.maxCll <= 65535
                        && hdr.maxFall >= 0                 && hdr.maxFall <= 65535
                        && hdr.ave.ambientIlluminance > 0   && hdr.ave.ambientIlluminance <= 65535
                        && hdr.ave.ambientLight.x >= 0      && hdr.ave.ambientLight.x <= 1
                        && hdr.ave.ambientLight.y >= 0      && hdr.ave.ambientLight.y <= 1
                        && hdr.ccv.cancelFlag >= 0          && hdr.ccv.cancelFlag <= 1
                        && hdr.ccv.persistenceFlag >= 0     && hdr.ccv.persistenceFlag <= 1
                        && hdr.ccv.primariesPresentFlag >= 0 && hdr.ccv.primariesPresentFlag <= 1
                        && hdr.ccv.maxLuminancePresentFlag >= 0
                        && hdr.ccv.maxLuminancePresentFlag <= 1
                        && hdr.ccv.minLuminancePresentFlag >= 0
                        && hdr.ccv.minLuminancePresentFlag <= 1
                        && hdr.ccv.avgLuminancePresentFlag >= 0
                        && hdr.ccv.avgLuminancePresentFlag <= 1
                        && hdr.ccv.red.x >= -100            && hdr.ccv.red.x <= 100
                        && hdr.ccv.red.y >= -100            && hdr.ccv.red.y <= 100
                        && hdr.ccv.green.x >= -100          && hdr.ccv.green.x <= 100
                        && hdr.ccv.green.y >= -100          && hdr.ccv.green.y <= 100
                        && hdr.ccv.blue.x >= -100           && hdr.ccv.blue.x <= 100
                        && hdr.ccv.blue.y >= -100           && hdr.ccv.blue.y <= 100
                        && hdr.ccv.maxLuminance >= 0 && hdr.ccv.maxLuminance <= 65535
                        && hdr.ccv.minLuminance >= 0 && hdr.ccv.minLuminance <= 65535
                        && hdr.ccv.avgLuminance >= 0 && hdr.ccv.avgLuminance <= 65535) {
                    HDRStaticInfo meta;
                    ...
                    meta.sType2.mCCVR.x = hdr.ccv.red.x / 0.00002 + 0.5;
                    meta.sType2.mCCVR.y = hdr.ccv.red.y / 0.00002 + 0.5;
                    meta.sType2.mCCVG.x = hdr.ccv.green.x / 0.00002 + 0.5;
                    meta.sType2.mCCVG.y = hdr.ccv.green.y / 0.00002 + 0.5;
                    meta.sType2.mCCVB.x = hdr.ccv.blue.x / 0.00002 + 0.5;
                    meta.sType2.mCCVB.y = hdr.ccv.blue.y / 0.00002 + 0.5;
                    ...
                    msg->setBuffer(KEY_HDR_STATIC_INFO,
                        ABuffer::CreateAsCopy(&meta, 1 + sizeof(HDRStaticInfo::Type2)));
                } else {
                    ALOGD("found invalid HDR static metadata %s", msg->debugString(8).c_str());
                }
            }
    ...
    { // convert color info
        ...
        if (params->findBuffer(KEY_HDR_STATIC_INFO, &hdrMeta)){
            ...
            if (meta->mID == HDRStaticInfo::kType1
                && hdrMeta->size() == (1 + sizeof(HDRStaticInfo::Type1))) {
                params->setFloat("hdr-type", 0);
                ...
                && hdrMeta->size() == (1 + sizeof(HDRStaticInfo::Type2))) {
                params->setFloat("hdr-type", 1);
                ...
                params->setFloat("smpte2086.min-luminance", meta->sType2.mMinDisplayLuminance * 0.0001);
                ...
                params->setFloat("ccv.primaries-present-flag", meta->sType2.mCCVPrimariesPresentFlag);
                params->setFloat("ccv.max-luminance-flag", meta->sType2.mCCVMaxContentLuminancePresentFlag);
                params->setFloat("ccv.min-luminance-flag", meta->sType2.mCCVMinContentLuminancePresentFlag);
                params->setFloat("ccv.avg-luminance-flag", meta->sType2.mCCVAvgContentLuminancePresentFlag);
                ...
                params->setFloat("ccv.max-luminance", meta->sType2.mMaxContentLuminance * 0.0000001);
                params->setFloat("ccv.min-luminance", meta->sType2.mMinContentLuminance * 0.0000001);
                params->setFloat("ccv.avg-luminance", meta->sType2.mAvgContentLuminance * 0.0000001);
            }
        }
    }
