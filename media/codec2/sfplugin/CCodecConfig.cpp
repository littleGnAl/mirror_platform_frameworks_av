...
sp<AMessage> CCodecConfig::getFormatForDomain(
    ...
    { // convert c2QpOffsetRectsInfo to qpOffsetRectsInfo
        sp<ABuffer> tmp;
...
ReflectedParamUpdater::Dict CCodecConfig::getReflectedFormat(
...

    {
        ...
        if (params->findString(KEY_QP_OFFSET_RECTS_INFO, &qpOffsetRectsInfo)) {
            ...
            // in config mode, set a mock rectangle to signal components to enable Feature_ROI
            if (configDomain == IS_CONFIG && c2QpOffsetRectsInfo.size() == 0) {
            ...
            if (c2QpOffsetRectsInfo.size() != 0) {
                const std::unique_ptr<C2StreamQpOffsetRectsInfo::output> regions =
                        C2StreamQpOffsetRectsInfo::output::AllocUnique(c2QpOffsetRectsInfo.size(),
                                                                       0u, c2QpOffsetRectsInfo);
