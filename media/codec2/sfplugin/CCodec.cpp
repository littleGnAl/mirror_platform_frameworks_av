...
void CCodec::configure(const sp<AMessage> &msg) {
    ...
    //Components are suppose to work with any input buffer size;
    //if the queried MAX_INPUT_SIZE is less than client configured
    //MAX_INPUT_SIZE we are going ahead with the client configure
    ...
    if (clientInputSize > queriedInputSize) {
        config->mInputFormat->setInt32(KEY_MAX_INPUT_SIZE,
            (int32_t)c2_min((uint32_t)clientInputSize,  uint32_t(INT32_MAX)));
    }
