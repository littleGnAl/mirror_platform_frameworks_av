...
status_t CCodecBufferChannel::queueInputBufferInternal(sp<MediaCodecBuffer> buffer) {
    ...
    if (buffer->size() > 0u) {
        ...
        if (input->frameComposer) {
            frameComposer = true;
            input->frameComposer.process(buffer, &items);
        } else {
...
status_t CCodecBufferChannel::start(
    ...
    if (inputFormat != nullptr) {
        ...
        C2StreamAudioEncoderFrameSizeInfo::input encoderFrameSize(0u);
