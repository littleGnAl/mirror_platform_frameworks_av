...
status_t CCodecBufferChannel::attachEncryptedBuffers(
    ...
    if (secure || (mCrypto == nullptr)) {
        if (cryptoInfos->value.size() != 1) {
            ALOGE("Cannot decrypt multple access units");
            return -ENOSYS;
        }
    ...
    std::unique_ptr<C2WriteView> wView = nullptr;
    ensureDecryptDestination(size);
    wView = std::make_unique<C2WriteView>(block->map().get());
    if (wView && wView->error() != C2_OK) {
...
status_t CCodecBufferChannel::queueSecureInputBuffers(
    ...
    if (secure || mCrypto == nullptr) {
        if (cryptoInfos->value.size() != 1) {
            ALOGE("Cannot decrypt multple access units on native handles");
            return -ENOSYS;
        }
        std::unique_ptr<CodecCryptoInfo> &info = cryptoInfos->value[0];
        if (info == nullptr) {
