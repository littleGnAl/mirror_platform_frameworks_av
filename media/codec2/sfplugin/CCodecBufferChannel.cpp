            ...
            if (c2Buffer) {
                ...
                if (inBufferInfo) {
                    ...
                    for (int nMeta = 0; nMeta < inBufferInfo->flexCount(); nMeta++) {
                        const C2LargeFrameMetadataStruct &bufferInfo =
                                inBufferInfo->m.values[nMeta];
                        bufferInfoParams.emplace_back(bufferInfo.flags,
                                static_cast<size_t>(bufferInfo.size),
                                static_cast<uint32_t>(bufferInfo.offset),
                                static_cast<size_t>(bufferInfo.presentationTimeUs));
                    }
