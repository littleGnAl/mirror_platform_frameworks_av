...
void decodeNFrames(const std::shared_ptr<android::Codec2Client::Component>& component,
    ...
    while (1) {
        ...
        flags = ((*Info)[frameID].flags & VTS_FLAG_CSD_FRAME) ? C2FrameData::FLAG_CODEC_CONFIG : 0;
...
TEST_P(Codec2VideoDecHidlTest, AdaptiveDecodeTest) {
    ...
    for (uint32_t i = 0; i < ADAPTIVE_STREAM_COUNT * 2; i++) {
        ...
        while (1) {
            ...
            ASSERT_NE(flags, 0xFF) << "unrecognized flag entry in info file: " << mInfoFile;
            eleInfo >> timestamp;
            timestamp += timestampOffset;
            Info.push_back({bytesCount, flags, timestamp});
            bool codecConfig = (flags & VTS_FLAG_CSD_FRAME) != 0;
            bool nonDisplayFrame = (flags & VTS_FLAG_NO_SHOW_FRAME) != 0;
...
TEST_P(Codec2VideoDecHidlTest, ThumbnailTest) {
    ...
    for (size_t i = 0; i < MAX_ITERATIONS; i++) {
        ...
        for (j = 0; j < Info.size(); j++) {
            if (Info[j].flags & VTS_FLAG_SYNC_FRAME) break;
        }
        std::ifstream eleStream;
        eleStream.open(mInputFile, std::ifstream::binary);
        ASSERT_EQ(eleStream.is_open(), true);
...
TEST_P(Codec2VideoDecHidlTest, FlushTest) {
    ...
    while (index < (int)Info.size()) {
        if (Info[index].flags & VTS_FLAG_SYNC_FRAME) {
            keyFrame = true;
            break;
        }
...
TEST_P(Codec2VideoDecHidlTest, DecodeTestEmptyBuffersInserted) {
    ...
    while (1) {
        if (!(frameId % 5)) {
            flags = !(frameId % 20) ? VTS_FLAG_CSD_FRAME : 0;
            bytesCount = 0;
        } else {
            if (!(eleInfo >> bytesCount)) break;
            eleInfo >> flags;
            ...
            ASSERT_NE(flags, 0xFF) << "unrecognized flag entry in info file: " << mInfoFile;
            eleInfo >> timestamp;
            codecConfig = (flags & VTS_FLAG_CSD_FRAME) != 0;
        }
        Info.push_back({bytesCount, flags, timestamp});
        frameId++;
    }
...
TEST_P(Codec2VideoDecCsdInputTests, CSDFlushTest) {
    ...
    while (1) {
        while (offset < (int)Info.size()) {
            if (Info[offset].flags & VTS_FLAG_SYNC_FRAME) {
                keyFrame = true;
                break;
            }
