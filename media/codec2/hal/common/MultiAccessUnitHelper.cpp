    ...
    for (auto worklet = work->worklets.begin();
        ...
        if (!(*worklet)->output.configUpdate.empty()) {
            for (auto& configUpdate : (*worklet)->output.configUpdate) {
                ...
                if (param->index() == C2StreamSampleRateInfo::output::PARAM_TYPE) {
                    mSampleRate =
                            C2StreamSampleRateInfo::output::From(param)->value;
                } else if (param->index() ==
                        C2StreamChannelCountInfo::output::PARAM_TYPE) {
                    mChannelCount =
                            C2StreamChannelCountInfo::output::From(param)->value;
                }
        ...
        for (int bufIdx = 0; bufIdx < (*worklet)->output.buffers.size(); ++bufIdx) {
            ...
            if (blocks.size() > 0) {
                ...
                while (copiedOutput < blocks.front().size()) {
                    ...
                    if (blocks.front().size() > maxOutSize) {
                        if (frame.mLargeWork) {
                            finalizeWork(frame);
                            addWork(frame.mLargeWork);
                            frame.reset();
                    ...
                    uint32_t toCopy = 0;
                    if (mInterface->kind() == C2Component::KIND_ENCODER) {
                        toCopy = blocks.front().size();
                    } else {
                        toCopy = c2_min(frame.mWview->size(),
                                blocks.front().size() - copiedOutput);
                        toCopy &= (~1);
                    ...
                    LOG(DEBUG) << "Copy: current offset  " << frame.mWview->offset()
                            ...
                            << " Buffer Offset " << frame.mWview->offset()
                            << " timestamp " << timestamp;
