...
void MultiAccessUnitHelper::getSampleRateAndChannelCount(
        uint32_t &sampleRate_, uint32_t &channelCount_) {
    if (mComponentInterface) {
        C2StreamSampleRateInfo::output sampleRate;
        C2StreamChannelCountInfo::output channelCount;
        c2_status_t res = mComponentInterface->query_vb(
...
c2_status_t MultiAccessUnitHelper::scatter(
    ...
    for (std::unique_ptr<C2Work>& w : largeWork) {
        ...
        if (!processedWork->empty()) {
            {
                frameInfo.mLargeFrameTuning = mInterface->getLargeFrameParam();
                std::lock_guard<std::mutex> l(mLock);
...
c2_status_t MultiAccessUnitHelper::processWorklets(MultiAccessUnitInfo &frame,
    ...
    for (auto worklet = work->worklets.begin();
        ...
        getSampleRateAndChannelCount(sampleRate, channelCount);
        if (sampleRate > 0 && channelCount > 0) {
            sampleTimeUs = (1000000u) / (sampleRate * channelCount * 2);
            frameSize = channelCount * 2;
            uint32_t thresholdGap = frame.mLargeFrameTuning.thresholdSize % frameSize;
            uint32_t maxSizeGap = frame.mLargeFrameTuning.maxSize % frameSize;
            if (thresholdGap != 0 || maxSizeGap != 0) {
                uint32_t newThreshold = frame.mLargeFrameTuning.thresholdSize
                        + (frameSize - thresholdGap);
                uint32_t newMaxSize = frame.mLargeFrameTuning.maxSize
                         + (frameSize - maxSizeGap);
                if ((mInterface->getLargeFrameParam().maxSize != newMaxSize)
                        || (mInterface->getLargeFrameParam().thresholdSize != newThreshold)) {
                    C2LargeFrame::output largeFrame(0u, newMaxSize, newThreshold);
                    std::vector<std::unique_ptr<C2SettingResult>> failures;
                    c2_status_t err = mInterface->config(
                            {&largeFrame}, C2_MAY_BLOCK, &failures);
                    // configUpdate required ?
                    LOG(DEBUG) << "new max " << newMaxSize
                            << " newThreshold " << newThreshold;
                    if (err == C2_OK) {
                        outputFramedata.configUpdate.push_back(C2Param::Copy(largeFrame));
                    } else {
                        LOG(DEBUG) << "Large frame config update failed";
                    }
                }
                frame.mLargeFrameTuning.thresholdSize = newThreshold;
                frame.mLargeFrameTuning.maxSize = newMaxSize;
            }
        }
                    ...
                    if (mInterface->kind() == C2Component::KIND_ENCODER) {
                        if (inputSize > frame.mLargeFrameTuning.maxSize) {
                            LOG(ERROR) << "Output buffer too small for audio AU, configured with "
                                << frame.mLargeFrameTuning.maxSize << " block size: "
