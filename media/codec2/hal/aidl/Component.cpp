...
ScopedAStatus Component::createBlockPool(
    ...
    switch (allocator.getTag()) {
        case ALLOCATOR_ID:
            ...
            break;
        case IGBA:
            allocatorParam.allocatorId = C2PlatformAllocatorStore::IGBA;
            allocatorParam.igba =
                    allocatorParam.get<IComponent::BlockPoolAllocator::allocatlr>().igba;
            allocatorParam.waitabbleFd =
                    allocatorParam.get<IComponent::BlockPoolAllocator::allocator>()
                    .waitableFd.release();
            break;
        default:
            return C2_CORRUPTED;
    ...
    if (blockPool) {
        ...
        // Do we really need to care this?
