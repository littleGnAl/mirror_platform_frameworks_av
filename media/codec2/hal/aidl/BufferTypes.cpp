...
template<>
void SetAHardwareBuffer(BaseBlock *block, const AHardwareBuffer *ahwb) {
    AHardwareBuffer_acquire(const_cast<AHardwareBuffer *>(ahwb));
    block->set<HWB_BLOCK>(AidlHardwareBuffer());
    (block->get<BaseBlock::hwbBlock>()).reset(const_cast<AHardwareBuffer *>(ahwb));
}
