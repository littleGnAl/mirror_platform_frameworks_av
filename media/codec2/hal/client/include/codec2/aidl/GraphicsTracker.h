...
public:
     ...
     * is possible. Configuring to a null surface is possible in the case,
     * an allocation request will be fulfilled by a direc allocation(not using
     ...
     *
     * @param[in] dequeueCount    max # of outstanding allocation to configure
     */
    c2_status_t configureDequeueCount(int dequeueCount);
...
private:
    ...
    std::recursive_mutex mConfigLock; // locks for configuration change.
                                      // Sometimes multiple configuration
                                      // changes done at once. In the case
                                      // recursive_lock is required.
    ...
    ::android::base::unique_fd mStopEventFd; // eventfd which indicates the life
                                           // cycle of the class bein stopped.

    std::unique_ptr<std::thread> mEventQueueThread; // Thread to handle interrupted
                                               // writes to eventfd{s}.
...
private:
    GraphicsTracker(int maxDequeueCount);
