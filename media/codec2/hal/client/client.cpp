...
public:
    std::shared_ptr<C2AidlGraphicBufferAllocator> getNewOrCurrent() {
    ...
    if (mAidlBase) {
        ...
        if (id == C2PlatformAllocatorStore::IGBA)  {
            std::shared_ptr<C2AidlGraphicBufferAllocator> gba =
                    mGraphicBufferAllocators->getNewOrCurrent();
...
c2_status_t Codec2Client::Component::destroyBlockPool(
    ...
    if (mAidlBase) {
        mGraphicBufferAllocators->removeOld(localId);
...
c2_status_t Codec2Client::Component::getListenerForOutputSurface(
        uint32_t generation,
        sp<::android::IProducerListener> *producer) {
    if (!mAidlBase) {
        return C2_OMITTED;
    }
    std::shared_ptr<C2AidlGraphicBufferAllocator> gba =
            mGraphicBufferAllocators->getNewOrCurrent();
    *producer = gba->createReleaseListener(generation);
    return C2_OK;
}
...
void Codec2Client::Component::stopUsingOutputSurface(
        C2BlockPool::local_id_t blockPoolId) {
    if (mAidlBase) {
        mGraphicBufferAllocators->resetCurrent();
        return;
    }
    std::scoped_lock lock(mOutputMutex);
