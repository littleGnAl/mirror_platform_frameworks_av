...
public:
    ...
    std::shared_ptr<C2AidlGraphicBufferAllocator> create() {
        ...
        mCurrent =
                C2AidlGraphicBufferAllocator::CreateGraphicBufferAllocator(3 /* maxDequeueCount */);
        ALOGD("GraphicBufferAllocator created");
        return mCurrent;
    }

    // Associates the blockpoll Id returned from HAL to the
    ...
    std::shared_ptr<C2AidlGraphicBufferAllocator> current() {
        std::unique_lock<std::mutex> l(mMutex);
        return mCurrent;
    }
    ...
    void remove(C2BlockPool::local_id_t id) {
        std::unique_lock<std::mutex> l(mMutex);
        mOlds.erase(id);
        if (mCurrentId.has_value() && mCurrentId == id) {
            mCurrent->reset();
            mCurrentId.reset();
            mCurrent.reset();
        }
