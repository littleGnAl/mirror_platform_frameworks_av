...
Codec2Client::Codec2Client(std::shared_ptr<AidlBase> const& base,
      ...
      : Configurable{
            [base]() -> std::shared_ptr<c2_aidl::IConfigurable> {
                ...
                ::ndk::ScopedAStatus transStatus = base->getConfigurable(&configurable);
                return transStatus.isOk() ? configurable : nullptr;
            }()
        },
        mAidlBase{base},
        mServiceIndex{serviceIndex} {
...
std::vector<std::string> Codec2Client::CacheServiceNames() {
    ...
    AServiceManager_forEachDeclaredInstance(
            "android.hardware.media.c2", &names, [](const char *name, void *context) {
...
c2_status_t Codec2Client::Component::setOutputSurface(
    ...
    if (mAidlBase) {
        // FIXME
        ::ndk::ScopedAStatus transStatus = mAidlBase->setOutputSurface(
                blockPoolId,
                ::aidl::android::hardware::NativeWindow(nullptr),
                c2_aidl::SurfaceSyncObj());
        return GetC2Status(transStatus, "setOutputSurface");
    }
...
void Codec2Client::Component::stopUsingOutputSurface(
    ...
    if (mAidlBase) {
        ::ndk::ScopedAStatus transStatus = mAidlBase->setOutputSurface(
                ...
                ::aidl::android::hardware::NativeWindow(nullptr),
                c2_aidl::SurfaceSyncObj());
        GetC2Status(transStatus, "stopUsingOutputSurface");
        return;
    }
