    ...
    if (property_get("persist.sys.locale", locale_value, NULL) > 0) {
        ...
        if (len == 3 && !strncmp(locale_value, "und", 3)) {
            ...
            mLocaleEnc = "windows-874-2000";
                   ...
                   !strncmp(locale_value, "ru", 2) || // Russian
...
void CharacterEncodingDetector::detectAndConvert() {
    ...
    if (size && mUtf8Conv) {
        ...
        if (allprintable) {
            ...
            bool chineseLocale = false;
            if (mLocaleEnc != NULL) {
                chineseLocale = !strcmp(mLocaleEnc, "GB18030") || !strcmp(mLocaleEnc, "Big5");
            }

            if (mLocaleEnc != NULL && !chineseLocale && !goodmatch && highest < 50) {
