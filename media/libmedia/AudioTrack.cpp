...
status_t AudioTrack::setPlaybackRate(const AudioPlaybackRate &playbackRate)
{
    ...
    if ((uint64_t)effectiveRate > (uint64_t)(mSampleRate * AUDIO_RESAMPLER_DOWN_RATIO_MAX)) {
        ALOGV("setPlaybackRate(%f, %f) failed. Resample rate exceeds max accepted value",
                playbackRate.mSpeed, playbackRate.mPitch);
        return BAD_VALUE;
    }

    if ((uint64_t)(effectiveRate * AUDIO_RESAMPLER_UP_RATIO_MAX) < (uint64_t)mSampleRate) {
