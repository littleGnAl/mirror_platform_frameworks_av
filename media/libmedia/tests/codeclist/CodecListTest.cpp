...
TEST(CodecListTest, CodecListByTypeTest) {
    ...
    std::vector<CddReq> cddReq{
            ...
            CddReq(MIMETYPE_AUDIO_AAC, true),    CddReq(MIMETYPE_AUDIO_MPEG, false),
            CddReq(MIMETYPE_VIDEO_AVC, true),    CddReq(MIMETYPE_VIDEO_AVC, false),
            CddReq(MIMETYPE_VIDEO_MPEG4, false), CddReq(MIMETYPE_VIDEO_VP8, true),
            CddReq(MIMETYPE_VIDEO_VP8, false),   CddReq(MIMETYPE_VIDEO_VP9, false),
    };
...
TEST(CodecInfoTest, ListInfoTest) {
    ...
    ASSERT_EQ(status, OK) << "XML Parsing failed.";
