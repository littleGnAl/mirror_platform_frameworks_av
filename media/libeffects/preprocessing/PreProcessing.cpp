...
int Session_SetConfig(preproc_session_t* session, effect_config_t* config) {
    ...
    // webrtc audio preprocessing modules process 10ms worth of data in each call.
    // This requires sampling rate to be a multiple of 100
    if (config->inputCfg.samplingRate % 100) {
        ALOGE("Sampling Rate must be a multiple of 100");
        return -EINVAL;
    }
...
int PreProcessingFx_Process(effect_handle_t self, audio_buffer_t* inBuffer,
    ...
    if (inBuffer->frameCount != session->frameCount) {
        ALOGW("inBuffer->frameCount %zu must correspond to 10ms at samlping rate %d",
              inBuffer->frameCount, session->samplingRate);
        return -EINVAL;
    }
