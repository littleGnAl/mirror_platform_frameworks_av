...
extern bool isAECEffect(const effect_uuid_t* uuid);
...
void EffectTestHelper::process(int16_t* input, int16_t* output) {
    ...
    for (size_t i = 0; i < mLoopCount; i++) {
        if (isAECEffect(mUuid)) ASSERT_NO_FATAL_FAILURE(setParam(AEC_PARAM_ECHO_DELAY, kAECDelay));
