...
TEST_P(SingleEffectTest, SimpleProcess) {
    ...
    if (isAECEffect(mUuid))
        ASSERT_NO_FATAL_FAILURE(effect.setParam(AEC_PARAM_ECHO_DELAY, kAECDelay));
...
TEST_P(SingleEffectComparisonTest, SimpleProcess) {
    ...
    if (isAECEffect(mUuid))
        ASSERT_NO_FATAL_FAILURE(monoEffect.setParam(AEC_PARAM_ECHO_DELAY, kAECDelay));
    ...
    for (size_t chMask : EffectTestHelper::kChMasks) {
        ...
        if (isAECEffect(mUuid))
            ASSERT_NO_FATAL_FAILURE(testEffect.setParam(AEC_PARAM_ECHO_DELAY, kAECDelay));
