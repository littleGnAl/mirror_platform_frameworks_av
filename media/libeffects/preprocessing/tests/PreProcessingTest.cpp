...
int main(int argc, const char *argv[]) {
  ...
  const option long_opts[] = {
      ...
      {"input", required_argument, nullptr, (int)ARG_INPUT},
      {"output", required_argument, nullptr, (int)ARG_OUTPUT},
      {"far", required_argument, nullptr, (int)ARG_FAR},
      {"fs", required_argument, nullptr, (int)ARG_FS},
  ...
  while (1) {
    ...
    if (samplesRead < frameLength) {
      memset(&in[samplesRead], 0, ioFrameSize * (frameLength - samplesRead));
    }
    if (farFp != nullptr) {
    ...
    if (outputFp != nullptr) {
      if (fwrite(out.data(), ioFrameSize, frameLength, outputFp) != (size_t)frameLength) {
