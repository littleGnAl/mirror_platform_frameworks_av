...
int preProcCreateEffect(effect_handle_t *pEffectHandle, uint32_t effectType,
  ...
  if (int status = AUDIO_EFFECT_LIBRARY_INFO_SYM.create_effect(
      &kPreProcUuids[effectType], sessionId, ioId, pEffectHandle); status != 0) {
...
int main(int argc, const char *argv[]) {
  ...
  for (int i = 0; i < PREPROC_NUM_EFFECTS; i++) {
    if (int status =
        preProcCreateEffect(&effectHandle[i], i, &config, sessionId, ioId); status != 0) {
  ...
  if (effectEn[PREPROC_AGC]) {
    if (int status = preProcSetConfigParam(
        ...
        effectHandle[PREPROC_AGC]); status != 0) {
  ...
  if (effectEn[PREPROC_NS]) {
    if (int status = preProcSetConfigParam(NS_PARAM_LEVEL,
                                       (uint32_t)preProcCfgParams.nsLevel,
                                       effectHandle[PREPROC_NS]); status != 0) {
      ...
      if (effectEn[i] == 1) {
        if (i == PREPROC_AEC) {
          if (int status = preProcSetConfigParam(
              ...
              effectHandle[PREPROC_AEC]); status != 0) {
        ...
        if (int status =
            (*effectHandle[i])
                ->process(effectHandle[i], &inputBuffer, &outputBuffer); status != 0) {
  ...
  for (int i = 0; i < PREPROC_NUM_EFFECTS; i++) {
    if (int status = AUDIO_EFFECT_LIBRARY_INFO_SYM.release_effect(effectHandle[i]); status != 0) {
