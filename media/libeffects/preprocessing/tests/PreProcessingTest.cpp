...
constexpr int kPreProcConfigChMaskCount =
    sizeof(kPreProcConfigChMask) / sizeof(kPreProcConfigChMask[0]);
...
int preProcCreateEffect(effect_handle_t *pEffectHandle, uint32_t effectType,
  ...
  int status = AUDIO_EFFECT_LIBRARY_INFO_SYM.create_effect(
      &kPreProcUuids[effectType], sessionId, ioId, pEffectHandle);
  if (status) {
...
int main(int argc, const char *argv[]) {
  ...
  while (true) {
    int opt =
        getopt_long(argc, (char *const *)argv, "i:o:", long_opts, nullptr);
    if (opt == -1) {
      break;
    }
    switch (opt) {
  ...
  for (int i = 0; i < PREPROC_NUM_EFFECTS; i++) {
    int status =
        preProcCreateEffect(&effectHandle[i], i, &config, sessionId, ioId);
    if (status != 0) {
  ...
  if (effectEn[PREPROC_AGC]) {
    int status = preProcSetConfigParam(
  ...
  if (effectEn[PREPROC_NS]) {
    int status = preProcSetConfigParam(NS_PARAM_LEVEL,
                                       (uint32_t)preProcCfgParams.nsLevel,
  ...
  int ioChannelCount =
      audio_channel_count_from_in_mask(preProcCfgParams.chMask);
  ...
  while (1) {
    ...
    if (outputFp != nullptr) {
      size_t samplesWritten =
          fwrite(out.data(), ioFrameSize, outputBuffer.frameCount, outputFp);
