...
RetCode HapticGeneratorContext::setHgVibratorInformation(
    ...
    if (mProcessorsRecord.bpf != nullptr) {
        mProcessorsRecord.bpf->setCoefficients(
                bpfCoefs(mParams.mVibratorInfo.resonantFrequencyHz, DEFAULT_BPF_Q, mSampleRate));
    }
    if (mProcessorsRecord.bsf != nullptr) {
        mProcessorsRecord.bsf->setCoefficients(
                bsfCoefs(mParams.mVibratorInfo.resonantFrequencyHz, mParams.mVibratorInfo.qFactor,
...
void HapticGeneratorContext::init_params(AudioChannelLayout inputChMask,
                                         AudioChannelLayout outputChMask) {
    ...
    mParams.mAudioChannelCount =
            getChannelCount(inputChMask, ~AudioChannelLayout::LAYOUT_HAPTIC_AB);
    mParams.mHapticChannelCount =
            getChannelCount(outputChMask, AudioChannelLayout::LAYOUT_HAPTIC_AB);
...
void HapticGeneratorContext::buildProcessingChain() {
    ...
    auto hpf = createHPF2(highPassCornerFrequency, mSampleRate, channelCount);
    addBiquadFilter(hpf);
    float lowPassCornerFrequency = 9000.0f;
    auto lpf = createLPF2(lowPassCornerFrequency, mSampleRate, channelCount);
    addBiquadFilter(lpf);
    ...
    hpf = createHPF2(highPassCornerFrequency, mSampleRate, channelCount);
    addBiquadFilter(hpf);
    lowPassCornerFrequency = 700.0f;
    lpf = createLPF2(lowPassCornerFrequency, mSampleRate, channelCount);
    addBiquadFilter(lpf);
    ...
    auto bpf = createBPF(mParams.mVibratorInfo.resonantFrequencyHz, DEFAULT_BPF_Q, mSampleRate,
                         channelCount);
    ...
    auto bsf = createBSF(mParams.mVibratorInfo.resonantFrequencyHz, mParams.mVibratorInfo.qFactor,
