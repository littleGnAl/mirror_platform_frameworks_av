                                          ...
                                          LVM_InstParams_t       *pInstParams)
{
    ...
    *phInstance  = (LVM_Handle_t)calloc(1, sizeof(*pInstance));
    ...
    BundleScratchSize = (LVM_INT32)
                         ...
                         * sizeof(LVM_FLOAT));
    pInstance->pScratch = malloc(BundleScratchSize);
    ...
    if (pInstParams->BufferMode == LVM_MANAGED_BUFFERS)
    {
        ...
        pInstance->pBufferManagement = (LVM_Buffer_t *)
                    malloc(sizeof(*(pInstance->pBufferManagement)));
     ...
     * Treble Enhancement
     */
    pInstance->pTE_Taps  = (LVM_TE_Data_t *)malloc(sizeof(*(pInstance->pTE_Taps)));
    if (pInstance->pTE_Taps == LVM_NULL)
    {
        return LVM_NULLADDRESS;
    }
    pInstance->pTE_State = (LVM_TE_Coefs_t *)malloc(sizeof(*(pInstance->pTE_State)));
    ...
    pInstance->pEQNB_BandDefs = (LVM_EQNB_BandDef_t *)
        malloc(pInstParams->EQNB_NumBands * sizeof(*(pInstance->pEQNB_BandDefs)));
    if (pInstance->pEQNB_BandDefs == LVM_NULL)
    {
        return LVM_NULLADDRESS;
    }
    pInstance->pEQNB_UserDefs = (LVM_EQNB_BandDef_t *)
         malloc(pInstParams->EQNB_NumBands * sizeof(*(pInstance->pEQNB_UserDefs)));
     ...
     */
    {
        pInstance->pHeadroom_BandDefs = (LVM_HeadroomBandDef_t *)
            malloc(LVM_HEADROOM_MAX_NBANDS * sizeof(*(pInstance->pHeadroom_BandDefs)));
        if (pInstance->pHeadroom_BandDefs == LVM_NULL)
        {
            return LVM_NULLADDRESS;
        }
        pInstance->pHeadroom_UserDefs = (LVM_HeadroomBandDef_t *)
            malloc((LVM_HEADROOM_MAX_NBANDS * sizeof(*(pInstance->pHeadroom_UserDefs))));
     ...
     */
    {
        ...
        if(pInstParams->PSA_Included==LVM_PSA_ON)
        {
            ...
            pInstance->pPSAInput = (LVM_FLOAT *)
                                malloc(MAX_INTERNAL_BLOCKSIZE * sizeof(LVM_FLOAT));
