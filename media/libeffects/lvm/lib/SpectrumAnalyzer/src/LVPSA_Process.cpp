    ...
    *******************************************************************************/
    ...
    if (pLVPSA_Inst->pSpectralDataBufferWritePointer != pWrite_Save) {
        AudioTimeInc = mult32x32in32_shiftr(
            ...
            (LVM_INT32)LVPSA_SampleRateInvTab[pLVPSA_Inst->CurrentParams.Fs], LVPSA_FsInvertShift);
...

{
    ...
    if ((LVM_INT32)(StatusDelta * pLVPSA_Inst->nBands) >
        ...
        pRead =
            ...
            (pLVPSA_Inst->SpectralDataBufferLength - (LVM_UINT32)StatusDelta) * pLVPSA_Inst->nBands;
    } else {
