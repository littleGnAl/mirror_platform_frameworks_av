...
#ifdef BIQUAD_OPT
    pLVREV_Private->pBqInstanceHPF->clear();
    pLVREV_Private->pBqInstanceLPF->clear();
#else
...
#ifdef BIQUAD_OPT
        for (int ii = 0; ii < LVREV_DELAYLINES_4; ii++) {
            pLVREV_Private->pBqInstanceRevLPF[ii]->clear();
        }
#else
...
#ifdef BIQUAD_OPT
        for (int ii = 0; ii < LVREV_DELAYLINES_2; ii++) {
            pLVREV_Private->pBqInstanceRevLPF[ii]->clear();
        }
#else
        LoadConst_Float(0, (LVM_FLOAT*)&pLVREV_Private->pFastData->RevLPTaps[1], 2);
...
#ifdef BIQUAD_OPT
        pLVREV_Private->pBqInstanceRevLPF[0]->clear();
#else
        LoadConst_Float(0, (LVM_FLOAT*)&pLVREV_Private->pFastData->RevLPTaps[0], 2);
#endif
        LoadConst_Float(0, pLVREV_Private->pDelay_T[0], LVREV_MAX_T0_DELAY);
