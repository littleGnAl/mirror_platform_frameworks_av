...
LVREV_ReturnStatus_en LVREV_GetInstanceHandle(LVREV_Handle_t* phInstance,
    ...
    for (size_t i = 0; i < pInstanceParams->NumDelays; i++) {
        pLVREV_Private->pDelay_T[i] =
                (LVM_FLOAT*)calloc(1, LVREV_MAX_T_DELAY[i] * sizeof(LVM_FLOAT));
        /* Scratch for each delay line output */
        pLVREV_Private->pScratchDelayLine[i] =
                (LVM_FLOAT*)calloc(1, sizeof(LVM_FLOAT) * MaxBlockSize);
    }
    ...
    pLVREV_Private->pScratch = (LVM_FLOAT*)calloc(1, sizeof(LVM_FLOAT) * MaxBlockSize);
    /* Mono->stereo input save for end mix */
    pLVREV_Private->pInputSave = (LVM_FLOAT*)calloc(1, FCC_2 * sizeof(LVM_FLOAT) * MaxBlockSize);
