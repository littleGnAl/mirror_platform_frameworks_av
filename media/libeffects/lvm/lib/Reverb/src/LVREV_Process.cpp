...
void ReverbBlock(LVM_FLOAT* pInput, LVM_FLOAT* pOutput, LVREV_Instance_st* pPrivate,
     ...
     */
#ifdef BIQUAD_OPT
    pPrivate->pBqInstanceHPF->process(pTemp, pTemp, NumSamples);
#else
     ...
     */
#ifdef BIQUAD_OPT
    pPrivate->pBqInstanceLPF->process(pTemp, pTemp, NumSamples);
#else
    ...
    for (j = 0; j < NumberOfDelayLines; j++) {
         ...
         */
#ifdef BIQUAD_OPT
        pPrivate->pBqInstanceRevLPF[j]->process(pDelayLine, pDelayLine, NumSamples);
#else
