...
#endif
        if (pInstanceParams->NumDelays == LVREV_DELAYLINES_4) {
            InstAlloc_AddMember(&FastData, MaxT_Delay[3] * sizeof(LVM_FLOAT));
            InstAlloc_AddMember(&FastData, MaxT_Delay[2] * sizeof(LVM_FLOAT));
            InstAlloc_AddMember(&FastData, MaxT_Delay[1] * sizeof(LVM_FLOAT));
            InstAlloc_AddMember(&FastData, MaxT_Delay[0] * sizeof(LVM_FLOAT));
        } else if (pInstanceParams->NumDelays == LVREV_DELAYLINES_2) {
            InstAlloc_AddMember(&FastData, MaxT_Delay[1] * sizeof(LVM_FLOAT));
            InstAlloc_AddMember(&FastData, MaxT_Delay[0] * sizeof(LVM_FLOAT));
