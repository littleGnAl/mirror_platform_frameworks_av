...
LVCS_ReturnStatus_en LVCS_Control(LVCS_Handle_t hInstance, LVCS_Params_t *pParams) {
    ...
    if ((pInstance->Params.SampleRate != pParams->SampleRate) ||
        ...
        Offset =
            (LVM_INT16)(pParams->SpeakerType + pParams->SourceFormat * (1 + LVCS_EX_HEADPHONES));
    ...
    if ((pParams->OperatingMode != OperatingModeSave) ||
        ...
        if (pInstance->bInOperatingModeTransition != LVM_TRUE) {
            ...
            pInstance->TimerParams.TimeInMs = (LVM_INT16)(
                ((pInstance->Reverberation.DelaySize << 2) / pInstance->TimerParams.SamplingRate) +
                1);
