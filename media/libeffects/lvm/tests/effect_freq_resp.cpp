...
TEST(TestEffect, AndroidEqualizerEffect) {
    ...
    toneFrequencies.push_back(centerFrequencies[numBands / 2]);  // for attenutation
    for (auto toneFrequency = 20; toneFrequency < 20000; toneFrequency += 100) {
    ...
    for (int preset = 0; preset < numPresets; preset++) {
        ...
        int i = 0;
        float gaindB[numBands], attenuationdB;
        for (auto toneFrequency : toneFrequencies) {
            ...
            status = effect.getParam<false>(EQ_PARAM_GET_BAND, toneFrequency * 1000, band);
            EXPECT_EQ(0, status);
            ...
            if (i == 0) {
                float achievedGaindB = 20 * log10(opAttr.maxAmplitude / inpAttr.maxAmplitude);
                attenuationdB = gaindB[numBands / 2] - achievedGaindB;
                i++;
                continue;
            }
            freqResponse.push_back(opAttr);
            float achievedGaindB = 20 * log10(opAttr.maxAmplitude / inpAttr.maxAmplitude);
            achievedGaindB += attenuationdB;
            // blind test 3.0db is noticeable
            EXPECT_GE(3.0, abs(achievedGaindB - gaindB[band[0]]))
                    << "For preset " << preset << " frequency " << toneFrequency << " band "
                    << band[0];
            EXPECT_LE(0, achievedGaindB * gaindB[band[0]])
                    << "For preset " << preset << " frequency " << toneFrequency << " band "
                    << band[0];
        }
    }
