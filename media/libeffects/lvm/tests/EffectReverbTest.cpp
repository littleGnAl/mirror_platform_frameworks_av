...
void EffectReverbHelper::configEffect() {
    ...
    if (isAuxMode(mUuid)) {
        config.inputCfg.channels = AUDIO_CHANNEL_OUT_MONO;
    }
...
void EffectReverbHelper::process(float* input, float* output) {
    ...
    for (size_t i = 0; i < mLoopCount; i++) {
        ...
        inBuffer.f32 += (isAuxMode(mUuid) ? mFrameCount : mFrameCount * mChannelCount);
...
TEST_P(SingleEffectTest, SimpleProcess) {
    ...
    if (isAuxMode(mUuid)) input.resize(mTotalFrameCount);
