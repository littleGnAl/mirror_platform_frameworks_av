...
class EffectReverbHelper {
  public:
    ...
    int reverbSetConfigParam(uint32_t paramType, uint32_t paramValue);
...
void EffectReverbHelper::configEffect() {
    ...
    if (AUDIO_CHANNEL_OUT_MONO == mChMask) {
        config.outputCfg.channels = AUDIO_CHANNEL_OUT_STEREO;
    }
    if (isAuxMode(mUuid) && config.inputCfg.channels != AUDIO_CHANNEL_OUT_MONO) {
        return;
    } else if (!isAuxMode(mUuid) && config.inputCfg.channels == AUDIO_CHANNEL_OUT_MONO) {
...
int EffectReverbHelper::reverbSetConfigParam(uint32_t paramType, uint32_t paramValue) {
    int reply = 0;
    uint32_t replySize = sizeof(reply);
...
void EffectReverbHelper::process(float* input, float* output) {
    ...
    for (size_t i = 0; i < mLoopCount; i++) {
        ...
        inBuffer.f32 += mFrameCount * mChannelCount;  // TODO: This ptr increment to be verified
  ...
  public:
    SingleEffectTest()
        : mChMask(kChMasks[std::get<0>(GetParam())]),
          mChannelCount(audio_channel_count_from_out_mask(mChMask) == FCC_1
                                ? FCC_2
                                : audio_channel_count_from_out_mask(mChMask)),
...
TEST_P(SingleEffectComparisonTest, SimpleProcess) {
    ...
    for (size_t chMask : kChMasks) {
        ...
        size_t channelCount = audio_channel_count_from_out_mask(chMask) == FCC_1
                                      ? FCC_2
                                      : audio_channel_count_from_out_mask(chMask);
        ...
        // TODO: Check this comparison logic for Aux-mode
        if ((channelCount > 0 && !isAuxMode(mUuid)) /* || channelCount > 1*/) {
            ASSERT_EQ(0,
