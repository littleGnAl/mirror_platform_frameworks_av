...
// sampleRates
// TODO: Add all sampling rates once they are handled by spatializer
...
// frame counts
// TODO: Add sizes smaller than 80 once they are handled by spatializer
...
// Test basic spatializer functionality for various combinations of sampling rates,
// channel masks, frame counts and so on.
...
// TODO: This is currently disabled as output from the spatializer seems to be wrong.
...
TEST_P(SingleEffectComparisonTest, DISABLED_SimpleProcess) {
    SCOPED_TRACE(testing::Message()
                 << "chMask: " << mInputChMask << " sampleRate: " << mSampleRate);
    int totalFrameCount = ALIGN((mSampleRate * 40) / 1000, kNumBufferSplits);  // 40 ms
...
// test doesn't validate the functionality of the params configured. It only checks the return status
// of API calls.
