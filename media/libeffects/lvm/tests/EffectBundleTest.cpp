...
TEST_P(SingleEffectComparisonTest, SimpleProcess) {
    ...
    std::vector<float> monoInput(mFrameCount * mLoopCount);
    ...
    adjust_channels(monoInput.data(), FCC_1, stereoInput.data(), FCC_2, sizeof(float),
                    mFrameCount * sizeof(float) * FCC_1);
    ...
    for (size_t chMask : kChMasks) {
        ...
        for (int i = 0; i < mFrameCount; ++i) {
            ...
            *fp = monoInput[i];
            std::fill(fp + 1, fp + channelCount, monoInput[i]);
        }
        ...
        if (mUuid == &kEffectUuids[0]) {
            ...
            ASSERT_EQ(0, memcmp(stereoRefI16.data(), stereoTestI16.data(), mFrameCount * FCC_2))
