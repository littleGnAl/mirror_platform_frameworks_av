...
class EffectBundleContext : public EffectContext {
  public:
    EffectBundleContext(int statusDepth, int inBufferSize, int outBufferSize,
                        std::shared_ptr<BundleContext> context)
        : EffectContext(statusDepth, inBufferSize, outBufferSize), mBundleContext(context) {
        LOG(DEBUG) << __func__;
    }

    void enableSession() {
        if (mBundleContext) {
            mBundleContext->enable();
        } else {
    ...
    int getSessionId() {
        return mBundleContext ? mBundleContext->getSessionId() : INVALID_SESSION_ID;
    }
    std::shared_ptr<BundleContext> getBundleContext() { return mBundleContext; }
...
class EffectBundleAidl : public BnEffect, EffectWorker {
  ...
  private:
    ...
    inline static std::unique_ptr<GlobalSession> mGloalSession;
