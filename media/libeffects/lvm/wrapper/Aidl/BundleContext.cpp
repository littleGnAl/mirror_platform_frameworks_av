...
RetCode BundleContext::setVolumeStereo(const Parameter::VolumeStereo& volume) {
    ...
    RETURN_VALUE_IF(LVM_SUCCESS != LVM_GetControlParameters(mInstance, &activeParams),
                    RetCode::ERROR_EFFECT_LIB_ERROR, "");

    activeParams.VC_Balance = pandB;

    RETURN_VALUE_IF(LVM_SUCCESS != LVM_SetControlParameters(mInstance, &activeParams),
                    RetCode::ERROR_EFFECT_LIB_ERROR, "");
