...
LVM_EQNB_BandDef_t *BundleContext::getDefaultEqualizerBandDefs() {
    static LVM_EQNB_BandDef_t BandDefs[lvm::MAX_NUM_BANDS];

    /* N-Band Equaliser parameters */
    for (int i = 0; i < lvm::MAX_NUM_BANDS; i++) {
        BandDefs[i].Frequency = lvm::kPresetsFrequencies[i];
        BandDefs[i].QFactor = lvm::kPresetsQFactors[i];
        BandDefs[i].Gain = lvm::kSoftPresets[0/* normal */][i];
    }
    return BandDefs;
}
