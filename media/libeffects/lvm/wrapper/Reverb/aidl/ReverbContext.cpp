...
RetCode ReverbContext::init() {
    ...
    mVolume.left = UNIT_VOLUME;
    mVolume.right = UNIT_VOLUME;
    mPrevVolume.left = UNIT_VOLUME;
    mPrevVolume.right = UNIT_VOLUME;
    volumeMode = VOLUME_FLAT;

    mSamplesToExitCount = DEFAULT_DECAY_TIME * mCommon.input.base.sampleRate / 1000;
    ...
    mRoomHfLevel = 0;
    mEnabled = LVM_FALSE;
    mDecayTime = DEFAULT_DECAY_TIME;
    mDecayHfRatio = DEFAULT_DAMPING * 20;
    mDensity = DEFAULT_ROOM_SIZE * 10;
    mDiffusion = DEFAULT_DENSITY * 10;
    mLevel = -6000;
...
RetCode ReverbContext::setEnvironmentalReverbRoomLevel(const int& roomLevel) {
    ...
    LVREV_ControlParams_st params;
    {
        ...
        int combinedLevel = (roomLevel + mLevel) - lvm::MAX_REVERB_LEVEL;
...
RetCode ReverbContext::setEnvironmentalReverbRoomHfLevel(const int& roomHfLevel) {
...
RetCode ReverbContext::setEnvironmentalReverbDecayTime(const int& decayTime) {
...
RetCode ReverbContext::setEnvironmentalReverbDecayHfRatio(const int& decayHfRatio) {
...
RetCode ReverbContext::setEnvironmentalReverbLevel(const int& level) {
    ...
    LVREV_ControlParams_st params;
    {
        ...
        int combinedLevel = (level + mRoomLevel) - lvm::MAX_REVERB_LEVEL;
        params.Level = convertLevel(combinedLevel);
...
RetCode ReverbContext::setEnvironmentalReverbDelay(const int& delay) {
...
RetCode ReverbContext::setEnvironmentalReverbDiffusion(const int& diffusion) {
...
RetCode ReverbContext::setEnvironmentalReverbDensity(const int& density) {
...
RetCode ReverbContext::setEnvironmentalReverbBypass(const bool& bypass) {
    mBypass = bypass;
    return RetCode::SUCCESS;
}
...
void ReverbContext::initControlParameter(LVREV_ControlParams_st& params) {
    ...
    params.LPF = DEFAULT_LPF;
    params.HPF = DEFAULT_HPF;
    params.T60 = DEFAULT_DECAY_TIME;
    params.Density = DEFAULT_DENSITY;
    params.Damping = DEFAULT_DAMPING;
    params.RoomSize = DEFAULT_ROOM_SIZE;
}
...
IEffect::Status ReverbContext::lvmProcess(float* in, float* out, int samples) {
    ...
    if (isAuxiliary()) {
        ...
        if (channels >= FCC_2) {
            for (int i = 0; i < frameCount; i++) {
                inFrames[FCC_2 * i] = in[channels * i] * SEND_LEVEL;
                inFrames[FCC_2 * i + 1] = in[channels * i + 1] * SEND_LEVEL;
            }
        } else {
            for (int i = 0; i < frameCount; i++) {
                inFrames[FCC_2 * i] = inFrames[FCC_2 * i + 1] = in[i] * SEND_LEVEL;
            }
        }
    }
    ...
    if (isAuxiliary()) {
        ...
        if (mVolume != mPrevVolume && volumeMode == VOLUME_RAMP) {
            ...
            if (mVolume.left != UNIT_VOLUME || mVolume.right != UNIT_VOLUME) {
                for (int i = 0; i < frameCount; i++) {
