...
class ReverbContext final : public EffectContext {
  public:
    ...
    RetCode setEnvironmentalReverbRoomLevel(const int& roomLevel);
    int getEnvironmentalReverbRoomLevel() const { return mRoomLevel; }
    RetCode setEnvironmentalReverbRoomHfLevel(const int& roomHfLevel);
    int getEnvironmentalReverbRoomHfLevel() const { return mRoomHfLevel; }
    RetCode setEnvironmentalReverbDecayTime(const int& decayTime);
    int getEnvironmentalReverbDecayTime() const { return mDecayTime; }
    RetCode setEnvironmentalReverbDecayHfRatio(const int& decayHfRatio);
    int getEnvironmentalReverbDecayHfRatio() const { return mDecayHfRatio; }
    RetCode setEnvironmentalReverbLevel(const int& level);
    int getEnvironmentalReverbLevel() const { return mLevel; }
    RetCode setEnvironmentalReverbDelay(const int& delay);
    int getEnvironmentalReverbDelay() const { return mDelay; }
    RetCode setEnvironmentalReverbDiffusion(const int& diffusion);
    int getEnvironmentalReverbDiffusion() const { return mDiffusion; }
    RetCode setEnvironmentalReverbDensity(const int& density);
    int getEnvironmentalReverbDensity() const { return mDensity; }
    RetCode setEnvironmentalReverbBypass(const bool& bypass);
  ...
  private:
    static inline const float UNIT_VOLUME = 1;
    static inline const float SEND_LEVEL = 0.75f;
    static inline const int DEFAULT_LEVEL = 0;         /* Default level */
    static inline const int DEFAULT_LPF = 23999;       /* Default low pass filter, in Hz */
    static inline const int DEFAULT_HPF = 50;          /* Default high pass filter, in Hz */
    static inline const int DEFAULT_DECAY_TIME = 1490; /* Default Decay time, in ms */
    static inline const int DEFAULT_DENSITY = 100;     /* Default Echo density */
    static inline const int DEFAULT_DAMPING = 21;      /* Default Damping */
    static inline const int DEFAULT_ROOM_SIZE = 100;   /* Default Room size */
