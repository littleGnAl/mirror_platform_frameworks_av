...
bool DynamicsProcessingImpl::isLimiterConfigInRange(
    ...
    auto func = [](const DynamicsProcessing::LimiterConfig& arg)
            -> std::tuple<int32_t, bool, int32_t, float, float, float, float, float> {
        return {arg.channel,       arg.enable, arg.linkGroup,   arg.attackTimeMs,
...
bool DynamicsProcessingImpl::isMbcBandConfigInRange(
    ...
    auto func = [](const DynamicsProcessing::MbcBandConfig& arg)
            -> std::tuple<int32_t, int32_t, bool, float, float, float, float, float, float, float,
                          float, float, float> {
        return {arg.channel,       arg.band,
                arg.enable,        arg.cutoffFrequencyHz,
                arg.attackTimeMs,  arg.releaseTimeMs,
                arg.ratio,         arg.thresholdDb,
                arg.kneeWidthDb,   arg.noiseGateThresholdDb,
                arg.expanderRatio, arg.preGainDb,
                arg.postGainDb};
    };
    auto minT = func(min), maxT = func(max);
    for (auto cfg : cfgs) {
...
bool DynamicsProcessingImpl::isEqBandConfigInRange(
    ...
    auto func = [](const DynamicsProcessing::EqBandConfig& arg)
            -> std::tuple<int32_t, int32_t, bool, float, float> {
        return {arg.channel, arg.band, arg.enable, arg.cutoffFrequencyHz, arg.gainDb};
    };
    auto minT = func(min), maxT = func(max);
    for (auto cfg : cfgs) {
        if (!isTupleInRange(func(cfg), minT, maxT)) return false;
...
bool DynamicsProcessingImpl::isChannelConfigInRange(
    ...
    auto func = [](const DynamicsProcessing::ChannelConfig& arg) -> std::tuple<int32_t, bool> {
        return {arg.channel, arg.enable};
    };
    auto minT = func(min), maxT = func(max);
    for (auto cfg : cfgs) {
        if (!isTupleInRange(func(cfg), minT, maxT)) return false;
...
bool DynamicsProcessingImpl::isEngineConfigInRange(
    ...
    auto func = [](const DynamicsProcessing::EngineArchitecture& arg)
            -> std::tuple<DynamicsProcessing::ResolutionPreference, float, bool, int32_t, bool,
                          int32_t, bool, int32_t, bool> {
        return {arg.resolutionPreference, arg.preferredProcessingDurationMs,
                arg.preEqStage.inUse,     arg.preEqStage.bandCount,
                arg.postEqStage.inUse,    arg.postEqStage.bandCount,
                arg.mbcStage.inUse,       arg.mbcStage.bandCount,
                arg.limiterInUse};
    };
