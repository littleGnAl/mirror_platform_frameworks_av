...
static constexpr char DUMMY_PATH[] = "/sdcard/AsyncIO_test_data.txt";
...
TEST_F(AsyncIOTest, testRead) {
    ...
    EXPECT_EQ(write(dummy_file, dummyDataStr.c_str(), TEST_PACKET_SIZE), TEST_PACKET_SIZE);
    struct aiocb aio;
    struct aiocb *aiol[] = {&aio};
    aio.aio_fildes = dummy_file;
    aio.aio_buf = buf;
    aio.aio_offset = 0;
...
TEST_F(AsyncIOTest, testWrite) {
    ...
    EXPECT_EQ(aio_suspend(aiol, 1, nullptr), 0);
    EXPECT_EQ(aio_return(&aio), TEST_PACKET_SIZE);
    EXPECT_EQ(read(dummy_file, buf, TEST_PACKET_SIZE), TEST_PACKET_SIZE);
    EXPECT_STREQ(buf, dummyDataStr.c_str());
}
...
TEST_F(AsyncIOTest, testPoolWrite) {
    ...
    aio_pool_end();
    EXPECT_EQ(read(dummy_file, buf, TEST_PACKET_SIZE * POOL_COUNT), TEST_PACKET_SIZE * POOL_COUNT);
