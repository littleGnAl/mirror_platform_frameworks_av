...
int MtpFfsCompatHandle::writeHandle(int fd, const void* data, int len) {
...
int MtpFfsCompatHandle::readHandle(int fd, void* data, int len) {
...
int MtpFfsCompatHandle::receiveFile(mtp_file_range mfr, bool zero_packet) {
    ...
    if (ret % packet_size == 0 || zero_packet) {
        if (TEMP_FAILURE_RETRY(::read(mBulkOut, data, 1)) != 0) {
            return -1;
        }
    }
    return 0;
...
int MtpFfsCompatHandle::sendFile(mtp_file_range mfr) {
    ...
    if (ret % packet_size == 0) {
        ...
        if (TEMP_FAILURE_RETRY(::write(mBulkIn, data, 1)) != 0) {
            return -1;
        }
    }
