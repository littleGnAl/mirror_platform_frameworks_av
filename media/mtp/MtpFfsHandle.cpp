...
int MtpFfsHandle::start() {
    ...
    posix_madvise(mBuffer1.data(), MAX_FILE_CHUNK_SIZE, POSIX_MADV_SEQUENTIAL | POSIX_MADV_WILLNEED);
    posix_madvise(mBuffer2.data(), MAX_FILE_CHUNK_SIZE, POSIX_MADV_SEQUENTIAL | POSIX_MADV_WILLNEED);
...
int MtpFfsHandle::receiveFile(mtp_file_range mfr) {
    ...
    while (file_length > 0 || write) {
        if (file_length > 0) {
            const int chunk_size = offset > MAX_FILE_CHUNK_SIZE ?
                MAX_FILE_CHUNK_SIZE : mMaxRead;
            length = std::min(static_cast<uint32_t>(chunk_size), file_length);
...
int MtpFfsHandle::sendFile(mtp_file_range mfr) {
    ...
    while(file_length > 0) {
        ...
        if (file_length > 0) {
            const int chunk_size = offset > MAX_FILE_CHUNK_SIZE ?
                MAX_FILE_CHUNK_SIZE : mMaxWrite;
            length = std::min(static_cast<uint64_t>(chunk_size), file_length);
