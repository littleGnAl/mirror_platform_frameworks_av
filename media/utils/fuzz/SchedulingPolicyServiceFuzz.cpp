...
static sp<android::ISchedulingPolicyService> sSchedulingPolicyService;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  // We are just using scheduling_policy to avoid the need
  // to register a new service.
  const sp<IServiceManager> sm(defaultServiceManager());

  const String16 name("scheduling_policy");
  sSchedulingPolicyService =
      interface_cast<android::ISchedulingPolicyService>(sm->checkService(name));

  requestCpusetBoost(enable, sSchedulingPolicyService);
  return 0;
}
