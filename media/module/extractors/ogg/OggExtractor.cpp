...
status_t MyOggExtractor::findNextPage(
    ...
    *pageOffset = startOffset;
    ...
    while(1) {
        ...
        // work with big buffers to amoritze readAt() costs
        ...
        // overlap enough for detect this. n >= lenOggS, so this always advances.
...
status_t MyOggExtractor::findPrevGranulePosition(
    ...
    *granulePos = 0;
    ...
    ALOGD("findPrevGranulePosition( %" PRIu64 ",-)", pageOffset);
    const int FIND_BUF_SIZE = 2048;
    const int lenOggS = strlen("OggS");
    ...
    off64_t firstAfter = pageOffset + 3 - 1;    // NOT in the buffer
    ...
    while(prevPageOffset == pageOffset) {
        ...
        // our buffer could have begun with "ggS"
        // so want to include those bytes in the next iteration
        firstAfter = nextOffset + 3 - 1;
    }
