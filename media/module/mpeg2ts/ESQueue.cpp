...
static status_t parseDTSHDSyncFrame(
    ...
    if (dtshdSyncWord == DTSHD_SYNC_CORE_16BIT_BE) { // DTS Digital Surround Header
        ...
        if (headerSizeType == 0) {
            RETURN_ERROR_IF_NOT_ENOUGH_BYTES_LEFT(bits, (8 + 16 + 1));

            extHeadersize = bits.getBits(8) + 1;
            extSSFsize = bits.getBits(16) + 1;
        } else {
            RETURN_ERROR_IF_NOT_ENOUGH_BYTES_LEFT(bits, (12 + 20 + 1));
    ...
    if (metaData != NULL) {
        if (isLBR) {
            ...
            (*metaData)->setCString(kKeyAudioProfile, 0x2); // CodecProfileLevel.DTS_HDProfileLBR
        } else {
