...
TEST_F(AmrwbDecoderTest, MultiCreateAmrwbDecoderTest) {
    ...
    for (int count = 0; count < kMaxCount; count++) {
        pvDecoder_AmrWb_Init(&amrHandle, (void*)decoderBuf.get(), &decoderCookie);
...
TEST_P(AmrwbDecoderTest, ResetDecoderTest) {
    ...
    int32_t decoderErr = DecodeFrames(decoderCookie, (void*)decoderBuf.get(), outFileHandle, kNumFrameReset);
    ASSERT_EQ(decoderErr, 0) << "DecodeFrames returned error";

    // Reset Decoder
    pvDecoder_AmrWb_Reset((void*)decoderBuf.get(), 1);

    // Start decoding again
    decoderErr = DecodeFrames(decoderCookie, (void*)decoderBuf.get(), outFileHandle);
    ASSERT_EQ(decoderErr, 0) << "DecodeFrames returned error";
