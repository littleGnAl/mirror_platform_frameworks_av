...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    sp<DrmManagerClientImpl> drmManager =
                             DrmManagerClientImpl::create(&uniqueId, fdp.ConsumeBool());
    ...
    drmManager->validateAction(uniqueId, path, action,
                    ActionDescription(fdp.ConsumeIntegral<int>(), fdp.ConsumeIntegral<int>()));
    ...
    sp<DecryptHandle> decryptHandle =
                        drmManager->openDecryptSession(uniqueId, DrmBuffer(Data, Size), mimeType);
