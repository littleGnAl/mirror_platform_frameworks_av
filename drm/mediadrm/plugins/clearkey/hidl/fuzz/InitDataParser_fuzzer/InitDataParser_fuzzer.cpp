...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  const std::vector<std::string> mimeTypeStrings = {"cenc", "audio/mp4",
    "video/mp4", "webm", "audio/webm", "video/webm",
    fuzzed_data.ConsumeBytesAsString(8)};
  const std::string mimeType = mimeTypeStrings[
    fuzzed_data.ConsumeIntegral<uint8_t>() % mimeTypeStrings.size()];
