...
namespace android {
    ...
    ssize_t
    MockCryptoPlugin::decrypt(bool secure, const uint8_t key[DECRYPT_ARRAY_SIZE],
            const uint8_t iv[DECRYPT_ARRAY_SIZE], Mode mode,
            ...
            void *dstPtr, AString * /* errorDetailMsg */)
    {
        ALOGD("MockCryptoPlugin::decrypt(secure=%d, key=%s, iv=%s, mode=%d, "
              ...
              "subSamples=%s, dst=%p)",
              (int)secure,
              arrayToString(key, DECRYPT_ARRAY_SIZE).string(),
              arrayToString(iv, DECRYPT_ARRAY_SIZE).string(),
