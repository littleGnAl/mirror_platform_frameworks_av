...
static std::map<std::string, std::shared_ptr<IRemotelyProvisionedComponent>>
getDrmRemotelyProvisionedComponents(const std::string& in_appPackageName) {
    ...
    AServiceManager_forEachDeclaredInstance(
            ...
            [](const char* instance, void* context) {
                ...
                if (!status.isOk()) {
                    ALOGE("getSupportedCryptoSchemes failed with error code %d",
                ...
                if (!status.isOk()) {
                    ALOGE("createDrmPlugin failed with error code %d",
                          status.getServiceSpecificError());
                    return;
                }

                std::shared_ptr<IRemotelyProvisionedComponent> comp =
                        std::make_shared<DrmRemotelyProvisionedComponent>(mDrm);
                std::string compName = "DrmRemotelyProvisionedComponent_" + std::string(instance);
                static_cast<std::map<std::string, std::shared_ptr<IRemotelyProvisionedComponent>>*>(
                        context)[compName] = comp;
            });
    return comps;
}
}  // namespace android::mediadrm
