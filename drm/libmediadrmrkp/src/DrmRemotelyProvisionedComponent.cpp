...
ScopedAStatus DrmRemotelyProvisionedComponent::generateCertificateRequestV2(
    ...
    std::vector<uint8_t> signedDeviceCsrPayload;
    status = mDrm->getPropertyByteArray("signedDeviceCsrPayload", &signedDeviceCsrPayload);
    if (!status.isOk()) {
        ALOGE("getPropertyByteArray signedDeviceCsrPayload failed. Details: [%s].",
              status.getDescription().c_str());
        return status;
    }
    ...
    *out = cppbor::Array()
                   ...
                   .add(cppbor::EncodedItem(std::move(signedDeviceCsrPayload)))
                   .encode();
    return ScopedAStatus::ok();
}
}  // namespace android::mediadrm