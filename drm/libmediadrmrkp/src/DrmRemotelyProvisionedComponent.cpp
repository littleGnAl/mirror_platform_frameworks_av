...
ScopedAStatus DrmRemotelyProvisionedComponent::getVerifiedDeviceInfo(cppbor::Map& deviceInfoMap) {
    ...
    auto status = mDrm->getPropertyByteArray("deviceInfo", &verifiedDeviceInfo);
    if (!status.isOk()) {
        ALOGE("getPropertyByteArray deviceInfo failed. Details: [%s].",
...
ScopedAStatus DrmRemotelyProvisionedComponent::getDeviceInfoV2(std::vector<uint8_t>* deviceInfo) {
    ...
    std::map<std::string, std::string> keyToProp{{"brand", "ro.product.brand"},
...
ScopedAStatus DrmRemotelyProvisionedComponent::generateCertificateRequestV2(
    ...
    status = getDeviceInfoV2(&deviceInfo);
    if (!status.isOk()) {
        ALOGE("getDeviceInfoV2 failed. Details: [%s].", status.getDescription().c_str());
        return status;
    }
    ...
    // hardware/interfaces/security/rkp/aidl/android/hardware/security/keymint/IRemotelyProvisionedComponent.aidl
    *out = cppbor::Array()
                   .add(1 /* version */)
                   .add(cppbor::Map() /* UdsCerts */)
                   .add(cppbor::EncodedItem(std::move(bcc)))
                   .add(cppbor::EncodedItem(std::move(csr)))
                   .encode();
    return ScopedAStatus::ok();
}
}  // namespace android::mediadrm