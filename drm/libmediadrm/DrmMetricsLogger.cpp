...
DrmStatus DrmMetricsLogger::isCryptoSchemeSupported(const uint8_t uuid[UUID_SIZE], const String8& mimeType,
...
DrmStatus DrmMetricsLogger::createPlugin(const uint8_t uuid[UUID_SIZE], const String8& appPackageName) {
    std::memcpy(mUuid.data(), uuid, UUID_SIZE);
