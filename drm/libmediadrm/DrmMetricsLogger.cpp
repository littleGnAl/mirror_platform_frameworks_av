...
int MediaSecurityLevelToJavaSecurityLevel(status_t securityLevel) {
#define STATUS_CASE(status) \
    case status: \
        return status

    switch (securityLevel) {
        STATUS_CASE(kSecurityLevelUnknown);
        STATUS_CASE(kSecurityLevelSwSecureCrypto);
        STATUS_CASE(kSecurityLevelSwSecureDecode);
        STATUS_CASE(kSecurityLevelHwSecureCrypto);
        STATUS_CASE(kSecurityLevelHwSecureDecode);
        STATUS_CASE(kSecurityLevelHwSecureAll);
        STATUS_CASE(kSecurityLevelMax);
#undef STATUS_CASE
    }
...
void DrmMetricsLogger::reportMediaDrmSessionOpened(const std::vector<uint8_t>& sessionId) const {
    ...
    if (it != mSessionMap.end()) {
        ...
        mediametrics_setInt64(handle, "requested_seucrity_level",
                    MediaSecurityLevelToJavaSecurityLevel(it->second.mTargetSecurityLevel));
        mediametrics_setInt64(handle, "opened_seucrity_level",
                    MediaSecurityLevelToJavaSecurityLevel(it->second.mActualSecurityLevel));
    }
    mediametrics_selfRecord(handle);
    mediametrics_delete(handle);
}
...
void DrmMetricsLogger::reportMediaDrmErrored(const DrmStatus& error_code, const char* api,
    ...
    if (!sessionId.empty()) {
        ...
        if (it != mSessionMap.end()) {
            mediametrics_setCString(handle, "session_nonce", it->second.mNonce.c_str());
            mediametrics_setInt64(handle, "seucrity_level",
                        MediaSecurityLevelToJavaSecurityLevel(it->second.mActualSecurityLevel));
        }
    }
