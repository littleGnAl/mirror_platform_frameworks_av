...
DrmStatus DrmMetricsLogger::openSession(DrmPlugin::SecurityLevel securityLevel,
    ...
    session.sessionNonceMsb = getrandom((void*)msbSesBuffer, 8, GRND_NONBLOCK);
    session.sessionNonceLsb = getrandom((void*)lsbSesBuffer, 8, GRND_NONBLOCK);
    if (checkGetRandom(session.sessionNonceMsb, __func__) == OK &&
        checkGetRandom(session.sessionNonceLsb, __func__) == OK) {
        DrmStatus status = mImpl->openSession(securityLevel, sessionId);
        if (status == OK) {
            std::vector<uint8_t> sessionKey = sessionConversion(sessionId);
            session.targetSecurityLevel = securityLevel;
            DrmPlugin::SecurityLevel actualSecLevel;
            if (DrmMetricsLogger::getSecurityLevel(sessionId, &(actualSecLevel)) != OK) {
                actualSecLevel = DrmPlugin::kSecurityLevelUnknown;
            }
            session.actualSecurityLevel = actualSecLevel;
            mSessionMap.insert({sessionKey, session});
            reportMediaDrmOpenSession(sessionKey);
        } else {
            reportMediaDrmErrored(status, __func__);
        }
        return status;
...
DrmStatus DrmMetricsLogger::closeSession(Vector<uint8_t> const& sessionId) {
    ...
    mSessionMap.erase(sessionConversion(temp));
    DrmStatus status = mImpl->closeSession(sessionId);
    if (status != OK) {
        reportMediaDrmErrored(status, __func__, sessionConversion(sessionId));
    }
    return status;
}
...
void DrmMetricsLogger::reportMediaDrmOpenSession(std::vector<uint8_t> sessionId) const {
    ...
    mediametrics_setInt64(handle, "session_nonce_msb", mSessionMap.at(sessionId).sessionNonceMsb);
    mediametrics_setInt64(handle, "session_nonce_lsb", mSessionMap.at(sessionId).sessionNonceLsb);
    mediametrics_setInt64(handle, "target_seucrity_level",
                          mSessionMap.at(sessionId).targetSecurityLevel);
    mediametrics_setInt64(handle, "actual_seucrity_level",
                          mSessionMap.at(sessionId).actualSecurityLevel);
    mediametrics_setInt32(handle, "frontend", mFrontend);
    mediametrics_selfRecord(handle);
    mediametrics_delete(handle);
}
...
void DrmMetricsLogger::reportMediaDrmErrored(DrmStatus error_code, const char* api,
    ...
    mediametrics_handle_t handle(mediametrics_create("mediadrm_errored"));
    mediametrics_setInt64(handle, "obj_nonce_msb", mObjNonceMsb);
    mediametrics_setInt64(handle, "obj_nonce_lsb", mObjNonceLsb);
    if (!sessionId.empty()) {
        mediametrics_setInt64(handle, "session_nonce_msb",
                              mSessionMap.at(sessionId).sessionNonceMsb);
        mediametrics_setInt64(handle, "session_nonce_lsb",
                              mSessionMap.at(sessionId).sessionNonceLsb);
    }
    mediametrics_setInt64(handle, "uuid_msb", be64toh(mUuid[0]));
    mediametrics_setInt64(handle, "uuid_lsb", be64toh(mUuid[1]));
...
DrmStatus DrmMetricsLogger::checkGetRandom(int64_t nonce, const char* api) {
    ssize_t bytes = getrandom(&nonce, sizeof(nonce), GRND_NONBLOCK);
...
std::vector<uint8_t> DrmMetricsLogger::sessionConversion(Vector<uint8_t> const& sessionId) const {
