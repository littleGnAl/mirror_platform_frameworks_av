...
class DrmMetricsLogger : public IDrm {
  public:
    ...
    void reportMediaDrmOpenSession() const;

    void reportMediaDrmErrored(DrmStatus error_code, int32_t API_Value, bool uuid = false,
                               Vector<uint8_t> sessionId = Vector<uint8_t>()) const;
  ...
  private:
    ...
    int64_t mSesNonceMsb, mSesNonceLsb;
    int64_t msbBuffer[8], lsbBuffer[8];
    int64_t mTargetSecurityLevel;
    int64_t mActualSecurityLevel;
    int64_t mInitCheck;
    std::map<Vector<uint8_t>*, std::vector<int64_t>> mSessionMap;
