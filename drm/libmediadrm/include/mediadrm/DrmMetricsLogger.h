...
struct sessionValues {
    int64_t sessionNonceMsb;
    int64_t sessionNonceLsb;
    int64_t targetSecurityLevel;
    int64_t actualSecurityLevel;
};
...
class DrmMetricsLogger : public IDrm {
  public:
    ...
    void reportMediaDrmOpenSession(std::vector<uint8_t> sessionId) const;
    ...
    std::vector<uint8_t> sessionConversion(Vector<uint8_t> const& sessionId) const;

  private:
    sp<IDrm> mImpl;
    int64_t mUuid[2] = {};
    int64_t mObjNonceMsb, mObjNonceLsb;
    int64_t msbObjBuffer[8], lsbObjBuffer[8];
    std::map<std::vector<uint8_t>, sessionValues> mSessionMap;
