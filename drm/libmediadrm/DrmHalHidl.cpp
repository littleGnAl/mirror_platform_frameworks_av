...
DrmStatus DrmHalHidl::setPropertyByteArray(String8 const& name, Vector<uint8_t> const& value) const {
    Mutex::Autolock autoLock(mLock);
    INIT_CHECK();
...
DrmStatus DrmHalHidl::getMetrics(const sp<IDrmMetricsConsumer>& consumer) {
    ...
    if (mPluginV1_1 != NULL) {
        ...
        return status.isOk() ? DrmStatus(err) : DrmStatus(DEAD_OBJECT);
    }
...
DrmStatus DrmHalHidl::requiresSecureDecoder(const char* mime, DrmPlugin::SecurityLevel securityLevel,
                                           bool* required) const {
    Mutex::Autolock autoLock(mLock);
    if (mPluginV1_4 == NULL) {
        return DrmStatus(false);
