...
DrmStatus DrmHalAidl::closeSession(Vector<uint8_t> const& sessionId) {
    ...
    status_t response = statusAidlToDrmStatus(status);
    if (status.isOk()) {
...
DrmStatus DrmHalAidl::removeKeys(Vector<uint8_t> const& keySetId) {
    ...
    return DrmStatus(statusAidlToDrmStatus(status));
}
...
DrmStatus DrmHalAidl::setPropertyByteArray(String8 const& name, Vector<uint8_t> const& value) const {
    Mutex::Autolock autoLock(mLock);
    INIT_CHECK();
...
DrmStatus DrmHalAidl::requiresSecureDecoder(const char* mime, DrmPlugin::SecurityLevel securityLevel,
                                           bool* required) const {
    Mutex::Autolock autoLock(mLock);
    INIT_CHECK();
    ...
    status_t err = statusAidlToDrmStatus(status);
    if (!status.isOk()) {
        DrmUtils::LOG2BE("requiresSecureDecoder txn failed: %d", status.getServiceSpecificError());
    }
