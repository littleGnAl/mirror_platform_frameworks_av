...
public:
    H2C2ComponentStore(sp<IComponentStore> const& store)
          : mStore{store},
    ...
    virtual std::shared_ptr<C2ParamReflector> getParamReflector() const
        ...
        struct SimpleParamReflector : public C2ParamReflector {
            ...
            SimpleParamReflector(sp<IComponentStore> base)
                : mBase(base) { }
    ...
    virtual std::vector<std::shared_ptr<const C2Component::Traits>>
        ...
        std::vector<std::shared_ptr<C2Component::Traits const>> traits;
        std::string serviceName = getName();
        Return<void> transResult = mStore->listComponents(
                [&traits, &serviceName](Status s,
                    ...
                    for (size_t i = 0; i < t.size(); ++i) {
                        std::shared_ptr<C2Component::Traits> newTraits =
                            std::make_shared<C2Component::Traits>();
                        if (!objcpy(newTraits.get(), t[i])) {
                            LOG(ERROR) << "listComponents -- corrupted output.";
                            traits.resize(i);
                            return;
                        }
                        newTraits->owner = serviceName;
                        traits[i] = newTraits;
                    }
                });
        if (!transResult.isOk()) {
