...
TEST_P(AudioPatchBridgingTest, SingleAudioPatch)
{
    ...
    auto createAudioPatch = [&]() {
        ...
        EXPECT_EQ(android::OK, ret) << "AudioSystem::createAudiopatch failed between source "
    ...
    auto checkAudioPatchEstablished = [&]() {
        if (audioPatchHandle != AUDIO_PATCH_HANDLE_NONE) {
            ...
            EXPECT_EQ(android::OK, Helper::getPatchByHandle(audioPatchHandle, curAudioPatch))
    ...
    auto releaseAudioPatch = [&]() {
        ...
        if (audioPatchHandle != AUDIO_PATCH_HANDLE_NONE) {
            ...
            EXPECT_EQ(android::OK, ret) << "AudioSystem::releaseAudioPatch failed between source "
                                        << expectedSourcePort.ext.device.address << " and sink "
                                        << expectedSinkPort.ext.device.address;
        }
    };
    auto checkAudioPatchRemoved = [&]() {
        ...
        EXPECT_EQ(android::OK, Helper::getPatches(patches, numPatches)) << "Could not list patches";
...
TEST_P(FullDuplexAudioPatchTest, UsingCreateAudioPatch)
{
    ...
    auto connectDevices = [&]() {
        ...
        ASSERT_EQ(android::OK, Helper::findPort(expectedDownlinkSinkPort, downlinkSinkPort))
                << "Could not find downlink sink port: "
                << expectedDownlinkSinkPort.ext.device.address;

        ASSERT_EQ(android::OK, Helper::findPort(expectedUplinkSourcePort, uplinkSourcePort))
    ...
    auto createAudioPatches = [&]() {
        ...
        EXPECT_EQ(android::OK, ret) << "AudioSystem::createAudiopatch for uplink failed";
        ...
        android::status_t ret = AudioSystem::createAudioPatch(&downlinkPatch, &downlinkPatchHandle);
        EXPECT_EQ(android::OK, ret) << "AudioSystem::createAudiopatch for downlink failed";
    };
    auto checkEstablishedAudioPatches = [&]() {
        ...
        if (downlinkPatchHandle != AUDIO_PATCH_HANDLE_NONE &&
            ...
            EXPECT_EQ(android::OK, Helper::getPatchByHandle(uplinkPatchHandle, curUlPatch))
            ...
            EXPECT_EQ(android::OK, Helper::getPatchByHandle(downlinkPatchHandle, curDlPatch))
                    << " Uplink AudioPatch missing to connect Source Port "
                    << expectedUplinkSourcePort.ext.device.address
                    << " to Sink Port " << expectedUplinkSinkPort.ext.device.address;
    ...
    auto releaseAudioPatches = [&]() {
        ...
        if (downlinkPatchHandle != AUDIO_PATCH_HANDLE_NONE) {
            ...
            EXPECT_EQ(android::OK, ret) << "AudioSystem::releaseAudioPatch for uplink failed";
        }
        if (uplinkPatchHandle != AUDIO_PATCH_HANDLE_NONE) {
            ret = AudioSystem::releaseAudioPatch(uplinkPatchHandle);
            EXPECT_EQ(android::OK, ret) << "AudioSystem::releaseAudioPatch for downlink failed";
        }
    };
