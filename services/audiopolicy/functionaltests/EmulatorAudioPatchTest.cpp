...
TEST_P(AudioPatchBridgingTest, SingleAudioPatch)
{
    ...
    EXPECT_TRUE(Helper::checkEstablishedPatch(audioPatchHandle, expectedSourcePort, expectedSinkPort, useSwBridging, AUDIO_STREAM_PATCH));

    releaseAudioPatch();

    // Ensure HW or SW Bridging is disabled
    Helper::checkPatchRemoved(sourcePort, sinkPort, audioPatchHandle);
...
TEST_P(FullDuplexAudioPatchTest, UsingCreateAudioPatch)
{
    ...
    auto checkEstablishedAudioPatches = [&]() {
        ...
        EXPECT_TRUE(Helper::checkEstablishedPatch(downlinkPatchHandle, downlinkSourcePort,
                                                  downlinkSinkPort, useSwBridging,
                                                  AUDIO_STREAM_PATCH));
        EXPECT_TRUE(Helper::checkEstablishedPatch(uplinkPatchHandle, uplinkSourcePort,
    ...
    auto checkAudioPatchesRemoved = [&]() {
        Helper::checkPatchRemoved(uplinkSourcePort, uplinkSinkPort, uplinkPatchHandle);
        Helper::checkPatchRemoved(downlinkSourcePort, downlinkSinkPort, downlinkPatchHandle);
    };
...
TEST_P(AudioModeBridgingTest, UsingAudioMode)
{
    ...
    auto checkReleasedUplinkPatch = [&]() {
        Helper::checkPatchRemoved(uplinkSourcePort, uplinkSinkPort);
    };
    auto checkReleasedDownlinkPatch = [&]() {
        Helper::checkPatchRemoved(downlinkSourcePort, downlinkSinkPort);
    };
