...
using AudioPatchTestParams = std::tuple<const audio_port_v7 /*sourcePort*/,
    const audio_port_v7 /*sinkPort*/, bool /*useSwBridging, if false use HW bridging*/>;
...
TEST_P(AudioPatchBridgingTest, SingleAudioPatch)
{
    ...
    audio_port_v7 expectedSourcePort = std::get<0>(GetParam());
    audio_port_v7 expectedSinkPort = std::get<1>(GetParam());
    ...
    auto releaseAudioPatch = [&](bool alreadyRemoved) {
        // Then, release the patches
        if (audioPatchHandle != AUDIO_PATCH_HANDLE_NONE) {
            ret = AudioSystem::releaseAudioPatch(audioPatchHandle);
            EXPECT_EQ(alreadyRemoved ? BAD_VALUE : OK, ret)
                    << "AudioSystem::releaseAudioPatch failed between source "
                    << expectedSourcePort.ext.device.address << " and sink "
                    << expectedSinkPort.ext.device.address
    ...
    EXPECT_TRUE(checkEstablishedPatch(audioPatchHandle, expectedSourcePort, expectedSinkPort,
                                      useSwBridging, AUDIO_STREAM_PATCH));
    disconnectDevice();
    bool patchAlreadyRemoved = checkPatchRemoved(sourcePort, sinkPort, audioPatchHandle);
    releaseAudioPatch(patchAlreadyRemoved);
}
...
using FullDuplexAudioPatchTestParams =
    std::tuple<const audio_port_v7 /*uplinkSourcePort*/,
        const audio_port_v7 /*uplinkSinkPort*/,
        const audio_port_v7 /*downlinkSourcePort*/,
        const audio_port_v7 /*downSinkPort*/,
...
{
    status_t ret;
    audio_port_v7 expectedUplinkSourcePort = std::get<0>(GetParam());
    audio_port_v7 expectedUplinkSinkPort = std::get<1>(GetParam());

    audio_port_v7 expectedDownlinkSourcePort = std::get<2>(GetParam());
    audio_port_v7 expectedDownlinkSinkPort = std::get<3>(GetParam());
    bool useSwBridging = std::get<4>(GetParam());


    // Register the devices & ensure ports are not available
    audio_port_v7 uplinkSinkPort;
    audio_port_v7 downlinkSourcePort;
    audio_port_v7 downlinkSinkPort {};
    audio_port_v7 uplinkSourcePort {};
    ...
    auto createAudioPatches = [&]() {
        ...
        downlinkPatch.sources[0].sample_rate = downlinkSourcePort.audio_profiles[0].sample_rates[0];
        downlinkPatch.sources[0].channel_mask = downlinkSourcePort.audio_profiles[0].channel_masks[0];
        downlinkPatch.sources[0].format = downlinkSourcePort.audio_profiles[0].format;
    ...
    auto checkAudioPatchesRemoved = [&]() {
        bool ulRemoved = checkPatchRemoved(uplinkSourcePort, uplinkSinkPort, uplinkPatchHandle);
        if (ulRemoved) {
            uplinkPatchHandle = AUDIO_PATCH_HANDLE_NONE;
        }
        bool dlRemoved =
...
static const std::vector<FullDuplexAudioPatchTestParams> gFullDuplexAudioPatchTestParams = {
    {
        ...
        { .role = AUDIO_PORT_ROLE_SINK, .type = AUDIO_PORT_TYPE_DEVICE,
          .ext.device.type = AUDIO_DEVICE_OUT_TELEPHONY_TX, .ext.device.address = "hfp_client_out_sw"},
        { .role = AUDIO_PORT_ROLE_SOURCE, .type = AUDIO_PORT_TYPE_DEVICE,
          .ext.device.type = AUDIO_DEVICE_IN_TELEPHONY_RX, .ext.device.address = "hfp_client_in_sw"},
        { .role = AUDIO_PORT_ROLE_SINK, .type = AUDIO_PORT_TYPE_DEVICE,
