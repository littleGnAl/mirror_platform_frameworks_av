...
using AudioModeBridgingTest = ::testing::TestWithParam<FullDuplexAudioPatchTestParams>;
...
TEST_P(AudioModeBridgingTest, UsingAudioMode)
{
    ...
    auto checkEstablishedUplinkPatch = [&]() {
        ...
        for (uint32_t i = 0; i < numPatches; i++) {
            ...
            if (patch.sinks[0].ext.device.type == uplinkSinkPort.ext.device.type) {
                if (expectUsingSwBridging && patch.num_sources != 2) {
                    continue;
                }
                ...
                EXPECT_EQ(patch.sources[0].type, uplinkSourcePort.type);
                EXPECT_EQ(patch.sources[0].role, uplinkSourcePort.role);
                EXPECT_EQ(patch.sources[0].ext.device.type, uplinkSourcePort.ext.device.type);

                EXPECT_EQ(patch.sinks[0].type, uplinkSinkPort.type);
                EXPECT_EQ(patch.sinks[0].role, uplinkSinkPort.role);

                uplinkPatchFound = true;
            }
        }
        EXPECT_TRUE(uplinkPatchFound) << " Uplink AudioPatch missing to connect Tx Sink Port "
                                      << expectedUplinkSinkPort.ext.device.address;
    };
    ...
    auto checkEstablishedDownlinkPatch = [&]() {
        ...
        for (uint32_t i = 0; i < numPatches; i++) {
            ...
            if (patch.sources[0].ext.device.type == downlinkSourcePort.ext.device.type) {
                if (expectUsingSwBridging && patch.num_sources != 2) {
                    // if DL source port is always available, a patch between declared mix & device
                    // is always created
                    continue;
                }
                ...
                EXPECT_EQ(patch.sources[0].type, downlinkSourcePort.type);
                EXPECT_EQ(patch.sources[0].role, downlinkSourcePort.role);

                EXPECT_EQ(patch.sinks[0].type, downlinkSinkPort.type);
                EXPECT_EQ(patch.sinks[0].role, downlinkSinkPort.role);
                EXPECT_EQ(patch.sinks[0].ext.device.type, downlinkSinkPort.ext.device.type);

                downlinkPatchFound = true;
            }
        }
        EXPECT_TRUE(downlinkPatchFound) << " Downlink AudioPatch missing to connect Rx Source Port "
                                        << expectedDownlinkSourcePort.ext.device.address;
    };
    ...
    auto checkReleasedUplinkPatch = [&]() {
        ...
        for (uint32_t i = 0; i < numPatches; i++) {
            ...
            EXPECT_FALSE((patch.sinks[0].ext.device.type == uplinkSinkPort.ext.device.type)
                    && (patch.sources[0].type == expectedUplinkSourcePort.type));
        }
    };
    auto checkReleasedDownlinkPatch = [&]() {
        ...
        for (uint32_t i = 0; i < numPatches; i++) {
            ...
            EXPECT_FALSE((patch.sinks[0].ext.device.type == downlinkSinkPort.ext.device.type)
                    && (patch.sources[0].type == downlinkSourcePort.type));
        }
    };
    ...
    Helper::disconnectPort(downlinkSinkPort);
    checkReleasedDownlinkPatch();
    checkEstablishedUplinkPatch();

    // Reconnect DL Sink Port
    ASSERT_TRUE(Helper::connectPort(expectedDownlinkSinkPort, downlinkSinkPort));
    checkEstablishedPatches();
