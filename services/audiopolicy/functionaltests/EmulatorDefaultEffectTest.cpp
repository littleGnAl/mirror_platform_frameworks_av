...
static bool isEffectAddedOnTrack(const effect_uuid_t * uuid,
    ...
    android::sp<android::AudioEffect> postProcEffect =
            new android::AudioEffect(NULL, gOpPackageName, uuid,
                                     gDefaultOutputEffectPriority - 1 , NULL, NULL,
                                     audioTrack->getSessionId(), audioTrack->getOutput());

    EXPECT_NE(postProcEffect, nullptr);
    return (postProcEffect != nullptr) &&
            (postProcEffect->initCheck() == android::ALREADY_EXISTS);
}
...
static bool isAgcEffectAddedOnRecord(const std::unique_ptr<AudioRecordTest> &audioRecord) {
    ...
    // NOTE: default output effects are created with priority 0
    //       default input effects are create with priority -1
    android::sp<android::AudioEffect> preProcEffect =
            new android::AudioEffect(FX_IID_AGC, gOpPackageName, NULL,
                                     gDefaultInputEffectPriority - 1, NULL, NULL,
                                     audioRecord->getSessionId(), audioRecord->getInput());

    EXPECT_NE(preProcEffect, nullptr);
    return (preProcEffect != nullptr) &&
            (preProcEffect->initCheck() == android::ALREADY_EXISTS);
}
...
static bool isAgcEffectDefaultOnRecord(const std::unique_ptr<AudioRecordTest> &audioRecord) {
    ...
    effect_descriptor_t *descriptors =
            new effect_descriptor_t[android::AudioEffect::kMaxPreProcessing];
    uint32_t numEffects = android::AudioEffect::kMaxPreProcessing;
    android::status_t ret = android::AudioEffect::queryDefaultPreProcessing(
...
static bool checkVolumeEffectParameters(const std::unique_ptr<AudioTrackTest> &audioTrack) {
    android::sp<android::AudioEffect> postProcEffect =
            new android::AudioEffect(FX_IID_VOLUME, gOpPackageName, NULL, 10, NULL, NULL,
                                     audioTrack->getSessionId(), audioTrack->getOutput());
    android::status_t ret;
    EXPECT_NE(postProcEffect, nullptr);
    if (postProcEffect != nullptr) {
        ret = postProcEffect->initCheck();
        EXPECT_EQ(android::OK, ret) << "Failed to initcheck postproc effect ret=" << ret;
    }
    if (postProcEffect == nullptr || ret != android::OK) {
        return false;
    }
    int errorCount = 0;
    ...
    for(int16_t volume = -8000; volume < -1000; volume+=1000) {
        ...
        EXPECT_EQ(android::OK, ret) << "Failed to set param on postproc effect ret=" << ret;
        errorCount += (ret != android::OK) ? 1 : 0;
        int16_t readVolume;
        ret = Helper::getEffectParameter(postProcEffect, VOLUME_PARAM_LEVEL,
                                         VOLUME_PARAM_SIZE_MAX, &readVolume, sizeof(volume));
        EXPECT_EQ(android::OK, ret) << "Failed to get param on postproc effect ret=" << ret;
        errorCount += (ret != android::OK) ? 1 : 0;
...
static bool checkBBEffectParameters(const std::unique_ptr<AudioTrackTest> &audioTrack) {
    ...
    android::sp<android::AudioEffect> postProcEffect =
            new android::AudioEffect(SL_IID_BASSBOOST, gOpPackageName, NXP_SL_IID_BASSBOOST,
                                     gDefaultOutputEffectPriority + 1, NULL, NULL,
                                     audioTrack->getSessionId(), audioTrack->getOutput());
    android::status_t ret;
    EXPECT_NE(postProcEffect, nullptr);
...
static bool checkAgcEffectParameters(const std::unique_ptr<AudioRecordTest> &audioRecord) {
    ...
    if (preProcEffect != nullptr) {
        ...
        EXPECT_EQ(android::OK, ret) << "Failed to initcheck preProcEffect effect ret=" << ret;
    }
    if (preProcEffect == nullptr || ret != android::OK) {
        return false;
    }
    int errorCount = 0;
...
TEST(DefaultEffectTest, UsingDefaultEffectApiForPlayback)
{
    ...
    android::AudioEffect::guidToString(NXP_SL_IID_BASSBOOST, bassboostUuidStr,
                                       EFFECT_STRING_LEN_MAX);
    android::AudioEffect::guidToString(SL_IID_BASSBOOST, bassboostTypeStr, EFFECT_STRING_LEN_MAX);

    audio_port sinkPort {};
    audio_port sinkPortForceUseSpeaker {};

    ret = AudioSystem::setForceUse(AUDIO_POLICY_FORCE_FOR_MEDIA, AUDIO_POLICY_FORCE_NO_BT_A2DP);
    EXPECT_EQ(android::OK, ret) << "setForceUse failed";

    auto checkDevices = [&]() {
        ASSERT_EQ(android::OK, Helper::findPort(expectedSinkPort, sinkPort))
        ...
        ASSERT_EQ(android::OK, ret) << "Could not find port :"
    ...
    ASSERT_EQ(android::OK, audioTrack->createTrack())
    ...
    ret = android::AudioEffect::removeStreamDefaultEffect(bassboostDefaultId);
    EXPECT_EQ(android::OK, ret) << "removeStreamDefaultEffect for Media usage failed";
...
TEST_P(UsingAudioEffectXmlForPlayback, UsingAudioEffectXmlForPlayback)
{
    ...
    if (attributes.usage == AUDIO_USAGE_VIRTUAL_SOURCE) {
        ...
        audio_port sinkPort {};
        ASSERT_EQ(android::OK, Helper::findPort(speakerSinkPort, sinkPort))
    ...
    EXPECT_EQ(isEffectAddedOnTrack(ANDROID_FX_IID_VOLUME, audioTrack), expectedByDefault)
            << " effect unexpected for " << android::toString(attributes);
...
TEST_P(UsingAudioEffectXmlForRecord, UsingAudioEffectXmlForRecord)
{
    ...
    android::AudioEffect::guidToString(ANDROID_FX_IID_AGC, agcUuidStr, EFFECT_STRING_LEN_MAX);
    android::AudioEffect::guidToString(FX_IID_AGC, agcTypeStr, EFFECT_STRING_LEN_MAX);
    ...
    android::status_t ret = audioRecord->record(captureRoutedPortId);
    ASSERT_EQ(ret, android::OK) << "failed to start recording";
...
TEST(DefaultEffectTest, UsingDefaultEffectApiForRecord)
{
    ...
    ret = android::AudioEffect::addSourceDefaultEffect(
                agcTypeStr, gOpPackageName, agcUuidStr, gDefaultInputEffectPriority,
                AUDIO_SOURCE_CAMCORDER, &agcDefaultId);
    EXPECT_EQ(android::OK, ret) << "addSourceDefaultEffect for AUDIO_SOURCE_CAMCORDER failed";
    ...
    // Remove default effect
    ret = android::AudioEffect::removeSourceDefaultEffect(agcDefaultId);
    EXPECT_EQ(android::OK, ret) << "removeSourceDefaultEffect for AUDIO_SOURCE_CAMCORDER failed";
