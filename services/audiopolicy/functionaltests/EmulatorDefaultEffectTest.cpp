...
static constexpr const char* gOpPackageName = "AudioSourceEffectTest";
...
static bool isEffectAddedOnTrack(const effect_uuid_t * uuid,
    ...
    attributionSource.packageName = std::string(gOpPackageName);
    attributionSource.token = sp<BBinder>::make();

    sp<AudioEffect> postProcEffect = new AudioEffect(attributionSource);
...
static bool isAgcEffectAddedOnRecord(const std::unique_ptr<AudioRecordTest> &audioRecord) {
    ...
    sp<AudioEffect> preProcEffect = new AudioEffect(attributionSource);
...
TEST(DefaultEffectTest, UsingDefaultEffectApiForPlayback)
{
    ...
    audio_port_v7 expectedSinkPort { .role = AUDIO_PORT_ROLE_SINK, .type = AUDIO_PORT_TYPE_DEVICE,
                .ext.device.type = AUDIO_DEVICE_OUT_BUS, .ext.device.address = "BUS00_MEDIA"};
    audio_port_v7 forceUseSpeakerExpectedSinkPort { .role = AUDIO_PORT_ROLE_SINK,
    ...
    audio_port_v7 sinkPortForceUseSpeaker {};
    ...
    ret = AudioEffect::addStreamDefaultEffect(
                bassboostTypeStr, String16(gOpPackageName), bassboostUuidStr,
                gDefaultOutputEffectPriority, AUDIO_USAGE_MEDIA, &bassboostDefaultId);
...
TEST_P(UsingAudioEffectXmlForPlayback, UsingAudioEffectXmlForPlayback)
{
    ...
    if (attributes.usage == AUDIO_USAGE_VIRTUAL_SOURCE) {
        audioTrack.reset();
        audio_port_v7 speakerSinkPort { .role = AUDIO_PORT_ROLE_SINK,
                    .type = AUDIO_PORT_TYPE_DEVICE,
                    .ext.device.type = AUDIO_DEVICE_OUT_SPEAKER, .ext.device.address = "SPEAKER"};
...
TEST(DefaultEffectTest, UsingDefaultEffectApiForRecord)
{
    ...
    ret = AudioEffect::addSourceDefaultEffect(
                agcTypeStr, String16(gOpPackageName), agcUuidStr, gDefaultInputEffectPriority,
