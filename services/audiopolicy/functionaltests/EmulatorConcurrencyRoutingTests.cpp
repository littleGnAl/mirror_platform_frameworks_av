 ...
 *
 *  <route type="mix" sink="bus4_usage_call"
 *          sources="mixport_bus4_usage_call_output,mixport_bus7_usage_system_sound_output"/>
 *
 *  It means Output profile for system will also accept to be routed on bus4_usage_call
 * Notification Output profile will not, so the device assigned will remain the same
 *
 ...
 *
 * AudioTrack3 (stream type notification) does not support the sink bus4_usage_call, so will not
...
TEST(RoutingConcurrencyTest, RoutingConcurrency)
{
    ...
    std::string address1 = "BUS04_CALL";
    ...
    std::string address2 = "BUS07_SYSTEM_SOUND";
    android::product_strategy_t strategy2 = android::PRODUCT_STRATEGY_NONE;
    audio_port expectedRoutedPort2;

    audio_stream_type_t stream3 = AUDIO_STREAM_NOTIFICATION;
    std::string name3;
    audio_devices_t type3 = AUDIO_DEVICE_OUT_BUS;
    std::string address3 = "BUS06_NOTIFICATION";
    android::product_strategy_t strategy3 = android::PRODUCT_STRATEGY_NONE;
    ...
    if (!name3.empty()) {
        strategy3 = Helper::getStrategyByName(name3);
        ASSERT_NE(android::PRODUCT_STRATEGY_NONE, strategy3) << "Invalid strategy " << name2;
    }
    Helper::getPort(name3, type3, address3, expectedRoutedPort3);
    ...
    Helper::launchPlayer(audioTrack1, strategy1, stream1, AUDIO_PORT_HANDLE_NONE,
    ...
    // Launch track3  -> shall not be routed as not supporting BUS04_CALL device
    std::unique_ptr<AudioTrackTest> audioTrack3;
    Helper::launchPlayer(audioTrack3, strategy3, stream3, AUDIO_PORT_HANDLE_NONE,
                         expectedRoutedPort3.id, false /*shallBeRouted*/);
