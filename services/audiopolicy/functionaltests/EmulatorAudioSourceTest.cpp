...
TEST_P(AudioSourceBridgingTest, UsingAudioSourceAPI)
{
    ...
    Helper::checkEstablishedPatch(sourcePort, sinkPort, expectUsingSwBridging, expectedStreamType);
    ...
    Helper::checkPatchRemoved(sourcePort, sinkPort);
    ...
    Helper::disconnectPort(sinkPort);
    Helper::checkPatchRemoved(sourcePort, sinkPort);

    // Re-Connect the sinkport
    Helper::connectPort(expectedSinkPort, sinkPort);
    Helper::checkEstablishedPatch(sourcePort, sinkPort, expectUsingSwBridging, expectedStreamType);
...
TEST_P(AudioSourceForceUseTogglingTest, SetForceUseToggling)
{
    ...
    auto connectDevice = [&]() {
        ...
        ASSERT_EQ(OK, Helper::findPort(forceUseExpectedSinkPort, sinkPortForcedConfig))
...
gAudioSourceForceUseTogglingTestParams = {
    { {.content_type = AUDIO_CONTENT_TYPE_MUSIC, .usage = AUDIO_USAGE_ASSISTANCE_NAVIGATION_GUIDANCE,
       .tags = "oem=ta"},
      { .role = AUDIO_PORT_ROLE_SOURCE, .type = AUDIO_PORT_TYPE_DEVICE,
        .ext.device.type = AUDIO_DEVICE_IN_FM_TUNER, .ext.device.address = "FM_SW"},
      { .role = AUDIO_PORT_ROLE_SINK, .type = AUDIO_PORT_TYPE_DEVICE,
