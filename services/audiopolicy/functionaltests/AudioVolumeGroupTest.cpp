...
public:
    void SetUp() override
    {
        ...
        ret = android::AudioSystem::listAudioProductStrategies(mStrategies);
        ASSERT_EQ(ret, NO_ERROR) << "AudioSystem::listAudioProductStrategies failed with error: "
                                 << ret;

        ret = android::AudioSystem::listAudioVolumeGroups(mVolumeGroups);
        ASSERT_EQ(ret, NO_ERROR) << "AudioSystem::listAudioProductStrategies failed with error: "
                                 << ret;
    ...
    android::AudioProductStrategyVector mStrategies;
    android::AudioVolumeGroupVector mVolumeGroups;
...
TEST(StreamTypeVolumeTest, VolumeSetByStreamTypeOrAttributes)
{
    ...
    for (audio_stream_type_t stream = AUDIO_STREAM_MIN; stream < AUDIO_STREAM_PUBLIC_CNT;
        ...
        android::product_strategy_t strategyId =
                (android::product_strategy_t)AudioSystem::getStrategyForStream(stream);
        if (strategyId == android::PRODUCT_STRATEGY_NONE) {
            ...
            android::AudioProductStrategy *strategy = nullptr;
            ...
            ASSERT_EQ(ret, android::NO_ERROR) << "Failed to retrieve strategies";
            for (android::AudioProductStrategy &ps : strategies) {
...
using StrategyPlaybackVolumeTestParams =
    std::tuple<const std::string /*strategyName*/,
               const android::AudioProductStrategy /*strategy*/>;
...
TEST_P(AudioProductStrategiesPlaybackVolumeTest, PlaybackVolumeSetByAttributes)
{
    ...
    auto attribute = strategy.getAudioAttributes().front().getAttributes();
    ASSERT_TRUE(attribute != android::defaultAttr) << "Strategy " << name << " has no valid attributes";
    auto group = strategy.getAudioAttributes().front().getGroupId();

    audio_devices_t device = AUDIO_DEVICE_OUT_SPEAKER;
    int indexRead = 0;
    android::StreamTypeVector volumeStreams;
    std::vector<audio_attributes_t> volumeAttributes;

    if (group == android::VOLUME_GROUP_NONE) {
    ...
    for (audio_stream_type_t stream = AUDIO_STREAM_MIN; stream < AUDIO_STREAM_PUBLIC_CNT;
        ...
        auto attributes = android::AudioSystem::streamTypeToAttributes(stream);
        android::volume_group_t groupId;
        EXPECT_EQ(AudioSystem::getVolumeGroupFromAudioAttributes(attributes, groupId), NO_ERROR);

        std::cout << "Stream " << android::toString(stream) << " attributes "
                  << android::toString(attributes) << " groupdId " << groupId << std::endl;
    ...
    for (auto &volumeAttribute : volumeAttributes) {
        ...
        EXPECT_EQ(index, indexRead)
                ...
                <<  Helper::dumpProductStrategy(strategy.getId(), true)
                << " and attributes " << android::toString(volumeAttribute)
    ...
    for (audio_stream_type_t stream = AUDIO_STREAM_MIN; stream < AUDIO_STREAM_PUBLIC_CNT;
        ...
        if (shallVolumeIndexMatch[stream]) {
            EXPECT_EQ(index, indexRead)
                    ...
                    <<  Helper::dumpProductStrategy(strategy.getId(), true)
                     << " and stream " <<  android::toString(stream)
                     << ", expected " << index << ", got " << indexRead;
        } else {
    ...
    for (const auto &stream : volumeStreams) {
        ...
        EXPECT_EQ(ret, NO_ERROR) << "setStreamVolumeIndex failed for "
                                 <<  Helper::dumpProductStrategy(strategy.getId(), true)
                                 << " with stream " << android::toString(stream);
        ...
        for (const auto &groupStream : volumeStreams) {
            ...
            EXPECT_EQ(ret, NO_ERROR) << "getStreamVolumeIndex failed for "
                                     <<  Helper::dumpProductStrategy(strategy.getId(), true)
                                     << " with stream " << android::toString(groupStream);
            EXPECT_EQ(index, indexRead) << "getStreamVolumeIndex wrong value for "
                                        <<  Helper::dumpProductStrategy(strategy.getId(), true)
                                        << " and stream " << android::toString(groupStream)
                                        << " expected " << index << ", got " << indexRead;
        }
        ...
        for (const auto &volumeAttribute : volumeAttributes) {
            ...
            EXPECT_EQ(index, indexRead)
                    ...
                    <<  Helper::dumpProductStrategy(strategy.getId(), true)
                    << " while setting by stream " << android::toString(stream)
                    << " expected " << index << ", got " << indexRead;
        }
    }
...
const std::vector<StrategyPlaybackVolumeTestParams> getStrategyPlaybackVolumeTestParams()
{
    ...
    android::AudioProductStrategyVector strategies;
    android::AudioSystem::listAudioProductStrategies(strategies);
