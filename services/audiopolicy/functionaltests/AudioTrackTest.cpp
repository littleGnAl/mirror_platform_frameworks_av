...
android::status_t AudioTrackTest::playSine(audio_port_handle_t &routedPortId/*, int playTimeMs*/)
{
    android::status_t status = mAudioTrack->start();
...
android::status_t AudioTrackTest::stop()
{
    if (mAudioTrack == nullptr) {
...
android::status_t AudioTrackTest::createTrack()
{
    ...
    if (mProductStrategy != android::PRODUCT_STRATEGY_NONE) {
        android::AudioProductStrategyVector strategies;
        android::status_t ret = AudioSystem::listAudioProductStrategies(strategies);
        ...
        for (const auto &strategy : strategies) {
            if (strategy.getId() == mProductStrategy) {
                ...
                std::cout << "createTrack for strategy: " << strategy.getName()
                          << " with stream= " << android::toString(mStream)
                          << ", attributes= " << android::toString(mAudioAttributes) << std::endl;

                attributes =
                        (mAudioAttributes == android::defaultAttr) ? nullptr : &mAudioAttributes;
                ...
                if (attributes == nullptr && (uint32_t(mStream) >= AUDIO_STREAM_PUBLIC_CNT)) {
                    ...
                    std::cerr << "Strategy " << strategy.getName() << " has invalid attributes "
                              << "and non-public stream " << android::toString(mStream)
                              << std::endl;
        ...
        if (attributes == nullptr) {
            ...
            *attributes = android::AudioSystem::streamTypeToAttributes(mStream);
            std::cerr << "translating stream " << android::toString(mStream)
                      << " to attributes " << android::toString(*attributes) << std::endl;
        }
        if (mStream == AUDIO_STREAM_DEFAULT) {
            mStream = android::AudioSystem::attributesToStreamType(*attributes);
        }
    }
...
android::status_t AudioTrackTest::setVolume(float volume)
{
    return mAudioTrack == nullptr ? android::NO_INIT : mAudioTrack->setVolume(volume);
}
