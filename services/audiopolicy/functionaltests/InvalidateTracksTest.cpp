...
public:
    audio_port_v7 mUplinkSinkPort;
    audio_port_v7 mDownlinkSourcePort;
    audio_port_v7 mDownlinkSinkPort {};
    audio_port_v7 mUplinkSourcePort {};

    audio_port_v7 mExpectedUplinkSourcePort = {
    ...
    audio_port_v7 mExpectedUplinkSinkPort = {
    ...
    audio_port_v7 mExpectedDownlinkSourcePort = {
        .role = AUDIO_PORT_ROLE_SOURCE, .type = AUDIO_PORT_TYPE_DEVICE,
        .ext.device.type = AUDIO_DEVICE_IN_TELEPHONY_RX,
        .ext.device.address = "hfp_client_in_hw" };
    audio_port_v7 mExpectedDownlinkSinkPort = {
        .role = AUDIO_PORT_ROLE_SINK, .type = AUDIO_PORT_TYPE_DEVICE,
...
TEST_P(SetPhoneStateInvalidateTrackTest, SetPhoneStateInvalidateTrack)
{
    ...
    // Check routed port
    audio_port_v7 routedDevicePort;
    playbackRoutedPortId = audioTrack->getRoutedDeviceId();
    status_t ret = getPortById(playbackRoutedPortId, routedDevicePort);
    if (playbackRoutedPortId != AUDIO_PORT_HANDLE_NONE) {
        EXPECT_EQ(NO_ERROR, ret)
                << "Failed to identify port for playback by attributese: "
                <<  android::toString(attributes)
        ...
        // Check Patch
        playbackRoutedPortId = checkPatch(audioTrack->getOutput());
        EXPECT_TRUE(playbackRoutedPortId != AUDIO_PORT_HANDLE_NONE)
                << "No patch found involving mix port " << audioTrack->getOutput()
                << " and device port " << playbackRoutedPortId;
    }
...
TEST_P(StartAudioSourceInvalidateTracksTest, SinglePlayerSingleAudioSource)
{
    ...
    const audio_port_v7 expectedSourcePort = std::get<3>(GetParam());
