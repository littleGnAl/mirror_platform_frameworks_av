...
{
    return AudioSystem::getDeviceConnectionState(port.ext.device.type, port.ext.device.address) ==
...
std::string Helper::getPortInfo(audio_port_handle_t portId)
{
    ...
    status_t ret = AudioSystem::listAudioPorts(AUDIO_PORT_ROLE_NONE, AUDIO_PORT_TYPE_NONE,
...
/*static*/
void Helper::checkPatchRemoved(const audio_port &sourcePort, const audio_port &sinkPort, const audio_patch_handle_t &audioPatchHandle) {
    struct audio_patch *patches = nullptr;
    unsigned int numPatches = 0;
    EXPECT_EQ(OK, getPatches(patches, numPatches)) << "Could not list patches";
    for (uint32_t i = 0; i < numPatches; i++) {
