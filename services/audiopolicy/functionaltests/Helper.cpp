...
/*static*/
std::vector<audio_port_v7> Helper::getAvailablePorts()
{
    uint32_t numPorts = 0;
    uint32_t generation;
    ...
    struct audio_port_v7 nPorts[numPorts];
    ...
    std::vector<audio_port_v7> portVec;
...
bool Helper::isPortConnected(const audio_port_v7 &port)
{
...
bool Helper::connectPort(const audio_port_v7 &portToConnect, audio_port_v7 &connectedPort)
{
...
bool Helper::disconnectPort(const audio_port_v7 &portToDisconnect)
{
    audio_port_v7 disconnectedPort;
...
/*static*/
std::string Helper::dumpPort(const audio_port_v7 &port)
{
    std::ostringstream result;
    std::string deviceInfo;
    if (port.type == AUDIO_PORT_TYPE_DEVICE) {
...
/*static*/
audio_port_handle_t Helper::checkPatch(audio_io_handle_t mixHandle)
{
    struct audio_patch patch;
    if (getPatchForOutputMix(mixHandle, patch) == OK) {
        for( unsigned int i = 0; i < patch.num_sinks; i++ ) {
            if (patch.sinks[i].type == AUDIO_PORT_TYPE_DEVICE) {
                return patch.sinks[i].id;
            }
        }
...
/*static*/
void Helper::checkEstablishedPatch(const audio_patch &patch, const audio_port_v7 &sinkPort,
                                   const audio_port_v7 &sourcePort,
...
bool Helper::checkEstablishedPatch(const audio_patch_handle_t &audioPatchHandle,
                                   const audio_port_v7 &sourcePort, const audio_port_v7 &sinkPort,
                                   bool useSwBridge, audio_stream_type_t streamType) {
...
/*static*/
void Helper::checkEstablishedPatch(const audio_port_v7 &sourcePort, const audio_port_v7 &sinkPort,
                                   bool useSwBridge, audio_stream_type_t streamType) {
    struct audio_patch *patches = nullptr;
...
/*static*/
bool Helper::checkPatchRemoved(const audio_port_v7 &sourcePort, const audio_port_v7 &sinkPort,
    ...
    for (uint32_t i = 0; i < numPatches; i++) {
        ...
        if (audioPatchHandle != AUDIO_PATCH_HANDLE_NONE) {
            ...
            if (audioPatchHandle == patch.id) {
                return false;
            }
        }
        if (patch.sources[0].type == AUDIO_PORT_TYPE_DEVICE) {
            bool portsStillConnected =
                    (patch.sources[0].ext.device.type == sourcePort.ext.device.type) &&
                    ...
                    (strncmp(patch.sinks[0].ext.device.address,
                     sinkPort.ext.device.address, AUDIO_DEVICE_MAX_ADDRESS_LEN) == 0);
            EXPECT_FALSE(portsStillConnected);
            if (portsStillConnected) {
                return false;
            }
        }
    }
...
/*static*/
status_t Helper::findPort(const audio_port_v7 &expectedPort, audio_port_v7 &foundPort)
{
...
void Helper::getPort(audio_devices_t type, const std::string &address, audio_port_v7 &port) {
    audio_port_v7 expectedPort {};
...
void Helper::playbackOnExplicitDevice(const audio_port_v7 &explicitDevicePort,
...
void Helper::captureFromExplicitDevice(const audio_port_v7 &explicitDevicePort,
                                       audio_port_handle_t &routedPort)
{
