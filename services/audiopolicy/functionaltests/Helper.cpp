...
{
    android::status_t ret = AudioSystem::setMode(mode);
...
{
    android::status_t ret = android::AudioSystem::setDeviceConnectionState(
                          ...
                          audio_port_handle_t expectedRoutingPortId)
{
    ...
    ASSERT_EQ(android::OK, audioTrack->createTrack()) << "Failed to create AudioTrack for "
            << (useExplicitRouting ? dumpPort(explicitRoutingPortId) : android::toString(stream));

    audio_port_handle_t playbackRoutedPortId = AUDIO_PORT_HANDLE_NONE;
    ASSERT_EQ(android::OK, audioTrack->playSine(playbackRoutedPortId))
    ...
    EXPECT_TRUE(Helper::checkPatch(audioTrack->getOutput(), expectedRoutingPortId))
            << "No patch found involving mix port " << audioTrack->getOutput()
            << " and device port " << expectedRoutingPortId;
}
...
std::string Helper::getPortInfo(audio_port_handle_t portId)
{
    auto dumpPort = [](const auto& port) {
       ...
       if (port.type == AUDIO_PORT_TYPE_DEVICE) {
           ...
           if (port.ext.device.type & AUDIO_DEVICE_BIT_IN) {
               android::InputDeviceConverter::maskToString(port.ext.device.type, typeName);
           } else {
               android::OutputDeviceConverter::maskToString(port.ext.device.type, typeName);
           }
    ...
    android::status_t ret = android::AudioSystem::listAudioPorts(AUDIO_PORT_ROLE_NONE,
    ...
    ret = android::AudioSystem::listAudioPorts(AUDIO_PORT_ROLE_NONE, AUDIO_PORT_TYPE_NONE,
...
/*static*/
android::status_t Helper::getPortById(const audio_port_handle_t& portId,
...
std::string Helper::dumpPortConfig(const audio_port_config &port)
{
    ...
    if (port.type == AUDIO_PORT_TYPE_DEVICE) {
        if (port.ext.device.type & AUDIO_DEVICE_BIT_IN) {
            android::InputDeviceConverter::maskToString(port.ext.device.type, deviceInfo);
        } else {
            android::OutputDeviceConverter::maskToString(port.ext.device.type, deviceInfo);
        }
...
{
    audio_port port;
    if (getPortById(portHandle, port) != android::OK) {
        return {};
    }
    std::ostringstream result;
    std::string deviceInfo;
...
/*static*/
android::status_t Helper::getPatchByHandle(audio_patch_handle_t handle, audio_patch &patch)
{
...
/*static*/
android::status_t Helper::getPatches(struct audio_patch *&patches, unsigned int &numPatches)
{
    ...
    do {
        ...
        android::status_t ret = AudioSystem::listAudioPatches(&numPatches, nullptr, &sizeGeneration);
        if (ret != NO_ERROR) {
...
bool Helper::checkPatch(audio_io_handle_t mixHandle, audio_port_handle_t portId)
{
    ...
    if (getPatchForOutputMix(mixHandle, patch) == android::OK) {
        return patchInvolvesSinkDevicePort(patch, portId);
    } else if (getPatchForInputMix(mixHandle, patch) == android::OK) {
...
bool Helper::checkPatch(audio_io_handle_t mixHandle, audio_devices_t deviceType)
{
    ...
    if (audio_is_output_devices(deviceType)) {
        return (getPatchForOutputMix(mixHandle, patch) == android::OK) &&
                patchInvolvesDeviceTypes(patch, deviceType);
    }
    return (getPatchForInputMix(mixHandle, patch) == android::OK) &&
            patchInvolvesDeviceTypes(patch, deviceType);
}

/*static*/
android::status_t Helper::getPatchForOutputMix(audio_io_handle_t mixHandle, audio_patch &patch)
{
    uint32_t numPatches = 0;
    struct audio_patch *patches = nullptr;
    status_t ret = getPatches(patches, numPatches);
...
/*static*/
android::status_t Helper::getPatchForInputMix(audio_io_handle_t mixHandle, audio_patch &patch)
{
    uint32_t numPatches = 0;
    struct audio_patch *patches = nullptr;
    status_t ret = getPatches(patches, numPatches);
...
std::string Helper::toString(audio_devices_t type)
{
    ...
    if (audio_is_output_device(type)) {
        if (not android::OutputDeviceConverter::toString(type, deviceLiteral)) {
            ALOGE("failed to convert output device: %d", type);
            return {"AUDIO_DEVICE_NONE"};
        }
    } else if (audio_is_input_device(type)) {
        if (not android::InputDeviceConverter::toString(type, deviceLiteral)) {
...
/*static*/
android::status_t Helper::findPort(const audio_port &expectedPort, audio_port &foundPort)
{
...
/*static*/
android::status_t Helper::findPort(audio_port_role_t role, audio_port_type_t type,
...
void Helper::getPort(const std::string &/*name*/, audio_devices_t type,
                     const std::string &address, audio_port &port) {
    audio_port expectedPort {};
    expectedPort.role = AUDIO_PORT_ROLE_SINK;
    expectedPort.type = AUDIO_PORT_TYPE_DEVICE;
    expectedPort.ext.device.type = type;
    strncpy(expectedPort.ext.device.address, address.c_str(), AUDIO_DEVICE_MAX_ADDRESS_LEN);
    android::status_t ret = findPort(expectedPort.role, expectedPort.type,
                                     expectedPort.ext.device.type,
                                     expectedPort.ext.device.address,
                                     port);

    ASSERT_EQ(ret, android::NO_ERROR) << "Could not find port for device type "
...
void Helper::waitEndOfActiveStreams()
{
    ...
    for (audio_stream_type_t stream = AUDIO_STREAM_MIN; stream < AUDIO_STREAM_PUBLIC_CNT;
        ...
        bool isActive = false;
        android::AudioSystem::isStreamActive(stream,
                                             &isActive,
...
android::product_strategy_t Helper::getStrategyByName(const std::string &name)
{
    android::AudioProductStrategyVector strategies;
    android::status_t ret = android::AudioSystem::listAudioProductStrategies(strategies);
    if (ret != android::NO_ERROR) {
        return android::PRODUCT_STRATEGY_NONE;
    }
...
/*static*/
std::string Helper::getStrategyInfo(android::product_strategy_t psId)
{
    std::string result;
    android::AudioProductStrategyVector strategies;
    android::status_t ret = android::AudioSystem::listAudioProductStrategies(strategies);
...
void Helper::launchPlayer(std::unique_ptr<AudioTrackTest> &audioTrack,
                          ...
                          bool shallBeRouted)
{
    bool useProductStrategy = strategy != android::PRODUCT_STRATEGY_NONE;
    bool useExplicitRouting = explicitRoutingPortId != AUDIO_PORT_HANDLE_NONE;
    ...
    EXPECT_TRUE(audioTrack->waitForDeviceCb(expectedRoutingPortId)) << "Timeout for Device cb";
...
//static
std::string Helper::dumpProductStrategy(android::product_strategy_t psId, bool oneline)
{
    std::ostringstream result;
    android::AudioProductStrategyVector strategies;
    android::status_t ret = android::AudioSystem::listAudioProductStrategies(strategies);
    ...
    for (const auto &strategy : strategies) {
        if (strategy.getId() == psId) {
            ...
            for (const auto & attributes : strategy.getAudioAttributes()) {
                ...
                if (!android::AudioContentTypeConverter::toString(aa.content_type,
                                                                  contentTypeLiteral)) {
                ...
                if (!android::UsageTypeConverter::toString(aa.usage, usageLiteral)) {
                    ALOGE("failed to convert usage : %d", aa.usage);
                    return {};
                }

                std::string flagsLiteral;
                android::AudioFlagConverter::maskToString(aa.flags, flagsLiteral);
            ...
            for (const auto & attributes : strategy.getAudioAttributes()) {
                ...
                if (!android::StreamTypeConverter::toString(stream, streamTypeLiteral)) {
...
//static
android::status_t Helper::getAudioVolumeGroups(android::AudioVolumeGroupVector &groups)
{
    android::status_t ret = android::AudioSystem::listAudioVolumeGroups(groups);
    if (ret != android::NO_ERROR) {
...
std::string Helper::dumpVolumeGroups()
{
    ...
    android::AudioVolumeGroupVector groups;
    android::status_t ret = getAudioVolumeGroups(groups);
    if (ret != android::NO_ERROR) {
        return {};
    }
...
//static
android::StreamTypeVector Helper::getVolumeGroupsStreams(android::volume_group_t groupId)
{
    android::AudioVolumeGroupVector groups;
    android::status_t ret = getAudioVolumeGroups(groups);
    if (ret != android::NO_ERROR) {
...
//static
std::vector<audio_attributes_t> Helper::getVolumeGroupsAttributes(android::volume_group_t groupId)
{
    android::AudioVolumeGroupVector groups;
    android::status_t ret = getAudioVolumeGroups(groups);
    if (ret != android::NO_ERROR) {
...
android::status_t Helper::setEffectParameter(const android::sp<android::AudioEffect>& effect,
                                             ...
                                             void *pValue, uint32_t valueSize)
{
    ...
    *(int32_t *)p->data = param;
    ...
    EXPECT_EQ(android::OK, status) << "Failed to set param status=" << status;
    if (android::NO_ERROR == status) {
        EXPECT_EQ(android::OK, status) << "Failed to set param p->status=" << status;
        status = p->status;
    }
    return status;
}

android::status_t Helper::getEffectParameter(const android::sp<android::AudioEffect>& effect,
                                             ...
                                             uint32_t valueSize)
{
    ...
    *(int32_t *)p->data = param;
    ...
    EXPECT_EQ(android::OK, status) << "Failed to get param status=" << status;
    if (android::NO_ERROR == status) {
        status = p->status;
        EXPECT_EQ(android::OK, status) << "Failed to get param p->status=" << status;
        if (android::NO_ERROR == status) {
                                       ...
                                       audio_port_handle_t &routedPort)
{
    ...
    ASSERT_EQ(android::OK, audioRecord->record(routedPort))
...
bool Helper::isPublicStrategy(const android::AudioProductStrategy &strategy)
{
    bool isPublicStrategy = true;
    for (const auto &attribute : strategy.getAudioAttributes()) {
        if (attribute.getAttributes() == android::defaultAttr &&
