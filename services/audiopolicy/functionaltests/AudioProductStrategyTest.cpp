...
static void playbackFollowingProductStrategy(
        const android::product_strategy_t strategy, audio_port &routedDevicePort,
        const audio_port_handle_t expectedPort = AUDIO_PORT_HANDLE_NONE)
{
    ...
    EXPECT_EQ(android::NO_ERROR, Helper::getPortById(playbackRoutedPortId, routedDevicePort))
                              ...
                              const audio_port_handle_t expectedPort = AUDIO_PORT_HANDLE_NONE)
{
    ...
    ASSERT_EQ(android::OK, audioTrack->createTrack())
            << "Failed to create AudioTrack for stream: " <<  android::toString(stream);
...
TEST(AudioProductStrategiesTest, PlayByStrategy)
{
    ...
    ret = android::AudioSystem::listAudioProductStrategies(strategies);
    ...
    for (const auto &strategy : strategies) {
        ...
        for (const auto &attribute : strategy.getAudioAttributes()) {
            ...
            EXPECT_EQ(routedDevicePortForAttr.id, expectedRoutedPort.id)
                ...
                <<  Helper::dumpProductStrategy(strategy.getId(), true)
                 << " for attributes " << android::toString(attribute.getAttributes())
    ...
    for (const auto &strategy : strategies) {
        ...
        for (const auto &attribute : strategy.getAudioAttributes()) {
            ...
            EXPECT_EQ(routedDevicePortForAttr.id, expectedRoutedPort.id)
                << "AudioProductStrategy : "
                <<  Helper::dumpProductStrategy(strategy.getId(), true)
                 << " for stream " << android::toString(stream)
                << " was routed on " << Helper::dumpPort(routedDevicePortForAttr)
                << " expecting to be routed on" << Helper::dumpPort(expectedRoutedPort);
        }
    }
...
public:
    void SetUp() override
    {
        ...
        for (audio_stream_type_t stream = AUDIO_STREAM_MIN; stream < AUDIO_STREAM_PUBLIC_CNT;
            ...
            bool isActive = false;
            ret = android::AudioSystem::isStreamActive(stream, &isActive, 0);
            if (isActive) {
                activeCount += 1;
            }
        }
