...
void AudioPolicyManagerTestMsd::SetUpManagerConfig() {
    ...
    msdIec958OutputProfile->setFlags(
            AUDIO_OUTPUT_FLAG_DIRECT | AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD);
...
TEST_P(AudioPolicyManagerTestMsd, CreateAudioPatchFromHdmiInToMsd) {
    audio_patch_handle_t handle = AUDIO_PATCH_HANDLE_NONE;
    uid_t uid = 42;
    const PatchCountCheck patchCount = snapshotPatchCount();
    ASSERT_FALSE(mManager->getAvailableInputDevices().isEmpty());
    ...
    ASSERT_EQ(patch->mPatch.num_sources, 1);
    ASSERT_EQ(patch->mPatch.num_sinks, 1);
    ASSERT_EQ(patch->mPatch.sources[0].role, AUDIO_PORT_ROLE_SOURCE);
    ASSERT_EQ(patch->mPatch.sinks[0].role, AUDIO_PORT_ROLE_SINK);
    ASSERT_EQ(patch->mPatch.sources[0].format, AUDIO_FORMAT_IEC60958);
    ASSERT_EQ(patch->mPatch.sinks[0].format, AUDIO_FORMAT_IEC60958);
    ASSERT_EQ(patch->mPatch.sources[0].channel_mask, AUDIO_CHANNEL_IN_STEREO);
    ASSERT_EQ(patch->mPatch.sinks[0].channel_mask, AUDIO_CHANNEL_OUT_STEREO);
    ASSERT_EQ(patch->mPatch.sources[0].sample_rate, 48000);
    ASSERT_EQ(patch->mPatch.sinks[0].sample_rate, 48000);
    ASSERT_EQ(1, patchCount.deltaFromSnapshot());
}
