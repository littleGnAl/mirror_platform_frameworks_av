                                               ...
                                               bool force)
{
    ...
    if (outputDesc == mPrimaryOutput && (isVoiceVolSrc || isBtScoVolSrc)) {
        ...
        if (isVoiceVolSrc)
            voiceDevices = mEngine->getOutputDevicesForAttributes(attributes_initializer(AUDIO_USAGE_VOICE_COMMUNICATION), nullptr, false).types();
        if (isBtScoVolSrc)
            voiceDevices = mEngine->getOutputDevicesForAttributes(attributes_initializer(AUDIO_USAGE_BLUETOOTH_SCO), nullptr, false).types();
