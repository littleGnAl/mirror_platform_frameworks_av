...
void AudioPolicyManager::disconnectTelephonyAudioSource(sp<SourceClientDescriptor> &clientDesc)
{
    ...
    stopAudioSource(clientDesc->portId());
    clientDesc.clear();
}
                                              ...
                                              uid_t uid)
{
    ...
    status_t status = isAudioPatchValid(patch);
    if (status != NO_ERROR) {
        return status;
    }
    ...
    status = connectAudioSourceToSink(sourceDesc, sinkDevice, patch, *handle, uid, delayMs);
    if (status != NO_ERROR) {
        return status;
    }
...
status_t AudioPolicyManager::isAudioPatchValid(const struct audio_patch *patch) const
{
    if (patch == nullptr) {
        return BAD_VALUE;
    }
...
{
    ALOGV("%s", __func__);
    if (handle == NULL || sourceDesc == nullptr) {
        return BAD_VALUE;
    }
    status_t status = isAudioPatchValid(patch);
    if (status != NO_ERROR) {
            ...
            for (size_t i = 0; i < patch->num_sinks; i++) {
                ...
                if (!sourceDesc->isInternal()) {
                    ...
                    if (output != AUDIO_IO_HANDLE_NONE) {
                        ...
                        ALOGE("%s no output for device %s",
