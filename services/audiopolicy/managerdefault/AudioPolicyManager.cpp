...
uint32_t AudioPolicyManager::updateCallRouting(const DeviceVector &rxDevices, uint32_t delayMs)
{
    ...
    if (mCallRxPatch != 0) {
        releaseAudioPatchInt(mCallRxPatch->getHandle());
        mCallRxPatch.clear();
    }
    // release TX patch if any
    if (mCallTxPatch != 0) {
        releaseAudioPatchInt(mCallTxPatch->getHandle());
        mCallTxPatch.clear();
    }
...
sp<AudioPatch> AudioPolicyManager::createTelephonyPatch(
    ...
    status_t status = createAudioPatchInt(patchBuilder.patch(), &patchHandle, mUidCached, delayMs);
                                               ...
                                               uid_t uid)
{
    ...
    return createAudioPatchInt(patch, handle, uid, 0 /* delayMs */);
}

status_t AudioPolicyManager::createAudioPatchInt(const struct audio_patch *patch,
                                                 audio_patch_handle_t *handle,
                                                 ...
                                                 const sp<SourceClientDescriptor>& sourceDesc)
{
    ...
    if (patch->sources[0].type == AUDIO_PORT_TYPE_MIX) {
        ...
        ALOGV("%s setting device %08x on output %d",
              __func__, devices.types(), outputDesc->mIoHandle);
                ...
                if (!srcDevice->hasSameHwModuleAs(sinkDevice) ||
                    ...
                    if (sourceDesc != nullptr) {
                        ...
                        getOutputForAttrInt(&resultAttr, &output, AUDIO_SESSION_NONE, &attributes,
                                            ...
                                            &secondaryOutputs);
                        if (output == AUDIO_IO_HANDLE_NONE) {
                            ALOGV("%s no output for device %08x", __FUNCTION__, sinkDevice->type());
                            return INVALID_OPERATION;
                        }
                    } else {
                        SortedVector<audio_io_handle_t> outputs =
                    ...
                    if (output != AUDIO_IO_HANDLE_NONE) {
                        sp<SwAudioOutputDescriptor> outputDesc = mOutputs.valueFor(output);
                        if (outputDesc->isDuplicated()) {
                            ALOGV("%s output for device %08x is duplicated", __FUNCTION__,
                                  sinkDevice->type());
                            return INVALID_OPERATION;
                        }
                        audio_port_config srcMixPortConfig = {};
                        outputDesc->toAudioPortConfig(&srcMixPortConfig, &patch->sinks[0]);
                        srcMixPortConfig.ext.mix.usecase.stream = AUDIO_STREAM_PATCH;
                        if (sourceDesc != nullptr) {
                                                  ...
                                                  uid_t uid)
{
    ...
    return releaseAudioPatchInt(handle);
}

status_t AudioPolicyManager::releaseAudioPatchInt(audio_patch_handle_t handle)
{
                                              ...
                                              uid_t uid)
{
    ...
    *portId = AudioPort::getNextUniqueId();
    ...
    sp<SourceClientDescriptor> sourceDesc =
        new SourceClientDescriptor(*portId, uid, *attributes, srcDevice,
...
status_t AudioPolicyManager::connectAudioSource(const sp<SourceClientDescriptor>& sourceDesc)
{
    ...
    status_t status = createAudioPatchInt(patchBuilder.patch(), &handle, mUidCached, 0, sourceDesc);
    ...
    if (swOutput != 0) {
        ...
        if (status != NO_ERROR) {
            ...
            swOutput->removeClient(sourceDesc->portId());
            swOutput->stop();
            sourceDesc->setSwOutput(nullptr);
            releaseAudioPatchInt(handle);
            return INVALID_OPERATION;
...
status_t AudioPolicyManager::disconnectAudioSource(const sp<SourceClientDescriptor>& sourceDesc)
{
    ...
    return releaseAudioPatchInt(sourceDesc->getPatchHandle());
}
