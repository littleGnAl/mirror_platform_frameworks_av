...
void AudioPolicyManager::connectTelephonyRxAudioSource()
{
    ...
    if (status != NO_ERROR) {
        ALOGE("%s failed to start Telephony Rx AudioSource", __func__);
    }
}
                                                ...
                                                const audio_attributes_t *srcAttr)
{
    if (srcAttr != NULL) {
        ...
        *dstAttr = *srcAttr;
        ...
        // Give music attributes as default or report error?
        *dstAttr = attributes_initializer(AUDIO_USAGE_MEDIA);
    }
        ...
        output_type_t *outputType)
{
    ...
    *outputType = API_OUTPUT_INVALID;
    ...
    audio_stream_type_t stream = mEngine->getStreamTypeForAttributes(*resultAttr);
