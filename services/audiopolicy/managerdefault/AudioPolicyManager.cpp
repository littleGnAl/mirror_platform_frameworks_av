                                               ...
                                               uid_t uid)
{
    ...
    return createAudioPatchInternal(patch, handle, uid, 0 /* delayMs */);
}
    ...
    if (patch->sources[0].type == AUDIO_PORT_TYPE_MIX) {
        ...
        if (patch->sinks[0].type == AUDIO_PORT_TYPE_MIX) {
            ...
            for (size_t i = 0; i < patch->num_sinks; i++) {
                ...
                // TODO: shall we also factorize the rule on gains comming from connectAudioSource?
...
status_t AudioPolicyManager::disconnectAudioSource(const sp<SourceClientDescriptor>& sourceDesc)
{
    ...
    // release patch internal may fail in case of direct output, already handled by releaseOutput
    (void) releaseAudioPatchInternal(sourceDesc->getPatchHandle());
    return NO_ERROR;
}
