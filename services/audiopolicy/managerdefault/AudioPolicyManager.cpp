    ...
    if (patch->sources[0].type == AUDIO_PORT_TYPE_MIX) {
        ...
        if (patch->sinks[0].type == AUDIO_PORT_TYPE_MIX) {
            ...
            for (size_t i = 0; i < patch->num_sinks; i++) {
                ...
                // In case of Hw bridge, it is a WA. the mixPort used is the one declared in
                // Settings XML to reach the sink so that is can be declared as available.
                audio_io_handle_t output = AUDIO_IO_HANDLE_NONE;
...
void AudioPolicyManager::checkOutputForAttributes(const audio_attributes_t &attr)
{
    ...
    if (srcOutputs != dstOutputs || invalidate) {
        ...
        for (audio_io_handle_t srcOut : srcOutputs) {
            ...
            if (source != 0 && !isCallRxAudioSource(source)) {
