...
Status Spatializer::setLevel(Spatialization::Level level) {
    ALOGV("%s level %s", __func__, media::audio::common::toString(level).c_str());
    mLocalLog.log("%s with %s", __func__, media::audio::common::toString(level).c_str());
...
Status Spatializer::setDesiredHeadTrackingMode(HeadTracking::Mode mode) {
    ALOGV("%s mode %s", __func__, media::audio::common::toString(mode).c_str());

    if (!mSupportsHeadTracking) {
        return binderStatusFromStatusT(INVALID_OPERATION);
    }
    mLocalLog.log("%s with %s", __func__, media::audio::common::toString(mode).c_str());
    std::lock_guard lock(mLock);
    switch (mode) {
...
std::string Spatializer::toString(unsigned level) const {
    ...
    for (auto& level : mLevels) {
        base::StringAppendF(&ss, " %s", media::audio::common::toString(level).c_str());
    }
    base::StringAppendF(&ss, "], mLevel: %s", media::audio::common::toString(mLevel).c_str());

    base::StringAppendF(&ss, "\n%smHeadTrackingModes: [", prefixSpace.c_str());
    for (auto& mode : mHeadTrackingModes) {
        base::StringAppendF(&ss, " %s", media::audio::common::toString(mode).c_str());
    }
    base::StringAppendF(&ss, "], Desired: %s, Actual %s\n",
                        media::toString(mDesiredHeadTrackingMode).c_str(),
                        media::audio::common::toString(mActualHeadTrackingMode).c_str());
