...
status_t AudioPolicyEffects::loadAudioEffectXmlConfig() {
    ...
    auto loadDeviceProcessingChain = [](auto &processingChain, auto& deviceEffects) {
        for (auto& deviceProcess : processingChain) {
            auto effectDescs = std::make_unique<DeviceEffects>(
                        deviceProcess.type, deviceProcess.address);
            for (auto& effect : deviceProcess.effects) {
                // Waiting for new constructor...
//                effectDescs->mEffects.push_back(
//                        std::make_unique<AudioEffect>(&effect.get().uuid,
//                                                      deviceProcess.type,
//                                                      deviceProcess.address));
                effectDescs->mEffects.push_back(
                        std::make_unique<AudioEffect>(&effect.get().uuid, String16("android")));
            }
