...
bool SwAudioOutputDescriptor::setVolume(float volumeDb, bool muted,
                                        ...
                                        bool force)
{
    if (!AudioOutputDescriptor::setVolume(volumeDb, muted, vs, deviceTypes, delayMs, force)) {
        return false;
    }
    for (const auto& devicePort : devices()) {
...
{
    bool changed =
            AudioOutputDescriptor::setVolume(volumeDb, muted, vs, deviceTypes, delayMs, force);
    if (changed) {
