...
status_t AudioPolicyMix::dump(int fd, int spaces, int index) const
{
    ...
    if (not MixTypeConverter::toString(mMix.mMixType, mixTypeLiteral)) {
    ...
    if (not RouteFlagTypeConverter::toString(mMix.mRouteFlags, routeFlagLiteral)) {
        ALOGE("%s: failed to convert route flags %d", __FUNCTION__, mMix.mRouteFlags);
        return BAD_VALUE;
    }
    ...
    for (const auto &criterion : mMix.mCriteria) {
        ...
        if (not UsageTypeConverter::toString(criterion.mValue.mUsage, usageLiteral)) {
        ...
        if (mMix.mMixType == MIX_TYPE_RECORDERS) {
            ...
            if (not SourceTypeConverter::toString(criterion.mValue.mSource, sourceLiteral)) {
        ...
        if (not RuleTypeConverter::toString(criterion.mRule, ruleLiteral)) {
