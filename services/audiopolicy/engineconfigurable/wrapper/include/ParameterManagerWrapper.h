...
public:
    ...
    /**
     * @brief setDeviceConnectionState propagates a state even on a given device(s)
     * @param types bit mask of the device(s) whose state has changed
     * @param address of the device whose state has changed
     * @param state new state of the given devices
     * @return NO_ERROR if new state corretly propagated to Engine Parameter-Framework, error
     * code otherwise.
     */
    status_t setDeviceConnectionState(audio_devices_t types, const std::string address,
