...
def parseAndroidAudioFile(androidaudiobaseheaderFile, androidaudiocommonbaseheaderFile):
    ...
    for line_number, line in enumerate(androidaudiobaseheaderFile):
        ...
        if match:
            ...
            criterion_literal = ''.join((w.capitalize() for w in match.groupdict()['literal'].split('_')))
            ...
            if criterion_name == "InputDevicesMaskType":
                if criterion_literal == "Default":
                    ...
                    try:
                        ...
                        logging.info("The value {}:{} is for criterion {} is not a number, ignoring".format(criterion_numerical_value, criterion_literal, criterion_name))
                        continue
                    criterion_numerical_value = str(int(criterion_numerical_value, 0) & ~2147483648)

            if criterion_name == "OutputDevicesMaskType":
                if criterion_literal == "Default":
                    logging.info("The value {}:{} is for criterion {}, OutputDevicesMaskType changing to stub".format(criterion_numerical_value, criterion_literal, criterion_name))
                    criterion_numerical_value = str(int("0x40000000", 0))
...
def main():
    ...
    all_criteria = parseAndroidAudioFile(args.androidaudiobaseheader, args.androidaudiocommonbaseheader)
