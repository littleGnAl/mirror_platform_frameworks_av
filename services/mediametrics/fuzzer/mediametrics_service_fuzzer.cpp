...
class MediaMetricsServiceFuzzer {
   public:
    MediaMetricsServiceFuzzer() = default;
    ~MediaMetricsServiceFuzzer() = default;
...
void MediaMetricsServiceFuzzer::invokeItemManipulation(const uint8_t *data, size_t size) {
    ...
    while (fdp.remaining_bytes()) {
        uint8_t action = fdp.ConsumeIntegralInRange<uint8_t>(0, 15);
        std::string key = fdp.ConsumeRandomLengthString();
        int32_t i32 = 0;
        int64_t i64 = 0, b = 0, h = 0;
        double d = 0;
        char *s = nullptr;
        const char *arr[1] = {""};
        ...
        switch (action) {
            ...
            case 10:
                ...
                break;
            case 11:
                item.setRate(key.c_str(), fdp.ConsumeIntegral<int32_t>(),
                             fdp.ConsumeIntegral<int32_t>());
                break;
            case 12:
                ...
                break;
            case 13:
                item.filter(key.c_str());
                break;
            case 14:
                arr[0] = const_cast<char *>(key.c_str());
                item.filterNot(1, arr);
                break;
            case 15:
                item.toString().c_str();
                break;
    ...
    item.writeToByteString(&byteData, &length);
    item2.readFromByteString(byteData, length);
    if (byteData) {
        free(byteData);
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    auto *mediaMetricsServiceFuzzer = new MediaMetricsServiceFuzzer();
    if (!mediaMetricsServiceFuzzer) {
        return 0;
    }
    mediaMetricsServiceFuzzer->process(data, size);
    delete mediaMetricsServiceFuzzer;
    return 0;
}
