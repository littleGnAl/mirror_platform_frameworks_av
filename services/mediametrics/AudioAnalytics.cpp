...
#ifdef STATSD_ENABLE
    result = mediametrics::stats::stats_write(args...);
#endif
    return result;
}
        ...
        int32_t subCode) const
{
    ...
    if (eventStr == AMEDIAMETRICS_PROP_EVENT_VALUE_CREATE) {
        ...
        const int32_t event = mediametrics::stats::
        ...
        const auto [ result, str ] = sendToStatsd(AudioRecordStatusFields,
                CONDITION(mediametrics::stats::MEDIAMETRICS_AUDIORECORDSTATUS_REPORTED)
                , atom_status
                , message.c_str()
                , subCode
                , uid
        ...
        mStatsdLog->log(mediametrics::stats::MEDIAMETRICS_AUDIORECORDSTATUS_REPORTED, str);
        return true;
    }
    return false;
}
        ...
        int32_t subCode) const
{
    ...
    if (eventStr == AMEDIAMETRICS_PROP_EVENT_VALUE_CREATE) {
        ...
        const auto [ result, str ] = sendToStatsd(AudioTrackStatusFields,
                CONDITION(mediametrics::stats::MEDIAMETRICS_AUDIOTRACKSTATUS_REPORTED)
                , atom_status
                , message.c_str()
                , subCode
                , uid
        ...
        ALOGV("%s: statsd %s", __func__, str.c_str());
        mStatsdLog->log(mediametrics::stats::MEDIAMETRICS_AUDIOTRACKSTATUS_REPORTED, str);
        return true;
    }
    return false;
}
    ...
    case RECORD: {
        ...
        if (clientCalled  // only log if client app called AudioRecord.
            ...
            const auto [ result, str ] = sendToStatsd(AudioRecordDeviceUsageFields,
                    CONDITION(mediametrics::stats::MEDIAMETRICS_AUDIORECORDDEVICEUSAGE_REPORTED)
            ...
            mAudioAnalytics.mStatsdLog->log(
                    mediametrics::stats::MEDIAMETRICS_AUDIORECORDDEVICEUSAGE_REPORTED, str);
        }
    } break;
    case THREAD: {
        std::string type;
        ...
        if (mAudioAnalytics.mDeliverStatistics) {
            const auto [ result, str ] = sendToStatsd(AudioThreadDeviceUsageFields,
                CONDITION(mediametrics::stats::MEDIAMETRICS_AUDIOTHREADDEVICEUSAGE_REPORTED)
            ...
            ALOGV("%s: statsd %s", __func__, str.c_str());
            mAudioAnalytics.mStatsdLog->log(
                    mediametrics::stats::MEDIAMETRICS_AUDIOTHREADDEVICEUSAGE_REPORTED, str);
        }
    } break;
    case TRACK: {
        std::string callerName;
        ...
        if (clientCalled // only log if client app called AudioTracks
            ...
            const auto [ result, str ] = sendToStatsd(AudioTrackDeviceUsageFields,
                    CONDITION(mediametrics::stats::MEDIAMETRICS_AUDIOTRACKDEVICEUSAGE_REPORTED)
            ...
            ALOGV("%s: statsd %s", __func__, str.c_str());
            mAudioAnalytics.mStatsdLog->log(
                    mediametrics::stats::MEDIAMETRICS_AUDIOTRACKDEVICEUSAGE_REPORTED, str);
        }
        } break;
    }
    ...
    if (outputDevices.find("AUDIO_DEVICE_OUT_BLUETOOTH_A2DP") != std::string::npos) {
        ...
        if (mAudioAnalytics.mDeliverStatistics) {
            ...
            const auto [ result, str ] = sendToStatsd(AudioDeviceConnectionFields,
                    CONDITION(mediametrics::stats::MEDIAMETRICS_AUDIODEVICECONNECTION_REPORTED)
                    , ENUM_EXTRACT(inputDeviceBits)
                    , ENUM_EXTRACT(outputDeviceBits)
            ...
            ALOGV("%s: statsd %s", __func__, str.c_str());
            mAudioAnalytics.mStatsdLog->log(
                    mediametrics::stats::MEDIAMETRICS_AUDIODEVICECONNECTION_REPORTED, str);
        }
    }
}
...
void AudioAnalytics::AAudioStreamInfo::endAAudioStream(
    ...
    if (mAudioAnalytics.mDeliverStatistics) {
        mediametrics::stats::BytesField bf_serialized(
            serializedDeviceTypes.c_str(), serializedDeviceTypes.size());
        const auto result = sendToStatsd(
                CONDITION(mediametrics::stats::MEDIAMETRICS_AAUDIOSTREAM_REPORTED)
                , path
                , direction
                , framesPerBurst
                , bufferSizeInFrames
        ...
        ALOGV("%s: statsd %s", __func__, str.c_str());
        mAudioAnalytics.mStatsdLog->log(mediametrics::stats::MEDIAMETRICS_AAUDIOSTREAM_REPORTED, str);
    }
}
...
static int32_t classifySettingEvent(bool isSetAlready, bool withinBoot) {
    if (isSetAlready) {
        return mediametrics::stats::MEDIAMETRICS_SPATIALIZER_DEVICE_ENABLED_REPORTED__EVENT__SPATIALIZER_SETTING_EVENT_NORMAL;
    }
    if (withinBoot) {
        return mediametrics::stats::MEDIAMETRICS_SPATIALIZER_DEVICE_ENABLED_REPORTED__EVENT__SPATIALIZER_SETTING_EVENT_BOOT;
    }
    return mediametrics::stats::MEDIAMETRICS_SPATIALIZER_DEVICE_ENABLED_REPORTED__EVENT__SPATIALIZER_SETTING_EVENT_FIRST;
}
        ...
        const std::shared_ptr<const android::mediametrics::Item> &item)
{
    ...
    if (delim == suffix.npos) {
        ...
        const auto [ result, str ] = sendToStatsd(SpatializerCapabilitiesFields,
                CONDITION(mediametrics::stats::MEDIAMETRICS_SPATIALIZERCAPABILITIES_REPORTED)
        ...
        mAudioAnalytics.mStatsdLog->log(
                mediametrics::stats::MEDIAMETRICS_SPATIALIZERCAPABILITIES_REPORTED, str);
        ...
        if (!enabled.empty()) {
            if (enabled != deviceState.enabled) {
                ...
                const auto [ result, str ] = sendToStatsd(SpatializerDeviceEnabledFields,
                        CONDITION(mediametrics::stats::MEDIAMETRICS_SPATIALIZERDEVICEENABLED_REPORTED)
                        , deviceTypeStatsd
                        , settingEventStatsd
                        , enabledStatsd
                        );
                mAudioAnalytics.mStatsdLog->log(
                        mediametrics::stats::MEDIAMETRICS_SPATIALIZERDEVICEENABLED_REPORTED, str);
            }
        }
        if (!hasHeadTracker.empty()) {
            if (hasHeadTracker != deviceState.hasHeadTracker) {
                ...
                const auto [ result, str ] = sendToStatsd(HeadTrackerDeviceSupportedFields,
                        CONDITION(mediametrics::stats::MEDIAMETRICS_HEADTRACKERDEVICESUPPORTED_REPORTED)
                        , deviceTypeStatsd
                        , settingEventStatsd
                        , supportedStatsd
                        );
                mAudioAnalytics.mStatsdLog->log(
                        mediametrics::stats::MEDIAMETRICS_HEADTRACKERDEVICESUPPORTED_REPORTED, str);
            }
        }
        if (!headTrackerEnabled.empty()) {
            if (headTrackerEnabled != deviceState.headTrackerEnabled) {
                ...
                const auto [ result, str ] = sendToStatsd(HeadTrackerDeviceEnabledFields,
                        CONDITION(mediametrics::stats::MEDIAMETRICS_HEADTRACKERDEVICEENABLED_REPORTED)
                        , deviceTypeStatsd
                        , settingEventStatsd
                        , enabledStatsd
                        );
                mAudioAnalytics.mStatsdLog->log(
                        mediametrics::stats::MEDIAMETRICS_HEADTRACKERDEVICEENABLED_REPORTED, str);
            }
        }
