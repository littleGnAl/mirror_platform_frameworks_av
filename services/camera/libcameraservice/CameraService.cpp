...
status_t CameraService::enumerateProviders() {
    ...
    for (auto& cameraId : mCameraProviderManager->getCameraDeviceIds()) {
        ...
        if (!cameraFound) {
            addStates(id8, StatusInternal::NOT_PRESENT);
        }
...
void CameraService::addStates(const String8 id, StatusInternal status) {
...
void CameraService::onDeviceStatusChanged(const String8& id,
    ...
    if (state == nullptr) {
        if (newStatus == StatusInternal::PRESENT) {
            ...
            addStates(id, StatusInternal::NOT_PRESENT);
...
CameraService::CameraState::CameraState(const String8& id, int cost,
        const std::set<String8>& conflicting, StatusInternal status) : mId(id),
