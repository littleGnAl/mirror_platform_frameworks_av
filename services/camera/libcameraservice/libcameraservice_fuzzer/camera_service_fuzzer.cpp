...
void CameraFuzzer::invokeCameraAPIs() {
    for (int32_t cameraId = 0; cameraId < mNumCameras; ++cameraId) {
        ...
        bool shouldConnectLegacy = mFuzzedDataProvider->ConsumeBool();
        if (shouldConnectLegacy) {
            int32_t halVersion = mFuzzedDataProvider->ConsumeIntegral<int32_t>();
            rc = mCameraService->connectLegacy(this, cameraId, halVersion, opPackageName, AID_MEDIA,
                                               &cameraDevice);
        } else {
