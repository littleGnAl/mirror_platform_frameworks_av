...
status_t CameraProviderManager::tryToInitializeHidlProviderLocked(
...

    {
        ...
        interface->getDebugInfo([&pid](const ::android::hidl::base::V1_0::DebugInfo& info) {
            pid = info.pid;
        });
        std::lock_guard<std::mutex> lock(mProviderPidMapLock);
        mProviderPidMap[providerInfo->mProviderInstance] = pid;
...
std::vector<pid_t> CameraProviderManager::getProviderPids() {
    std::lock_guard<std::mutex> lock(mProviderPidMapLock);

    std::vector<pid_t> pids;
    std::transform(mProviderPidMap.begin(), mProviderPidMap.end(), std::back_inserter(pids),
