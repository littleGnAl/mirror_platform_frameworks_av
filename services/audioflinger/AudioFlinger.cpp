...
status_t AudioFlinger::checkVolumeSource(VolumeSource volumeSource) const
{
    ...
    // Shall we check VolumeSource(s) associated to NON PUBLIC STREAM(S)???
                                             ...
                                             audio_io_handle_t output)
{
    ...
    LOG_ALWAYS_FATAL_IF(volumeSource == toVolumeSource(AUDIO_STREAM_PATCH) && value != 1.0f,
...
status_t AudioFlinger::setVolumeSourceMute(VolumeSource volumeSource, bool muted)
{
    ...
    ALOG_ASSERT(volumeSource != patchVolumeSource, "attempt to mute AUDIO_STREAM_PATCH");

    if (volumeSource == patchVolumeSource) {
...
/*static*/
VolumeSource AudioFlinger::toVolumeSource(audio_stream_type_t streamType)
{
    return toVolumeSource(AudioSystem::streamTypeToAttributes(streamType));
}
