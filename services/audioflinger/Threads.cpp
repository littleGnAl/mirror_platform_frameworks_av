...
bool AudioFlinger::PlaybackThread::invalidateTracks_l(const std::vector<audio_port_handle_t> &ports)
{
    ...
    for (const sp<PlaybackThread::Track> &track : mTracks) {
        auto port = std::find(
                    std::begin(portsToInvalidate), std::end(portsToInvalidate), track->portId());
        if (track->isExternalTrack() && port != std::end(portsToInvalidate)) {
            portsToInvalidate.erase(port);
            track->invalidate();
            trackMatch = true;
        }
    }
        ...
        const std::vector<audio_port_handle_t> &ports)
{
    ...
    auto portsToInvalidate = ports;
    for (const sp<MmapTrack> &track : mActiveTracks) {
        auto portToInvalidate = std::find(
                    std::begin(portsToInvalidate), std::end(portsToInvalidate), track->portId());
        if (portToInvalidate != std::end(portsToInvalidate)) {
            track->invalidate();
            portsToInvalidate.erase(portToInvalidate);
        }
    }
    broadcast_l();
}
