...
{
    return (uint32_t)((mNormalFrameCount * NANOS_PER_MILLISECOND) / mSampleRate);
}
...
{
    return (uint32_t)((mNormalFrameCount * NANOS_PER_MILLISECOND) / mSampleRate) / 2;
}
...
uint32_t AudioFlinger::DirectOutputThread::idleSleepTimeUs() const
{
    uint32_t time;
    if (audio_has_proportional_frames(mFormat)) {
        time = (uint32_t)((mFrameCount * NANOS_PER_MILLISECOND) / mSampleRate) / 2;
    } else {
        time = kDirectMinSleepTimeUs;
    }
    return time;
...
uint32_t AudioFlinger::DirectOutputThread::suspendSleepTimeUs() const
{
    uint32_t time;
    if (audio_has_proportional_frames(mFormat)) {
        time = (uint32_t)((mFrameCount * NANOS_PER_MILLISECOND) / mSampleRate);
    } else {
        time = kDirectMinSleepTimeUs;
    }
    return time;
