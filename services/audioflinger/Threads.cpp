    ...
    if (mNormalSink != 0) {
        ...
        if (mUseFastMixer == FastMixer_Dynamic) {
            if ((mNormalSink == mOutputSink) && (mDynamicOutSinkFormat != mFormat)) {
                memcpy_by_audio_format(mDynamicOutSinkBuffer, mDynamicOutSinkFormat,
                        mSinkBuffer, mFormat, mNormalFrameCount * mChannelCount);
        ...
        // mNormalSink below
{
    ...
    if (initFastMixer) {
        ...
        if (mUseFastMixer == FastMixer_Dynamic) {
            ...
            mDynamicOutSinkBufferSize = mChannelCount * audio_bytes_per_sample(mFormat)
                    * mNormalFrameCount;
...
void AudioFlinger::MixerThread::threadLoop_standby()
{
    ...
    if (mFastMixer != 0) {
        ...
        if (!(state->mCommand & FastMixerState::IDLE)) {
            ...
            state->mColdFutexAddr = &mFastMixerFutex;
            state->mColdGen++;
            // warm up immediately to avoid glitch if use dynamic mode
            state->mIsWarm = (mUseFastMixer == FastMixer_Dynamic);
            mFastMixerFutex = 0;
            sq->end();
