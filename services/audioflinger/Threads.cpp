...
{
    return (uint32_t)((mNormalFrameCount * 1000000) / mSampleRate);
}
...
{
    return (uint32_t)((mNormalFrameCount * 1000000) / mSampleRate) / 2;
}
...
uint32_t AudioFlinger::DirectOutputThread::idleSleepTimeUs() const
{
    uint32_t time;
    if (audio_has_proportional_frames(mFormat)) {
        time = (uint32_t)((mFrameCount * 1000000) / mSampleRate) / 2;
    } else {
        time = kDirectMinSleepTimeUs;
    }
    return time;
...
uint32_t AudioFlinger::DirectOutputThread::suspendSleepTimeUs() const
{
    uint32_t time;
    if (audio_has_proportional_frames(mFormat)) {
        time = (uint32_t)((mFrameCount * 1000000) / mSampleRate);
    } else {
        time = kDirectMinSleepTimeUs;
    }
    return time;
