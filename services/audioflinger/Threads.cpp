...
void AudioFlinger::DuplicatingThread::ioConfigChanged(audio_io_config_event event, pid_t pid,
                                                   audio_port_handle_t portId) {
    sp<AudioIoDescriptor> desc = new AudioIoDescriptor();
    ALOGV("DuplicatingThread::ioConfigChanged, thread %p, event %d, mId %d, device ID %d", this, event, mId, mPatch.sinks[0].id);
    ...
    case AUDIO_OUTPUT_CONFIG_CHANGED:
        ...
        ALOGD("DuplicatingThread::ioConfigChanged, event %d, mId %d, device Id %d", event, mId, mPatch.sinks[0].id);
        desc->mChannelMask = mChannelMask;
        desc->mSamplingRate = mSampleRate;
        desc->mFormat = mFormat;
        desc->mFrameCount = mNormalFrameCount; // FIXME see
