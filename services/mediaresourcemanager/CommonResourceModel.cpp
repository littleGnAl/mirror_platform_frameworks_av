...
bool CommonResourceModel::getClients(
    ...
    if (requestInfo.mResources.size() > 1) {
        // Look to find the client(s) with the other resources.
    ...
    if (clients->empty()) {
        // Since we couldn't find the client to reclaim from, free one client with the same type.
    ...
    // Another attempt for codecs as it can be secure or unsecure
    if (clients->empty() &&
        (requestInfo.mResources[0].type == MediaResource::Type::kSecureCodec ||
         requestInfo.mResources[0].type == MediaResource::Type::kNonSecureCodec)) {
        // Since we couldn't find the client to reclaim from,
        // free one codec with the different type.
