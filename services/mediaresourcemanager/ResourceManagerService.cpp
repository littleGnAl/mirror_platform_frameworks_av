...
protected:
    ...
    AIBinder_DeathRecipient* mRecipient = nullptr;
...
DeathNotifier::DeathNotifier(const std::shared_ptr<IResourceManagerClient>& client,
                             ...
                             const ClientInfoParcel& clientInfo,
                             AIBinder_DeathRecipient* recipient)
    : mClient(client), mService(service), mClientInfo(clientInfo), mRecipient(recipient) {
    link();
}
...
static ResourceInfos& getResourceInfosForEdit(int pid, PidResourceInfosMap& map) {
    ...
    if (found == map.end()) {
        ...
        auto added = map.emplace(pid, infosForPid);
        found = added.first;
    }
...
static ResourceInfo& getResourceInfoForEdit(uid_t uid, int64_t clientId,
    ...
    if (found == infos.end()) {
        ...
        auto added = infos.emplace(clientId, info);
        found = added.first;
    }
