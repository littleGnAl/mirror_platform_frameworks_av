...
void ResourceManagerService::getClientForResource_l(int callingPid,
    ...
    if (getLowestPriorityBiggestClient_l(callingPid, res->type, res->subType, idList, &client)) {
...
Status ResourceManagerService::reclaimResource(const ClientInfoParcel& clientInfo,
    ...
    *_aidl_return = false;
    ...
    PidUidVector idList;
    {
        ...
        if (secureCodec != NULL) {
            if (!mSupportsMultipleSecureCodecs) {
                if (!getAllClients_l(callingPid, MediaResource::Type::kSecureCodec,
                            secureCodec->subType, &idList, &clients)) {
                    return Status::ok();
                }
            }
        ...
        if (nonSecureCodec != NULL) {
            if (!mSupportsSecureWithNonSecureCodec) {
                if (!getAllClients_l(callingPid, MediaResource::Type::kSecureCodec,
                        nonSecureCodec->subType, &idList, &clients)) {
                    return Status::ok();
                }
            }
        }
        if (drmSession != NULL) {
            getClientForResource_l(callingPid, drmSession, &idList, &clients);
            if (clients.size() == 0) {
                return Status::ok();
            }
        }
        ...
        if (clients.size() == 0) {
            ...
            getClientForResource_l(callingPid, graphicMemory, &idList, &clients);
        }

        if (clients.size() == 0) {
            // if we are here, run the third pass to free one codec with the same type.
            getClientForResource_l(callingPid, secureCodec, &idList, &clients);
            getClientForResource_l(callingPid, nonSecureCodec, &idList, &clients);
        }
        ...
        if (clients.size() == 0) {
            ...
            if (secureCodec != NULL) {
                ...
                getClientForResource_l(callingPid, &temp, &idList, &clients);
            }
            if (nonSecureCodec != NULL) {
    ...
    *_aidl_return = reclaimUnconditionallyFrom(clients);
    ...
    pushReclaimAtom(clientInfo, clients, idList, *_aidl_return);
...
bool ResourceManagerService::getAllClients_l(int callingPid, MediaResource::Type type,
    ...
    idList->insert(std::end(*idList), std::begin(tempIdList), std::end(tempIdList));
    return true;
}
...
bool ResourceManagerService::getLowestPriorityBiggestClient_l(int callingPid,
    ...
    if (getBiggestClientPendingRemoval_l(callingPid, type, subType, uid, client)) {
        idList->emplace_back(callingPid, uid);
        return true;
    }
    ...
    idList->emplace_back(lowestPriorityPid, uid);
    return true;
}
