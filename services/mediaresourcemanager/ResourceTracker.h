...
public:
    ...
    // The first call on a client adds the client and the resources.
    // And the subsequent call updates the resources associated with the client.
    ...
    // If checkValid is set, then the (pid of the) caller is validated.
    // returns true on success, false otherwise.
    ...
    // If applicable, match the primary type too.
    // returns true on success, false otherwise.
    bool getLowestImportantBiggestClient(int targetPid, int32_t importance,
                                         MediaResource::Type type,
                                         MediaResource::SubType subType,
                                         MediaResource::SubType primarySubType,
                                         const std::vector<ClientInfo>& clients,
    ...
    // The resource conflict is defined by the ResourceModel (such a
    ...
    // priority processes.
    // If there is a higher or same priority process with the given resource,
...
private:
    ...
    // If none exists, this method will create an empty object and return it.
