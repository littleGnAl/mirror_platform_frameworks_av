 ...
 *  - the calling/requesting client's priority
 *  - the resources requesting (to be reclaimed from others)
 */
struct ReclaimRequestInfo {
    int mCallingPid = -1;
    const std::vector<MediaResourceParcel>& mResources;
};
...
struct ResourceRequestInfo {
    int mCallingPid = -1;
    const MediaResourceParcel* mResource;
};
...
public:
    ...
    virtual bool getClients(const ReclaimRequestInfo& requestInfo,
                            ...
                            std::vector<std::shared_ptr<IResourceManagerClient>>* clients) = 0;
};
