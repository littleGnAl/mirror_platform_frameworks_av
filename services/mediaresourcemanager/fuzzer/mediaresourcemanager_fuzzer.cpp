...
const int32_t kMinThreads = 1;
const int32_t kMaxThreads = 6;
...
void ResourceManagerServiceFuzzer::setResources() {
    ...
    size_t numThreads =
        mFuzzedDataProvider->ConsumeIntegralInRange<size_t>(kMinThreads, kMaxThreads / 2);
    ...
    int i, j;
    for (i = 0, j = numThreads / 2; i < numThreads / 2; ++i, ++j) {
        ...
        pthread_create(&pt[j], nullptr, removeResource, &threadArgs);
        mediaResource.clear();
    }
