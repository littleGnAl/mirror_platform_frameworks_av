 ...
 *
 * Find the lowest important (than that of requesting client) client from the
 * same process (that is requesting for the resource).
 * If there are multiple clients of lowest importance, then pick the biggest
 * client.
 *
 * NOTE that this reclaim policy is used as a secondraory policy.
 * So, it is not required to address the codec conflict here as it
 * has been addressed by the Resource Model and the primary Reclaim Policy
 * already.
 */
class ClientImportanceReclaimPolicy : public IReclaimPolicy {
public:
    ClientImportanceReclaimPolicy(const std::shared_ptr<ResourceTracker>& resourceTracker);

    virtual ~ClientImportanceReclaimPolicy();

    /*
     * Based on the client importance, identify and return a client from the list
...
private:
    ...
    // The client should have lowest possible importance than that of the
    // requesting client's importance.
